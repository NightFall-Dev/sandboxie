...
static NTSTATUS Process_HookProcessNotify(PCREATE_PROCESS_NOTIFY_ROUTINE NewNotifyRoutine);

#endif _WIN64

static ULONG Process_GetTraceFlag(PROCESS* proc, const WCHAR* setting);

static void Process_NotifyProcess(HANDLE ParentId, HANDLE ProcessId, BOOLEAN Create);

static void Process_CreateTerminated(HANDLE ProcessId, ULONG SessionId);

static PROCESS* Process_Create(HANDLE ProcessId, const BOX* box, const WCHAR* image_path, KIRQL* out_irql);

static void Process_NotifyProcess_Delete(HANDLE ProcessId);

static void Process_Delete(HANDLE ProcessId);

static void Process_NotifyImage(const UNICODE_STRING* FullImageName, HANDLE ProcessId, IMAGE_INFO* ImageInfo);
...
_FX BOOLEAN Process_Init(void)
{
	...
	if ((status == STATUS_INVALID_PARAMETER) && (Driver_OsVersion < DRIVER_WINDOWS_VISTA))
	{
...
_FX void Process_Unload(BOOLEAN FreeLock)
{
	...
	if (Process_NotifyProcessInstalled)
	{
		...
		{
			PCREATE_PROCESS_NOTIFY_ROUTINE OldNotifyProcess = (PCREATE_PROCESS_NOTIFY_ROUTINE)InterlockedExchange((volatile LONG*)Process_pOldNotifyProcess, 0);
			if (OldNotifyProcess)
			{
...
_FX NTSTATUS Process_HookProcessNotify(PCREATE_PROCESS_NOTIFY_ROUTINE NewNotifyRoutine)
{
	...
	if (ptr[0] != 0x8B || ptr[1] != 0xFF || (ptr[-5] != 0xCC && ptr[-5] != 0x90) || (ptr[-4] != 0xCC && ptr[-4] != 0x90) || (ptr[-3] != 0xCC && ptr[-3] != 0x90) || (ptr[-2] != 0xCC && ptr[-2] != 0x90) || (ptr[-1] != 0xCC && ptr[-1] != 0x90))
	{
		if (*(USHORT*)ptr == 0xF9EB && ptr[-5] == 0xE9)
		{
			...
			if (*tramp != 0xB9 || memcmp(tramp + 5, HookBytes, sizeof(HookBytes)) != 0)
			{
				tramp = NULL;
			}
		}
	...
	if (ptr[0x0B] != 0x56 || ptr[0x0C] != 0x57 || ptr[0x0D] != 0x74 || ptr[0x0F] != 0xBF || ptr[0x14] != 0x57)
	{
		return STATUS_UNKNOWN_REVISION;
	}
	...
	Process_pOldNotifyProcess = (PCREATE_PROCESS_NOTIFY_ROUTINE*)&(HOOK_TRAMP_CODE_TO_TRAMP_HEAD(tramp))->target;
	...
	*(ULONG*)ptr = // jmp   original_routine
	...
	status = PsSetCreateProcessNotifyRoutine((PCREATE_PROCESS_NOTIFY_ROUTINE)OldNotifyRoutine, TRUE);
	...
	*Process_pOldNotifyProcess = (PCREATE_PROCESS_NOTIFY_ROUTINE)OldNotifyRoutine;
		...
		while (proc)
		{
			if (proc->pid == ProcessId)
			{
				if (proc->terminated)
				{
					...
					{
						proc->terminated = 9;
						PsSetThreadHardErrorsAreDisabled((PETHREAD)KeGetCurrentThread(), TRUE);
					}
					//
...
_FX PROCESS* Process_Create(HANDLE ProcessId, const BOX* box, const WCHAR* image_path, KIRQL* out_irql)
{
	POOL* pool;
	PROCESS* proc;
	...
	if (!NT_SUCCESS(status))
	{
		Log_Status_Ex_Session(MSG_1231, 0x33, status, L"???", box->session_id);
	...
	if (image_path)
	{
		...
		if (image_name)
		{
			...
			if ((len == Driver_HomePathNt_Len) && (wcsncmp(image_path, Driver_HomePathNt, len) == 0))
			{
			...
			if (len)
			{
				...
				proc->image_name     = Mem_Alloc(proc->pool, proc->image_name_len);
				if (proc->image_name)
				{
					memcpy(proc->image_name, image_name, proc->image_name_len);
				}
				else
				{
					Log_Msg_Session(MSG_1201, NULL, NULL, box->session_id);
	...
	if ((!proc->image_name) || (!proc->image_path))
	{
		...
		Log_Status_Ex_Session(MSG_1231, 0x11, status, L"???", box->session_id);
...
_FX void Process_NotifyProcess(HANDLE ParentId, HANDLE ProcessId, BOOLEAN Create)
{
	//
...
_FX void Process_NotifyProcess_Create(HANDLE ProcessId, HANDLE ParentId, BOX* box)
{
	void *nbuf1, *nbuf2;
	ULONG nlen1, nlen2;
	WCHAR *nptr1, *nptr2;
	...
	Process_GetProcessName(Driver_Pool, (ULONG_PTR)ProcessId, &nbuf1, &nlen1, &nptr1);
	if (!nbuf1)
	{
	...
	else if (!box)
	{
		...
		if (added_to_dfp_list)
		{
			...
			Process_GetProcessName(Driver_Pool, (ULONG_PTR)ParentId, &nbuf2, &nlen2, &nptr2);
	...
	if (box)
	{
		...
		if (new_proc)
		{
			...
			if (!bHostInject)
			{
				...
				if (add_process_to_job && process_is_forced && (Driver_OsVersion >= DRIVER_WINDOWS_VISTA) && Process_IsInPcaJob(pid))
				{
					add_process_to_job = FALSE;
			...
			Process_Low_Inject(pid, session_id, create_time, nptr1, add_process_to_job, bHostInject);
		}
	}
...
_FX void Process_NotifyImage(const UNICODE_STRING* FullImageName, HANDLE ProcessId, IMAGE_INFO* ImageInfo)
{
	...
	if ((!proc) || proc->initialized)
	{
		...
		if (proc && (!proc->ntdll32_base))
		{
			...
			{
				WCHAR* path = FullImageName->Buffer + FullImageName->Length / sizeof(WCHAR) - 19;
				if (_wcsicmp(path, _Ntdll32) == 0)
				{
					proc->ntdll32_base = (ULONG)(ULONG_PTR)ImageInfo->ImageBase;
				}
			}
		}
