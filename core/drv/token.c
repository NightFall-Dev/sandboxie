...
NTSTATUS Sbie_SeFilterToken_KernelMode(IN PACCESS_TOKEN ExistingToken, IN ULONG Flags, IN PTOKEN_GROUPS SidsToDisable OPTIONAL, IN PTOKEN_PRIVILEGES PrivilegesToDelete OPTIONAL, IN PTOKEN_GROUPS RestrictedSids OPTIONAL, OUT PACCESS_TOKEN* NewToken);

NTSTATUS Sbie_SepFilterToken_KernelMode(void* TokenObject, ULONG_PTR SidCount, ULONG_PTR SidPtr, ULONG_PTR LengthIncrease, void** NewToken);
...
static void* Token_RestrictHelper1(void* TokenObject, ULONG* OutIntegrityLevel, PROCESS* proc);

static NTSTATUS Token_RestrictHelper2(void* TokenObject, ULONG* OutIntegrityLevel, ULONG SessionId);

static void* Token_RestrictHelper3(void* TokenObject, TOKEN_GROUPS* Groups, TOKEN_PRIVILEGES* Privileges, PSID UserSid, ULONG FilterFlags, ULONG SessionId);

static BOOLEAN Token_AssignPrimary(void* ProcessObject, void* TokenObject, ULONG SessionId);


//---------------------------------------------------------------------------


typedef NTSTATUS (*P_SepFilterToken)(void* TokenObject, ULONG_PTR Zero2, ULONG_PTR Zero3, ULONG_PTR Zero4, ULONG_PTR Zero5, ULONG_PTR Zero6, ULONG_PTR Zero7, ULONG_PTR RestrictedSidCount, ULONG_PTR RestrictedSidPtr, ULONG_PTR VariableLengthIncrease, void** NewTokenObject);

typedef NTSTATUS(__fastcall* P_SepFilterToken_W81)(void* TokenObject, ULONG_PTR Zero2, ULONG_PTR Zero3, ULONG_PTR Zero4, ULONG_PTR Zero5, ULONG_PTR Zero6, ULONG_PTR Zero7, ULONG_PTR RestrictedSidCount, ULONG_PTR RestrictedSidPtr, ULONG_PTR VariableLengthIncrease, void** NewTokenObject);
...
static UCHAR Token_AdministratorsSid[16] = {1, // Revision
    2,                                         // SubAuthorityCount
    0,
    0,
    0,
    ...
    ((DOMAIN_ALIAS_RID_ADMINS & 0xFF00) >> 8),
    0,
    0};

static UCHAR Token_PowerUsersSid[16] = {1, // Revision
    2,                                     // SubAuthorityCount
    0,
    0,
    0,
    ...
    ((DOMAIN_ALIAS_RID_POWER_USERS & 0xFF00) >> 8),
    0,
    0};
...
_FX BOOLEAN Token_Init(void)
{
	...
	ULONG len = (NumBasePrivs + NumVistaPrivs) * sizeof(LUID_AND_ATTRIBUTES) + 32; // for TOKEN_PRIVILEGES.PrivilegeCount, and spare room
...
#undef MySetPrivilege
	...
	len = 2 * sizeof(SID_AND_ATTRIBUTES) + 32; // for TOKEN_GROUPS.GroupCount, and spare room
...
_FX BOOLEAN Token_Init_SepFilterToken(void)
{
	...
	if (ptr)
	{
		...
		for (i = 0; i < 256; ++i)
		{
			...
			if (*(ULONG*)ptr == 0xE80875FF || *(ULONG*)ptr == 0xE80C75FF)
			{
				ptr += *(LONG*)(ptr + 4) + 8;
				Token_SepFilterToken = (P_SepFilterToken)ptr;
...
_FX void* Token_Query(void* TokenObject, TOKEN_INFORMATION_CLASS InfoClass, ULONG SessionId)
{
	void* ReturnValue;
	NTSTATUS status;
...
_FX NTSTATUS Token_QuerySidString(void* TokenObject, UNICODE_STRING* SidString)
{
	TOKEN_USER* TokenUserData = NULL;
	NTSTATUS status           = SeQueryInformationToken(TokenObject, TokenUser, &TokenUserData);
	if (NT_SUCCESS(status))
	{
		status = RtlConvertSidToUnicodeString(SidString, TokenUserData->User.Sid, TRUE);
...
_FX BOOLEAN Token_CheckPrivilege(void* TokenObject, ULONG PrivilegeLowPart, ULONG SessionId)
{
	BOOLEAN HavePrivilege = FALSE;

	TOKEN_PRIVILEGES* privs = Token_Query(TokenObject, TokenPrivileges, SessionId);
	if (privs)
	{
		ULONG i;
...
_FX void* Token_QueryPrimary(TOKEN_INFORMATION_CLASS InfoClass, ULONG SessionId)
{
	void* TokenObject;
	void* ReturnValue;
	...
	if (!TokenObject)
	{
		...
		Log_Status_Ex_Session(MSG_1222, 0x15, STATUS_UNSUCCESSFUL, NULL, SessionId);
	}
	else
	{
...
_FX void* Token_FilterPrimary(PROCESS* proc, void* ProcessObject)
{
	...
	if (!PrimaryToken)
	{
		Log_Status_Ex_Session(MSG_1222, 0x31, STATUS_NO_TOKEN, NULL, proc->box->session_id);
		return NULL;
	}

	// DbgPrint("   Process Token %08X - %d <%S>\n", PrimaryToken, proc->pid, proc->image_name);

	proc->drop_rights = Conf_Get_Boolean(proc->box->name, L"DropAdminRights", 0, FALSE);

	DropRights = (proc->drop_rights ? -1 : 0);

	// DbgPrint("   Drop rights %d - %d <%S>\n", proc->drop_rights, proc->pid, proc->image_name);

	ReturnToken = Token_Filter(PrimaryToken, DropRights, proc->box->session_id);
...
_FX void* Token_Filter(void* TokenObject, ULONG DropRights, ULONG SessionId)
{
		...
		for (i = 0; i < groups->GroupCount; ++i)
		{
			...
			for (j = 0; j < Token_FilterGroups->GroupCount; ++j)
			{
				...
				if (RtlEqualSid(entry_i->Sid, entry_j->Sid))
				{
					...
					const ULONG attrs_to_check = entry_i->Attributes & (SE_GROUP_USE_FOR_DENY_ONLY | SE_GROUP_ENABLED);
	...
	if (!ShouldFilterToken)
	{
		...
		for (i = 0; i < privs->PrivilegeCount; ++i)
		{
			...
			{
				LUID_AND_ATTRIBUTES* entry_j = &Token_FilterPrivileges->Privileges[j];
	...
	if (ShouldFilterToken)
	{
		...
		status = Sbie_SeFilterToken_KernelMode(TokenObject, 0, DisabledGroups, Token_FilterPrivileges, NULL, &ReturnToken);
...
_FX NTSTATUS Token_FilterDacl(void* TokenObject, ULONG SessionId)
{
	...
	x_dacl = (TOKEN_DEFAULT_DACL*)ExAllocatePoolWithTag(PagedPool, x_dacl_len, tzuk);
	if (!x_dacl)
	{
	...
	RtlAddAccessAllowedAce(x_dacl_ptr, ACL_REVISION, GENERIC_ALL, user->User.Sid);
	...
	status = ObOpenObjectByPointer(TokenObject, OBJ_KERNEL_HANDLE, NULL, 0, NULL, KernelMode, &TokenHandle);
	if (!NT_SUCCESS(status))
	{
		TokenHandle = NULL;
		goto finish;
	...
	status = ZwSetInformationToken(TokenHandle, TokenDefaultDacl, x_dacl, x_dacl_len);
...
_FX void* Token_Restrict(void* TokenObject, ULONG FilterFlags, ULONG* OutIntegrityLevel, PROCESS* proc)
{
	TOKEN_GROUPS* groups;
	...
	{
		void* FixedTokenObject = Token_RestrictHelper1(TokenObject, OutIntegrityLevel, proc);

		if (FixedTokenObject)
		{
			TOKEN_PRIVILEGES* privs_arg = (FilterFlags & DISABLE_MAX_PRIVILEGE) ? NULL : privs;

			NewTokenObject =
			    Token_RestrictHelper3(FixedTokenObject, groups, privs_arg, user->User.Sid, FilterFlags, proc->box->session_id);
...
_FX BOOLEAN Token_ResetPrimary(PROCESS* proc)
{
	...
	else if (Driver_OsVersion >= DRIVER_WINDOWS_8 && Driver_OsVersion <= DRIVER_WINDOWS_10)
	{
	...
	else if (Driver_OsVersion >= DRIVER_WINDOWS_VISTA && Driver_OsVersion <= DRIVER_WINDOWS_7)
	{
		UserAndGroups_offset = 0x90;
	}
	else if (Driver_OsVersion >= DRIVER_WINDOWS_8 && Driver_OsVersion <= DRIVER_WINDOWS_10)
	...
	if (!NT_SUCCESS(status))
	{
		Log_Status_Ex_Session(MSG_1222, 0x37, status, NULL, proc->box->session_id);
	}
	else
	{
		...
		else
		{
			...
			if (UserAndGroups_offset)
			{
				...
				{
					SidAndAttrsInTokenOrig = *(SID_AND_ATTRIBUTES**)((ULONG_PTR)(proc->primary_token) + UserAndGroups_offset);
...
_FX BOOLEAN Token_IsSharedSid_W8(void* TokenObject)
{
	...
	if (TokenObject && Driver_OsVersion >= DRIVER_WINDOWS_8 && Driver_OsVersion <= DRIVER_WINDOWS_10)
	{
#ifdef _WIN64
		...
		if ((ULONG_PTR)SidAndAttrsInToken->Sid > (ULONG_PTR)SidAndAttrsInToken && (ULONG_PTR)SidAndAttrsInToken->Sid <= ((ULONG_PTR)SidAndAttrsInToken + (nUserAndGroupCount + 5) * sizeof(SID_AND_ATTRIBUTES)))
		{
...
_FX void* Token_RestrictHelper1(void* TokenObject, ULONG* OutIntegrityLevel, PROCESS* proc)
{
	void* NewTokenObject;
	SID_AND_ATTRIBUTES* SidAndAttrsInToken = NULL;
	...
	if (RestrictedSids_offset)
	{
		...
		ULONG RestrictSidCountInToken = *(ULONG*)((ULONG_PTR)TokenObject + RestrictedSidCount_offset);
		SID_AND_ATTRIBUTES* RestrictSidsInToken = *(SID_AND_ATTRIBUTES**)((ULONG_PTR)TokenObject + RestrictedSids_offset);
		...
		status = Sbie_SepFilterToken_KernelMode(TokenObject, SidCount, SidPtr, 128, &TempNewTokenObject);
		...
		if (NT_SUCCESS(status))
		{
			status = Sbie_SeFilterToken_KernelMode(TempNewTokenObject, 0, NULL, NULL, RestrictSids, &NewTokenObject);
		...
		if (RestrictSidsInToken)
		{
			...
			ULONG_PTR AddressToSetZero = ((ULONG_PTR)NewTokenObject + RestrictedSidCount_offset);
			*(ULONG*)AddressToSetZero  = 0;

			AddressToSetZero              = ((ULONG_PTR)NewTokenObject + RestrictedSids_offset);
	...
	else
	{
		...
		if (UserAndGroups_offset)
		{
			SidAndAttrsInToken = *(SID_AND_ATTRIBUTES**)((ULONG_PTR)NewTokenObject + UserAndGroups_offset);
		}

		if (SidAndAttrsInToken)
		{
			UCHAR* SidInToken = (UCHAR*)SidAndAttrsInToken->Sid;
			if (SidInToken && SidInToken[1] >= 1)
			{ // SubAuthorityCount >= 1
				...
				{
					if (Driver_OsVersion >= DRIVER_WINDOWS_8 && Driver_OsVersion <= DRIVER_WINDOWS_10 && Token_IsSharedSid_W8(NewTokenObject))
					{
		...
		if (NT_SUCCESS(status))
		{
			status = Token_RestrictHelper2(NewTokenObject, OutIntegrityLevel, proc->box->session_id);
		}
...
_FX NTSTATUS Token_RestrictHelper2(void* TokenObject, ULONG* OutIntegrityLevel, ULONG SessionId)
{
	NTSTATUS status;
	ULONG label;
	...
	label = (ULONG)(ULONG_PTR)Token_Query(TokenObject, TokenIntegrityLevel, SessionId);
	...
	else
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			status = ZwSetInformationToken(TokenHandle, TokenIntegrityLevel, &SidAndAttrs, sizeof(SidAndAttrs));
...
_FX void* Token_RestrictHelper3(void* TokenObject, TOKEN_GROUPS* Groups, TOKEN_PRIVILEGES* Privileges, PSID UserSid, ULONG FilterFlags, ULONG SessionId)
{
	void* NewTokenObject;
	...
	Disabled_len   = sizeof(TOKEN_GROUPS) + (Groups->GroupCount + 2) * sizeof(SID_AND_ATTRIBUTES);
...
_FX NTSTATUS Token_AssignPrimaryHandle(void* ProcessObject, HANDLE TokenKernelHandle, ULONG SessionId)
{
	NTSTATUS status;
	...
	status = ObOpenObjectByPointer(ProcessObject, OBJ_KERNEL_HANDLE, NULL, 0, NULL, KernelMode, &ProcessHandle);
	if (!NT_SUCCESS(status))
	{
	...
	if (Driver_OsVersion >= DRIVER_WINDOWS_VISTA)
	{
		...
		if (Driver_OsVersion >= DRIVER_WINDOWS_10)
		{
			...
			else if (Driver_OsBuild >= 18290)
			{ //Windows 10 RS6 FR
				Flags2_Offset          = 0x308;
				MitigationFlags_Offset = 0x828;
				SignatureLevel_Offset  = 0x6d0;
			}
			else if (Driver_OsBuild >= 17661)
			{ //Windows 10 RS5 FR
				Flags2_Offset          = 0x300;
		...
		{
			if (*PtrPrimaryTokenFrozen & 0x400)
			{ // DisableDynamicCode = 0x400
...
_FX BOOLEAN Token_AssignPrimary(void* ProcessObject, void* TokenObject, ULONG SessionId)
{
	NTSTATUS status;
	HANDLE TokenHandle;
	...
	if (NT_SUCCESS(status))
	{
		status = Token_AssignPrimaryHandle(ProcessObject, TokenHandle, SessionId);
...
_FX BOOLEAN Token_ReplacePrimary(PROCESS* proc)
{
	...
	else
	{
		...
		if (OriginalToken)
		{
			...
			void* RestrictedToken = Token_Restrict(OriginalToken, SANDBOX_INERT | DISABLE_MAX_PRIVILEGE, &proc->integrity_level, proc);
			...
			if (RestrictedToken)
			{
				...
				if (Token_AssignPrimary(ProcessObject, RestrictedToken, proc->box->session_id))
				{
					proc->primary_token = OriginalToken;

					OriginalToken = NULL;

					if (proc->drop_rights && proc->integrity_level != tzuk && proc->integrity_level >= 0x3000)
					{
...
_FX NTSTATUS Sbie_SeFilterToken_KernelMode(IN PACCESS_TOKEN ExistingToken, IN ULONG Flags, IN PTOKEN_GROUPS SidsToDisable OPTIONAL, IN PTOKEN_PRIVILEGES PrivilegesToDelete OPTIONAL, IN PTOKEN_GROUPS RestrictedSids OPTIONAL, OUT PACCESS_TOKEN* NewToken)
{
	...
	else
	{
		...
		status = ZwCreateFile(&hFile, GENERIC_READ | GENERIC_WRITE, &oa, &ios, 0, FILE_ATTRIBUTE_NORMAL, FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE, 0, 0);

		if (NT_SUCCESS(status))
		{
			Sbie_SeFilterTokenArg arg = {ExistingToken, Flags, SidsToDisable, PrivilegesToDelete, RestrictedSids, NewToken, &statusRet};

			status = ZwDeviceIoControlFile(hFile, NULL, NULL, NULL, &ios, API_SBIEDRV_FILTERTOKEN_CTLCODE, &arg, sizeof(Sbie_SeFilterTokenArg), NULL, 0);
...
_FX NTSTATUS Sbie_SepFilterToken_KernelMode(void* TokenObject, ULONG_PTR SidCount, ULONG_PTR SidPtr, ULONG_PTR LengthIncrease, void** NewToken)
{
	UNICODE_STRING usSboxDevName = {0};
	IO_STATUS_BLOCK ios          = {0};
	NTSTATUS status              = STATUS_SUCCESS;
	...
	else
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			status = ZwDeviceIoControlFile(hFile, NULL, NULL, NULL, &ios, API_SBIEDRV_PFILTERTOKEN_CTLCODE, &arg, sizeof(Sbie_SepFilterTokenArg), NULL, 0);
...
_FX NTSTATUS Sbie_SepFilterTokenHandler(void* TokenObject, ULONG_PTR SidCount, ULONG_PTR SidPtr, ULONG_PTR LengthIncrease, void** NewToken)
{
	NTSTATUS status = 0;

	if (Driver_OsVersion >= DRIVER_WINDOWS_81)
	{
		status = ((P_SepFilterToken_W81)Token_SepFilterToken)(TokenObject, 0, 0, 0, 0, 0, 0, SidCount, SidPtr, LengthIncrease, NewToken);
	}
	else
	{
		status = Token_SepFilterToken(TokenObject, 0, 0, 0, 0, 0, 0, SidCount, SidPtr, LengthIncrease, NewToken);
	}
...
ULONG GetThreadTokenOwnerPid()
{
	...
	if (NT_SUCCESS(ZwOpenThreadToken(NtCurrentThread(), TOKEN_ALL_ACCESS, FALSE, &hHandle)) && NT_SUCCESS(ObReferenceObjectByHandle(hHandle, TOKEN_ALL_ACCESS, *SeTokenObjectType, KernelMode, &impToken, NULL)))
	{
