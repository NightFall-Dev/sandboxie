...
static BOOLEAN Conf_Expand_Override(CONF_EXPAND_ARGS* args, const WCHAR* Prefix, const WCHAR* ValueName, WCHAR* PageSizeBuffer);

static NTSTATUS Conf_Expand_RegValue(CONF_EXPAND_ARGS* args, ULONG RootKey, const WCHAR* SubKeyPath, const WCHAR* ValueName, WCHAR* PageSizeBuffer);

static NTSTATUS Conf_Expand_Template(CONF_EXPAND_ARGS* args, const WCHAR* varname, WCHAR* varvalue);

static WCHAR* Conf_Expand_Helper(CONF_EXPAND_ARGS* args, const WCHAR* model_value, WCHAR* varvalue);

static WCHAR* Conf_Expand_2(CONF_EXPAND_ARGS* args, const WCHAR* model_value);
...
_FX BOOLEAN Conf_Expand_Override(CONF_EXPAND_ARGS* args, const WCHAR* Prefix, const WCHAR* ValueName, WCHAR* PageSizeBuffer)
{
	ULONG IniName_len;
	WCHAR* IniName;
...
_FX NTSTATUS Conf_Expand_RegValue(CONF_EXPAND_ARGS* args, ULONG RootKey, const WCHAR* SubKeyPath, const WCHAR* ValueName, WCHAR* PageSizeBuffer)
{
	NTSTATUS status;
	...
	qrt[0].Flags        = RTL_QUERY_REGISTRY_REQUIRED | RTL_QUERY_REGISTRY_DIRECT | RTL_QUERY_REGISTRY_NOEXPAND;
	...
	{
		KeyPath_len = (32 + wcslen(args->sid) + wcslen(SubKeyPath)) * sizeof(WCHAR);
	...
	{
		KeyPath_len = (32 + wcslen(SubKeyPath)) * sizeof(WCHAR);
		KeyPath     = Mem_Alloc(args->pool, KeyPath_len);
		if (!KeyPath)
		{
			return STATUS_INSUFFICIENT_RESOURCES;
	...
	if (status == STATUS_SUCCESS)
	{
		...
		while (1)
		{
			...
			if (ptr2 - ptr1 == ValueName_len && _wcsnicmp(ptr1, ValueName, ValueName_len) == 0)
			{
...
_FX NTSTATUS Conf_Expand_Template(CONF_EXPAND_ARGS* args, const WCHAR* varname, WCHAR* varvalue)
{
	ULONG varname2_len;
	WCHAR* varname2;
	const WCHAR* IniValue;
...
_FX WCHAR* Conf_Expand_Helper(CONF_EXPAND_ARGS* args, const WCHAR* model_value, WCHAR* varvalue)
{
	static const WCHAR* _Registry_User       = L"\\REGISTRY\\USER\\%SID%";
	static const WCHAR* _Windows             = L"\\REGISTRY\\MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion";
	...
	static const WCHAR* _DefaultSpoolDirectoryPrefix = L"%SystemRoot%\\System32\\spool\\";
	NTSTATUS status;
	...
	else if (_wcsicmp(varname, L"user") == 0 || _wcsicmp(varname, L"username") == 0)
	{
		Conf_Expand_UserName(args, varvalue);
	}
	...
	{
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_ABSOLUTE, _Windows, _ProgramFilesDir, varvalue);
	}
	else if (_wcsicmp(varname, _SystemRoot) == 0)
	{
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_WINDOWS_NT, Driver_Empty, varname, varvalue);
	}
	else if (_wcsicmp(varname, L"SystemDrive") == 0)
	{
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_WINDOWS_NT, Driver_Empty, _SystemRoot, varvalue);
	...
	{
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_CONTROL, L"Print\\Printers", varname, varvalue);
	...
	else if (_wcsicmp(varname, L"UserProfile") == 0)
	{
		...
		else
		{
			...
			status = Conf_Expand_RegValue(args, RTL_REGISTRY_WINDOWS_NT, KeyName, L"ProfileImagePath", varvalue);

			Mem_Free(KeyName, KeyNameLen);
		}
	}
	else if (_wcsicmp(varname, L"AllUsersProfile") == 0 || _wcsicmp(varname, L"ProgramData") == 0)
	{
		//
		...
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_WINDOWS_NT, _ProfileList, L"ProgramData", varvalue);
		...
		if (status == STATUS_OBJECT_NAME_NOT_FOUND)
		{
			...
			status = Conf_Expand_RegValue(args, RTL_REGISTRY_WINDOWS_NT, _ProfileList, L"ProfilesDirectory", varvalue);
			...
			if (NT_SUCCESS(status))
			{
				...
				status = Conf_Expand_RegValue(args, RTL_REGISTRY_WINDOWS_NT, _ProfileList, L"AllUsersProfile", varvalue + PrefixLen);
			}
		}
	...
	else if (_wcsicmp(varname, _homedrive) == 0 || _wcsicmp(varname, _homepath) == 0 || _wcsicmp(varname, _homeshare) == 0)
	{
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_USER, _VolatileEnvironment, varname, varvalue);

		//
		// "temp" environment variables from HKCU\Environment
		//
	}
	else if (_wcsicmp(varname, _temp) == 0 || _wcsicmp(varname, _tmp) == 0)
	{
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_USER, _Environment, _temp, varvalue);

		if (status != STATUS_SUCCESS)
		{
			status = Conf_Expand_RegValue(args, RTL_REGISTRY_USER, _Environment, _tmp, varvalue);
		}
	}
	else if (_wcsicmp(varname, _SystemTemp) == 0)
	{
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_CONTROL, L"Session Manager\\Environment", L"Temp", varvalue);
	...
	{
		static const WCHAR* User_Shell_Folders_Key = L"Software\\Microsoft\\Windows\\CurrentVersion\\"
		                                             L"Explorer\\User Shell Folders";
		static const WCHAR* Shell_Folders_Key = L"Software\\Microsoft\\Windows\\CurrentVersion\\"
		...
		status = Conf_Expand_RegValue(args, RTL_REGISTRY_USER, User_Shell_Folders_Key, varname, varvalue);

		if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_OBJECT_PATH_NOT_FOUND)
		{
			status = Conf_Expand_RegValue(args, RTL_REGISTRY_USER, Shell_Folders_Key, varname, varvalue);
		}

		if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_OBJECT_PATH_NOT_FOUND)
		{
			status = Conf_Expand_RegValue(args, RTL_REGISTRY_MACHINE, User_Shell_Folders_Key, varname, varvalue);
		}

		if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_OBJECT_PATH_NOT_FOUND)
		{
			status = Conf_Expand_RegValue(args, RTL_REGISTRY_MACHINE, Shell_Folders_Key, varname, varvalue);
		}
	}
	...
	len = ((ptr1 - model_value) + wcslen(varvalue) + wcslen(ptr2 + 1) + 1) * sizeof(WCHAR);
	buf = Mem_Alloc(args->pool, len);
	if (!buf)
	{
		status = STATUS_INSUFFICIENT_RESOURCES;
...
_FX WCHAR* Conf_Expand_2(CONF_EXPAND_ARGS* args, const WCHAR* model_value)
{
	...
	while (new_value)
	{
		...
		{
			Log_Status_Ex(MSG_CONF_EXPAND, 0, STATUS_UNSUCCESSFUL, L"(TooLong)");
			return NULL;
		}
		...
		if (retries > 10)
		{
			Log_Status_Ex(MSG_CONF_EXPAND, 0, STATUS_UNSUCCESSFUL, L"(Recursion)");
			return NULL;
		}
	}
...
_FX WCHAR* Conf_Expand(CONF_EXPAND_ARGS* args, const WCHAR* model_value, const WCHAR* setting_name)
{
