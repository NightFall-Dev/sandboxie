...
_FX NTSTATUS DriverEntry(IN DRIVER_OBJECT* DriverObject, IN UNICODE_STRING* RegistryPath)
{
	...
	if (ok)
	{
		Driver_RegistryPath = Mem_AllocStringEx(Driver_Pool, RegistryPath->Buffer, TRUE);
...
#define MyAddAccessAllowedAce(pAcl, pSid) \
	RtlAddAccessAllowedAceEx(pAcl, ACL_REVISION, CONTAINER_INHERIT_ACE | OBJECT_INHERIT_ACE | INHERITED_ACE, GENERIC_ALL, pSid);
	...
	static UCHAR WorldSid[12] = {
	    ...
	    0,
	    0,
	    1,                 // SECURITY_WORLD_SID_AUTHORITY // IdentifierAuthority
	...
	if (Driver_OsVersion >= DRIVER_WINDOWS_VISTA)
	{
		...
		UCHAR ace_space[32];
		SYSTEM_MANDATORY_LABEL_ACE* pAce = (SYSTEM_MANDATORY_LABEL_ACE*)ace_space;
		...
		RtlCreateSecurityDescriptor(Driver_LowLabelSd, SECURITY_DESCRIPTOR_REVISION);
		RtlSetDaclSecurityDescriptor(Driver_LowLabelSd, TRUE, LowLabelAcl2, FALSE);
		RtlSetSaclSecurityDescriptor(Driver_LowLabelSd, TRUE, LowLabelAcl1, FALSE);
	}
...
_FX BOOLEAN Driver_FindHomePath(UNICODE_STRING* RegistryPath)
{
	...
	if ((info.part.Type != REG_SZ && info.part.Type != REG_EXPAND_SZ) || info.part.DataLength < 4)
	{
...
#define FIND_SERVICE(svc, prmcnt)                                              \
	{                                                                          \
		...
		if (!ptr)                                                              \
			return FALSE;                                                      \
		if (!Hook_GetService(ptr, NULL, prmcnt, NULL, (void**)&svc))           \
		{                                                                      \
...
_FX BOOLEAN Driver_CheckThirdParty(const WCHAR* DriverName, ULONG DriverType)
{
	NTSTATUS status;
	...
	status = RtlQueryRegistryValues(RTL_REGISTRY_SERVICES, DriverName, qrt, NULL, NULL);
