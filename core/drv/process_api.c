...
static NTSTATUS Process_Api_CopyBoxPathsToUser(BOX* box, ULONG* file_path_len, ULONG* key_path_len, ULONG* ipc_path_len, UNICODE_STRING64* file_path, UNICODE_STRING64* key_path, UNICODE_STRING64* ipc_path);
...
_FX NTSTATUS Process_Api_Start(PROCESS* proc, ULONG64* parms)
{
	...
	else
	{
		...
		box = Box_CreateEx(Driver_Pool, boxname, SidString.Buffer, SessionId, TRUE);
	...
	if (NT_SUCCESS(status))
	{
		...
		{
			Process_NotifyProcess_Create(user_pid_parm, Api_ServiceProcessId, box);
...
_FX NTSTATUS Process_Api_Query(PROCESS* proc, ULONG64* parms)
{
	...
	__try
	{
		...
		Api_CopyStringToUser((UNICODE_STRING64*)parms[2], proc->box->name, proc->box->name_len);

		// image name unicode string can be specified in parameter 3

		Api_CopyStringToUser((UNICODE_STRING64*)parms[3], proc->image_name, proc->image_name_len);

		// sid unicode string can be specified in parameter 4

		Api_CopyStringToUser((UNICODE_STRING64*)parms[4], proc->box->sid, proc->box->sid_len);
...
_FX NTSTATUS Process_Api_QueryBoxPath(PROCESS* proc, ULONG64* parms)
{
	...
	{
		WCHAR boxname[34];
		BOOLEAN ok = Api_CopyBoxNameFromUser(boxname, (WCHAR*)args->box_name.val);
		if (!ok)
		{
			return STATUS_INVALID_PARAMETER;
		}
	...
	status = Process_Api_CopyBoxPathsToUser(box,
...
_FX NTSTATUS Process_Api_QueryProcessPath(PROCESS* proc, ULONG64* parms)
{
	...
	status = Process_Api_CopyBoxPathsToUser(proc->box,
	    args->file_path_len.val,
	    args->key_path_len.val,
	    args->ipc_path_len.val,
	    args->file_path.val,
...
_FX NTSTATUS Process_Api_CopyBoxPathsToUser(BOX* box, ULONG* file_path_len, ULONG* key_path_len, ULONG* ipc_path_len, UNICODE_STRING64* file_path, UNICODE_STRING64* key_path, UNICODE_STRING64* ipc_path)
{
	NTSTATUS status;
...
_FX NTSTATUS Process_Enumerate(const WCHAR* boxname, BOOLEAN all_sessions, ULONG session_id, ULONG* pids, ULONG* count)
{
	NTSTATUS status;
	PROCESS* proc1;
	ULONG num;
	...
	__try
	{
		...
		while (proc1)
		{
			...
			{
				BOOLEAN same_box     = (!boxname[0]) || (_wcsicmp(box1->name, boxname) == 0);
				BOOLEAN same_session = (all_sessions || box1->session_id == session_id);
...
_FX NTSTATUS Process_Api_Enum(PROCESS* proc, ULONG64* parms)
{
	...
	status = Process_Enumerate(boxname, all_sessions, session_id, user_pids, NULL);
	if (!NT_SUCCESS(status))
	{
		return status;
	}
