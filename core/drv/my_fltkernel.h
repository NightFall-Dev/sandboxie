...
typedef FLT_PREOP_CALLBACK_STATUS(FLTAPI* PFLT_PRE_OPERATION_CALLBACK)(__inout PFLT_CALLBACK_DATA Data, __in PCFLT_RELATED_OBJECTS FltObjects, __deref_out_opt PVOID* CompletionContext);
...
typedef VOID(FLTAPI* PFLT_CONTEXT_CLEANUP_CALLBACK)(__in PFLT_CONTEXT Context, __in FLT_CONTEXT_TYPE ContextType);


typedef PVOID(FLTAPI* PFLT_CONTEXT_ALLOCATE_CALLBACK)(__in POOL_TYPE PoolType, __in SIZE_T Size, __in FLT_CONTEXT_TYPE ContextType);

typedef VOID(FLTAPI* PFLT_CONTEXT_FREE_CALLBACK)(__in PVOID Pool, __in FLT_CONTEXT_TYPE ContextType);
...
typedef NTSTATUS(FLTAPI* PFLT_INSTANCE_SETUP_CALLBACK)(__in PCFLT_RELATED_OBJECTS FltObjects, __in FLT_INSTANCE_SETUP_FLAGS Flags, __in DEVICE_TYPE VolumeDeviceType, __in FLT_FILESYSTEM_TYPE VolumeFilesystemType);


typedef NTSTATUS(FLTAPI* PFLT_INSTANCE_QUERY_TEARDOWN_CALLBACK)(__in PCFLT_RELATED_OBJECTS FltObjects, __in FLT_INSTANCE_QUERY_TEARDOWN_FLAGS Flags);


typedef VOID(FLTAPI* PFLT_INSTANCE_TEARDOWN_CALLBACK)(__in PCFLT_RELATED_OBJECTS FltObjects, __in FLT_INSTANCE_TEARDOWN_FLAGS Reason);


typedef NTSTATUS(FLTAPI* PFLT_GENERATE_FILE_NAME)(__in PFLT_INSTANCE Instance, __in PFILE_OBJECT FileObject, __in_opt PFLT_CALLBACK_DATA CallbackData, __in FLT_FILE_NAME_OPTIONS NameOptions, __out PBOOLEAN CacheFileNameInformation, __out PFLT_NAME_CONTROL FileName);


typedef NTSTATUS(FLTAPI* PFLT_NORMALIZE_NAME_COMPONENT)(__in PFLT_INSTANCE Instance, __in PCUNICODE_STRING ParentDirectory, __in USHORT VolumeNameLength, __in PCUNICODE_STRING Component, __out_bcount(ExpandCompnentNameLength) PFILE_NAMES_INFORMATION ExpandComponentName, __in ULONG ExpandComponentNameLength, __in FLT_NORMALIZE_NAME_FLAGS Flags, __deref_out_opt PVOID* NormalizationContext);


typedef VOID(FLTAPI* PFLT_NORMALIZE_CONTEXT_CLEANUP)(__in_opt PVOID* NormalizationContext);


typedef NTSTATUS(FLTAPI* PFLT_TRANSACTION_NOTIFICATION_CALLBACK)(__in PCFLT_RELATED_OBJECTS FltObjects, __in PFLT_CONTEXT TransactionContext, __in ULONG NotificationMask);
...
NTSTATUS
FLTAPI
FltRegisterFilter(__in PDRIVER_OBJECT Driver, __in CONST FLT_REGISTRATION* Registration, __deref_out PFLT_FILTER* RetFilter);

VOID FLTAPI FltUnregisterFilter(__in PFLT_FILTER Filter);

NTSTATUS
FLTAPI
FltStartFiltering(__in PFLT_FILTER Filter);

VOID FLTAPI FltSetCallbackDataDirty(__inout PFLT_CALLBACK_DATA Data);
...
__checkReturn __drv_maxIRQL(APC_LEVEL)
NTSTATUS
FLTAPI
FltGetFileNameInformation(__in PFLT_CALLBACK_DATA CallbackData, __in FLT_FILE_NAME_OPTIONS NameOptions, __deref_out PFLT_FILE_NAME_INFORMATION* FileNameInformation);

__drv_maxIRQL(APC_LEVEL) VOID FLTAPI FltReleaseFileNameInformation(__in PFLT_FILE_NAME_INFORMATION FileNameInformation);
...
NTSTATUS
FLTAPI
FltEnumerateFilters(__out_ecount_part_opt(FilterListSize, *NumberFiltersReturned) PFLT_FILTER* FilterList, __in ULONG FilterListSize, __out PULONG NumberFiltersReturned);

VOID FLTAPI FltObjectDereference(__inout PVOID FltObject);

NTSTATUS
FLTAPI
FltGetFilterInformation(__in PFLT_FILTER Filter, __in FILTER_INFORMATION_CLASS InformationClass, __out_bcount_part_opt(BufferSize, *BytesReturned) PVOID Buffer, __in ULONG BufferSize, __out PULONG BytesReturned);

NTSTATUS
FLTAPI
FltObjectReference(__inout PVOID FltObject);

NTSTATUS
FLTAPI
FltEnumerateInstances(__in_opt PFLT_VOLUME Volume, __in_opt PFLT_FILTER Filter, __out_ecount_part_opt(InstanceListSize, *NumberInstancesReturned) PFLT_INSTANCE* InstanceList, __in ULONG InstanceListSize, __out PULONG NumberInstancesReturned);
