...
static NTSTATUS File_TranslateShares(const WCHAR* dos_path, POOL* pool, WCHAR** out_path, ULONG* out_len);

static NTSTATUS File_TranslateSymlinks(WCHAR* name, ULONG max_len);

void File_InitReparsePoints(BOOLEAN init);

static WCHAR* File_TranslateReparsePoints_2(const WCHAR* path, ULONG len, POOL* pool, ULONG PassNum);

static CACHE_PATH* File_TranslateReparsePoints_3(const WCHAR* path, ULONG path_len, POOL* pool, ULONG PassNum);
...
_FX NTSTATUS File_TranslateDosToNt(const WCHAR* dos_path, POOL* pool, WCHAR** out_path, ULONG* out_len)
{
	NTSTATUS status;
	WCHAR* name;
	...
	*out_len  = 0;
	...
	if ((!dos_path[0]) || dos_path[1] != L':' || (dos_path[2] != L'\\' && dos_path[2] != L'\0'))
	{
...
_FX NTSTATUS File_TranslateShares(const WCHAR* dos_path, POOL* pool, WCHAR** out_path, ULONG* out_len)
{
	WCHAR* path;
	ULONG PrefixLen;
	ULONG len;
	...
	if (dos_path[0] == L'\\' && dos_path[1] == L'\\' && dos_path[2] != L'\0' && dos_path[2] != L'\\')
	{
...
_FX WCHAR* File_TranslateReparsePoints(const WCHAR* path, POOL* pool)
{
	...
	if (len > File_RedirectorLen && (path[File_RedirectorLen] == L'\\' || path[File_RedirectorLen] == L'\0') && _wcsnicmp(path, File_Redirector, File_RedirectorLen) == 0)
	{
		return NULL;
	}

	if (len > File_MupLen && (path[File_MupLen] == L'\\' || path[File_MupLen] == L'\0') && _wcsnicmp(path, File_Mup, File_MupLen) == 0)
	{
		return NULL;
	}

	if (len > File_NamedPipeLen && (path[File_NamedPipeLen] == L'\\' || path[File_NamedPipeLen] == L'\0') && _wcsnicmp(path, File_NamedPipe, File_NamedPipeLen) == 0)
	{
		return NULL;
	}
...
_FX WCHAR* File_TranslateReparsePoints_2(const WCHAR* path, ULONG len, POOL* pool, ULONG PassNum)
{
	LARGE_INTEGER now;
	CACHE_PATH* entry;
	WCHAR* retpath;
...
_FX CACHE_PATH* File_TranslateReparsePoints_3(const WCHAR* path, ULONG path_len, POOL* pool, ULONG PassNum)
{
	NTSTATUS status;
	OBJECT_ATTRIBUTES objattrs;
	...
	{
		alloc_len = sizeof(CACHE_PATH) + (path_len + 1) * sizeof(WCHAR) + (dst_len + 1) * sizeof(WCHAR);
