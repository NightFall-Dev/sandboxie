...
static HANDLE* Syscall_ReplaceTargetHandle(void* UserHandleStackPtr, BOOLEAN CheckIfNullHandlePtr, HANDLE** TlsPtr_out, HANDLE* TlsValue_out);

static HANDLE Syscall_RestoreTargetHandle(void* UserHandleStackPtr, HANDLE* UserHandlePtr, HANDLE* TlsPtr, HANDLE TlsValue);

static NTSTATUS Syscall_CheckObject(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, void* OpenedObject, OBJECT_HANDLE_INFORMATION* HandleInfo);

static NTSTATUS Syscall_DuplicateHandle_2(HANDLE TargetProcessHandle, HANDLE TargetHandle, void* ExpectedTargetProcessObject, PROCESS* proc);

static SYSCALL_ENTRY* Syscall_DuplicateHandle_3(USHORT TypeLength, WCHAR* TypeBuffer);
...
_FX HANDLE* Syscall_ReplaceTargetHandle(void* UserHandleStackPtr, BOOLEAN CheckIfNullHandlePtr, HANDLE** TlsPtr_out, HANDLE* TlsValue_out)
{
	...
	RandomIndex = (ULONG)(((ULONG_PTR)PsGetCurrentThread() * time.LowPart) % 64);
...
_FX HANDLE Syscall_RestoreTargetHandle(void* UserHandleStackPtr, HANDLE* UserHandlePtr, HANDLE* TlsPtr, HANDLE TlsValue)
{
	HANDLE NewHandle;
	...
	__try
	{
		...
		ShouldBeTlsPtr = InterlockedExchangePointer(UserHandleStackPtr, UserHandlePtr);
...
_FX NTSTATUS Syscall_CheckObject(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, void* OpenedObject, OBJECT_HANDLE_INFORMATION* HandleInfo)
{
	OBJECT_NAME_INFORMATION* Name;
	ULONG NameLength;
	NTSTATUS status;

	status = Obj_GetNameOrFileName(proc->pool, OpenedObject, &Name, &NameLength);
	if (NT_SUCCESS(status))
	{
		status = syscall_entry->handler2_func(proc, OpenedObject, &Name->Name, HandleInfo->GrantedAccess);

		if ((status != STATUS_SUCCESS) && (status != STATUS_BAD_INITIAL_PC))
		{
			WCHAR msg[256];
...
_FX NTSTATUS Syscall_OpenHandle(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
	...
	UserHandlePtr = Syscall_ReplaceTargetHandle((HANDLE*)&user_args[0], FALSE, &TlsPtr, &TlsValue);
	...
	NewHandle = Syscall_RestoreTargetHandle(&user_args[0], UserHandlePtr, TlsPtr, TlsValue);
	...
	status = ObReferenceObjectByHandle(NewHandle, 0, NULL, UserMode, &OpenedObject, &HandleInfo);
	...
	if (NT_SUCCESS(status))
	{
		...
		status = Syscall_CheckObject(proc, syscall_entry, OpenedObject, &HandleInfo);
...
_FX NTSTATUS Syscall_GetNextProcess(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
...
_FX NTSTATUS Syscall_DeviceIoControlFile(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	...
	return NtDeviceIoControlFile((HANDLE)user_args[0], // FileHandle
...
_FX NTSTATUS Syscall_DuplicateHandle(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
	NTSTATUS orig_status;
	HANDLE* UserHandlePtr;
	...
	if (user_args[2])
	{ // TargetProcessHandle parameter

		status = ObReferenceObjectByHandle((HANDLE)user_args[2], 0, *PsProcessType, UserMode, &TargetProcessObject, NULL);
	...
	UserHandlePtr = Syscall_ReplaceTargetHandle((HANDLE*)&user_args[3], TRUE, &TlsPtr, &TlsValue);
	...
	NewHandle = Syscall_RestoreTargetHandle(&user_args[3], UserHandlePtr, TlsPtr, TlsValue);
	...
	else
	{
		...
		status = Syscall_DuplicateHandle_2((HANDLE)user_args[2], NewHandle, TargetProcessObject, proc);
	}
...
_FX NTSTATUS Syscall_DuplicateHandle_2(HANDLE TargetProcessHandle, HANDLE TargetHandle, void* ExpectedTargetProcessObject, PROCESS* proc)
{
	NTSTATUS status;
	...
	status = ObReferenceObjectByHandle(TargetProcessHandle, 0, *PsProcessType, UserMode, &CurrentTargetProcessObject, NULL);
	...
	status = ObReferenceObjectByHandle(TargetHandle, 0, NULL, UserMode, &OpenedObject, &HandleInfo);
	...
	else if (TypeLength == 5 * sizeof(WCHAR) && wmemcmp(TypeBuffer, L"Token", 5) == 0)
	{
		//
		...
		status = Thread_CheckTokenObject(proc, OpenedObject, HandleInfo.GrantedAccess);
	}
...
_FX SYSCALL_ENTRY* Syscall_DuplicateHandle_3(USHORT TypeLength, WCHAR* TypeBuffer)
{
	...
	if ((TypeLength == 4 && wmemcmp(TypeBuffer, _Port, 4) == 0) || (TypeLength == 9 && wmemcmp(TypeBuffer + 5, _Port, 4) == 0))
	{
	...
	while (entry)
	{
		if (entry->handler2_func && entry->name_len == name_len && memcmp(entry->name, SyscallName, name_len) == 0)
		{
			return entry;
		}
