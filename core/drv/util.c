...
_FX BOOLEAN MyIsProcessRunningAsSystemAccount(HANDLE ProcessId)
{
	...
	NTSTATUS status = Process_GetSidStringAndSessionId(NULL, ProcessId, &SidString, &SessionId);
	...
	if (NT_SUCCESS(status))
	{
		...
		system = (_wcsicmp(SidString.Buffer, Driver_S_1_5_18) == 0 || _wcsicmp(SidString.Buffer, Driver_S_1_5_19) == 0
...
_FX BOOLEAN MyIsCallerMyServiceProcess(void)
{
	...
	if (MyIsCurrentProcessRunningAsLocalSystem())
	{
		...
		if (nbuf)
		{
			...
			if ((uni->Length > Driver_HomePathNt_Len * sizeof(WCHAR)) && (0 == _wcsnicmp(uni->Buffer, Driver_HomePathNt, Driver_HomePathNt_Len)))
			{
...
_FX NTSTATUS MyGetSessionId(ULONG* SessionId)
{
	...
	len    = sizeof(info);
	status = ZwQueryInformationProcess(NtCurrentProcess(), ProcessSessionInformation, &info, sizeof(info), &len);
...
_FX NTSTATUS MyGetParentId(HANDLE* ParentId)
{
	...
	len    = sizeof(info);
	status = ZwQueryInformationProcess(NtCurrentProcess(), ProcessBasicInformation, &info, len, &len);
...
BOOLEAN DoesRegValueExist(ULONG RelativeTo, WCHAR* Path, WCHAR* ValueName)
{
	...
	qrt[0].Flags = RTL_QUERY_REGISTRY_REQUIRED | RTL_QUERY_REGISTRY_DIRECT | RTL_QUERY_REGISTRY_NOVALUE | RTL_QUERY_REGISTRY_NOEXPAND;
	qrt[0].Name         = ValueName;
	qrt[0].EntryContext = &uni;
	qrt[0].DefaultType  = REG_NONE;

	status = RtlQueryRegistryValues(RelativeTo, Path, qrt, NULL, NULL);
