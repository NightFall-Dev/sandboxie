...
_FX void* Syscall_GetMasterServiceTable(void)
{
	...
	i      = 0;
	status = ZwQuerySystemInformation(SystemModuleInformation, ptr, PAGE_SIZE, &i);
	...
	for (i = 0; i < 32; ++i)
	{
		if ((ptr[0] == 0x4A || ptr[0] == 0x4B) && ptr[1] == 0x83 && ptr[8] == 0x00)
		{
			if (ofs32a == -1)
			{
				ofs32a = *(ULONG*)&ptr[4];
	...
	if (ofs32a >= 0x400000 || ofs32b >= 0x400000)
	{
		...
		for (i = 0; i < 32; ++i)
		{
			if (ptr[0] == 0x48)
			{
				if (ptr[1] == 0x83 && ptr[2] == 0xBC && ptr[3] == 0x18 && ptr[8] == 0x00 && ofs32a == -1)
				{
					ofs32a = *(ULONG*)&ptr[4];
				}
...
{
	SERVICE_DESCRIPTOR* ShadowTable = (SERVICE_DESCRIPTOR*)Syscall_GetServiceTable();
	if (ShadowTable)
	{
