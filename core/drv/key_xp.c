...
_FX void Key_Check_KB979683(const WCHAR* KbName)
{
	...
	if (Driver_OsVersion <= DRIVER_WINDOWS_XP)
	{
		...
		if (!NT_SUCCESS(status))
		{
			...
			status = ZwCreateFile(&handle, FILE_GENERIC_READ, &objattrs, &MyIoStatusBlock, NULL, 0, FILE_SHARE_ANY, FILE_OPEN, FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
		}
...
_FX BOOLEAN Key_Init_XpHook(void)
{
	...
	if (!Obj_HookParseProc(Key_ObjectName, Key_MyParseProc, &Key_NtParseProc, &Key_JumpStub))
	{
		return FALSE;
	}
	...
	if (Driver_CheckThirdParty(L"SAVOnAccessControl", SERVICE_FILE_SYSTEM_DRIVER))
	{
...
_FX void Key_HookWaitForSingleObject(void)
{
	static UCHAR HookBytes[] = {
	    ...
	    0x44,
	    0x24,
	    0x14, // mov   eax,dword ptr [esp+5*4]
	    0x85,
	    0xC0, // test  eax,eax
	    0x74,
	    0x18, // jz    j1
	...
	if (ptr[0] != 0x8B || ptr[1] != 0xFF || (ptr[-5] != 0xCC && ptr[-5] != 0x90) || (ptr[-4] != 0xCC && ptr[-4] != 0x90) || (ptr[-3] != 0xCC && ptr[-3] != 0x90) || (ptr[-2] != 0xCC && ptr[-2] != 0x90) || (ptr[-1] != 0xCC && ptr[-1] != 0x90))
	{
		if (*(USHORT*)ptr == 0xF9EB && ptr[-5] == 0xE9)
		{
			...
			if (*tramp != 0xE8 || memcmp(tramp + 10, HookBytes, sizeof(HookBytes)) != 0)
			{
				tramp = NULL;
			}
		}
	...
	Key_WaitForSingleObject_Pid    = (ULONG*)(&(HOOK_TRAMP_CODE_TO_TRAMP_HEAD(tramp))->size);
	...
	*(ULONG*)(ptr + 1) = (ULONG)((UCHAR*)PsGetCurrentProcessId - (ptr + 5));
	ptr += 5;
