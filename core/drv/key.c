...
static BOOLEAN Key_MountHive3(PROCESS* proc, OBJECT_ATTRIBUTES* target, OBJECT_ATTRIBUTES* source);
...
_FX BOOLEAN Key_InitProcess(PROCESS* proc)
{
	...
	{
		ok = Process_GetPaths2(proc, &proc->write_key_paths, &proc->closed_key_paths, _WritePath, TRUE);
		if (ok)
		{
			ok = Process_GetPaths(proc, &proc->closed_key_paths, _WritePath, TRUE);
		}
		if (!ok)
		{
...
_FX NTSTATUS Key_MyParseProc_2(OBJ_PARSE_PROC_ARGS_2)
{
	...
	if (!IsBoxedPath)
	{
		...
		Process_MatchPath(proc->pool, Name->Name.Buffer, Name->Name.Length / sizeof(WCHAR), open_key_paths, &proc->closed_key_paths, &is_open, &is_closed);
		...
		{
			Process_MatchPath(proc->pool, Name->Name.Buffer, Name->Name.Length / sizeof(WCHAR), &proc->read_key_paths, NULL, &is_open, &is_closed);
		...
		if (status == STATUS_ACCESS_DENIED && (!proc->sbiedll_loaded) && Name->Name.Length == 17 * sizeof(WCHAR)
	...
	if (proc->key_trace & (TRACE_ALLOW | TRACE_DENY))
	{
		...
		if (letter)
		{
			swprintf(access_str, L"(K%c) %08X", letter, AccessState->OriginalDesiredAccess);
...
mount_loop:
	...
	while (mount)
	{
		...
		if (mount->root_key_len == proc->box->key_path_len && _wcsicmp(mount->root_key, proc->box->key_path) == 0)
		{
			//
			...
			if (mount->hive_path_len == hive_path_len && _wcsicmp(mount->hive_path, hive_path) == 0)
			{
				break;
			}
			...
			{
				Log_Status(MSG_MOUNT_FAILED, 0x11, STATUS_CANNOT_LOAD_REGISTRY_FILE);
				failed = TRUE;
				break;
			}
	...
	if ((!failed) && (!mount))
	{
		...
		else
		{
			...
			mount->root_key = Mem_Alloc(Driver_Pool, proc->box->key_path_len);
			if (!mount->root_key)
			{
			...
			{
				memcpy(mount->root_key, proc->box->key_path, proc->box->key_path_len);
...
_FX BOOLEAN Key_MountHive2(PROCESS* proc, KEY_MOUNT* mount)
{
	...
	InitializeObjectAttributes(&source, &hive_path_uni, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);

	RtlInitUnicodeString(&root_key_uni, mount->root_key);
	InitializeObjectAttributes(&target, &root_key_uni, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
	...
	RtlInitUnicodeString(&hivelist_uni, L"\\REGISTRY\\MACHINE\\SYSTEM\\CurrentControlSet\\Control\\hivelist");
	InitializeObjectAttributes(&hivelist, &hivelist_uni, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
	...
	if (NT_SUCCESS(status))
	{
		...
		if (NT_SUCCESS(status))
		{
			status = ZwQueryValueKey(handle, &root_key_uni, KeyValuePartialInformation, info, info_len, &info_len_dummy);
		}
		...
		if (NT_SUCCESS(status))
		{
			...
			if (kvpi->DataLength < hive_path_uni.Length
			    || _wcsnicmp((WCHAR*)kvpi->Data, hive_path_uni.Buffer, hive_path_uni.Length / sizeof(WCHAR) + 1) != 0)
			{
...
_FX BOOLEAN Key_MountHive3(PROCESS* proc, OBJECT_ATTRIBUTES* target, OBJECT_ATTRIBUTES* source)
{
	...
	old_token_dacl = Token_QueryPrimary(TokenDefaultDacl, proc->box->session_id);
	...
	{
		status = ZwOpenProcessTokenEx(NtCurrentProcess(), TOKEN_QUERY | TOKEN_ADJUST_DEFAULT, OBJ_KERNEL_HANDLE, &token);
		...
		else
		{
			...
			status = ZwSetInformationToken(token, TokenDefaultDacl, &new_token_dacl, sizeof(TOKEN_DEFAULT_DACL));
			...
			else
			{
				...
				if (STATUS_SUCCESS == ObOpenObjectByName(&objattrs, *IoFileObjectType, KernelMode, NULL, 0, NULL, &handle))
				{
					ZwClose(handle);
				}
				...
				status2 = ZwSetInformationToken(token, TokenDefaultDacl, old_token_dacl, sizeof(TOKEN_DEFAULT_DACL));
...
_FX NTSTATUS Key_Api_SetLowLabel(PROCESS* proc, ULONG64* parms)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		if (NT_SUCCESS(status))
		{
			status = ZwSetSecurityObject(handle, DACL_SECURITY_INFORMATION | LABEL_SECURITY_INFORMATION, Driver_LowLabelSd);
