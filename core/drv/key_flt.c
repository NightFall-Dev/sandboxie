...
_FX BOOLEAN Key_Init_Filter(void)
{
	...
	pCmRegisterCallbackEx = (P_CmRegisterCallbackEx)MmGetSystemRoutineAddress(&uni);
	...
	{
		status = pCmRegisterCallbackEx(Key_Callback, &Driver_Altitude, Driver_Object, NULL, &Key_Cookie, NULL);
	}
...
_FX NTSTATUS Key_Callback(void* Context, void* Arg1, void* Arg2)
{
	...
	//
	if (NotifyEvent != RegNtPreCreateKeyEx && NotifyEvent != RegNtPreOpenKeyEx)
	{
		return status;
	}
	...
	if (SearchUnicodeString(Info->CompleteName, L"CLSID\\{0002df01-0000-0000-c000-000000000046}", TRUE))
	{
		if (Conf_Get_Boolean(NULL, L"BlockIEEmbedding", 0, FALSE))
		{
			...
			if (nbuf)
			{
				if (!proc)
				{
					if ((_wcsicmp(nptr, L"winword.exe") == 0) || (_wcsicmp(nptr, L"powerpnt.exe") == 0) || (_wcsicmp(nptr, L"excel.exe") == 0) || (_wcsicmp(nptr, L"explorer.exe") == 0) || (_wcsicmp(nptr, L"svchost.exe") == 0))
					{
...
NTSTATUS Key_PreDataInject(REG_QUERY_VALUE_KEY_INFORMATION* pPreInfo, ULONG spid, PROCESS* proc)
{
	...
	if (targetName)
	{
		...
		if (rc == STATUS_SUCCESS)
		{
			__try
			{
				rc = ZwQueryValueKey(handle,
				    pPreInfo->ValueName,
				    pPreInfo->KeyValueInformationClass,
				    pPreInfo->KeyValueInformation,
				    pPreInfo->Length,
				    pPreInfo->ResultLength);
			}
			__except (EXCEPTION_EXECUTE_HANDLER)
			{
