...
static BOOLEAN Box_InitKeys(POOL* pool, BOX* box, const WCHAR* sidstring, ULONG session_id);

static BOOLEAN Box_InitConfExpandArgs(POOL* pool, BOX* box);

static BOOLEAN Box_InitPaths(POOL* pool, BOX* box);

static BOOLEAN Box_ExpandString(BOX* box, const WCHAR* model, const WCHAR* suffix, WCHAR** path, ULONG* path_len);
...
_FX BOX* Box_Alloc(POOL* pool, const WCHAR* boxname, ULONG session_id)
{
	...
	if (!box)
	{
		Log_Status_Ex_Session(MSG_BOX_CREATE, 0x10, STATUS_INSUFFICIENT_RESOURCES, boxname, session_id);
		return NULL;
	}
...
_FX BOX* Box_Create(POOL* pool, const WCHAR* boxname, BOOLEAN init_paths)
{
	...
	ULONG SessionId;
	NTSTATUS status = Process_GetSidStringAndSessionId(NtCurrentProcess(), NULL, &SidString, &SessionId);

	if (NT_SUCCESS(status))
	{
		box = Box_CreateEx(pool, boxname, SidString.Buffer, SessionId, init_paths);
...
_FX BOOLEAN Box_InitKeys(POOL* pool, BOX* box, const WCHAR* sidstring, ULONG session_id)
{
	//
	// copy sidstring
	//
	...
	if (!box->sid)
	{
		Log_Status_Ex_Session(MSG_BOX_CREATE, 0x12, STATUS_INSUFFICIENT_RESOURCES, box->name, session_id);
		return FALSE;
	}
...
_FX BOOLEAN Box_InitConfExpandArgs(POOL* pool, BOX* box)
{
	...
	if (!box->expand_args)
	{
		Log_Status_Ex_Session(MSG_BOX_CREATE, 0x13, STATUS_INSUFFICIENT_RESOURCES, box->name, box->session_id);
		return FALSE;
	}
...
{
	static const WCHAR* _FileRootPath_Default = L"\\??\\%SystemDrive%\\Sandbox\\%USER%\\%SANDBOX%";
	static const WCHAR* _KeyRootPath_Default  = L"\\REGISTRY\\USER\\Sandbox_%USER%_%SANDBOX%";
	static const WCHAR* _IpcRootPath_Default  = L"\\Sandbox\\%USER%\\%SANDBOX%\\Session_%SESSION%";
	...
	ok = Box_ExpandString(box, value, suffix, &box->file_path, &box->file_path_len);
	if (!ok)
	{
		Log_Status_Ex_Session(MSG_BOX_CREATE, 0x21, STATUS_UNSUCCESSFUL, box->name, box->session_id);
		return FALSE;
	}
	...
	if (!ok)
	{
		Log_Status_Ex_Session(MSG_BOX_CREATE, 0x22, STATUS_UNSUCCESSFUL, box->name, box->session_id);
		return FALSE;
	}
	...
	ok = Box_ExpandString(box, value, suffix, &box->key_path, &box->key_path_len);
	if (!ok)
	{
		Log_Status_Ex_Session(MSG_BOX_CREATE, 0x23, STATUS_UNSUCCESSFUL, box->name, box->session_id);
		return FALSE;
	}
	...
	ok = Box_ExpandString(box, value, suffix, &box->ipc_path, &box->ipc_path_len);
	if (!ok)
	{
		Log_Status_Ex_Session(MSG_BOX_CREATE, 0x24, STATUS_UNSUCCESSFUL, box->name, box->session_id);
		return FALSE;
	}
	...
	if (!box->pipe_path)
	{
		Log_Status_Ex_Session(MSG_BOX_CREATE, 0x25, STATUS_UNSUCCESSFUL, box->name, box->session_id);
		return FALSE;
	}
...
_FX BOOLEAN Box_ExpandString(BOX* box, const WCHAR* model, const WCHAR* suffix, WCHAR** path, ULONG* path_len)
{
...
#define CLONE_MEMBER(m)                                                                                                 \
	...
	{                                                                                                                   \
		...
		if (!box->m)                                                                                                    \
		{                                                                                                               \
			Log_Status_Ex_Session(MSG_BOX_CREATE, 0x20, STATUS_INSUFFICIENT_RESOURCES, model->name, model->session_id); \
			Box_Free(box);                                                                                              \
			return NULL;                                                                                                \
		}                                                                                                               \
...
_FX BOOLEAN Box_IsBoxedPath_Helper(UNICODE_STRING* uni, const WCHAR* box_path, ULONG box_path_len)
{
	...
	if (uni->Buffer[box_path_len] != L'\\' && uni->Buffer[box_path_len] != L'\0')
	{
		return FALSE;
	}
