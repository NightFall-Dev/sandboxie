...
static FLT_PREOP_CALLBACK_STATUS File_PreOperation(PFLT_CALLBACK_DATA Data, PCFLT_RELATED_OBJECTS FltObjects, void** CompletionContext);

static NTSTATUS File_CreateOperation(PROCESS* proc, FLT_IO_PARAMETER_BLOCK* Iopb, UNICODE_STRING* FileName);

static NTSTATUS File_RenameOperation(PROCESS* proc, FLT_IO_PARAMETER_BLOCK* Iopb);

static NTSTATUS File_QueryTeardown(PCFLT_RELATED_OBJECTS FltObjects, FLT_INSTANCE_QUERY_TEARDOWN_FLAGS Flags);

static NTSTATUS File_CheckFileObject(PROCESS* proc, void* Object, UNICODE_STRING* NameString, ACCESS_MASK GrantedAccess);

static void File_CheckFontAccess(PFLT_CALLBACK_DATA Data, FLT_IO_PARAMETER_BLOCK* Iopb, NTSTATUS* out_status);
...
static const FLT_OPERATION_REGISTRATION File_Callbacks[] = {
    ...
    FILE_CALLBACK(IRP_MJ_CREATE) FILE_CALLBACK(IRP_MJ_CREATE_NAMED_PIPE) FILE_CALLBACK(IRP_MJ_CREATE_MAILSLOT) FILE_CALLBACK(IRP_MJ_SET_INFORMATION)
...
_FX BOOLEAN File_Init_Filter(void)
{
	...
	status = FltRegisterFilter(Driver_Object, &File_Registration, &File_FilterCookie);
...
#define SBIE_FILE_GENERIC_WRITE (FILE_WRITE_DATA | FILE_WRITE_ATTRIBUTES | FILE_WRITE_EA | FILE_APPEND_DATA)


_FX FLT_PREOP_CALLBACK_STATUS File_PreOperation(PFLT_CALLBACK_DATA Data, PCFLT_RELATED_OBJECTS FltObjects, void** CompletionContext)
{
	NTSTATUS status;
	PROCESS* proc;
	...
	{
		if (Iopb->MajorFunction == IRP_MJ_CREATE && Data->Thread == PsGetCurrentThread())
		{
			BOOLEAN bSetDirty = FALSE;

			PFILE_OBJECT FileObject = Iopb->TargetFileObject;

			File_ReplaceTokenIfFontRequest(Iopb->Parameters.Create.SecurityContext->AccessState,
	...
	else if (Iopb->MajorFunction != IRP_MJ_CREATE && Iopb->MajorFunction != IRP_MJ_CREATE_NAMED_PIPE && Iopb->MajorFunction != IRP_MJ_CREATE_MAILSLOT)
	{
		goto finish;
	}
	...
	if (Iopb->MajorFunction == IRP_MJ_CREATE && Iopb->Parameters.Create.SecurityContext
	    && (Iopb->Parameters.Create.SecurityContext->DesiredAccess & SBIE_FILE_GENERIC_WRITE) && MyIsProcessRunningAsSystemAccount(PsGetCurrentProcessId()))
	{
		void* nbuf  = 0;
		ULONG nlen  = 0;
		WCHAR* nptr = 0;
		...
		if (nbuf)
		{
			    ...
			    !SearchUnicodeString(&Iopb->TargetFileObject->FileName, L"\\pipe\\spoolss", TRUE))
			{
				...
				if (ulOwnerPid)
				{
					...
					if (proc && proc != PROCESS_TERMINATED && !proc->m_boolAllowSpoolerPrintToFile) // if process specifically allowed to use spooler print to file, we can skip everything below
					{
						...
						else if (!UnicodeStringStartsWith(&pTargetFileNameInfo->Name, proc->box->spooler_directory, TRUE)) // spooler work folder is allowed.
						{
							...
							else
							{
								...
								usTargetFile.MaximumLength =
								    pTargetFileNameInfo->Name.MaximumLength - pTargetFileNameInfo->Volume.Length;
			...
			if (!UnicodeStringStartsWith(&Data->Iopb->TargetFileObject->FileName, L"\\", TRUE)) // just a filename and not a full path?
			{
				...
				if (FltGetFileNameInformation(Data, FLT_FILE_NAME_NORMALIZED, &pTargetFileNameInfo) == STATUS_SUCCESS)
				{
					if (pTargetFileNameInfo)
					{
						...
						nTempFullPathLen = pTargetFileNameInfo->Name.Length + Data->Iopb->TargetFileObject->FileName.Length
						    - pTargetFileNameInfo->Volume.Length + 4;
...
_FX NTSTATUS File_CreateOperation(PROCESS* proc, FLT_IO_PARAMETER_BLOCK* Iopb, UNICODE_STRING* FileName)
{
	...
	MyContext.OriginalDesiredAccess = Parms->Create.SecurityContext->AccessState->OriginalDesiredAccess;

	DeviceObject = FileObject->DeviceObject;

	status = File_Generic_MyParseProc(proc, DeviceObject, DeviceObject->DeviceType, FileName, &MyContext, TRUE);
...
_FX NTSTATUS File_RenameOperation(PROCESS* proc, FLT_IO_PARAMETER_BLOCK* Iopb)
{
	FLT_PARAMETERS* Parms;
	...
	if (FileObject->FileName.Length >= 2 && FileObject->FileName.Buffer && FileObject->FileName.Buffer[0] != L'\\')
	{
		PFILE_OBJECT RelatedFileObject = FileObject->RelatedFileObject;
		if (RelatedFileObject && RelatedFileObject->FileName.Length >= 2 && RelatedFileObject->FileName.Buffer
	...
	return File_Generic_MyParseProc(proc, FileObject, FileObject->DeviceObject->DeviceType, &FileName, &MyContext, FALSE);
}
...
_FX NTSTATUS File_QueryTeardown(PCFLT_RELATED_OBJECTS FltObjects, FLT_INSTANCE_QUERY_TEARDOWN_FLAGS Flags)
{
	//
...
_FX NTSTATUS File_CheckFileObject(PROCESS* proc, void* Object, UNICODE_STRING* NameString, ACCESS_MASK GrantedAccess)
{
	NTSTATUS status;
	...
	if (!proc->sbiedll_loaded)
	{
		...
		if (Backslash && Underscore && _wcsicmp(Underscore, L"_kl1") == 0 && _wcsnicmp(Backslash, L"\\swmon_", 7) == 0)
		{
	...
	if (FileObject->DeviceObject->DeviceType != FILE_DEVICE_NAMED_PIPE && FileObject->DeviceObject->DeviceType != FILE_DEVICE_MAILSLOT)
	{
		//
	...
	status = Obj_GetParseName(proc->pool, FileObject, &FileName, &Name, &NameLength);
	if (NT_SUCCESS(status) && (Name != &Obj_Unnamed))
	{
		BOOLEAN OnlyDeviceName = FALSE;
		ULONG len              = wcslen(Name->Name.Buffer);
		if (FileObject->DeviceObject->DeviceType == FILE_DEVICE_NAMED_PIPE && len == 17) /* \Device\NamedPipe */
		{
			OnlyDeviceName = TRUE;
		}
		else if (FileObject->DeviceObject->DeviceType == FILE_DEVICE_MAILSLOT && len == 16) /* \Device\MailSlot */
		{
			OnlyDeviceName = TRUE;
		}
