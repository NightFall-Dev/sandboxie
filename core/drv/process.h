...
void Process_NotifyProcess_Create(HANDLE ProcessId, HANDLE ParentId, BOX* box);
...
BOOLEAN Process_GetPaths(PROCESS* proc, LIST* list, const WCHAR* setting_name, BOOLEAN AddStar);


// Process_GetPaths2:  similar to Process_GetPaths, but adds the path
// only if it does not already match the second path-list

BOOLEAN Process_GetPaths2(PROCESS* proc, LIST* list, LIST* list2, const WCHAR* setting_name, BOOLEAN AddStar);
...
BOOLEAN Process_AddPath(PROCESS* proc, LIST* list, const WCHAR* setting_name, BOOLEAN AddFirst, const WCHAR* value, BOOLEAN AddStar);
...
NTSTATUS Process_CheckProcessName(PROCESS* proc, LIST* open_paths, ULONG_PTR idProcess, const WCHAR** pSetting);
...
BOX* Process_GetForcedStartBox(HANDLE ProcessId, HANDLE ParentId, const WCHAR* ImagePath, BOOLEAN bHostInject);
