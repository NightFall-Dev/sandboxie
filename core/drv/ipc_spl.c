...
_FX NTSTATUS Ipc_Api_GetRpcPortFromPid(enum ENUM_DYNAMIC_PORT_TYPE ePortType, PROCESS* proc, ULONG64* parms)
{
	...
	if (NT_SUCCESS(status))
	{
		if (PsGetProcessSessionId(ProcessObject) == 0)
		{
			...
			Process_GetProcessName(Driver_Pool, (ULONG_PTR)pArgs->process_id.val, &nbuf, &nlen, &nptr);

			if (nbuf)
			{
				if (_wcsicmp(nptr, pArgs->exe_name.val) == 0 && MyIsProcessRunningAsSystemAccount(pArgs->process_id.val))
				{
...
_FX NTSTATUS Ipc_Api_GetRpcPortName_2(enum ENUM_DYNAMIC_PORT_TYPE ePortType, PEPROCESS ProcessObject, WCHAR* pDstPortName)
{
	...
	while (1)
	{
		...
		dummy_len = 0;
		status    = ZwQueryDirectoryObject(handle, buf, len, FALSE, TRUE, &context, &dummy_len);
	...
	while (1)
	{
		...
		if (TypeName->Length == 9 * sizeof(WCHAR) && TypeName->Buffer && _wcsicmp(TypeName->Buffer, L"ALPC Port") == 0)
		{
			if ((ObjName->Length > 5 * sizeof(WCHAR)) && (ObjName->Length < 64 * sizeof(WCHAR)) && _wcsnicmp(ObjName->Buffer, L"LRPC-", 5) == 0)
			{
				swprintf(name, L"%s\\%s", _rpc_control, ObjName->Buffer);

				RtlInitUnicodeString(&objname, name);

				status = ObReferenceObjectByName(&objname, OBJ_CASE_INSENSITIVE, NULL, 0, *LpcPortObjectType, KernelMode, NULL, &PortObject);
