...
#define F_MAKE_COMBO(dataSize, addrSize, rex) \
	((((dataSize) & 0xFF)) | (((addrSize) & 0xFF) << 8) | (((rex) & 0xFF) << 16))
...
static UCHAR* Hook_Analyze_ModRM(UCHAR* modrm, HOOK_INST* inst);

static UCHAR* Hook_Analyze_Prefix(UCHAR* addr, BOOLEAN is64, ULONG* flags);
...
ALIGNED BOOLEAN Hook_Analyze(void* address, BOOLEAN probe_address, BOOLEAN is64, HOOK_INST* inst)
{
	UCHAR* addr;
	WCHAR text[64];
...
ALIGNED UCHAR* Hook_Analyze_Inst(UCHAR* addr, HOOK_INST* inst)
{
	...
	{
	case 1: return Hook_Analyze_ModRM(addr + 1, inst);
	...
	case 4: return Hook_Analyze_ModRM(addr + 1, inst) + 1;

	case 5: return Hook_Analyze_ModRM(addr + 1, inst) + F_DATA_SIZE();

	case 6: return addr + 1;

	case 7: return addr + 1 + F_ADDR_SIZE();
	}
...
ALIGNED UCHAR* Hook_Analyze_CtlXfer(UCHAR* addr, HOOK_INST* inst)
{
	...
	if (op1 == 0xEA)
	{
		...
		if (seg == 0x001B || seg == 0x0023)
		{                              // application CS == 0x001B
...
ALIGNED UCHAR* Hook_Analyze_ModRM(UCHAR* modrm, HOOK_INST* inst)
{
	...
	if (disp != 0xFF)
	{
		...
		else if ((disp & 0xC0) == 0x80 || (disp & 7) == 5)
		{
