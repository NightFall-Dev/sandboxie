...
static void* Dll_GetProc2(DLL_ENTRY* dll, const UCHAR* ProcName, BOOLEAN returnOffset);
...
_FX DLL_ENTRY* Dll_Load(const WCHAR* DllBaseName)
{
	...
	status = ZwCreateFile(&dll->hFile, FILE_GENERIC_READ, &objattrs, &MyIoStatusBlock, NULL, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN, FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	...
	status = ZwQueryInformationFile(dll->hFile, &MyIoStatusBlock, &info, sizeof(FILE_STANDARD_INFORMATION), FileStandardInformation);
	if (!NT_SUCCESS(status))
	{
		err = 0x13;
		goto finish;
	...
	status = ZwCreateSection(&dll->hSection, SECTION_MAP_READ | SECTION_QUERY, NULL, &info.EndOfFile, PAGE_READONLY, SEC_RESERVE, dll->hFile);
	if (!NT_SUCCESS(status))
	{
		dll->hSection = NULL;
		err           = 0x14;
	...
	status = ZwMapViewOfSection(dll->hSection, NtCurrentProcess(), &dll->base, 0, 0, 0, &mapsize, ViewUnmap, 0, PAGE_READONLY);
	if (!NT_SUCCESS(status))
	{
		dll->base = NULL;
		err       = 0x15;
	...
	dll->exports = (IMAGE_EXPORT_DIRECTORY*)Dll_RvaToAddr(dll, data_dirs[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress);
...
_FX void* Dll_RvaToAddr(DLL_ENTRY* dll, ULONG rva)
{
	...
	for (i = 0; i < dll->nthdr->FileHeader.NumberOfSections; ++i)
	{
		...
		{
			void* addr = dll->base + rva - section->VirtualAddress + section->PointerToRawData;
			return addr;
		}
...
_FX void* Dll_GetProc(const WCHAR* DllName, const UCHAR* ProcName, BOOLEAN returnOffset)
{
	void* proc = NULL;
...
_FX ULONG Dll_GetNextProc(DLL_ENTRY* dll, const UCHAR* SearchName, UCHAR** FoundName, ULONG* FoundIndex)
{
