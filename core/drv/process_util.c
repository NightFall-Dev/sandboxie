...
static BOOLEAN Process_MatchImageGroup(BOX* box, const WCHAR* group, ULONG group_len, const WCHAR* test_str, ULONG depth);

static BOOLEAN Process_AddPath_2(PROCESS* proc, LIST* list, const WCHAR* value, const WCHAR* setting_name, BOOLEAN AddFirst, BOOLEAN AddStar, BOOLEAN RemoveBackslashes, BOOLEAN CheckReparse);
...
_FX BOOLEAN Process_IsSameBox(PROCESS* proc, PROCESS* proc2, ULONG_PTR proc2_pid)
{
	KIRQL irql;
	BOOLEAN ok;
	BOOLEAN locked;
	...
	else if (proc2 && (!proc2->terminated) && (!proc2->untouchable))
	{
		...
		ok = (proc->box->session_id == proc2->box->session_id) && (proc->box->name_len == proc2->box->name_len)
...
_FX BOOLEAN Process_MatchImage(BOX* box, const WCHAR* pat_str, ULONG pat_len, const WCHAR* test_str, ULONG depth)
{
	PATTERN* pat;
	WCHAR *tmp, *expnd;
	ULONG tmp_len;
	...
	if (*pat_str == L'<')
	{
		...
		ok = Process_MatchImageGroup(box, Pattern_Source(pat), 0, test_str, depth + 1);
...
_FX BOOLEAN Process_MatchImageGroup(BOX* box, const WCHAR* group, ULONG group_len, const WCHAR* test_str, ULONG depth)
{
	ULONG index;
	BOOLEAN match = FALSE;
	...
	for (index = 0; (!match); ++index)
	{
		...
		while (*value)
		{
			...
			if (value_len)
			{
				if (*value != L'<')
				{
					match = Process_MatchImage(box, value, value_len, test_str, depth + 1);
				}
				else if (depth < 6)
				{
					match = Process_MatchImageGroup(box, value, value_len, test_str, depth + 1);
				}
				if (match)
				{
					break;
...
_FX BOOLEAN Process_GetPaths(PROCESS* proc, LIST* list, const WCHAR* setting_name, BOOLEAN AddStar)
{
	ULONG index;
	const WCHAR* value;
	BOOLEAN ok = TRUE;
	...
	for (index = 0;; ++index)
	{
		...
		if (!Process_AddPath(proc, list, setting_name, FALSE, value, AddStar))
		{
			ok = FALSE;
			break;
		}
...
_FX BOOLEAN Process_GetPaths2(PROCESS* proc, LIST* list, LIST* list2, const WCHAR* setting_name, BOOLEAN AddStar)
{
	LIST dummy_list;
	PATTERN* pat;
	const WCHAR* value;
	...
	while (1)
	{
		...
		{
			Process_MatchPath(proc->pool, value, len, NULL, list2, &is_open, &is_closed);
		}
...
_FX BOOLEAN Process_AddPath(PROCESS* proc, LIST* list, const WCHAR* setting_name, BOOLEAN AddFirst, const WCHAR* value, BOOLEAN AddStar)
{
	WCHAR* tmp;
	ULONG len;
	...
	if (setting_name)
	{
		...
		else if (_wcsnicmp(setting_name, Process_Read, 4) == 0 || _wcsnicmp(setting_name, Process_Open, 4) == 0)
		{
		...
		if (setting_name_ptr)
		{
			...
			else if (_wcsnicmp(setting_name_ptr, L"File", 4) == 0 || _wcsnicmp(setting_name_ptr, L"Pipe", 4) == 0)
			{
				RemoveBackslashes = TRUE;
				CheckReparse      = TRUE;
			}
	...
	if (tmp)
	{
		...
		if (len)
		{
			match = Process_MatchImage(proc->box, value, len, proc->image_name, 1);
			if (inv)
			{
				match = !match;
			}
	...
	ok = Process_AddPath_2(proc, list, value, setting_name, AddFirst, AddStar, RemoveBackslashes, CheckReparse);
	...
	if (ok && CheckReparse && (value[0] == L'?' || value[0] == L'*') && (value[1] == L':'))
	{
		...
		{
			*tmp = (WCHAR)len;
			ok   = Process_AddPath_2(proc, list, tmp, setting_name, AddFirst, AddStar, RemoveBackslashes, CheckReparse);
		}
		Mem_FreeString(tmp);
	}
...
_FX BOOLEAN Process_AddPath_2(PROCESS* proc, LIST* list, const WCHAR* value, const WCHAR* setting_name, BOOLEAN AddFirst, BOOLEAN AddStar, BOOLEAN RemoveBackslashes, BOOLEAN CheckReparse)
{
	PATTERN* pat;
	WCHAR *expand, *tmp;
	ULONG len;
...
_FX const WCHAR* Process_MatchPath(POOL* pool, const WCHAR* path, ULONG path_len, LIST* open_list, LIST* closed_list, BOOLEAN* is_open, BOOLEAN* is_closed)
{
	PATTERN* pat;
	WCHAR* path_lwr;
	ULONG path_lwr_len;
...
_FX void Process_GetProcessName(POOL* pool, ULONG_PTR idProcess, void** out_buf, ULONG* out_len, WCHAR** out_ptr)
{
	NTSTATUS status;
	...
	*out_ptr = NULL;
	...
	status = ZwQueryInformationProcess(handle, ProcessImageFileName, NULL, 0, &len);
	...
	if (status == STATUS_INFO_LENGTH_MISMATCH)
	{
		...
		if (uni)
		{
			...
			status = ZwQueryInformationProcess(handle, ProcessImageFileName, uni, len + 8, &len);
...
_FX NTSTATUS Process_CheckProcessName(PROCESS* proc, LIST* open_paths, ULONG_PTR idProcess, const WCHAR** pSetting)
{
	NTSTATUS status;
	PATTERN* pat;
	void* nbuf;
...
_FX NTSTATUS Process_GetSidStringAndSessionId(HANDLE ProcessHandle, HANDLE ProcessId, UNICODE_STRING* SidString, ULONG* SessionId)
{
	NTSTATUS status;
	...
	{
		const KPROCESSOR_MODE AccessMode = ((ProcessHandle == NtCurrentProcess()) ? KernelMode : UserMode);

		status = ObReferenceObjectByHandle(ProcessHandle, 0, *PsProcessType, AccessMode, &ProcessObject, NULL);
	}
	else if (ProcessId)
	{
...
_FX void Process_TrackProcessLimit(PROCESS* proc)
{
	...
	Process_Enumerate(proc->box->name, FALSE, proc->box->session_id, NULL, &v);
...
_FX BOOLEAN Process_IsInPcaJob(HANDLE ProcessId)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		if (JobObject)
		{
			status = Obj_GetName(Driver_Pool, (void*)JobObject, &Name, &NameLength);
			if (NT_SUCCESS(status) && (Name != &Obj_Unnamed))
			{
				if (Name->Name.Length == 60 * sizeof(WCHAR) && 0 == _wcsnicmp(Name->Name.Buffer, L"\\BaseNamedObjects\\PCA_", 22))
				{
					IsInPcaJob = TRUE;
				}
			...
			else if (NT_SUCCESS(status) && (Name == &Obj_Unnamed) && Driver_OsVersion >= DRIVER_WINDOWS_8)
			{
				//
