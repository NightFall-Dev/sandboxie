...
ULONG findFlagOffset(UCHAR* ptr)
{
	...
	if (c)
	{
		...
		if (instrSize)
		{
			c = c + instrSize;
			if (ptr[c] == 0xf7)
			{ //test [reg + flag_offset], 4 byte mask
				offset = *(ULONG*)&ptr[c + 2];
			}
			else if (*(USHORT*)&ptr[c] == 0xba0f)
			{ //bt [reg + flag_offset],1 byte value
...
PSYSTEM_SERVICE_TABLE GetShadowTableAddress()
{
	...
	for (i = 0; i < 1024; i++)
	{
		...
		if (!MmIsAddressValid(pTable) || ((PVOID)pTable == (PVOID)&KeServiceDescriptorTable) || (memcmp(pTable, &KeServiceDescriptorTable, sizeof(SYSTEM_SERVICE_TABLE))))
		{
			pCheckByte++;
			pTable = NULL;
		}
...
_FX void* Syscall_GetServiceTable(void)
{
	...
	if (Driver_OsVersion >= DRIVER_WINDOWS_XP && Driver_OsVersion <= DRIVER_WINDOWS_2003)
	{
		...
		if (ShadowTable[0] != MasterTable[0] && Driver_OsVersion == DRIVER_WINDOWS_2003)
		{
...
_FX ULONG Syscall_GetIndexFromNtdll(UCHAR* code, const UCHAR* name, ULONG name_len)
{
	ULONG index = -1;
...
_FX BOOLEAN Syscall_GetKernelAddr(ULONG index, void** pKernelAddr, ULONG* pParamCount)
{
	ULONG* ServiceTable = Syscall_GetServiceTable();
	if (ServiceTable)
	{
		USHORT MaxSyscallIndexPlusOne = (USHORT)ServiceTable[2];
		if ((index < 0x1000) && ((index & 0xFFF) < MaxSyscallIndexPlusOne))
		{
...
_FX void Syscall_HandleOADriver(void)
{
	...
	if (Driver_CheckThirdParty(L"OADevice", SERVICE_FILE_SYSTEM_DRIVER))
	{
		...
		UCHAR* OADriverApisToFix[] = {"OpenSection", "ConnectPort", "SecureConnectPort", NULL};
		ULONG i;
		...
		for (i = 0; OADriverApisToFix[i]; ++i)
		{
			...
			if (entry)
			{
				void* new_ntos_func = Syscall_HandleOADriver_2(entry->ntos_func);
				if (new_ntos_func)
				{
...
_FX void* Syscall_HandleOADriver_2(void* ntos_func)
{
	...
	for (i = 0; i < 48; ++i)
	{
		...
		if (jump_inst && *(USHORT*)jump_inst == 0x840F)
		{ // je
			...
			ULONG jump_target = *(ULONG*)(jump_inst + 2) + (ULONG)jump_inst + 6;
			if (!jump_target_1)
			{
