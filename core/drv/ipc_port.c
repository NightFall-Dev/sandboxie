...
static NTSTATUS Ipc_CheckPortRequest(PROCESS* proc, HANDLE PortHandle, PORT_MESSAGE* msg);

static NTSTATUS Ipc_CheckPortRequest_WinApi(PROCESS* proc, OBJECT_NAME_INFORMATION* Name, PORT_MESSAGE* msg);

static NTSTATUS Ipc_CheckPortRequest_Lsa(PROCESS* proc, OBJECT_NAME_INFORMATION* Name, PORT_MESSAGE* msg);

static NTSTATUS Ipc_CheckPortRequest_PowerManagement(PROCESS* proc, OBJECT_NAME_INFORMATION* Name, PORT_MESSAGE* msg);
...
_FX void* Ipc_GetServerPort(void* Object)
{
	...
	{
		port_object = ((struct LPC_PORT_OBJECT_XP_2003*)Object)->ConnectionPort;
	}
	else if (Driver_OsVersion >= DRIVER_WINDOWS_VISTA)
	{
		port_object = *(((struct ALPC_PORT_OBJECT_VISTA*)Object)->ConnectionPortPtr);
	}
...
_FX NTSTATUS Ipc_CheckPortRequest(PROCESS* proc, HANDLE PortHandle, PORT_MESSAGE* msg)
{
	NTSTATUS status;
	...
	status = ObReferenceObjectByHandle(PortHandle, 0, *LpcPortObjectType, UserMode, &client_port_object, NULL);
...
_FX NTSTATUS Ipc_CheckPortRequest_PowerManagement(PROCESS* proc, OBJECT_NAME_INFORMATION* Name, PORT_MESSAGE* msg)
{
	NTSTATUS Status;
...
_FX NTSTATUS Ipc_CheckPortRequest_WinApi(PROCESS* proc, OBJECT_NAME_INFORMATION* Name, PORT_MESSAGE* msg)
{
	...
	name_ptr = Name->Name.Buffer + Name->Name.Length / sizeof(WCHAR) - 16;
	...
	__try
	{
		...
		if (msg->u1.s1.TotalLength >= sizeof(WINAPI_MESSAGE))
		{
			...
			if (msg2->api_code == WINAPI_EXITWINDOWSEX || msg2->api_code == WINAPI_ENDTASK)
			{
				status = STATUS_ACCESS_DENIED;
			}
			...
			// MS11-063
			if (((Driver_OsVersion == DRIVER_WINDOWS_XP || Driver_OsVersion == DRIVER_WINDOWS_VISTA) && msg2->api_code == WINAPI_SRVDEVICEEVENT) || (Driver_OsVersion == DRIVER_WINDOWS_7 && msg2->api_code == WINAPI_SRVDEVICEEVENT_WIN7))
			{
				Log_Msg0(MSG_1316)
					;
					status = STATUS_ACCESS_DENIED;
...
_FX NTSTATUS Ipc_CheckPortRequest_Lsa(PROCESS* proc, OBJECT_NAME_INFORMATION* Name, PORT_MESSAGE* msg)
{
	NTSTATUS status;
	...
	__try
	{
		...
		if (Driver_OsVersion >= DRIVER_WINDOWS_VISTA)
		{
			...
			while (len > 9 + 1)
			{
				if (ptr[0] == L'N' && ptr[9] != 0 && wmemcmp(ptr, L"Negotiate", 9) == 0)
				{
					status = STATUS_ACCESS_DENIED;
					break;
				}
		...
		else
		{
			...
			if (msg->u1.s1.TotalLength >= sizeof(LSA_MESSAGE_XP))
			{
				...
				ProbeForRead(msg2, sizeof(LSA_MESSAGE_XP), sizeof(ULONG_PTR));

				if (msg2->api_code == 2 && // LsaCallAuthenticationPackage
				    msg2->auth_pkg_code == Ipc_MSV10_AuthPackageNumber && msg2->buf_len >= sizeof(ULONG))
				{
					ULONG* buf = msg2->buf;
...
_FX NTSTATUS Ipc_ImpersonatePort(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
...
_FX NTSTATUS Ipc_RequestPort(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
...
_FX NTSTATUS Ipc_AlpcSendWaitReceivePort(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	HANDLE PortHandle     = (HANDLE*)user_args[0];
...
_FX NTSTATUS Ipc_NtRequestPort(HANDLE PortHandle, void* RequestMessage)
{
...
_FX NTSTATUS Ipc_NtRequestWaitReplyPort(HANDLE PortHandle, void* RequestMessage, void* ReplyMessage)
{
	IPC_PORT_HEADER(RequestMessage);
	if (NT_SUCCESS(status))
	{
		status = __sys_NtRequestWaitReplyPort(PortHandle, RequestMessage, ReplyMessage);
	}
	return status;
}
