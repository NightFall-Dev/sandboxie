...
_FX BOOLEAN Gui_InitProcess(PROCESS* proc)
{
	...
	static const WCHAR* openclasses[] = {L"Shell_TrayWnd",
	    L"TrayNotifyWnd",
	...
	ok = Process_GetPaths(proc, &proc->open_win_classes, Gui_OpenClass_Name, FALSE);

	if (ok)
	{
		for (i = 0; openclasses[i] && ok; ++i)
		{
			ok = Process_AddPath(proc, &proc->open_win_classes, NULL, TRUE, openclasses[i], FALSE);
		}
	}
	...
	if (ok)
	{
		...
		{
			ok = Process_AddPath(proc, &proc->open_win_classes, NULL, TRUE, L"Sandbox:*:ConsoleWindowClass", FALSE);
			AddMSTaskSwWClass = TRUE;
		}
		else if ((!proc->image_copy) && (_wcsicmp(proc->image_name, L"excel.exe") == 0 || _wcsicmp(proc->image_name, L"powerpnt.exe") == 0))
		{
			AddMSTaskSwWClass = TRUE;
		}
		if (ok && AddMSTaskSwWClass)
		{
			ok = Process_AddPath(proc, &proc->open_win_classes, NULL, TRUE, L"MSTaskSwWClass", FALSE);
		}
	}
...
_FX void Gui_Check_OpenWinClass(PROCESS* proc)
{
	...
	while (1)
	{
		const WCHAR* value = Conf_Get(proc->box->name, Gui_OpenClass_Name, (index | CONF_GET_NO_EXPAND | CONF_GET_NO_TEMPLS));
		if (!value)
		{
			break;
		}
...
_FX GUI_CLIPBOARD* Gui_GetClipboard(void)
{
	...
	{ // Covers Win 8 up through Win 10-18980
	...
	status = ObReferenceObjectByHandle(WinStaHandle, 0, *ExWindowStationObjectType, KernelMode, &WinStaObject, NULL);
...
{
	if (Gui_ClipboardIntegrityIndex != -1)
	{ // do nothing if UIPI is off
		...
		for (i = 0; i < Clipboard->count; ++i)
		{
			...
			if ((il == 0x0000) || (il == 0x1000) || (il == 0x2000) || (il == 0x3000) || (il == 0x4000))
			{
