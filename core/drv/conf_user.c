...
static BOOLEAN Conf_GetUserNameForSid(const WCHAR* SidString, ULONG SessionId, WCHAR* varvalue);
...
_FX BOOLEAN Conf_GetUserNameForSid(const WCHAR* SidString, ULONG SessionId, WCHAR* varvalue)
{
	...
	for (retries = 0; (retries < 40) && (!Driver_Unloading); ++retries)
	{
		...
		while (user1)
		{
			if (user1->sid_len == sid_len && _wcsicmp(user1->sid, SidString) == 0)
			{
				break;
			}
		...
		if (!message_sent)
		{
			ULONG msg_len = sizeof(SVC_LOOKUP_SID_MSG) + (sid_len + 4) * sizeof(WCHAR);
			...
			message_sent = Api_SendServiceMessage(SVC_LOOKUP_SID, msg_len, msg);
		...
		KeWaitForSingleObject(Conf_Users_Event, Executive, KernelMode, FALSE, &time);
	}
...
_FX BOOLEAN Conf_GetGroupsForSid(WCHAR* PageSizeBuffer, ULONG SessionId)
{
	...
	if (NT_SUCCESS(status))
	{
		for (i = 0; i < groups->GroupCount; ++i)
		{
			...
			if (attrs & (SE_GROUP_USE_FOR_DENY_ONLY | SE_GROUP_LOGON_ID | SE_GROUP_INTEGRITY | SE_GROUP_INTEGRITY_ENABLED))
			{
				continue;
			}
			...
			status = RtlConvertSidToUnicodeString(&uni, groups->Groups[i].Sid, FALSE);
		...
		if (Driver_OsVersion >= DRIVER_WINDOWS_VISTA && (bufptr - PageSizeBuffer < 1024))
		{
...
_FX BOOLEAN Conf_IsBoxEnabled(const WCHAR* BoxName, const WCHAR* SidString, ULONG SessionId)
{
	const WCHAR* value;
	WCHAR* buffer;
	BOOLEAN enabled;
...
NTSTATUS Conf_Api_SetUserName(PROCESS* proc, ULONG64* parms)
{
	...
	while (user1)
	{
		if (user1->sid_len == user->sid_len && _wcsicmp(user1->sid, user->sid) == 0)
		{
