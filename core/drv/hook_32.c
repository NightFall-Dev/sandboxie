...
_FX ULONG Hook_Find_ZwRoutine_1(ULONG ServiceNum, void** out_routine)
{
	...
	while (addr != last_routine)
	{
		...
		if (*(ULONG*)addr == 0x08C2C033 && addr[4] == 0x00)
		{
			...
			for (i = 5; i < 20; ++i)
			{
				if (addr[i + 0] == 0xC2 && addr[i + 1] == 0x08 && addr[i + 2] == 0x00)
				{
					break;
				}
			}
...
_FX ULONG Hook_Find_ZwRoutine_2(ULONG ServiceNum, void** out_routine)
{
	...
	__try
	{
		...
		for (addr = (UCHAR*)wcstombs + 0x4000; addr < (UCHAR*)wcstombs + 0x6000; ++addr)
		{
			if (*addr == 0xB8 && sizeof(ZwXxx_Code) == RtlCompareMemory(addr + 5, ZwXxx_Code, sizeof(ZwXxx_Code)))
			{
				found = TRUE;
				break;
			}
