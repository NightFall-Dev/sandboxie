...
typedef HANDLE (*P_NtUserQueryWindow)(void* hWnd, ULONG_PTR QueryClass);

typedef void* (*P_NtUserGetForegroundWindow)(void);

typedef ULONG_PTR (*P_NtUserPostThreadMessage)(ULONG idThread, ULONG Msg, void* wParam, void* lParam);

typedef ULONG_PTR (*P_NtUserSendInput)(ULONG_PTR nInputs, void* pInputs, ULONG_PTR cbInput);

typedef ULONG_PTR (*P_NtUserBlockInput)(ULONG_PTR fBlockIt);

typedef ULONG_PTR (*P_NtUserSetWindowsHookEx)(void* hModule, UNICODE_STRING* DllName, ULONG idThread, ULONG HookType, void* HookProc, ULONG_PTR Flags);

typedef ULONG_PTR (*P_NtUserSetWinEventHook)(ULONG EventMin, ULONG EventMax, void* hModule, UNICODE_STRING* DllName, void* HookProc, ULONG idProcess, ULONG idThread, ULONG Flags);
...
static BOOLEAN Gui_HookService(void** pSourceFunc, ULONG_PTR* pJumpStub, ULONG_PTR TargetFunc);


//---------------------------------------------------------------------------


static ULONG_PTR Gui_NtUserPostThreadMessage(ULONG idThread, ULONG Msg, void* wParam, void* lParam);

static ULONG_PTR Gui_NtUserSendInput(ULONG_PTR nInputs, void* pInputs, ULONG_PTR cbInput);

static ULONG_PTR Gui_NtUserBlockInput(ULONG_PTR BlockIt);

static ULONG_PTR Gui_NtUserSetWindowsHookEx(void* hModule, UNICODE_STRING* DllName, ULONG idThread, ULONG HookType, void* HookProc, ULONG_PTR Flags);

static ULONG_PTR Gui_NtUserSetWinEventHook(ULONG EventMin, ULONG EventMax, void* hModule, UNICODE_STRING* DllName, void* HookProc, ULONG idProcess, ULONG idThread, ULONG Flags);
...
#define GET_SERVICE(svc, prmcnt)                                                                       \
	if (ok)                                                                                            \
	{                                                                                                  \
		if (!Hook_GetService((void*)(ULONG_PTR)__idx_##svc, NULL, prmcnt, (void**)&__sys_##svc, NULL)) \
		{                                                                                              \
...
#define HOOK_SERVICE_X(pfx, svc)                                                       \
	if (ok)                                                                            \
	{                                                                                  \
		if (!Gui_HookService((void**)&__sys_##svc, &__jmp_##svc, (ULONG_PTR)pfx##svc)) \
		{                                                                              \
...
static void Gui_HookFreezeDpc(KDPC* dpc, GUI_HOOKSERVICE_CONTEXT* context, void* unused1, void* unused2);
...
_FX BOOLEAN Gui_HookService(void** pSourceFunc, ULONG_PTR* pJumpStub, ULONG_PTR TargetFunc)
{
	NTSTATUS status;
	void* Trampoline;
	...
	context = ExAllocatePoolWithTag(NonPagedPool, sizeof(GUI_HOOKSERVICE_CONTEXT), tzuk);
	if (!context)
	{
...
retry:
	...
	{ // call (TrendMicro)
		...
		if (memcmp(context->SourceAddr, SpySweeperProlog, sizeof(SpySweeperProlog)) == 0)
		{
			//
		...
		else if (memcmp(context->SourceAddr, TrendMicroProlog, sizeof(TrendMicroProlog)) == 0)
		{
		...
		else if (*(UCHAR*)context->SourceAddr != 0xE8)
		{
			...
			context->SourceAddr = (void*)((ULONG_PTR)context->SourceAddr + 2);
		}
	}
	...
	Trampoline = Hook_BuildTramp(context->SourceAddr, Trampoline, FALSE, FALSE);
	...
	*pSourceFunc = Trampoline;
	...
	if (!*pJumpStub)
	{
		*pJumpStub = Process_BuildHookEntry(TargetFunc, (ULONG_PTR)Trampoline, NULL);
		if (!*pJumpStub)
		{
			status = STATUS_UNSUCCESSFUL;
			goto finish;
	...
	if (0 != memcmp(context->SaveBytes, context->SaveBytesAddr, SaveBytesLen))
	{
		goto retry;
	}
	...
	if (WriteAddr)
	{
		WriteAddr += (UCHAR*)context->SourceAddr - (UCHAR*)context->SaveBytesAddr;
	}
	else
	{
	...
	if (NT_SUCCESS(status))
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			if (0 == memcmp(context->SaveBytes, context->SaveBytesAddr, SaveBytesLen))
			{
				...
				Hook_BuildJump(WriteAddr, context->SourceAddr, context->TargetAddr);
...
_FX NTSTATUS Gui_HookSaveThreads(GUI_HOOKSERVICE_CONTEXT* context)
{
	...
	while (1)
	{
		...
		dummy_len = 0;
		status    = ZwQuerySystemInformation(SystemProcessesAndThreadsInformation, buf, len, &dummy_len);
		if (NT_SUCCESS(status))
		{
			break;
		}
	...
	while (1)
	{
		...
		info = (SYSTEM_PROCESS_INFORMATION*)((UCHAR*)info + info->NextEntryOffset);
	}
	...
	context->Threads = ExAllocatePoolWithTag(NonPagedPool, sizeof(ULONG_PTR) * AllThreadCount, tzuk);
	if (!context->Threads)
	{
		ExFreePoolWithTag(buf, tzuk);
	...
	while (1)
	{
		...
		{
			PETHREAD Thread;
			HANDLE idThread = info->Threads[info->ThreadCount - 1].ClientId.UniqueThread;
...
_FX NTSTATUS Gui_HookFreeze(GUI_HOOKSERVICE_CONTEXT* context, char op)
{
	...
	if (op == 'i')
	{
		...
		while (mask)
		{
			if (mask & 1)
			{
				...
				context->dpcs[i] = ExAllocatePoolWithTag(NonPagedPool, sizeof(KDPC), tzuk);
				if (!context->dpcs[i])
				{
					return STATUS_INSUFFICIENT_RESOURCES;
				}
	...
	if (op == 'f')
	{
		...
		for (i = 0; i < MAXIMUM_PROCESSORS; ++i)
		{
			if (context->dpcs[i] && i != cur_proc)
			{
				KeInitializeDpc(context->dpcs[i], Gui_HookFreezeDpc, context);
		...
		while (InterlockedCompareExchange(&context->NumRaised, num, num) != num)
		{
			for (i = 0; i < 10; ++i)
			{
				YieldProcessor();
	...
	else if (op == 't')
	{
		...
		while (InterlockedCompareExchange(&context->NumRaised, 0, 0) != 0)
		{
			for (i = 0; i < 10; ++i)
			{
				YieldProcessor();
...
_FX void Gui_HookFreezeDpc(KDPC* dpc, GUI_HOOKSERVICE_CONTEXT* context, void* unused1, void* unused2)
{
	//
...
static NTSTATUS Gui_CheckBoxedThread(PROCESS* proc, ULONG idThread, ULONG* out_idProcess);
...
_FX ULONG_PTR Gui_NtUserPostThreadMessage(ULONG idThread, ULONG Msg, void* wParam, void* lParam)
{
	GUI_NTUSER_IF
	{
		NTSTATUS status;
		...
		else
		{
			...
			{
				status = Process_CheckProcessName(proc, &proc->open_win_classes, idProcess, NULL);
			}
		}
		...
		if (Session_MonitorCount)
		{
			...
			Process_GetProcessName(proc->pool, idProcess, &nbuf, &nlen, &nptr);
			if (nbuf)
			{
		...
		if (!NT_SUCCESS(status))
		{
			...
			{
				WCHAR access_str[80];
				swprintf(access_str, L"(GD) ThrdMessage %05d (%04X) to tid=%06d    pid=%06d", Msg, Msg, idThread, idProcess);
...
_FX ULONG_PTR Gui_NtUserSendInput(ULONG_PTR nInputs, void* pInputs, ULONG_PTR cbInput)
{
	GUI_NTUSER_IF
	{
		//
		...
		while (nInputs && nInputs < 16)
		{
			...
			{
				PROCESS* proc2;
				HANDLE idProcess = __sys_NtUserQueryWindow(active_hwnd, QUERYWINDOW_PROCESS);
				if (idProcess)
				{
...
_FX ULONG_PTR Gui_NtUserSetWindowsHookEx(void* hModule, UNICODE_STRING* DllName, ULONG idThread, ULONG HookType, void* HookProc, ULONG_PTR Flags)
{
	GUI_NTUSER_IF
	{
		//
		...
		if (HookType == WH_JOURNALRECORD || HookType == WH_JOURNALPLAYBACK || HookType == WH_KEYBOARD_LL || HookType == WH_MOUSE_LL)
		{
		...
		if (proc->gui_trace & (TRACE_ALLOW | TRACE_DENY))
		{
			...
			if (letter)
			{
				swprintf(access_str, L"(G%c) WinHook %04d on tid=%06d pid=%06d", letter, HookType, idThread, idProcess);
				Log_Debug_Msg(access_str, Driver_Empty);
			}
		}
	...
	return __sys_NtUserSetWindowsHookEx(hModule, DllName, idThread, HookType, HookProc, Flags);
}
...
_FX ULONG_PTR Gui_NtUserSetWinEventHook(ULONG EventMin, ULONG EventMax, void* hModule, UNICODE_STRING* DllName, void* HookProc, ULONG idProcess, ULONG idThread, ULONG Flags)
{
	GUI_NTUSER_IF
	{
		...
		if (proc->gui_trace & (TRACE_ALLOW | TRACE_DENY))
		{
			...
			if (letter)
			{
				swprintf(access_str, L"(G%c) AccHook on tid=%06d pid=%06d", letter, idThread, idProcess);
				Log_Debug_Msg(access_str, Driver_Empty);
			}
		}
	...
	return __sys_NtUserSetWinEventHook(EventMin, EventMax, hModule, DllName, HookProc, idProcess, idThread, Flags);
}
...
_FX NTSTATUS Gui_CheckBoxedThread(PROCESS* proc, ULONG idThread, ULONG* out_idProcess)
{
	NTSTATUS status;
	HANDLE handle;
	...
	status = ZwOpenThread(&handle, THREAD_QUERY_INFORMATION, &objattrs, &cid);

	if (NT_SUCCESS(status))
	{
		len    = sizeof(THREAD_BASIC_INFORMATION);
		status = ZwQueryInformationThread(handle, ThreadBasicInformation, &info, len, &len);
		if (NT_SUCCESS(status))
		{
