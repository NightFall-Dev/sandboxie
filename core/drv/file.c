...
#define FILE_DENIED_ACCESS \
	~(STANDARD_RIGHTS_READ | GENERIC_READ | SYNCHRONIZE | READ_CONTROL | FILE_READ_DATA | FILE_READ_EA | FILE_READ_ATTRIBUTES | FILE_EXECUTE)
...
static BOOLEAN File_InitPaths(PROCESS* proc, LIST* open_file_paths, LIST* closed_file_paths, LIST* read_file_paths, LIST* write_file_paths);

static BOOLEAN File_BlockInternetAccess(PROCESS* proc);

static BOOLEAN File_BlockInternetAccess2(PROCESS* proc, const WCHAR* name, int modifier);

static NTSTATUS File_Generic_MyParseProc(PROCESS* proc, PVOID ParseObject, ULONG device_type, PUNICODE_STRING RemainingName, MY_CONTEXT* MyContext, BOOLEAN msg1313);

static NTSTATUS File_CreatePagingFile(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args);

static void File_ReplaceTokenIfFontRequest(ACCESS_STATE* AccessState, PDEVICE_OBJECT DeviceObject, UNICODE_STRING* FileName, BOOLEAN* pbSetDirty);

static NTSTATUS File_Api_SetShortName2(PROCESS* proc, OBJECT_ATTRIBUTES* objattrs, FILE_NAME_INFORMATION* name, ULONG name_len);
...
_FX BOOLEAN File_CreateBoxPath(PROCESS* proc)
{
	...
	InitializeObjectAttributes(&objattrs, &objname, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, Driver_PublicSd);
	...
	if (!NT_SUCCESS(status))
	{
		Log_Status_Ex(MSG_FILE_CREATE_BOX_PATH, 0, status, proc->box->file_path);
	}
...
{
	static char* _Tip = "This folder is a work area created by the program " MY_PRODUCT_NAME_STRING ".\r\n";

	static char* _DesktopIni = "IconIndex=8\r\n"
	                           "InfoTip=";
	...
	InitializeObjectAttributes(&objattrs, &objname, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, FileHandle, Driver_PublicSd);

	//
	// hide the newly created sandbox directory
	//

	status = ZwQueryInformationFile(FileHandle, &IoStatusBlock, &info, sizeof(FILE_BASIC_INFORMATION), FileBasicInformation);

	if (NT_SUCCESS(status))
	{
		info.FileAttributes &= ~(FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM);
		info.FileAttributes |= FILE_ATTRIBUTE_READONLY;
		status = ZwSetInformationFile(FileHandle, &IoStatusBlock, &info, sizeof(FILE_BASIC_INFORMATION), FileBasicInformation);
	}
	...
	if (File_DesktopIniText)
	{
		...
		{
			status = ZwWriteFile(handle, NULL, NULL, NULL, &IoStatusBlock, File_DesktopIniText, strlen(File_DesktopIniText), NULL, NULL);
	...
	if (NT_SUCCESS(status) && IoStatusBlock.Information == FILE_CREATED)
	{
		status = ZwWriteFile(handle, NULL, NULL, NULL, &IoStatusBlock, _Tip, strlen(_Tip), NULL, NULL);
...
_FX void File_AdjustBoxFilePath(PROCESS* proc, HANDLE handle)
{
	...
	status = ObReferenceObjectByHandle(handle, 0, *IoFileObjectType, KernelMode, &object, NULL);
	...
	if (NT_SUCCESS(status))
	{
		...
		if (Name && (Name != &Obj_Unnamed))
		{
			...
			if (path_len != box->file_path_len || _wcsicmp(path, box->file_path) != 0)
			{
				//
...
_FX BOOLEAN File_InitPaths(PROCESS* proc, LIST* open_file_paths, LIST* closed_file_paths, LIST* read_file_paths, LIST* write_file_paths)
{
	...
	static const WCHAR* openpipes[] = {L"\\Device\\NamedPipe\\", // named pipe root
	    ...
	    NULL};
	static const WCHAR* strClosedFiles[] = {// Windows Remote Management (WinRM) is a large security hole.  A sandboxed app running in an elevated cmd shell can send any admin command to the host.
	...
	if (ok && Conf_Get_Boolean(proc->box->name, Driver_OpenProtectedStorage, 0, FALSE))
	{
		ok = Process_AddPath(proc, open_file_paths, NULL, TRUE, _PstPipe, FALSE);
	}

	for (i = 0; openpipes[i] && ok; ++i)
	{
		ok = Process_AddPath(proc, open_file_paths, NULL, TRUE, openpipes[i], FALSE);
	}
	...
	if (ok)
	{
		...
		ok = Process_AddPath(proc, closed_file_paths, NULL, TRUE, File_Redirector, FALSE);
		if (ok)
		{
			ok = Process_AddPath(proc, closed_file_paths, NULL, TRUE, File_Mup, FALSE);
		}
	}
	...
	ok = Process_GetPaths2(proc, write_file_paths, closed_file_paths, _WritePath, TRUE);
	if (ok)
	{
		ok = Process_GetPaths(proc, closed_file_paths, _WritePath, TRUE);
	}
	if (!ok)
	{
	...
	if (proc->image_sbie)
	{
		static const WCHAR* _ShimEng = L"%SystemRoot%\\System32\\shimeng.dll";
		static const WCHAR* _AppHelp = L"%SystemRoot%\\System32\\apphelp.dll";

		ok = Process_AddPath(proc, closed_file_paths, _ClosedPath, TRUE, _ShimEng, FALSE);
		if (ok)
		{
			ok = Process_AddPath(proc, closed_file_paths, _ClosedPath, TRUE, _AppHelp, FALSE);
		}
	}
...
_FX BOOLEAN File_BlockInternetAccess(PROCESS* proc)
{
	...
	proc->file_warn_internet = Conf_Get_Boolean(proc->box->name, L"NotifyInternetAccessDenied", 0, TRUE);
	...
	Process_MatchPath(proc->pool, L"InternetAccessDevices", 21, NULL, &proc->closed_file_paths, &is_open, &is_closed);
...
_FX BOOLEAN File_BlockInternetAccess2(PROCESS* proc, const WCHAR* name, int modifier)
{
	...
	return Process_AddPath(proc, &proc->closed_file_paths, NULL, TRUE, device_name, add_star);
}
...
{
	BOOLEAN ok = File_InitPaths(proc, &proc->open_file_paths, &proc->closed_file_paths, &proc->read_file_paths, &proc->write_file_paths);
	if (ok)
	{
	...
	if (ok && proc->image_path && (!proc->image_sbie))
	{
		...
		BOOLEAN is_open, is_closed;
		Process_MatchPath(proc->pool, proc->image_path, wcslen(proc->image_path), NULL, &proc->closed_file_paths, &is_open, &is_closed);
...
_FX NTSTATUS File_Generic_MyParseProc(PROCESS* proc, PVOID ParseObject, ULONG device_type, PUNICODE_STRING RemainingName, MY_CONTEXT* MyContext, BOOLEAN msg1313)
{
	...
	if (device_type != FILE_DEVICE_DISK && device_type != FILE_DEVICE_NAMED_PIPE && device_type != FILE_DEVICE_MAILSLOT && device_type != FILE_DEVICE_NETWORK && device_type != FILE_DEVICE_MULTI_UNC_PROVIDER && device_type != FILE_DEVICE_NETWORK_FILE_SYSTEM && device_type != FILE_DEVICE_DFS)
	{
		if ((proc->file_trace & TRACE_IGNORE) || Session_MonitorCount)
		{
			...
			status = Obj_GetParseName(proc->pool, ParseObject, RemainingName, &Name, &NameLength);
			if (NT_SUCCESS(status))
			{
			...
			if (ignore_str)
			{
				swprintf(ignore_str, L"(FI) %08X %s", device_type, device_name_ptr);
				...
				if (Session_MonitorCount && device_type != FILE_DEVICE_PHYSICAL_NETCARD)
				{
	...
	if (!NT_SUCCESS(status))
	{
		...
		if (status == STATUS_OBJECT_PATH_INVALID && ((!RemainingName) || RemainingName->Length == 0))
		{
	...
	if (device_type == FILE_DEVICE_NAMED_PIPE || device_type == FILE_DEVICE_MAILSLOT)
	{
		IsPipeDevice = TRUE;
		...
		if (_wcsnicmp(Name->Name.Buffer, _Device, _DeviceLen) == 0)
		{
			...
			if (PipeName)
			{
				...
				if (*PipeName == L'\\')
				{
					...
					{
						MonitorPrefixLen = (ULONG)(ULONG_PTR)(PipeName - Name->Name.Buffer);
						MonitorSuffixPtr = PipeName + proc->box->pipe_path_len / sizeof(WCHAR);
						IsBoxedPath      = TRUE;
						PipeName         = NULL;
					}
	...
	if (IsPipeDevice)
	{
		...
		if (PipeName && (*PipeName == L'\0') && (DesiredAccess & FILE_DENIED_ACCESS) == 0)
		{
			write_access = FALSE;
		}
	}
	...
	if (!IsBoxedPath)
	{
		...
		if (path_len >= Driver_HomePathNt_Len && (path[Driver_HomePathNt_Len] == L'\\' || path[Driver_HomePathNt_Len] == L'\0') && 0 == Box_NlsStrCmp(path, Driver_HomePathNt, Driver_HomePathNt_Len))
		{
			if (write_access)
			{
				status = STATUS_ACCESS_DENIED;
		...
		Process_MatchPath(proc->pool, path, path_len, open_file_paths, &proc->closed_file_paths, &is_open, &is_closed);
		...
		if ((!is_open) && (!is_closed))
		{
			...
			if (PrefixLen && path[PrefixLen] == L'\\' && path[PrefixLen + 1] != L'\0')
			{
				...
				if (ptr && ptr[0] && ptr[1])
				{
					...
					else
					{
						...
						Process_MatchPath(proc->pool, path2, len1, open_file_paths, &proc->closed_file_paths, &is_open, &is_closed);
						...
						if (write_access && is_open)
						{
							...
							Process_MatchPath(proc->pool, path2, len1, &proc->read_file_paths, NULL, &is_open, &is_closed);

							if (is_open)
							{   // match on OpenFilePath
						...
						if ((MyContext->OriginalDesiredAccess != FILE_READ_ATTRIBUTES) && (MyContext->OriginalDesiredAccess != SYNCHRONIZE) && !is_open
						    && Conf_Get_Boolean(proc->box->name, L"BlockNetworkFiles", 0, FALSE))
						{
							if (!write_access)
							{
								// see if path is open for reading
								Process_MatchPath(proc->pool, path2, len1, &proc->open_file_paths, NULL, &is_open, &is_closed);
							}
							if (!is_open)
							{
								is_closed = TRUE;
		...
		if (write_access && is_open)
		{
			Process_MatchPath(proc->pool, path, path_len, &proc->read_file_paths, NULL, &is_open, &is_closed);
		...
		if ((!proc->sbiedll_loaded) && status == STATUS_SUCCESS && (CreateOptions & FILE_DIRECTORY_FILE) == 0)
		{
	...
	if (file_trace & (TRACE_ALLOW | TRACE_DENY))
	{
		...
		if (letter)
		{
			swprintf(access_str, L"(F%c) %08X.%02X.%08X", letter, DesiredAccess, CreateDisposition & 0x0F, CreateOptions);
	...
	{
		Session_MonitorPut(MONITOR_FILE_OR_KEY | MONITOR_DENY, Name->Name.Buffer);
	}
	else if (msg1313 && status == STATUS_ACCESS_DENIED && device_type == FILE_DEVICE_DISK && RemainingName
...
_FX NTSTATUS File_CreatePagingFile(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
...
_FX void File_ReplaceTokenIfFontRequest(ACCESS_STATE* AccessState, PDEVICE_OBJECT DeviceObject, UNICODE_STRING* FileName, BOOLEAN* pbSetDirty)
{
	//
	...
	const ULONG _DesiredAccess = SYNCHRONIZE | READ_CONTROL | FILE_READ_ATTRIBUTES | FILE_EXECUTE | FILE_READ_EA | FILE_READ_DATA;
	PROCESS* proc;
	WCHAR* ptr;
	ULONG len, i;
	...
	if (i == len)
	{
		...
		if (DeviceObject)
		{
			...
			NTSTATUS status = Obj_GetParseName(proc->pool, DeviceObject, FileName, &Name, &NameLength);
			if (NT_SUCCESS(status))
			{
...
_FX NTSTATUS File_Api_Rename(PROCESS* proc, ULONG64* parms)
{
	...
	*name = L'\\';
	...
	Process_MatchPath(proc->pool, path, wcslen(path), &proc->open_file_paths, &proc->closed_file_paths, &is_open, &is_closed);
	...
	*name = L'\0';
	...
	status = ZwCreateFile(&dir_handle, FILE_GENERIC_WRITE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	...
	*name    = save_char;
	...
	else
	{
		...
		status = ZwSetInformationFile(args->file_handle.val, &IoStatusBlock, info, info_len, FileRenameInformation);
...
_FX NTSTATUS File_Api_GetName(PROCESS* proc, ULONG64* parms)
{
	...
	status = ObReferenceObjectByHandle(args->handle.val, 0, *IoFileObjectType, UserMode, &object, NULL);
	...
	if (NT_SUCCESS(status))
	{
		...
		if ((!object->DeviceObject) || (!object->FileName.Length) || (!object->FileName.Buffer))
		{
			ObDereferenceObject(object);
	...
	if (NT_SUCCESS(status))
	{
		...
		__try
		{
			if (object->DeviceObject->DeviceType == FILE_DEVICE_DISK)
			{
				...
				if (NT_SUCCESS(status))
				{
					len = Name->Name.Length + sizeof(WCHAR) * 2; // null padding
			...
			else
			{
				...
				status = Obj_GetName(pool, object->DeviceObject, &Name, &NameLength);
				if (NT_SUCCESS(status) && (Name == &Obj_Unnamed))
				{
					Name   = NULL;
					status = STATUS_OBJECT_PATH_NOT_FOUND;
				...
				if (NT_SUCCESS(status))
				{
					len = Name->Name.Length + object->FileName.Length + sizeof(WCHAR) * 2; // null padding
					if (len > args->name_len.val)
					{
						status = STATUS_BUFFER_TOO_SMALL;
					}
					else
					{
						...
						memcpy(user, Name->Name.Buffer, Name->Name.Length);
						user += Name->Name.Length / sizeof(WCHAR);
						memcpy(user, object->FileName.Buffer, object->FileName.Length);
...
_FX NTSTATUS File_Api_RefreshPathList(PROCESS* proc, ULONG64* parms)
{
	...
	ok = File_InitPaths(proc, &open_paths, &closed_paths, &read_paths, &write_paths);
...
_FX NTSTATUS File_Api_Open(PROCESS* proc, ULONG64* parms)
{
	...
	AccessCheckOptions = IO_FORCE_ACCESS_CHECK | IO_IGNORE_SHARE_ACCESS_CHECK;

	KeRaiseIrql(APC_LEVEL, &irql);
	ExAcquireResourceSharedLite(proc->file_lock, TRUE);

	Process_MatchPath(proc->pool, path, wcslen(path), NULL, &proc->closed_file_paths, &is_open, &is_closed);
	...
	status = IoCreateFileSpecifyDeviceObjectHint(&handle, DesiredAccess, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, CreateOptions, NULL, 0, CreateFileTypeNone, NULL, AccessCheckOptions, NULL);
...
{
	API_CHECK_INTERNET_ACCESS_ARGS* args = (API_CHECK_INTERNET_ACCESS_ARGS*)parms;
	WCHAR* user_devname;
	...
	if (1)
	{
		...
		Process_MatchPath(proc->pool, device_name, wcslen(device_name), NULL, &proc->closed_file_paths, &is_open, &is_closed);
...
_FX NTSTATUS File_Api_GetBlockedDll(PROCESS* proc, ULONG64* parms)
{
	...
	else
	{
		__try
		{
			...
			ProbeForWrite(user_buf, sizeof(WCHAR) * (len + 1), sizeof(WCHAR));
