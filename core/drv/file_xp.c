...
static BOOLEAN File_CreateMyContext(PACCESS_STATE AccessState, KPROCESSOR_MODE AccessMode, PVOID Context, MY_CONTEXT* MyContext);
...
_FX BOOLEAN File_Init_XpHook(void)
{
	...
	if (!Obj_HookParseProc(File_File_ObjectName, File_File_MyParseProc, &File_File_NtParseProc, &File_File_JumpStub))
	{
		return FALSE;
	}
	...
	if (!Obj_HookParseProc(File_Device_ObjectName, File_Device_MyParseProc, &File_Device_NtParseProc, &File_Device_JumpStub))
	{
		return FALSE;
	}
...
_FX BOOLEAN File_CreateMyContext(PACCESS_STATE AccessState, KPROCESSOR_MODE AccessMode, PVOID Context, MY_CONTEXT* MyContext)
{
	//
	...
	if (Context && ((OPEN_PACKET*)Context)->Type == IO_TYPE_OPEN_PACKET)
	{
		...
		MyContext->CreateDisposition = ((OPEN_PACKET*)Context)->CreateDisposition;
	...
	else if (MyContext->HaveContext && MyContext->Options & IO_OPEN_TARGET_DIRECTORY)
	{
		return TRUE;
	}
...
_FX NTSTATUS File_File_MyParseProc(OBJ_PARSE_PROC_ARGS)
{
	...
	if (File_CreateMyContext(AccessState, AccessMode, Context, &MyContext))
	{
		if (proc != PROCESS_TERMINATED)
		{
			...
			status = File_Generic_MyParseProc(proc, ParseObject, file_object->DeviceObject->DeviceType, RemainingName, &MyContext, FALSE);
		}
		else
		{
...
_FX NTSTATUS File_Device_MyParseProc(OBJ_PARSE_PROC_ARGS)
{
	...
	if (File_CreateMyContext(AccessState, AccessMode, Context, &MyContext))
	{
		if (proc != PROCESS_TERMINATED)
		{
			...
			status = File_Generic_MyParseProc(proc, ParseObject, device_object->DeviceType, RemainingName, &MyContext, TRUE);
		}
		else
		{
			status = STATUS_PROCESS_IS_TERMINATING;
	...
	else
	{
		...
		if (AccessMode == KernelMode)
		{
			...
			DEVICE_OBJECT* device_object = (DEVICE_OBJECT*)ParseObject;
			File_ReplaceTokenIfFontRequest(AccessState, ParseObject, RemainingName, &bSetDirty);
		}
	}
