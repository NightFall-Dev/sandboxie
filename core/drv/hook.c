...
static LONG Hook_FindSyscall2(void* addr, int depth, ULONG_PTR* targets, BOOLEAN check_target, LONG eax, LONG* skip);
...
_FX BOOLEAN Hook_GetService(void* DllProc, LONG* SkipIndexes, ULONG ParamCount, void** NtService, void** ZwService)
{
	LONG svc_num;
	void* svc_addr;
...
_FX LONG Hook_GetServiceIndex(void* DllProc, LONG* SkipIndexes)
{
	...
	targets = (ULONG_PTR*)ExAllocatePoolWithTag(PagedPool, PAGE_SIZE * NUM_TARGET_PAGES, tzuk);
	if (!targets)
	{
		Log_Msg0(MSG_1104)
			;
...
_FX LONG Hook_FindSyscall2(void* addr, int depth, ULONG_PTR* targets, BOOLEAN check_target, LONG eax, LONG* skip)
{
	HOOK_INST inst;
	ULONG edx;
	ULONG i;
	...
	for (count = 0; count < 1024; ++count)
	{
		...
		if (inst.kind == INST_CTLXFER_REG)
		{
			...
			else if (inst.op1 == 0xFF && inst.op2 == 0x12 && ((ULONG)edx & 0xF0000000) >= 0x70000000)
			{
				__try
				{
		...
		if (inst.kind == INST_CTLXFER)
		{
            ...
            */
			...
			eax = Hook_FindSyscall2((void*)inst.parm, depth + 1, targets, check_target, eax, skip);
