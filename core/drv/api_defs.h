...
#define API_SBIEDRV_CTLCODE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x801, METHOD_NEITHER, FILE_ANY_ACCESS)
...
#define API_MAX_SVC_DATA_LEN (MAX_PORTMSG_LENGTH - (sizeof(PORT_MESSAGE) + sizeof(ULONG)))
...
#define API_ARGS_BEGIN(x) \
	typedef struct _##x   \
	{                     \
		ULONG64 func_code;
#define API_ARGS_FIELD(t, m) \
	union {                  \
		ULONG64 val64;       \
		t val;               \
	} m;
#define API_ARGS_CLOSE(x) \
	}                     \
	x;
