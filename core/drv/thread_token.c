...
#define TOKEN_DENIED_ACCESS_MASK ~(STANDARD_RIGHTS_READ | GENERIC_READ | SYNCHRONIZE | TOKEN_QUERY | TOKEN_QUERY_SOURCE)
...
static NTSTATUS Thread_OpenProcessToken(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args);

static NTSTATUS Thread_OpenProcessTokenEx(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args);

static NTSTATUS Thread_OpenProcessToken_Common(PROCESS* proc, HANDLE ProcessHandle, ACCESS_MASK DesiredAccess, HANDLE* TokenHandle);

static NTSTATUS Thread_SetInformationProcess(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args);

static NTSTATUS Thread_SetInformationProcess_PrimaryToken(PROCESS* proc, HANDLE ProcessHandle, void* InfoBuffer, ULONG InfoLength);

static NTSTATUS Thread_SetInformationProcess_PrimaryToken_2(PROCESS* proc, void* ProcessObject, void* TokenObject, ULONG SessionId);

static void* Thread_SetInformationProcess_PrimaryToken_3(PROCESS* proc, void* TokenObject1, ULONG SessionId);


//---------------------------------------------------------------------------


static NTSTATUS Thread_OpenThreadToken(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args);

static NTSTATUS Thread_OpenThreadTokenEx(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args);

static NTSTATUS Thread_OpenThreadToken_Common(PROCESS* proc, HANDLE ThreadHandle, ACCESS_MASK DesiredAccess, BOOLEAN OpenAsSelf, HANDLE* TokenHandle);

static NTSTATUS Thread_OpenThreadToken_OpenHandle(void* TokenObject, BOOLEAN CopyOnOpen, BOOLEAN EffectiveOnly, SECURITY_IMPERSONATION_LEVEL ImpersonationLevel, ACCESS_MASK DesiredAccess, HANDLE* NewHandle, HANDLE OpenAsSelf_PrimaryTokenObject);

static NTSTATUS Thread_OpenThreadToken_SwitchToken(void* PrimaryTokenObject, void** TokenObject, BOOLEAN* CopyOnOpen, BOOLEAN* EffectiveOnly, SECURITY_IMPERSONATION_LEVEL* ImpersonationLevel);

static _FX NTSTATUS Thread_SetInformationThread(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args);

static NTSTATUS Thread_SetInformationThread_ImpersonationToken(PROCESS* proc, HANDLE ThreadHandle, void* InfoBuffer, ULONG InfoLength);

static NTSTATUS Thread_CheckTokenForImpersonation(void* TokenObject, BOOLEAN CheckTokenType, BOOLEAN DropRights, ULONG SessionId);

static NTSTATUS Thread_SetInformationThread_ChangeNotifyToken(PROCESS* proc);

static NTSTATUS Thread_ImpersonateAnonymousToken(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args);


//---------------------------------------------------------------------------


static NTSTATUS Thread_GetKernelHandleForUserHandle(HANDLE* OutKernelHandle, HANDLE InUserHandle);
...
_FX NTSTATUS Thread_OpenProcessToken(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
	...
	else
	{
		...
		status = Thread_OpenProcessToken_Common(proc, ProcessHandle, DesiredAccess, TokenHandle);
	}
...
_FX NTSTATUS Thread_OpenProcessTokenEx(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
...
_FX NTSTATUS Thread_OpenProcessToken_Common(PROCESS* proc, HANDLE ProcessHandle, ACCESS_MASK DesiredAccess, HANDLE* TokenHandle)
{
	...
	_ProcessAccess = (Driver_OsVersion >= DRIVER_WINDOWS_VISTA) ? PROCESS_QUERY_LIMITED_INFORMATION : PROCESS_QUERY_INFORMATION;

	status = ObReferenceObjectByHandle(ProcessHandle, _ProcessAccess, *PsProcessType, UserMode, &ProcessObject, NULL);
	...
	if (proc2 && proc2->primary_token)
	{
		...
		status = ObOpenObjectByPointer(PrimaryTokenObject, 0, NULL, DesiredAccess, *SeTokenObjectType, UserMode, &MyTokenHandle);
	...
	else
	{
		...
		else
		{
			...
			status = Thread_GetKernelHandleForUserHandle(&ProcessKernelHandle, ProcessHandle);
			if (NT_SUCCESS(status))
			{
				status = ZwOpenProcessToken(ProcessKernelHandle, DesiredAccess, &MyTokenHandle);
...
_FX NTSTATUS Thread_SetInformationProcess(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
	...
	if (((user_args[1] == ProcessAccessToken) || (user_args[1] == ProcessAccessTokenEx)) && proc->primary_token)
	{
		...
		status = Thread_SetInformationProcess_PrimaryToken(proc, ProcessHandle, InfoBuffer, InfoLength);
	}
	else
	{
		status = Syscall_Invoke(syscall_entry, user_args);
...
_FX NTSTATUS Thread_SetInformationProcess_PrimaryToken(PROCESS* proc, HANDLE ProcessHandle, void* InfoBuffer, ULONG InfoLength)
{
	void* ProcessObject;
	PROCESS* proc2;
	NTSTATUS status;
	...
	status = ObReferenceObjectByHandle(ProcessHandle, PROCESS_SET_INFORMATION, *PsProcessType, UserMode, &ProcessObject, NULL);
	if (!NT_SUCCESS(status))
	{
		return status;
	}
	...
	if (proc2 && (!proc2->initialized))
	{
		...
		else
		{
			...
			status = Thread_SetInformationProcess_PrimaryToken_2(proc, ProcessObject, PrimaryTokenObject, SessionId);
		}
...
_FX NTSTATUS Thread_SetInformationProcess_PrimaryToken_2(PROCESS* proc, void* ProcessObject, void* TokenObject, ULONG SessionId)
{
	...
	TokenObject2 = Thread_SetInformationProcess_PrimaryToken_3(proc, TokenObject, SessionId);
	...
	status = ObOpenObjectByPointer(TokenObject, OBJ_KERNEL_HANDLE, NULL, TOKEN_ALL_ACCESS, *SeTokenObjectType, KernelMode, &OldTokenHandle);
	...
	status = ZwDuplicateToken(OldTokenHandle, TOKEN_ALL_ACCESS, NULL, FALSE, TokenPrimary, &NewTokenHandle);
	...
	if (NT_SUCCESS(status))
	{
		...
		status = Thread_GetKernelHandleForUserHandle(&NewTokenKernelHandle, NewTokenHandle);

		if (NT_SUCCESS(status))
		{
			status = Token_AssignPrimaryHandle(ProcessObject, NewTokenKernelHandle, SessionId);
...
_FX void* Thread_SetInformationProcess_PrimaryToken_3(PROCESS* proc, void* TokenObject1, ULONG SessionId)
{
	void* TokenObject2;
	...
	TokenObject2 = PsReferenceImpersonationToken(PsGetCurrentThread(), &CopyOnOpen, &EffectiveOnly, &ImpersonationLevel);
	if (!TokenObject2)
	{
		//
	...
	if (RtlEqualLuid((LUID*)((ULONG_PTR)TokenObject1 + TokenId_offset), (LUID*)((ULONG_PTR)TokenObject2 + ParentTokenId_offset)))
	{
		//
	...
	if (RtlEqualLuid((LUID*)((ULONG_PTR)TokenObject1 + ParentTokenId_offset), (LUID*)((ULONG_PTR)TokenObject2 + ParentTokenId_offset)))
	{
		//
	...
	if (Token_CheckPrivilege(TokenObject1, SE_ASSIGNPRIMARYTOKEN_PRIVILEGE, SessionId) || Token_CheckPrivilege(TokenObject2, SE_ASSIGNPRIMARYTOKEN_PRIVILEGE, SessionId))
	{
		return TokenObject2;
	}
	...
	if (proc->image_sbie && _wcsicmp(proc->image_name, SANDBOXIE L"DcomLaunch.exe") == 0)
	{
		return TokenObject2;
	}
...
_FX NTSTATUS Thread_OpenThreadToken(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
	...
	else
	{
		...
		status = Thread_OpenThreadToken_Common(proc, ThreadHandle, DesiredAccess, OpenAsSelf, TokenHandle);
	}
...
_FX NTSTATUS Thread_OpenThreadTokenEx(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
...
_FX NTSTATUS Thread_OpenThreadToken_Common(PROCESS* proc, HANDLE ThreadHandle, ACCESS_MASK DesiredAccess, BOOLEAN OpenAsSelf, HANDLE* TokenHandle)
{
	...
	if (OpenAsSelf)
	{
		status = Thread_OpenThreadToken_SwitchToken(proc->primary_token, &TokenObject, &CopyOnOpen, &EffectiveOnly, &ImpersonationLevel);
	...
	_ThreadAccess = (Driver_OsVersion >= DRIVER_WINDOWS_VISTA) ? THREAD_QUERY_LIMITED_INFORMATION : THREAD_QUERY_INFORMATION;

	status = ObReferenceObjectByHandle(ThreadHandle, _ThreadAccess, *PsThreadType, UserMode, &ThreadObject, NULL);

	if (OpenAsSelf)
	{
		status2 = Thread_OpenThreadToken_SwitchToken(NULL, &TokenObject, &CopyOnOpen, &EffectiveOnly, &ImpersonationLevel);
	...
	if (proc2)
	{
		...
		thrd2 = Thread_GetOrCreate(proc2, PsGetThreadId(ThreadObject), FALSE);
		if (thrd2)
		{
		...
		{
			status = Thread_OpenThreadToken_OpenHandle(TokenObject, CopyOnOpen, EffectiveOnly, ImpersonationLevel, DesiredAccess, &MyTokenHandle, (OpenAsSelf ? PrimaryTokenObject : NULL));
	...
	else
	{
		...
		else
		{
			...
			if (OpenAsSelf)
			{
				status = Thread_OpenThreadToken_SwitchToken(PrimaryTokenObject, &TokenObject, &CopyOnOpen, &EffectiveOnly, &ImpersonationLevel);
			}

			if (NT_SUCCESS(status))
			{
				HANDLE ThreadKernelHandle;
				status = Thread_GetKernelHandleForUserHandle(&ThreadKernelHandle, ThreadHandle);

				if (NT_SUCCESS(status))
				{
					status = ZwOpenThreadToken(ThreadKernelHandle, DesiredAccess, FALSE, &MyTokenHandle);
...
_FX NTSTATUS Thread_OpenThreadToken_OpenHandle(void* TokenObject, BOOLEAN CopyOnOpen, BOOLEAN EffectiveOnly, SECURITY_IMPERSONATION_LEVEL ImpersonationLevel, ACCESS_MASK DesiredAccess, HANDLE* NewHandle, HANDLE OpenAsSelf_PrimaryTokenObject)
{
	NTSTATUS status;
	...
	if (ImpersonationLevel >= SecurityImpersonation && CopyOnOpen)
	{
		...
		if (NT_SUCCESS(status))
		{
			status = ZwOpenThreadToken(NtCurrentThread(), DesiredAccess, FALSE, NewHandle);
		}
	...
	else
	{
		...
		{
			status = Thread_OpenThreadToken_SwitchToken(OpenAsSelf_PrimaryTokenObject, &TokenObject2, &CopyOnOpen2, &EffectiveOnly2, &ImpersonationLevel2);
		}
		else
		{
			status = STATUS_SUCCESS;
		...
		if (NT_SUCCESS(status))
		{
			status = ObOpenObjectByPointer(TokenObject, 0, NULL, DesiredAccess, *SeTokenObjectType, UserMode, NewHandle);
		}
		...
		if (NT_SUCCESS(status) && CopyOnOpen)
		{
			...
			status = ZwDuplicateToken(OldHandle, DesiredAccess, &ObjectAttributes, EffectiveOnly, TokenImpersonation, NewHandle);
		...
		if (OpenAsSelf_PrimaryTokenObject)
		{
			NTSTATUS status2 = Thread_OpenThreadToken_SwitchToken(NULL, &TokenObject2, &CopyOnOpen2, &EffectiveOnly2, &ImpersonationLevel2);
...
_FX NTSTATUS Thread_OpenThreadToken_SwitchToken(void* PrimaryTokenObject, void** TokenObject, BOOLEAN* CopyOnOpen, BOOLEAN* EffectiveOnly, SECURITY_IMPERSONATION_LEVEL* ImpersonationLevel)
{
	NTSTATUS status;
	...
	if (PrimaryTokenObject)
	{
		...
		*TokenObject = PsReferenceImpersonationToken(PsGetCurrentThread(), CopyOnOpen, EffectiveOnly, ImpersonationLevel);
		...
		status = Thread_MyImpersonateClient(PsGetCurrentThread(), PrimaryTokenObject, FALSE, FALSE, SecurityImpersonation);
...
_FX NTSTATUS Thread_SetInformationThread(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
	...
	if (InfoClass == ThreadImpersonationToken && proc->primary_token)
	{
		...
		status = Thread_SetInformationThread_ImpersonationToken(proc, ThreadHandle, InfoBuffer, InfoLength);
	}
	else
	{
		status = Syscall_Invoke(syscall_entry, user_args);
...
_FX NTSTATUS Thread_SetInformationThread_ImpersonationToken(PROCESS* proc, HANDLE ThreadHandle, void* InfoBuffer, ULONG InfoLength)
{
	void* TokenObject;
	void* ThreadObject;
	void* ProcessObject;
...
#ifdef _WIN64
	        || (ULONG)(ULONG_PTR)MyTokenHandle == (ULONG)(ULONG_PTR)NtCurrentThread()
#endif _WIN64
	            ))
	{
		status = Thread_SetInformationThread_ChangeNotifyToken(proc);
		if (status == STATUS_THREAD_NOT_IN_PROCESS || status == STATUS_ALREADY_COMMITTED)
		{
			return status;
		}
	}
	...
	status = ObReferenceObjectByHandle(ThreadHandle, THREAD_SET_THREAD_TOKEN, *PsThreadType, UserMode, &ThreadObject, NULL);
	...
	if (MyTokenHandle)
	{
		status = ObReferenceObjectByHandle(MyTokenHandle, TOKEN_IMPERSONATE, *SeTokenObjectType, UserMode, &TokenObject, NULL);

		if (NT_SUCCESS(status))
		{
			status = Thread_CheckTokenForImpersonation(TokenObject, TRUE, proc->drop_rights, proc->box->session_id);
	...
	if (proc2)
	{
		...
		thrd2 = Thread_GetOrCreate(proc2, PsGetThreadId(ThreadObject), MustCreateThread);
		if (thrd2)
		{
			TokenObject                     = InterlockedExchangePointer(&thrd2->token_object, TokenObject);
...
_FX NTSTATUS Thread_CheckTokenForImpersonation(void* TokenObject, BOOLEAN CheckTokenType, BOOLEAN DropRights, ULONG SessionId)
{
	NTSTATUS status;
	...
	TOKEN_TYPE TokenType = CheckTokenType ? SeTokenType(TokenObject) : TokenImpersonation;
	...
	else
	{
		...
		void* FilteredTokenObject = Token_Filter(TokenObject, DropRights, SessionId);
...
_FX NTSTATUS Thread_SetInformationThread_ChangeNotifyToken(PROCESS* proc)
{
	...
	if (!proc->change_notify_token_flag)
	{
		...
		void* CurrentToken = PsReferenceImpersonationToken(PsGetCurrentThread(), &CopyOnOpen, &EffectiveOnly, &ImpersonationLevel);
		...
		FilteredTokenObject = Token_Restrict(CurrentToken, DISABLE_MAX_PRIVILEGE, NULL, proc);

		ObDereferenceObject(CurrentToken);

		if (FilteredTokenObject)
		{
			NTSTATUS status = Thread_MyImpersonateClient(PsGetCurrentThread(), FilteredTokenObject, FALSE, FALSE, SecurityImpersonation);
...
_FX void Thread_InitAnonymousToken(void)
{
	...
	pNtImpersonateAnonymousToken = (P_NtImpersonateAnonymousToken)syscall_entry->ntos_func;
	...
	if (NT_SUCCESS(status))
	{
		...
		Thread_AnonymousToken = PsReferenceImpersonationToken(PsGetCurrentThread(), &CopyOnOpen2, &EffectiveOnly2, &ImpersonationLevel2);
	}

	PsImpersonateClient(PsGetCurrentThread(), TokenObject, CopyOnOpen, EffectiveOnly, ImpersonationLevel);
}
...
_FX NTSTATUS Thread_ImpersonateAnonymousToken(PROCESS* proc, SYSCALL_ENTRY* syscall_entry, ULONG_PTR* user_args)
{
	NTSTATUS status;
	...
	if (proc->primary_token)
	{
		...
		{
			status = PsImpersonateClient(PsGetCurrentThread(), Thread_AnonymousToken, TRUE, FALSE, SecurityImpersonation);
	...
	{
		P_NtImpersonateAnonymousToken pNtImpersonateAnonymousToken = (P_NtImpersonateAnonymousToken)syscall_entry->ntos_func;
...
_FX NTSTATUS Thread_CheckTokenObject(PROCESS* proc, void* Object, ACCESS_MASK GrantedAccess)
{
	//
	...
	return Thread_CheckTokenForImpersonation(Object, FALSE, proc->drop_rights, proc->box->session_id);
}
...
_FX NTSTATUS Thread_GetKernelHandleForUserHandle(HANDLE* OutKernelHandle, HANDLE InUserHandle)
{
	//
	...
	status = ObReferenceObjectByHandle(InUserHandle, 0, NULL, UserMode, &Object, NULL);

	if (NT_SUCCESS(status))
	{
		status = ObOpenObjectByPointer(Object, OBJ_KERNEL_HANDLE, NULL, 0, NULL, KernelMode, OutKernelHandle);
