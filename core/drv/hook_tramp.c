...
static BOOLEAN Hook_Tramp_CountBytes(void* SysProc, ULONG* ByteCount, BOOLEAN is64, BOOLEAN probe);

static BOOLEAN Hook_Tramp_Copy(HOOK_TRAMP* tramp, void* SourceFunc, ULONG ByteCount, BOOLEAN is64, BOOLEAN probe);
...
_FX void* Hook_Tramp_Get(ULONG TrampSize)
{
	...
	if (!page)
	{
		...
		page->mapped_base = MmMapLockedPagesSpecifyCache(mdl, KernelMode, MmCached, NULL, FALSE, NormalPagePriority);
		if (!page->mapped_base)
		{
...
_FX BOOLEAN Hook_Tramp_CountBytes(void* SysProc, ULONG* ByteCount, BOOLEAN is64, BOOLEAN probe)
{
	...
	while (1)
	{
		...
		if (inst.op1 == 0xFF && inst.op2 == 0x25 && *(ULONG*)&addr[2] == 0)
		{
...
_FX BOOLEAN Hook_Tramp_Copy(HOOK_TRAMP* tramp, void* SourceFunc, ULONG ByteCount, BOOLEAN is64, BOOLEAN probe)
{
	...
	while ((ULONG_PTR)(src - (UCHAR*)SourceFunc) < ByteCount)
	{
		...
		if ((inst.op1 >= 0x70 && inst.op1 <= 0x7F) || (inst.op1 == 0x0F && (inst.op2 >= 0x80 && inst.op2 <= 0x8F)))
		{
			...
			else
			{
				...
				code[1]           = 0x80 | cond;
				*(ULONG*)&code[2] = (ULONG)(inst.parm - ((ULONG_PTR)code + 6));
		...
		else if (inst.op1 == 0xEB)
		{
			...
			else
			{
				...
				*(ULONG*)&code[1] = (ULONG)(inst.parm - ((ULONG_PTR)code + 5));
		...
		else if (inst.op1 == 0xFF && inst.op2 == 0x25 && *(ULONG*)&src[2] == 0)
		{
			//
...
_FX void* Hook_BuildTramp(void* SourceFunc, void* Trampoline, BOOLEAN is64, BOOLEAN probe)
{
	ULONG ByteCount;
	HOOK_TRAMP* tramp;
...
_FX void Hook_BuildJump(void* WritableAddr, void* ExecutableAddr, void* JumpTarget)
{
	...
	if (SourceAddr[0] == 0xFF && SourceAddr[1] == 0x25 && *(ULONG*)&SourceAddr[2] == 0)
	{
	...
	else
	{
		...
		*(ULONG_PTR*)&SourceAddr[3] = (ULONG_PTR)JumpTarget;
		...
		else
		{
			...
			*(ULONG*)&SourceAddr[1] = (ULONG)((UCHAR*)JumpTarget - (UCHAR*)ExecutableAddr - 5);
		}
