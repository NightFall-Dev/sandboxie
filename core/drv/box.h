...
BOX* Box_CreateEx(POOL* pool, const WCHAR* boxname, const WCHAR* sidstring, ULONG session_id, BOOLEAN init_paths);
...
#define Box_IsBoxedPath(box, m, uni) Box_IsBoxedPath_Helper((uni), ((box)->m##_path), ((box)->m##_path_len))

int Box_NlsStrCmp(const WCHAR* s1, const WCHAR* s2, ULONG len);

BOOLEAN Box_IsBoxedPath_Helper(UNICODE_STRING* uni, const WCHAR* box_path, ULONG box_path_len);
