...
static NTSTATUS Conf_Read_Sections(STREAM* stream, CONF_DATA* data, int* linenum);

static NTSTATUS Conf_Read_Settings(STREAM* stream, CONF_DATA* data, CONF_SECTION* section, WCHAR* line, int* linenum);
...
static NTSTATUS Conf_Merge_Global(CONF_DATA* data, ULONG session_id, CONF_SECTION* global);

static NTSTATUS Conf_Merge_Template(CONF_DATA* data, ULONG session_id, const WCHAR* tmpl_name, CONF_SECTION* section);

static const WCHAR* Conf_Get_Helper(const WCHAR* section_name, const WCHAR* setting_name, ULONG* index, BOOLEAN skip_tmpl);

static const WCHAR* Conf_Get_Section_Name(ULONG index, BOOLEAN skip_tmpl);

static const WCHAR* Conf_Get_Setting_Name(const WCHAR* section_name, ULONG index, BOOLEAN skip_tmpl);
...
_FX NTSTATUS Conf_Read(ULONG session_id)
{
	...
	if (!NT_SUCCESS(status))
	{
		...
		else
		{
			...
			Log_Status_Ex_Session(MSG_CONF_READ, 0, status, linenum_str, session_id);
		}
	}
	...
	if (NT_SUCCESS(status))
	{
		...
		if (!NT_SUCCESS(status))
		{
			Log_Status_Ex_Session(MSG_CONF_NO_TMPL_FILE, 0, status, NULL, session_id);
		}
		else
		{
	...
	if (!NT_SUCCESS(status))
	{
		...
		{
			Log_Msg_Session(MSG_CONF_LINE_TOO_LONG, linenum_str, NULL, session_id);
		}
		else if (status == STATUS_TOO_MANY_COMMANDS)
		{
			Log_Msg_Session(MSG_CONF_FILE_TOO_LONG, linenum_str, NULL, session_id);
		}
		else if (status == STATUS_INVALID_PARAMETER)
		{
			Log_Msg_Session(MSG_CONF_SYNTAX_ERROR, linenum_str, NULL, session_id);
		}
		else
		{
			Log_Status_Ex_Session(MSG_CONF_READ, 0, status, linenum_str, session_id);
...
_FX NTSTATUS Conf_Read_Sections(STREAM* stream, CONF_DATA* data, int* linenum)
{
...
_FX NTSTATUS Conf_Read_Settings(STREAM* stream, CONF_DATA* data, CONF_SECTION* section, WCHAR* line, int* linenum)
{
	NTSTATUS status;
	WCHAR* ptr;
	WCHAR* value;
...
_FX NTSTATUS Conf_Merge_Templates(CONF_DATA* data, ULONG session_id)
{
	...
	while (sandbox)
	{
		...
		if (_wcsnicmp(sandbox->name, Conf_Template_, 9) == 0 || _wcsnicmp(sandbox->name, Conf_UserSettings_, 13) == 0)
		{
			sandbox = next_sandbox;
			continue;
		}
...
_FX NTSTATUS Conf_Merge_Global(CONF_DATA* data, ULONG session_id, CONF_SECTION* global)
{
	NTSTATUS status;
	CONF_SECTION* sandbox;
	CONF_SETTING* setting;
...
_FX NTSTATUS Conf_Merge_Template(CONF_DATA* data, ULONG session_id, const WCHAR* tmpl_name, CONF_SECTION* section)
{
	//
	...
	while (tmpl)
	{
		if (wcslen(tmpl->name) >= 10 && _wcsnicmp(tmpl->name, Conf_Template_, 9) == 0 && _wcsicmp(tmpl->name + 9, tmpl_name) == 0)
		{
			break;
		}
	...
	{
		Log_Msg_Session(MSG_CONF_MISSING_TMPL, section->name, tmpl_name, session_id);
	}
...
_FX const WCHAR* Conf_Get_Helper(const WCHAR* section_name, const WCHAR* setting_name, ULONG* index, BOOLEAN skip_tmpl)
{
	WCHAR* value;
	CONF_SECTION* section;
	CONF_SETTING* setting;
...
_FX const WCHAR* Conf_Get_Setting_Name(const WCHAR* section_name, ULONG index, BOOLEAN skip_tmpl)
{
	WCHAR* value;
	CONF_SECTION* section;
...
_FX const WCHAR* Conf_Get(const WCHAR* section, const WCHAR* setting, ULONG index)
{
	const WCHAR* value;
	...
	if ((!have_section) && have_setting && _wcsicmp(setting, L"IniLocation") == 0)
	{
...
_FX BOOLEAN Conf_Get_Boolean(const WCHAR* section, const WCHAR* setting, ULONG index, BOOLEAN def)
{
	const WCHAR* value;
	BOOLEAN retval;
...
_FX ULONG Conf_Get_Number(const WCHAR* section, const WCHAR* setting, ULONG index, ULONG def)
{
	const WCHAR* value;
	ULONG retval;
...
_FX NTSTATUS Conf_IsValidBox(const WCHAR* section_name)
{
	...
	if (_wcsicmp(section_name, Conf_GlobalSettings) == 0 || _wcsicmp(section_name, Conf_TemplateSettings) == 0 || _wcsnicmp(section_name, Conf_Template_, 9) == 0 || _wcsnicmp(section_name, Conf_UserSettings_, 13) == 0)
	{
