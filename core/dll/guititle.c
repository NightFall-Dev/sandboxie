...
static int Gui_GetWindowTextW(HWND hWnd, WCHAR* lpWindowTitle, int nMaxCount);

static int Gui_GetWindowTextA(HWND hWnd, UCHAR* lpWindowTitle, int nMaxCount);
...
_FX BOOLEAN Gui_InitTitle(void)
{
	...
	if (*buf == L'y' || *buf == L'Y')
	{
		...
		Gui_BoxNameTitleW    = Dll_Alloc((Gui_BoxNameTitleLen + 3) * sizeof(WCHAR));
...
{
	if (len >= (int)Gui_TitleSuffixW_len * 2 && Gui_ShouldCreateTitle(hWnd))
	{
		...
		if (Gui_BoxNameTitleLen)
		{
			...
			if (len >= lenTitle && wmemcmp(lpWindowTitle, ptrTitle, lenTitle) == 0)
			{
				len -= lenTitle;
...
{
	if (len >= (int)Gui_TitleSuffixA_len * 2 && Gui_ShouldCreateTitle(hWnd))
	{
		...
		if (Gui_BoxNameTitleLen)
		{
			...
			if (len >= lenTitle && memcmp(lpWindowTitle, ptrTitle, lenTitle) == 0)
			{
				len -= lenTitle;
...
_FX int Gui_GetWindowTextW(HWND hWnd, WCHAR* lpWindowTitle, int nMaxCount)
{
...
_FX int Gui_GetWindowTextA(HWND hWnd, UCHAR* lpWindowTitle, int nMaxCount)
{
