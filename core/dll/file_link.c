...
static const FILE_DRIVE* File_GetDriveForPath(const WCHAR* Path, ULONG PathLen);

static const FILE_DRIVE* File_GetDriveForUncPath(const WCHAR* Path, ULONG PathLen, ULONG* OutPrefixLen);

static FILE_DRIVE* File_GetDriveForLetter(WCHAR drive_letter);

static const FILE_DRIVE* File_GetDriveAndLinkForPath(const WCHAR* Path, ULONG PathLen, const FILE_LINK** OutLink);

static BOOLEAN File_AddLink(BOOLEAN PermLink, const WCHAR* Src, const WCHAR* Dst);

static void File_RemovePermLinks(const WCHAR* path);

static WCHAR* File_TranslateTempLinks(WCHAR* TruePath, BOOLEAN StripLastPathComponent);

static WCHAR* File_TranslateTempLinks_2(WCHAR* input_str, ULONG input_len);

static FILE_LINK* File_AddTempLink(WCHAR* path);

static ULONG File_FixPermLinksForTempLink(WCHAR* name, ULONG name_len, ULONG max_len);

static FILE_LINK* File_FindPermLinksForMatchPath(const WCHAR* name, ULONG name_len);
...
_FX const FILE_DRIVE* File_GetDriveForPath(const WCHAR* Path, ULONG PathLen)
{
	...
	for (i = 0; i < 26; ++i)
	{
		...
		if (drive)
		{
			if (PathLen >= drive->len && _wcsnicmp(Path, drive->path, drive->len) == 0)
			{
				//
...
_FX const FILE_DRIVE* File_GetDriveForUncPath(const WCHAR* Path, ULONG PathLen, ULONG* OutPrefixLen)
{
	const FILE_DRIVE* drive;
	WCHAR* ptr1a;
	...
	*OutPrefixLen = 0;
	...
	for (i = 0; i < 26; ++i)
	{
		    ...
		    _wcsnicmp(drive->path, File_Mup, 8) == 0)
		{
			...
			if (ptr1b && (ptr1b - drive->path) == pfx_len && *(ptr1b + 1) == L';' && _wcsnicmp(Path, drive->path, pfx_len) == 0)
			{
...
_FX BOOLEAN File_AddLink(BOOLEAN PermLink, const WCHAR* Src, const WCHAR* Dst)
{
	ULONG src_len;
	ULONG dst_len;
	ULONG drive_letter;
	...
	alloc_len = sizeof(FILE_LINK) + (src_len + 4) * sizeof(WCHAR) + (dst_len + 4) * sizeof(WCHAR);
	...
	*link->dst = L'\0';
	...
	if (PermLink && link->src_len == link->dst_len && _wcsicmp(link->src, link->dst) == 0)
	{
		Dll_Free(link);
		return FALSE;
	}
	...
	if (PermLink)
	{
		...
		while (old_link)
		{
			if ((old_link->src_len == link->src_len && _wcsicmp(old_link->src, link->src) == 0)
			    || (old_link->dst_len == link->src_len && _wcsicmp(old_link->dst, link->src) == 0))
			{
				LeaveCriticalSection(File_DrivesAndLinks_CritSec);
	...
	else
	{
		...
		if (link->src_len == link->dst_len && _wcsicmp(link->src, link->dst) == 0)
		{
			link->same = TRUE;
		}
		else
...
_FX void File_RemovePermLinks(const WCHAR* path)
{
	...
	while (old_link)
	{
		...
		if (src_len >= path_len && (src[path_len] == L'\\' || src[path_len] == L'\0') && _wcsnicmp(path, src, path_len) == 0)
		{
...
#define FILE_IS_REDIRECTOR_OR_MUP(str, len) \
	((len >= File_RedirectorLen && 0 == _wcsnicmp(str, File_Redirector, File_RedirectorLen)) || (len >= File_MupLen && 0 == _wcsnicmp(str, File_Mup, File_MupLen)))
...
_FX WCHAR* File_TranslateTempLinks(WCHAR* TruePath, BOOLEAN StripLastPathComponent)
{
	...
	while (link)
	{
		if (link->src_len == TruePath_len && 0 == _wcsnicmp(link->src, TruePath, TruePath_len))
		{
			if (!link->same)
			{
				//
...
_FX WCHAR* File_TranslateTempLinks_2(WCHAR* input_str, ULONG input_len)
{
	...
	while (1)
	{
		...
		while (link)
		{
			if (link->src_len <= work_len && link->src_len > prefix_len
			    && (work_str[link->src_len] == L'\\' || work_str[link->src_len] == L'\0') && 0 == _wcsnicmp(link->src, work_str, link->src_len))
			{
		...
		if (link)
		{
			if (!link->same)
			{
				...
				WCHAR* new_str      = Dll_AllocTemp((dst_len + rem_len) * sizeof(WCHAR));
...
_FX FILE_LINK* File_AddTempLink(WCHAR* path)
{
	...
	status = pNtCreateFile(&handle, FILE_READ_ATTRIBUTES | SYNCHRONIZE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	...
	if (NT_SUCCESS(status))
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			len = File_FixPermLinksForTempLink(newpath, len, (PATH_BUF_LEN - 4) / sizeof(WCHAR));
...
_FX ULONG File_FixPermLinksForTempLink(WCHAR* name, ULONG name_len, ULONG max_len)
{
	//
	...
	while (link)
	{
		...
		if (
		    ...
		    name_len >= src_len && (name[src_len] == L'\\' || name[src_len] == L'\0') && _wcsnicmp(name, link->src, src_len) == 0)
		{
			const ULONG dst_len = link->dst_len;
			if (dst_len + name_len - src_len <= max_len)
			{
				if (src_len != dst_len)
				{
					wmemmove(name + dst_len, name + src_len, name_len - src_len + 1);
				}
...
_FX const FILE_DRIVE* File_GetDriveAndLinkForPath(const WCHAR* Path, ULONG PathLen, const FILE_LINK** OutLink)
{
	//
	// given a path 'name' of length 'name_len'
	// in a buffer with a maximum length of 'max_len'
	...
	*OutLink = NULL;
	...
	while (link)
	{
		...
		if (
		    ...
		    link != File_Wow64FileLink &&
#endif WOW64_FS_REDIR
		    PathLen >= src_len && (Path[PathLen] == L'\\' || Path[PathLen] == L'\0') && _wcsnicmp(Path, link->src, src_len) == 0)
		{
			*OutLink = link;
			break;
		}
...
_FX FILE_LINK* File_FindPermLinksForMatchPath(const WCHAR* name, ULONG name_len)
{
	//
	...
	while (link)
	{
		...
		if (
		    ...
		    link != File_Wow64FileLink &&
#endif WOW64_FS_REDIR
		    name_len >= dst_len && (name[dst_len] == L'\\' || name[dst_len] == L'\0') && _wcsnicmp(name, link->dst, dst_len) == 0)
		{
			return link;
		}
