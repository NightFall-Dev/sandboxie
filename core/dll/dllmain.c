...
_FX BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
...
_FX void Dll_InitInjected(void)
{
	...
	status = SbieApi_QueryProcessEx2((HANDLE)(ULONG_PTR)Dll_ProcessId, 255, Dll_BoxNameSpace, Dll_ImageNameSpace, Dll_SidStringSpace, &Dll_SessionId, NULL);
	...
	status = SbieApi_QueryBoxPath(NULL, NULL, NULL, NULL, &BoxFilePathLen, &BoxKeyPathLen, &BoxIpcPathLen);
	if (status != 0)
	{
		SbieApi_Log(2304, Dll_ImageName);
		ExitProcess(-1);
	...
	status = SbieApi_QueryBoxPath(NULL, (WCHAR*)Dll_BoxFilePath, (WCHAR*)Dll_BoxKeyPath, (WCHAR*)Dll_BoxIpcPath, &BoxFilePathLen, &BoxKeyPathLen, &BoxIpcPathLen);
	if (status != 0)
	{
		SbieApi_Log(2304, Dll_ImageName);
		ExitProcess(-1);
...
_FX void Dll_SelectImageType(void)
{
	...
	static const WCHAR* _ImageNames[] = {
	    ...
	    SANDBOXIE L"RpcSs.exe",
	    (WCHAR*)DLL_IMAGE_SANDBOXIE_RPCSS,
	    SANDBOXIE L"DcomLaunch.exe",
	    (WCHAR*)DLL_IMAGE_SANDBOXIE_DCOMLAUNCH,
	    SANDBOXIE L"Crypto.exe",
	    (WCHAR*)DLL_IMAGE_SANDBOXIE_CRYPTO,
	    SANDBOXIE L"WUAU.exe",
	    (WCHAR*)DLL_IMAGE_SANDBOXIE_WUAU,
	    SANDBOXIE L"BITS.exe",
	    (WCHAR*)DLL_IMAGE_SANDBOXIE_BITS,
	    SBIESVC_EXE,
	    (WCHAR*)DLL_IMAGE_SANDBOXIE_SBIESVC,

	    L"TrustedInstaller.exe",
	    (WCHAR*)DLL_IMAGE_TRUSTED_INSTALLER,
	    L"TiWorker.exe",
	    (WCHAR*)DLL_IMAGE_TRUSTED_INSTALLER,
	    L"wuauclt.exe",
	    (WCHAR*)DLL_IMAGE_WUAUCLT,
	    L"explorer.exe",
	    (WCHAR*)DLL_IMAGE_SHELL_EXPLORER,
	    L"rundll32.exe",
	    (WCHAR*)DLL_IMAGE_RUNDLL32,
	    L"dllhost.exe",
	    (WCHAR*)DLL_IMAGE_DLLHOST,

	    L"iexplore.exe",
	    (WCHAR*)DLL_IMAGE_INTERNET_EXPLORER,
	    L"firefox.exe",
	    (WCHAR*)DLL_IMAGE_MOZILLA_FIREFOX,
	    L"wmplayer.exe",
	    (WCHAR*)DLL_IMAGE_WINDOWS_MEDIA_PLAYER,
	    L"winamp.exe",
	    (WCHAR*)DLL_IMAGE_NULLSOFT_WINAMP,
	    L"kmplayer.exe",
	    (WCHAR*)DLL_IMAGE_PANDORA_KMPLAYER,
	    L"wlmail.exe",
	    (WCHAR*)DLL_IMAGE_WINDOWS_LIVE_MAIL,
	    L"ServiceModelReg.exe",
	    (WCHAR*)DLL_IMAGE_SERVICE_MODEL_REG,
	    L"wisptis.exe",
	    (WCHAR*)DLL_IMAGE_WISPTIS,
	    L"iron.exe",
	    (WCHAR*)DLL_IMAGE_GOOGLE_CHROME,
	    L"dragon.exe",
	    (WCHAR*)DLL_IMAGE_GOOGLE_CHROME,
	    L"chrome.exe",
	    (WCHAR*)DLL_IMAGE_GOOGLE_CHROME,
	    ...
	    (WCHAR*)DLL_IMAGE_GOOGLE_CHROME,
	    L"GoogleUpdate.exe",
	    (WCHAR*)DLL_IMAGE_GOOGLE_UPDATE,
	    L"AcroRd32.exe",
	    (WCHAR*)DLL_IMAGE_ACROBAT_READER,
	    L"Acrobat.exe",
	    (WCHAR*)DLL_IMAGE_ACROBAT_READER,
	    L"plugin-container.exe",
	    (WCHAR*)DLL_IMAGE_PLUGIN_CONTAINER,
	    L"Outlook.exe",
	    (WCHAR*)DLL_IMAGE_OFFICE_OUTLOOK,
	    L"Excel.exe",
	    (WCHAR*)DLL_IMAGE_OFFICE_EXCEL,
	    NULL,
	    NULL};
	...
	if (Dll_ImageType == DLL_IMAGE_UNSPECIFIED && _wcsnicmp(Dll_ImageName, L"FlashPlayerPlugin_", 18) == 0)
	{
	...
	if (Dll_ImageType == DLL_IMAGE_UNSPECIFIED
	    && (_wcsicmp(Dll_ImageName, L"SchTasks.exe") == 0 || _wcsicmp(Dll_ImageName, L"cvh.exe") == 0 // Office 2010 virt
	        || 0))
	{
	...
	if (Dll_RestrictedToken)
	{
		if (Dll_ImageType == DLL_IMAGE_GOOGLE_CHROME || Dll_ImageType == DLL_IMAGE_ACROBAT_READER || Dll_ImageType == DLL_IMAGE_FLASH_PLAYER_SANDBOX)
		{
...
_FX ULONG_PTR Dll_Ordinal1(ULONG_PTR arg1, ULONG_PTR arg2, ULONG_PTR arg3, ULONG_PTR arg4, ULONG_PTR arg5)
{
	struct _INJECT_DATA
	{ // keep in sync with core/low/inject.c
	...
	typedef ULONG_PTR (*P_RtlFindActivationContextSectionString)(ULONG_PTR arg1, ULONG_PTR arg2, ULONG_PTR arg3, ULONG_PTR arg4, ULONG_PTR arg5);
	...
	VirtualProtect((void*)(ULONG_PTR)inject->RtlFindActCtx, 5, inject->RtlFindActCtx_Protect, &dummy_prot);

	arg1 = (ULONG_PTR)inject->RtlFindActCtx_SavedArg1;

	RtlFindActCtx = (P_RtlFindActivationContextSectionString)inject->RtlFindActCtx;
