...
HANDLE Gui_GetPropCommon(HWND hWnd, const void* lpString, BOOLEAN unicode, ULONG LastError);
...
static ULONG_PTR Gui_GetWindowLong_DragDrop(HWND hWnd, int nIndex, ULONG_PTR value);

static ULONG_PTR Gui_SetWindowLong_DragDrop(HWND hWnd, int nIndex, ULONG_PTR dwNew, ULONG_PTR value);
...
static ULONG_PTR Gui_SetWindowLong8(HWND hWnd, int nIndex, ULONG_PTR dwNew, ULONG IsAscii);
...
static ULONG_PTR Gui_SetWindowLongPtrW(HWND hWnd, int nIndex, ULONG_PTR dwNew);

static ULONG_PTR Gui_SetWindowLongPtrA(HWND hWnd, int nIndex, ULONG_PTR dwNew);

static ULONG_PTR Gui_SetWindowLongPtr8(HWND hWnd, int nIndex, ULONG_PTR dwNew, ULONG IsAscii);
...
_FX void Gui_InitPropAtoms(void)
{
	...
	static const WCHAR* _OleDropTargetMarshalHwnd = L"OleDropTargetMarshalHwnd";
	static const WCHAR* _OleEndPointID            = L"OleEndPointID";
	static const WCHAR* _format                   = L"Sandbox:%s:%s";
	WCHAR name[128];

	if (!Gui_OleDropTargetInterface_Atom)
	{
		Gui_OleDropTargetInterface_Atom = GlobalAddAtomW(_OleDropTargetInterface);
	...
	{
		Gui_OleDropTargetMarshalHwnd_Atom = GlobalAddAtomW(_OleDropTargetMarshalHwnd);
	...
	{
		Gui_OleEndPointID_Atom = GlobalAddAtomW(_OleEndPointID);
...
_FX HANDLE Gui_GetPropCommon(HWND hWnd, const void* lpString, BOOLEAN unicode, ULONG LastError)
{
...
_FX ULONG_PTR Gui_GetLongCommon(HWND hWnd, int nIndex, ULONG which)
{
	...
	else
	{
		...
		req.index = nIndex;
		req.which = which;
		rpl       = Gui_CallProxy(&req, sizeof(req), sizeof(GUI_GET_WINDOW_LONG_RPL));
		if (!rpl)
		{
			return FALSE;
		}
...
_FX ULONG_PTR Gui_GetWindowLong_DragDrop(HWND hWnd, int nIndex, ULONG_PTR value)
{
	if (nIndex == GWL_EXSTYLE && (value & WS_EX_ACCEPTFILES))
	{
		...
		void* pDropTarget = __sys_GetPropW(hWnd, (LPCWSTR)Gui_DropTargetProp_Atom);
		if (pDropTarget)
		{
...
_FX ULONG_PTR Gui_SetWindowLong_DragDrop(HWND hWnd, int nIndex, ULONG_PTR dwNew, ULONG_PTR value)
{
...
_FX ULONG Gui_GetWindowLongW(HWND hWnd, int nIndex)
{
	...
	if (Gui_RenameClasses && (nIndex == GWLP_WNDPROC))
	{
		...
		{
			value = (ULONG_PTR)__sys_GetPropW(hWnd, (LPCWSTR)Gui_WindowProcOldW_Atom);
		}
	}
...
_FX ULONG Gui_GetWindowLongA(HWND hWnd, int nIndex)
{
	...
	if (Gui_RenameClasses && (nIndex == GWLP_WNDPROC))
	{
		...
		if ((w && value == (ULONG_PTR)w) || (a && value == (ULONG_PTR)a))
		{
			value = (ULONG_PTR)__sys_GetPropW(hWnd, (LPCWSTR)Gui_WindowProcOldA_Atom);
		}
	}
...
_FX ULONG_PTR Gui_SetWindowLong8(HWND hWnd, int nIndex, ULONG_PTR dwNew, ULONG IsAscii)
{
	ULONG_PTR value;
	Gui_InitPropAtoms();
	...
	if (Gui_RenameClasses && (nIndex == GWLP_WNDPROC))
	{
		...
		if ((w && value == (ULONG_PTR)w) || (a && value == (ULONG_PTR)a))
		{
			ATOM atom = IsAscii ? Gui_WindowProcOldA_Atom : Gui_WindowProcOldW_Atom;
...
_FX ULONG_PTR Gui_SetWindowLongPtr8(HWND hWnd, int nIndex, ULONG_PTR dwNew, ULONG IsAscii)
{
	ULONG_PTR value;
	Gui_InitPropAtoms();
...
_FX BOOLEAN Gui_Hook_SetWindowLongPtr8(void)
{
	...
	if (*(ULONG*)a == 0x0001B941 && *(ULONG*)w == 0xE9C93345)
	{
		...
		if ((a + 11 + a_offset) == (w + 8 + w_offset))
		{
			__sys_SetWindowLongPtr8 = (P_SetWindowLongPtr8)(w + 8 + w_offset);
...
_FX void Gui_SetWindowProc(HWND hWnd, BOOLEAN force)
{
	...
	{
		__sys_SetWindowLongPtrW(hWnd, GWLP_WNDPROC, (ULONG_PTR)Gui_WindowProcW);
	}
	else
	{
		__sys_SetWindowLongPtrA(hWnd, GWLP_WNDPROC, (ULONG_PTR)Gui_WindowProcA);
	}
