...
_FX BOOLEAN HNet_Init(HMODULE module)
{
	...
	IcfOpenDynamicFwPort = (P_IcfOpenDynamicFwPort)GetProcAddress(module, "IcfOpenDynamicFwPort");
...
static int WSA_WSANSPIoctl(HANDLE hLookup, DWORD dwControlCode, void* lpvInBuffer, DWORD cbInBuffer, void* lpvOutBuffer, DWORD cbOutBuffer, DWORD* lpcbBytesReturned, WSACOMPLETION* lpCompletion);

static int WSA_IsBlockedPort(const short* addr, int addrlen);

static int WSA_connect(SOCKET s, const void* name, int namelen);

static int WSA_WSAConnect(SOCKET s, const void* name, int namelen, void* lpCallerData, void* lpCalleeData, void* lpSQOS, void* lpGQOS);


//---------------------------------------------------------------------------


typedef int (*P_WSANSPIoctl)(HANDLE hLookup, DWORD dwControlCode, void* lpvInBuffer, DWORD cbInBuffer, void* lpvOutBuffer, DWORD cbOutBuffer, DWORD* lpcbBytesReturned, WSACOMPLETION* lpCompletion);

typedef int (*P_connect)(SOCKET s, const struct sockaddr* name, int namelen);

typedef int (*P_WSAConnect)(SOCKET s, const struct sockaddr* name, int namelen, void* lpCallerData, void* lpCalleeData, void* lpSQOS, void* lpGQOS);
...
_FX int WSA_WSANSPIoctl(HANDLE hLookup, DWORD dwControlCode, void* lpvInBuffer, DWORD cbInBuffer, void* lpvOutBuffer, DWORD cbOutBuffer, DWORD* lpcbBytesReturned, WSACOMPLETION* lpCompletion)
{
	...
	int rv = __sys_WSANSPIoctl(hLookup, dwControlCode, lpvInBuffer, cbInBuffer, lpvOutBuffer, cbOutBuffer, lpcbBytesReturned, lpCompletion);
...
_FX int WSA_WSAConnect(SOCKET s, const void* name, int namelen, void* lpCallerData, void* lpCalleeData, void* lpSQOS, void* lpGQOS)
{
	if (WSA_IsBlockedPort(name, namelen))
	{
		return SOCKET_ERROR;
...
_FX void WSA_InitBlockedPorts_AddPort(USHORT port1, USHORT port2, BOOLEAN setOrClear)
{
	ULONG index;
	...
	if (index >= WSA_MaxBlockedPorts)
	{
		...
		if (WSA_BlockedPorts)
		{
			memcpy(NewTable, WSA_BlockedPorts, WSA_MaxBlockedPorts * sizeof(ULONG_PTR));
		}
...
_FX void WSA_InitBlockedPorts(void)
{
	...
	while (1)
	{
		...
		NTSTATUS status = SbieApi_QueryConf(NULL, L"BlockPort", index, text, 1020 * sizeof(WCHAR));
		++index;
...
static ULONG NetApi_NetUseAdd(WCHAR* ServerName, ULONG Level, UCHAR* Buf, ULONG* ParmError);


//---------------------------------------------------------------------------


typedef ULONG (*P_NetUseAdd)(WCHAR* ServerName, ULONG Level, UCHAR* Buf, ULONG* ParmError);
...
_FX ULONG NetApi_NetUseAdd(WCHAR* ServerName, ULONG Level, UCHAR* Buf, ULONG* ParmError)
{
		...
		info0
			...
			if (req->level >= 0)
			{
				if (info0->ui0_local)
				{
					...
					wmemcpy(req->ui0_local, info0->ui0_local, req->ui0_local_len);
				}
				else
				{
					req->ui0_local_len = -1;
				...
				if (info0->ui0_remote)
				{
					...
					wmemcpy(req->ui0_remote, info0->ui0_remote, req->ui0_remote_len);
				}
				else
				{
			...
			if (req->level >= 1)
			{
				if (info1->ui1_password)
				{
					...
					wmemcpy(req->ui1_password, info1->ui1_password, req->ui1_password_len);
				}
				else
				{
			...
			if (req->level >= 2)
			{
				if (info2->ui2_username)
				{
					...
					wmemcpy(req->ui2_username, info2->ui2_username, req->ui2_username_len);
				}
				else
				{
				...
				if (info2->ui2_domainname)
				{
					...
					wmemcpy(req->ui2_domainname, info2->ui2_domainname, req->ui2_domainname_len);
				}
				else
				{
			...
			if (req->level >= 4)
			{
				...
				if (info4->ui4_auth_identity_length && info4->ui4_auth_identity)
				{
					...
					memcpy(req->ui4_auth_identity, info4->ui4_auth_identity, req->ui4_auth_identity_length);
				}
				else
				{
			...
			drive_number = 0;
			if (req->ui0_local_len != -1 && req->ui0_local_len >= 2 && req->ui0_local[1] == L':')
			{
			...
			{
				err = rpl->h.status;
				if (ParmError && err == ERROR_INVALID_PARAMETER && rpl->h.length >= sizeof(NETAPI_USE_ADD_RPL))
				{
...
static HRESULT Net_INetFwAuthorizedApplication_ImageNamePut(void* pUnknown, BSTR imageFileName);

/*static HRESULT Net_INetFwRule_ImageNameGet(
    void *pUnknown, BSTR *imageFileName);*/

static HRESULT Net_INetFwRule_ImageNamePut(void* pUnknown, BSTR imageFileName);

/*static HRESULT Net_Common_ImageNameGet(
    void *pUnknown, BSTR *imageFileName, ULONG_PTR pOrigImageNameGet);*/

static HRESULT Net_Common_ImageNamePut(void* pUnknown, BSTR imageFileName, ULONG_PTR pOrigImageNamePut);
...
_FX void Net_Firewall_Hook(ULONG type, void* pUnknown)
{
	...
	if (VirtualProtect(lpVtbl, sizeof(ULONG_PTR) * 2, PAGE_READWRITE, &prot))
	{
		/*if (lpVtbl[0] != pNewGet) {
...
_FX HRESULT Net_INetFwAuthorizedApplication_ImageNamePut(void* pUnknown, BSTR imageFileName)
{
	return Net_Common_ImageNamePut(pUnknown, imageFileName, Net_INetFwAuthorizedApplication_orig_ImageNamePut);
}
...
_FX HRESULT Net_INetFwRule_ImageNamePut(void* pUnknown, BSTR imageFileName)
{
	return Net_Common_ImageNamePut(pUnknown, imageFileName, Net_INetFwRule_orig_ImageNamePut);
}
...
_FX HRESULT Net_Common_ImageNamePut(void* pUnknown, BSTR imageFileName, ULONG_PTR pOrigImageNamePut)
{
	HANDLE hFile;
	ULONG len;
	WCHAR* path;
	...
	hFile = CreateFile(path, GENERIC_READ, FILE_SHARE_VALID_FLAGS, NULL, OPEN_EXISTING, 0, NULL);
	...
	hr = ((P_INetFwXxx_put_ImageName)pOrigImageNamePut)(pUnknown, imageFileName);
