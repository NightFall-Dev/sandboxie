...
static void* Scm_QueryBoxedServiceByName(const WCHAR* ServiceNm, ULONG with_service_status, ULONG with_service_config);

void* Scm_QueryServiceByName(const WCHAR* ServiceNm, ULONG with_service_status, ULONG with_service_config);

static void* Scm_QueryServiceByHandle(HANDLE hService, ULONG with_service_status, ULONG with_service_config);

static BOOL Scm_QueryServiceStatusEx(SC_HANDLE hService, SC_STATUS_TYPE InfoLevel, LPBYTE lpBuffer, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

static BOOL Scm_QueryServiceStatus(SC_HANDLE hService, SERVICE_STATUS* lpServiceStatus);

static BOOL Scm_QueryServiceConfigW(SC_HANDLE hService, void* lpServiceConfig, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

static BOOL Scm_QueryServiceConfigA(SC_HANDLE hService, void* lpServiceConfig, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

static BOOL Scm_QueryServiceConfig2W(SC_HANDLE hService, DWORD InfoLevel, LPBYTE lpBuffer, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

static BOOL Scm_QueryServiceConfig2A(SC_HANDLE hService, DWORD InfoLevel, LPBYTE lpBuffer, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

static BOOL Scm_EnumServicesStatusX(SC_HANDLE hSCManager, DWORD dwServiceType, DWORD dwServiceState, LPENUM_SERVICE_STATUS lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle, BOOL IsUnicode, BOOL IsExtended);

static BOOL Scm_EnumServicesStatusW(SC_HANDLE hSCManager, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle);

static BOOL Scm_EnumServicesStatusA(SC_HANDLE hSCManager, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle);

static BOOL Scm_EnumServicesStatusExW(SC_HANDLE hSCManager, SC_ENUM_TYPE InfoLevel, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle, void* GroupName);

static BOOL Scm_EnumServicesStatusExA(SC_HANDLE hSCManager, SC_ENUM_TYPE InfoLevel, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle, void* GroupName);

static BOOL Scm_QueryServiceLockStatusW(SC_HANDLE hService, void* lpServiceStatus, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

static BOOL Scm_QueryServiceLockStatusA(SC_HANDLE hService, void* lpServiceStatus, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

static BOOL Scm_GetServiceDisplayNameW(SC_HANDLE hSCManager, WCHAR* lpServiceName, WCHAR* lpDisplayName, LPDWORD lpcchBuffer);

static BOOL Scm_GetServiceDisplayNameA(SC_HANDLE hSCManager, UCHAR* lpServiceName, UCHAR* lpDisplayName, LPDWORD lpcchBuffer);

static BOOL Scm_GetServiceKeyNameW(SC_HANDLE hSCManager, WCHAR* lpDisplayName, WCHAR* lpServiceName, LPDWORD lpcchBuffer);

static BOOL Scm_GetServiceKeyNameA(SC_HANDLE hSCManager, UCHAR* lpDisplayName, UCHAR* lpServiceName, LPDWORD lpcchBuffer);

static BOOL Scm_EnumDependentServicesW(SC_HANDLE hService, DWORD dwServiceState, LPENUM_SERVICE_STATUS lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned);

static BOOL Scm_EnumDependentServicesA(SC_HANDLE hService, DWORD dwServiceState, LPENUM_SERVICE_STATUS lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned);

static BOOL Scm_QueryServiceObjectSecurity(SC_HANDLE hService, SECURITY_INFORMATION dwSecurityInformation, PSECURITY_DESCRIPTOR lpSecurityDescriptor, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

static BOOL Scm_SetServiceObjectSecurity(SC_HANDLE hService, SECURITY_INFORMATION dwSecurityInformation, PSECURITY_DESCRIPTOR lpSecurityDescriptor);
...
_FX WCHAR* Scm_GetServiceConfigString(SERVICE_QUERY_RPL* rpl, UCHAR type)
{
	...
	if (Dll_IsWow64)
	{
		...
		QUERY_SERVICE_CONFIG_64* cfg = (QUERY_SERVICE_CONFIG_64*)&rpl->service_config;
	...
	else
	{
		...
		QUERY_SERVICE_CONFIG* cfg = (QUERY_SERVICE_CONFIG*)&rpl->service_config;
...
_FX void* Scm_QueryBoxedServiceByName(const WCHAR* ServiceNm, ULONG with_service_status, ULONG with_service_config)
{
	NTSTATUS status;
	...
	else if (with_service_config == -1)
	{
		...
		if (NT_SUCCESS(status) && u.info.Type == REG_SZ && u.info.DataLength != 0)
		{
		...
		if (NT_SUCCESS(status) && u.info.DataLength != 0 && (u.info.Type == REG_SZ || u.info.Type == REG_EXPAND_SZ))
		{
			u.info_space[250] = L'\0';
		}
		else
		...
		if (len > 9)
		{
			...
			len       = ExpandEnvironmentStrings((WCHAR*)u.info.Data, ImagePath, len + 8);
		}
	...
	{
		service_config_len = sizeof(SERVICE_DESCRIPTION) + (wcslen(ServiceNm) + 1) * sizeof(WCHAR);
	...
	if (NT_SUCCESS(status) && u.info.Type == REG_DWORD && u.info.DataLength == 4)
	{
		...
		status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)pid, boxname, imagename, NULL, &session_id);

		if (NT_SUCCESS(status) && session_id == Dll_SessionId && _wcsicmp(boxname, Dll_BoxName) == 0)
		{
			...
			if (NT_SUCCESS(status) && u.info.DataLength != 0 && (u.info.Type == REG_SZ || u.info.Type == REG_EXPAND_SZ))
			{
				...
				else
				{
					...
					for (MySvc = MySvcs; *MySvc; MySvc += 2)
					{
						...
						len = (ULONG)(ULONG_PTR)(wcsrchr(MySvc[1], L'.') - MySvc[1]);
	...
	if (NT_SUCCESS(status) && u.info.Type == REG_DWORD && u.info.DataLength == 4)
	{
		rpl->service_status.dwServiceSpecificExitCode = *(ULONG*)u.info.Data;
	}
...
after_service_status:
	...
	if (with_service_config == -1)
	{
		...
		if (Dll_IsWow64)
		{
			p_lpBinaryPathName   = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG_64, lpBinaryPathName));
			p_lpLoadOrderGroup   = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG_64, lpLoadOrderGroup));
			p_lpDependencies     = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG_64, lpDependencies));
			p_lpServiceStartName = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG_64, lpServiceStartName));
			p_lpDisplayName      = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG_64, lpDisplayName));

			next = (WCHAR*)(base + sizeof(QUERY_SERVICE_CONFIG_64));
		}
		else
		{
			p_lpBinaryPathName   = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG, lpBinaryPathName));
			p_lpLoadOrderGroup   = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG, lpLoadOrderGroup));
			p_lpDependencies     = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG, lpDependencies));
			p_lpServiceStartName = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG, lpServiceStartName));
			p_lpDisplayName      = (ULONG_PTR*)(base + FIELD_OFFSET(QUERY_SERVICE_CONFIG, lpDisplayName));
		...
		*next = L'\0';
		...
		rpl->service_config.dwServiceType = rpl->service_status.dwServiceType;
...
_FX void* Scm_QueryServiceByName(const WCHAR* ServiceNm, ULONG with_service_status, ULONG with_service_config)
{
	union {
	...
	{
		return Scm_QueryBoxedServiceByName(ServiceNm, with_service_status, with_service_config);
	}
	...
	u.req.h.length = sizeof(SERVICE_QUERY_REQ) + (u.req.name_len + 1) * sizeof(WCHAR);
...
_FX void* Scm_QueryServiceByHandle(HANDLE hService, ULONG with_service_status, ULONG with_service_config)
{
	...
	return Scm_QueryServiceByName(ServiceNm, with_service_status, with_service_config);
}
...
_FX BOOL Scm_QueryServiceStatusEx(SC_HANDLE hService, SC_STATUS_TYPE InfoLevel, LPBYTE lpBuffer, DWORD cbBufSize, LPDWORD pcbBytesNeeded)
{
	SERVICE_QUERY_RPL* rpl;
...
_FX BOOL Scm_QueryServiceStatus(SC_HANDLE hService, SERVICE_STATUS* lpServiceStatus)
{
	SERVICE_STATUS_PROCESS status;
	ULONG len = sizeof(SERVICE_STATUS_PROCESS);
	BOOL ok   = Scm_QueryServiceStatusEx(hService, SC_STATUS_PROCESS_INFO, (BYTE*)&status, len, &len);
	if (ok)
	{
...
_FX BOOL Scm_QueryServiceConfigW(SC_HANDLE hService, void* lpServiceConfig, DWORD cbBufSize, LPDWORD pcbBytesNeeded)
{
	SERVICE_QUERY_RPL* rpl;
...
#undef COPY_FIELD_64_TO_32
	...
	else
	{
		...
		QUERY_SERVICE_CONFIG* cfg  = (QUERY_SERVICE_CONFIG*)&rpl->service_config;
		QUERY_SERVICE_CONFIGA* out = (QUERY_SERVICE_CONFIGA*)lpServiceConfig;
...
_FX BOOL Scm_QueryServiceConfigA(SC_HANDLE hService, void* lpServiceConfig, DWORD cbBufSize, LPDWORD pcbBytesNeeded)
{
...
_FX BOOL Scm_QueryServiceConfig2W(SC_HANDLE hService, DWORD InfoLevel, LPBYTE lpBuffer, DWORD cbBufSize, LPDWORD pcbBytesNeeded)
{
	SERVICE_QUERY_RPL* rpl;

	rpl = (SERVICE_QUERY_RPL*)Scm_QueryServiceByHandle(hService, FALSE, InfoLevel);
	if (!rpl)
	{
		return FALSE;
	}
	...
	{
		SERVICE_FAILURE_ACTIONSA* cfga = (SERVICE_FAILURE_ACTIONSA*)lpBuffer;
		if (cfga->lpRebootMsg)
		{
		...
		if (cfga->lpsaActions)
		{
			UCHAR* lpsaActions = (UCHAR*)cfga->lpsaActions + (ULONG_PTR)cfga;
...
_FX BOOL Scm_QueryServiceConfig2A(SC_HANDLE hService, DWORD InfoLevel, LPBYTE lpBuffer, DWORD cbBufSize, LPDWORD pcbBytesNeeded)
{
	UNICODE_STRING uni;
	ANSI_STRING ansi;
	ULONG len;
	...
	BOOL ok = Scm_QueryServiceConfig2W(hService, InfoLevel, lpBuffer, cbBufSize, pcbBytesNeeded);
	if (!ok)
	{
		return FALSE;
	}
...
#define MyUnicodeToAnsiString2A(m)                        \
	...
	if (InfoLevel == SERVICE_CONFIG_DESCRIPTION)
	{
		SERVICE_DESCRIPTIONW* info = (SERVICE_DESCRIPTIONW*)lpBuffer;

		MyUnicodeToAnsiString2A(lpDescription);
	}
	else if (InfoLevel == SERVICE_CONFIG_FAILURE_ACTIONS)
	{
		SERVICE_FAILURE_ACTIONSW* info = (SERVICE_FAILURE_ACTIONSW*)lpBuffer;
...
_FX BOOL Scm_EnumServicesStatusX(SC_HANDLE hSCManager, DWORD dwServiceType, DWORD dwServiceState, LPENUM_SERVICE_STATUS lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle, BOOL IsUnicode, BOOL IsExtended)
{
	...
	if ((dwServiceType & SERVICE_TYPE_ALL) == 0 || (dwServiceState & SERVICE_STATE_ALL) == 0)
	{
	...
	while (*names)
	{
		...
		hService = Scm_OpenServiceW(hSCManager, ServiceName, SERVICE_QUERY_STATUS);
		if (!hService)
		{
			continue;
		}

		if (!Scm_QueryServiceStatusEx(hService, SC_STATUS_PROCESS_INFO, (BYTE*)&ss, sizeof(SERVICE_STATUS_PROCESS), &svclen))
		{
			continue;
		}
...
_FX BOOL Scm_EnumServicesStatusW(SC_HANDLE hSCManager, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle)
{
	return Scm_EnumServicesStatusX(hSCManager, dwServiceType, dwServiceState, lpServices, cbBufSize, pcbBytesNeeded, lpServicesReturned, lpResumeHandle, TRUE, FALSE);
}
...
_FX BOOL Scm_EnumServicesStatusA(SC_HANDLE hSCManager, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle)
{
	return Scm_EnumServicesStatusX(hSCManager, dwServiceType, dwServiceState, lpServices, cbBufSize, pcbBytesNeeded, lpServicesReturned, lpResumeHandle, FALSE, FALSE);
}
...
_FX BOOL Scm_EnumServicesStatusExW(SC_HANDLE hSCManager, SC_ENUM_TYPE InfoLevel, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle, void* GroupName)
{
	...
	return Scm_EnumServicesStatusX(hSCManager, dwServiceType, dwServiceState, lpServices, cbBufSize, pcbBytesNeeded, lpServicesReturned, lpResumeHandle, TRUE, TRUE);
}
...
_FX BOOL Scm_EnumServicesStatusExA(SC_HANDLE hSCManager, SC_ENUM_TYPE InfoLevel, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle, void* GroupName)
{
	if (InfoLevel != SC_ENUM_PROCESS_INFO)
	{
		SetLastError(ERROR_INVALID_LEVEL);
	...
	return Scm_EnumServicesStatusX(hSCManager, dwServiceType, dwServiceState, lpServices, cbBufSize, pcbBytesNeeded, lpServicesReturned, lpResumeHandle, FALSE, TRUE);
}
...
_FX BOOL Scm_QueryServiceLockStatusW(SC_HANDLE hService, void* lpServiceStatus, DWORD cbBufSize, LPDWORD pcbBytesNeeded)
{
	*pcbBytesNeeded = sizeof(QUERY_SERVICE_LOCK_STATUSW) + sizeof(WCHAR);
	...
	((QUERY_SERVICE_LOCK_STATUSW*)lpServiceStatus)->lpLockOwner = (WCHAR*)((UCHAR*)lpServiceStatus + sizeof(QUERY_SERVICE_LOCK_STATUSW));
...
_FX BOOL Scm_QueryServiceLockStatusA(SC_HANDLE hService, void* lpServiceStatus, DWORD cbBufSize, LPDWORD pcbBytesNeeded)
{
	*pcbBytesNeeded = sizeof(QUERY_SERVICE_LOCK_STATUSA) + sizeof(WCHAR); // +2 bytes even in ASCII version
	if (cbBufSize < *pcbBytesNeeded)
	{
		SetLastError(ERROR_INSUFFICIENT_BUFFER);
		return FALSE;
	...
	memzero(lpServiceStatus, *pcbBytesNeeded);
	((QUERY_SERVICE_LOCK_STATUSA*)lpServiceStatus)->lpLockOwner = (UCHAR*)lpServiceStatus + sizeof(QUERY_SERVICE_LOCK_STATUSA);
	SetLastError(ERROR_SUCCESS);
	return TRUE;
}
...
_FX BOOL Scm_GetServiceDisplayNameW(SC_HANDLE hSCManager, WCHAR* lpServiceName, WCHAR* lpDisplayName, LPDWORD lpcchBuffer)
{
	SERVICE_QUERY_RPL* rpl;
	ULONG need_len;
	WCHAR* display;
...
_FX BOOL Scm_GetServiceDisplayNameA(SC_HANDLE hSCManager, UCHAR* lpServiceName, UCHAR* lpDisplayName, LPDWORD lpcchBuffer)
{
	ANSI_STRING ansi;
	UNICODE_STRING uni;
	SERVICE_QUERY_RPL* rpl;
...
_FX BOOL Scm_GetServiceKeyNameW(SC_HANDLE hSCManager, WCHAR* lpDisplayName, WCHAR* lpServiceName, LPDWORD lpcchBuffer)
{
...
_FX BOOL Scm_GetServiceKeyNameA(SC_HANDLE hSCManager, UCHAR* lpDisplayName, UCHAR* lpServiceName, LPDWORD lpcchBuffer)
{
	ANSI_STRING ansi;
	UNICODE_STRING uni;
	WCHAR *names_base, *names, *display;
...
_FX BOOL Scm_EnumDependentServicesW(SC_HANDLE hService, DWORD dwServiceState, LPENUM_SERVICE_STATUS lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned)
{
...
_FX BOOL Scm_EnumDependentServicesA(SC_HANDLE hService, DWORD dwServiceState, LPENUM_SERVICE_STATUS lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned)
{
	*lpServicesReturned = 0;
	SetLastError(0);
	return TRUE;
...
_FX BOOL Scm_QueryServiceObjectSecurity(SC_HANDLE hService, SECURITY_INFORMATION dwSecurityInformation, PSECURITY_DESCRIPTOR lpSecurityDescriptor, DWORD cbBufSize, LPDWORD pcbBytesNeeded)
{
	...
	static const UCHAR dacl[96] = {2, // AclRevision
	    0,                            // Sbz1
	    96,
	    0, // AclSize
	    4,
	...
	if ((!dwSecurityInformation) || (dwSecurityInformation & ~(DACL_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | OWNER_SECURITY_INFORMATION | SACL_SECURITY_INFORMATION)))
	{
		SetLastError(ERROR_INVALID_PARAMETER);
		return FALSE;
	}
...
_FX BOOL Scm_SetServiceObjectSecurity(SC_HANDLE hService, SECURITY_INFORMATION dwSecurityInformation, PSECURITY_DESCRIPTOR lpSecurityDescriptor)
{
	if (!Scm_GetHandleName(hService))
	{
		return FALSE;
