...
static ULONG_PTR Gdi_GdiDllInitialize_XP(HMODULE User32, BOOLEAN Init, ULONG_PTR Unknown3);

static ULONG_PTR Gdi_GdiDllInitialize_Vista(HMODULE User32, BOOLEAN Init);

static ULONG_PTR Gdi_GdiDllInitialize_Common(ULONG NumArgs, HMODULE User32, BOOLEAN Init, ULONG_PTR Unknown3);

#ifndef _WIN64

static HDC Gdi_CreateDCW(void* lpszDriver, void* lpszDevice, void* lpszOutput, void* lpInitData);

#endif !_WIN64

static WCHAR* Gdi_GetFontPath(const WCHAR* path);

static ULONG Gdi_GdiAddFontResourceW(const WCHAR* path, ULONG flags, void* reserved);

static ULONG Gdi_RemoveFontResourceExW(const WCHAR* path, ULONG flags, void* reserved);

static ULONG Gdi_GetFontResourceInfoW(const WCHAR* path, ULONG* buflen, void* bufptr, ULONG querytype);

static ULONG Gdi_CreateScalableFontResourceW(ULONG hidden, const WCHAR* res, const WCHAR* file, const WCHAR* path);

static void Gdi_AddFontsInBox(void);

static void Gdi_AddFontsInBox_2(HANDLE hFontsDir, void* buf8k, WCHAR* WinFonts);

static int Gdi_EnumFontFamiliesExA(HDC hdc, void* lpLogfont, void* lpEnumFontFamExProc, LPARAM lParam, DWORD dwFlags);

static int Gdi_EnumFontFamiliesExW(HDC hdc, void* lpLogfont, void* lpEnumFontFamExProc, LPARAM lParam, DWORD dwFlags);
...
#ifndef _WIN64
static BOOL Gdi_OpenPrinter2W(void* pPrinterName, HANDLE* phPrinter, void* pDefault, void* pOptions);
...
typedef HDC (*P_CreateDCW)(void* lpszDriver, void* lpszDevice, void* lpszOutput, void* lpInitData);

typedef ULONG (*P_GdiAddFontResourceW)(const WCHAR* path, ULONG flags, void* reserved);

typedef ULONG (*P_RemoveFontResourceExW)(const WCHAR* path, ULONG flags, void* reserved);

typedef ULONG (*P_GetFontResourceInfoW)(const WCHAR* path, ULONG* buflen, void* bufptr, ULONG querytype);

typedef ULONG (*P_CreateScalableFontResourceW)(ULONG hidden, const WCHAR* res, const WCHAR* file, const WCHAR* path);

typedef int (*P_EnumFontFamiliesEx)(HDC hdc, void* lpLogfont, void* lpEnumFontFamExProc, LPARAM lParam, DWORD dwFlags);
...
typedef LONG (*P_DocumentProperties)(HWND hWnd, HANDLE hPrinter, void* pDeviceName, void* pDevModeOutput, void* pDevModeInput, DWORD fMode);
...
static const WCHAR* Gdi_SplWow64InUseEventName = SBIE_BOXED_ L"SplWow64InUse";
...
_FX ULONG_PTR Gdi_GdiDllInitialize_XP(HMODULE User32, BOOLEAN Init, ULONG_PTR Unknown3)
{
...
_FX ULONG_PTR Gdi_GdiDllInitialize_Common(ULONG NumArgs, HMODULE User32, BOOLEAN Init, ULONG_PTR Unknown3)
{
	...
	if (NumArgs == 3)
	{
		typedef ULONG_PTR (*P_GdiDllInitialize_XP)(HMODULE User32, BOOLEAN Init, ULONG_PTR Unknown3);

		rc = ((P_GdiDllInitialize_XP)__sys_GdiDllInitialize)(User32, Init, Unknown3);
	}
	else if (NumArgs == 2)
	{
		typedef ULONG_PTR (*P_GdiDllInitialize_Vista)(HMODULE User32, BOOLEAN Init);

		rc = ((P_GdiDllInitialize_Vista)__sys_GdiDllInitialize)(User32, Init);
	}
	else
	{
		rc = 99;
...
_FX HDC Gdi_CreateDCW(void* lpszDriver, void* lpszDevice, void* lpszOutput, void* lpInitData)
{
	//
	...
	HDC hdc = __sys_CreateDCW(lpszDriver, lpszDevice, lpszOutput, lpInitData);

	if ((!hdc) && lpszDriver && _wcsicmp(lpszDriver, L"WINSPOOL") == 0)
	{
		P_DocumentProperties __sys_DocumentProperties = Ldr_GetProcAddrNew(L"winspool.drv", L"DocumentPropertiesW", "DocumentPropertiesW");
		...
		while (__sys_DocumentProperties && (!hdc) && (retry < 20))
		{
			...
			__sys_DocumentProperties(NULL, hPrinter, lpInitData, NULL, NULL, 0);

			hdc = __sys_CreateDCW(lpszDriver, lpszDevice, lpszOutput, lpInitData);
...
_FX WCHAR* Gdi_GetFontPath(const WCHAR* path)
{
	...
	hFile = CreateFile(path1, GENERIC_READ, FILE_SHARE_VALID_FLAGS, NULL, OPEN_EXISTING, 0, NULL);
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		if ((!NT_SUCCESS(status)) || (!SbieDll_TranslateNtToDosPath(path1)))
		{
			wmemcpy(path1, path, path_len);
			path1[path_len] = L'\0';
		}
...
_FX ULONG Gdi_GdiAddFontResourceW(const WCHAR* path, ULONG flags, void* reserved)
{
	ULONG rv, err;
...
_FX ULONG Gdi_RemoveFontResourceExW(const WCHAR* path, ULONG flags, void* reserved)
{
	ULONG rv, err;
...
_FX ULONG Gdi_GetFontResourceInfoW(const WCHAR* path, ULONG* buflen, void* bufptr, ULONG querytype)
{
	ULONG rv, err;
...
_FX ULONG Gdi_CreateScalableFontResourceW(ULONG hidden, const WCHAR* res, const WCHAR* file, const WCHAR* path)
{
	const WCHAR* res1;
	const WCHAR* file1;
	ULONG rv, err;
...
_FX void Gdi_AddFontsInBox(void)
{
	...
	if (!AlreadyAdded)
	{
		...
		if (WinFonts[0])
		{
			...
			hFile = CreateFile(WinFonts, GENERIC_READ, FILE_SHARE_VALID_FLAGS, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
		}
...
_FX void Gdi_AddFontsInBox_2(HANDLE hFontsDir, void* buf8k, WCHAR* WinFonts)
{
	...
	while (1)
	{
		...
		status = File_MyQueryDirectoryFile(hFontsDir, info, 8000, FileDirectoryInformation, FALSE, NULL, FALSE);
		...
		while (1)
		{
			...
			{
				memcpy(WinFonts + WinFonts_len + 1, info->FileName, info->FileNameLength);
				WinFonts[WinFonts_len + 1 + (info->FileNameLength / sizeof(WCHAR))] = L'\0';
...
_FX int Gdi_EnumFontFamiliesExA(HDC hdc, void* lpLogfont, void* lpEnumFontFamExProc, LPARAM lParam, DWORD dwFlags)
{
	Gdi_AddFontsInBox();
	return __sys_EnumFontFamiliesExA(hdc, lpLogfont, lpEnumFontFamExProc, lParam, dwFlags);
}
...
_FX int Gdi_EnumFontFamiliesExW(HDC hdc, void* lpLogfont, void* lpEnumFontFamExProc, LPARAM lParam, DWORD dwFlags)
{
	Gdi_AddFontsInBox();
	return __sys_EnumFontFamiliesExW(hdc, lpLogfont, lpEnumFontFamExProc, lParam, dwFlags);
}
...
_FX BOOLEAN Gdi_Full_Init(HMODULE module)
{
	...
	//
	CreateDCW = (P_CreateDCW)GetProcAddress(module, "CreateDCW");

	GdiAddFontResourceW = (P_GdiAddFontResourceW)GetProcAddress(module, "GdiAddFontResourceW");

	RemoveFontResourceExW = (P_RemoveFontResourceExW)GetProcAddress(module, "RemoveFontResourceExW");

	GetFontResourceInfoW = (P_GetFontResourceInfoW)GetProcAddress(module, "GetFontResourceInfoW");

	CreateScalableFontResourceW = (P_CreateScalableFontResourceW)GetProcAddress(module, "CreateScalableFontResourceWImpl");
	...
	EnumFontFamiliesExA = (P_EnumFontFamiliesEx)GetProcAddress(module, "EnumFontFamiliesExA");

	EnumFontFamiliesExW = (P_EnumFontFamiliesEx)GetProcAddress(module, "EnumFontFamiliesExW");

	GetStockObject = (P_GetStockObject)GetProcAddress(module, "GetStockObject");

	SBIEDLL_HOOK(Gdi_, EnumFontFamiliesExA);
	SBIEDLL_HOOK(Gdi_, EnumFontFamiliesExW);
	SBIEDLL_HOOK(Gdi_, GetStockObject);

	__sys_GetEnhMetaFileBits = (P_GetEnhMetaFileBits)GetProcAddress(module, "GetEnhMetaFileBits");

	__sys_GetBitmapBits = (P_GetBitmapBits)GetProcAddress(module, "GetBitmapBits");

	__sys_DeleteObject = (P_DeleteObject)GetProcAddress(module, "DeleteObject");

	__sys_DeleteEnhMetaFile = (P_DeleteEnhMetaFile)GetProcAddress(module, "DeleteEnhMetaFile");
...
_FX BOOLEAN Gdi_Init_Spool(HMODULE module)
{
	...
	if (Dll_OsBuild >= 8400)
	{
		__sys_OpenPrinter2W = (P_OpenPrinter2W)GetProcAddress(module, "OpenPrinter2W");
		__sys_ClosePrinter  = (P_ClosePrinter)GetProcAddress(module, "ClosePrinter");
	}
...
_FX BOOLEAN Gdi_Init(HMODULE module)
{
	...
	CreateScalableFontResourceW = (P_CreateScalableFontResourceW)GetProcAddress(module, "CreateScalableFontResourceW");
