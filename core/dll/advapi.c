...
static BOOL AdvApi_LookupAccountNameW(void* lpSystemName, void* lpAccountName, PSID Sid, LPDWORD cbSid, void* ReferencedDomainName, LPDWORD cchReferencedDomainName, PSID_NAME_USE peUse);

static ULONG AdvApi_RegConnectRegistryA(void* lpMachineName, HKEY hKey, HKEY* phkResult);

static ULONG AdvApi_RegConnectRegistryW(void* lpMachineName, HKEY hKey, HKEY* phkResult);
...
static BOOL AdvApi_SaferComputeTokenFromLevel(ULONG_PTR LevelHandle, HANDLE InAccessToken, HANDLE* OutAccessToken, ULONG dwFlags, void* lpReserved);

static ULONG AdvApi_GetEffectiveRightsFromAclW(PACL pacl, void* pTrustee, PACCESS_MASK pAccessRights);

/*static ULONG AdvApi_SaferiIsDllAllowed(
    HANDLE FileHandle, const UNICODE_STRING *FileName, ULONG_PTR Unknown);*/

static ULONG AdvApi_CreateRestrictedToken(HANDLE ExistingTokenHandle, DWORD Flags, DWORD DisableSidCount, PSID_AND_ATTRIBUTES SidsToDisable, DWORD DeletePrivilegeCount, PLUID_AND_ATTRIBUTES PrivilegesToDelete, DWORD RestrictedSidCount, PSID_AND_ATTRIBUTES SidsToRestrict, PHANDLE NewTokenHandle);

static DWORD AdvApi_SetSecurityInfo(HANDLE handle, SE_OBJECT_TYPE ObjectType, SECURITY_INFORMATION SecurityInfo, PSID psidOwner, PSID psidGroup, PACL pDacl, PACL pSacl);

static DWORD Ntmarta_SetSecurityInfo(HANDLE handle, SE_OBJECT_TYPE ObjectType, SECURITY_INFORMATION SecurityInfo, PSID psidOwner, PSID psidGroup, PACL pDacl, PACL pSacl);

static DWORD AdvApi_GetSecurityInfo(HANDLE handle, SE_OBJECT_TYPE ObjectType, SECURITY_INFORMATION SecurityInfo, PSID psidOwner, PSID psidGroup, PACL pDacl, PACL pSacl, PSECURITY_DESCRIPTOR* ppSecurityDescriptor);

//---------------------------------------------------------------------------


typedef BOOL (*P_SaferComputeTokenFromLevel)(ULONG_PTR LevelHandle, HANDLE InAccessToken, HANDLE* OutAccessToken, ULONG dwFlags, void* lpReserved);

typedef ULONG (*P_GetEffectiveRightsFromAcl)(PACL pacl, void* pTrustee, PACCESS_MASK pAccessRights);
...
#undef GETPROC
	...
	if (Dll_Windows < 10)
	{
		if (Dll_ImageType == DLL_IMAGE_SANDBOXIE_BITS || Dll_ImageType == DLL_IMAGE_SANDBOXIE_WUAU || Dll_ImageType == DLL_IMAGE_WUAUCLT)
		{
			void* AccessCheckByType = Ldr_GetProcAddrNew((Dll_KernelBase ? DllName_kernelbase : DllName_advapi32), L"AccessCheckByType", "AccessCheckByType");
...
_FX BOOL AdvApi_LookupAccountNameW(void* lpSystemName, void* lpAccountName, PSID Sid, LPDWORD cbSid, void* ReferencedDomainName, LPDWORD cchReferencedDomainName, PSID_NAME_USE peUse)
{
	BOOL ok = __sys_LookupAccountNameW(lpSystemName, lpAccountName, Sid, cbSid, ReferencedDomainName, cchReferencedDomainName, peUse);

	if ((!ok) && GetLastError() == ERROR_NONE_MAPPED && (!lpSystemName) && lpAccountName)
	{
		...
		if (name && _wcsnicmp(name, L"NT SERVICE\\", 11) == 0)
		{
			...
			ok = __sys_LookupAccountNameW(lpSystemName, name, Sid, cbSid, ReferencedDomainName, cchReferencedDomainName, peUse);
		}
	}
...
_FX ULONG AdvApi_RegConnectRegistryA(void* lpMachineName, HKEY hKey, HKEY* phkResult)
{
...
_FX ULONG AdvApi_RegConnectRegistryW(void* lpMachineName, HKEY hKey, HKEY* phkResult)
{
...
_FX ULONG AdvApi_CreateRestrictedToken(HANDLE ExistingTokenHandle, DWORD Flags, DWORD DisableSidCount, PSID_AND_ATTRIBUTES SidsToDisable, DWORD DeletePrivilegeCount, PLUID_AND_ATTRIBUTES PrivilegesToDelete, DWORD RestrictedSidCount, PSID_AND_ATTRIBUTES SidsToRestrict, PHANDLE NewTokenHandle)
{
	DWORD i, n;
	...
	for (i = 0; i < DeletePrivilegeCount; i++)
	{
		...
		{
			if ((PrivilegesToDelete[i].Luid.HighPart == luidChangeNotify.HighPart)
			    && (PrivilegesToDelete[i].Luid.LowPart == luidChangeNotify.LowPart))
			{
	...
	bResult = __sys_CreateRestrictedToken(ExistingTokenHandle,
	    Flags,
	    DisableSidCount,
	    SidsToDisable,
...
BOOL CALLBACK myEnumWindowStationProc(_In_ LPTSTR lpszWindowStation, _In_ LPARAM lParam);
...
_FX DWORD AdvApi_GetSecurityInfo(HANDLE handle, SE_OBJECT_TYPE ObjectType, SECURITY_INFORMATION SecurityInfo, PSID psidOwner, PSID psidGroup, PACL pDacl, PACL pSacl, PSECURITY_DESCRIPTOR* ppSecurityDescriptor)
{
	DWORD rc = 0;
...
_FX DWORD AdvApi_SetSecurityInfo(HANDLE handle, SE_OBJECT_TYPE ObjectType, SECURITY_INFORMATION SecurityInfo, PSID psidOwner, PSID psidGroup, PACL pDacl, PACL pSacl)
{
...
_FX BOOL AdvApi_SaferComputeTokenFromLevel(ULONG_PTR LevelHandle, HANDLE InAccessToken, HANDLE* OutAccessToken, ULONG dwFlags, void* lpReserved)
{
	if (!AdvApi_EnableSRP)
	{
	...
	return __sys_SaferComputeTokenFromLevel(LevelHandle, InAccessToken, OutAccessToken, dwFlags, lpReserved);
}
...
_FX BOOLEAN AdvApi_EnableDisableSRP(BOOLEAN Enable)
{
	...
	{
		P_SaferComputeTokenFromLevel SaferComputeTokenFromLevel = (P_SaferComputeTokenFromLevel)GetProcAddress(AdvApi_Module, "SaferComputeTokenFromLevel");
...
_FX ULONG AdvApi_GetEffectiveRightsFromAclW(PACL pacl, void* pTrustee, PACCESS_MASK pAccessRights)
{
	//
	...
	ULONG rc = __sys_GetEffectiveRightsFromAclW(pacl, pTrustee, pAccessRights);
...
// Ntmarta_Init
//---------------------------------------------------------------------------
DWORD Ntmarta_GetSecurityInfo(HANDLE handle, SE_OBJECT_TYPE ObjectType, SECURITY_INFORMATION SecurityInfo, PSID psidOwner, PSID psidGroup, PACL pDacl, PACL pSacl, PSECURITY_DESCRIPTOR* ppSecurityDescriptor);


#define SBIEDLL_HOOK2(pfx, proc)                                                       \
	*(ULONG_PTR*)&__sys_##pfx##proc = (ULONG_PTR)SbieDll_Hook(#proc, proc, pfx##proc); \
	if (!__sys_##pfx##proc)                                                            \
		return FALSE;
...
_FX DWORD Ntmarta_GetSecurityInfo(HANDLE handle, SE_OBJECT_TYPE ObjectType, SECURITY_INFORMATION SecurityInfo, PSID psidOwner, PSID psidGroup, PACL pDacl, PACL pSacl, PSECURITY_DESCRIPTOR* ppSecurityDescriptor)
{
	DWORD rc = 0;
...
_FX DWORD Ntmarta_SetSecurityInfo(HANDLE handle, SE_OBJECT_TYPE ObjectType, SECURITY_INFORMATION SecurityInfo, PSID psidOwner, PSID psidGroup, PACL pDacl, PACL pSacl)
{
	// this is a HACK to get Chrome 38 to work.
	if ((Dll_ImageType == DLL_IMAGE_GOOGLE_CHROME) && (ObjectType == SE_WINDOW_OBJECT) && (handle == NULL))
	{
