...
#define Com_RpcRaiseException(ExceptionCode) //RaiseException(ExceptionCode, EXCEPTION_NONCONTINUABLE, 0, NULL)

#define GETPROCADDR_DEF(name) P_##name name = (P_##name)GetProcAddress(module, #name)

#define GETPROCADDR_SYS(name) __sys_##name = (P_##name)GetProcAddress(module, #name)
...
static HRESULT Com_CoGetClassObject(REFCLSID rclsid, ULONG clsctx, void* pServerInfo, REFIID riid, void** ppv);

static HRESULT Com_CoGetObject(const WCHAR* pszName, void* pBindOptions, REFIID riid, void** ppv);

static HRESULT Com_CoCreateInstance(REFCLSID rclsid, void* pUnkOuter, ULONG clsctx, REFIID riid, void** ppv);

static HRESULT Com_CoCreateInstanceEx(REFCLSID rclsid, void* pUnkOuter, ULONG clsctx, void* pServerInfo, ULONG cmq, MULTI_QI* pmqs);

static BOOLEAN Com_Hook_CoUnmarshalInterface_W8(UCHAR* code);

static HRESULT __fastcall Com_CoUnmarshalInterface_W8(ULONG_PTR StreamAddr, ULONG64 zero, REFIID riid, void** ppv);

static HRESULT __fastcall Com_CoUnmarshalInterface_W81(ULONG_PTR StreamAddr, ULONG zero, REFIID riid, void** ppv);

static HRESULT Com_CoUnmarshalInterface_W10(ULONG_PTR StreamAddr, REFIID riid, void** ppv);

static HRESULT Com_CoUnmarshalInterface(IStream* pStream, REFIID riid, void** ppv);

static HRESULT Com_CoUnmarshalInterface_Common(IStream* pStream, REFIID riid, void** ppv, LARGE_INTEGER* seekpos);

static HRESULT Com_CoMarshalInterface(IStream* pStream, REFIID riid, IUnknown* pUnknown, DWORD dwDestContext, void* pvDestContext, DWORD mshlflags);

static HRESULT Com_CreateProxyStubFactoryBuffer(REFIID riid, IPSFactoryBuffer** pFactory, HMODULE* hProxyDll, ITypeInfo** pTypeInfo);
...
static HRESULT Com_IUnknown_New(ULONG objidx, ULONG methods, ULONG flags, COM_IUNKNOWN** ppv);

static HRESULT Com_IClassFactory_New(REFCLSID rclsid, const WCHAR* StringGUID, void** ppv);

static HRESULT Com_OuterIUnknown_New(ULONG objidx, REFIID iid, void** ppv);

static HRESULT Com_IRpcChannelBuffer_New(ULONG objidx, REFIID iid, void** ppv);
...
static HRESULT Com_IConnectionPoint_Advise(void* pUnknown, void* pSink, ULONG* pCookie);
...
static void Com_Trace(const WCHAR* TraceType, REFCLSID rclsid, REFIID riid, ULONG ProcNum, HRESULT hr);
...
typedef HRESULT (*P_CoGetClassObject)(REFCLSID rclsid, ULONG clsctx, void* pServerInfo, REFIID riid, void** ppv);

typedef ULONG (*P_CoGetObject)(const WCHAR* pszName, void* pBindOptions, REFIID riid, void** ppv);

typedef ULONG (*P_CoCreateInstance)(REFCLSID rclsid, void* pUnkOuter, ULONG dwClsContext, REFIID riid, void** ppv);

typedef ULONG (*P_CoCreateInstanceEx)(REFCLSID rclsid, void* pUnkOuter, ULONG dwClsContext, void* pServerInfo, ULONG cmq, MULTI_QI* pResults);

typedef ULONG (*P_CoUnmarshalInterface)(IStream* pStream, REFIID riid, void** ppv);

typedef ULONG(__fastcall* P_CoUnmarshalInterface_W8)(ULONG_PTR StreamAddr, ULONG64 zero, REFIID riid, void** ppv);

typedef ULONG(__fastcall* P_CoUnmarshalInterface_W81)(ULONG_PTR StreamAddr, ULONG zero, REFIID riid, void** ppv);

typedef ULONG (*P_CoUnmarshalInterface_W10)(ULONG_PTR StreamAddr, REFIID riid, void** ppv);

typedef ULONG (*P_CoMarshalInterface)(IStream* pStream, REFIID riid, IUnknown* pUnknown, DWORD dwDestContext, void* pvDestContext, DWORD mshlflags);
...
typedef HRESULT (*P_LoadTypeLibEx)(LPCOLESTR szFile, REGKIND regkind, ITypeLib** pptlib);

typedef HRESULT (*P_CreateProxyFromTypeInfo)(ITypeInfo* pTypeInfo, IUnknown* pUnkOuter, REFIID riid, IRpcProxyBuffer** ppProxy, void** ppv);

typedef HRESULT (*P_CreateStubFromTypeInfo)(ITypeInfo* pTypeInfo, REFIID riid, IUnknown* pUnkServer, IRpcStubBuffer** ppStub);
...
P_CoUnmarshalInterface_W10
	__sys_CoUnmarshalInterface_W10
		= NULL;
		...
		static const GUID IID_IStdIdentity = {0x0000001B, 0x0000, 0x0000, {0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46}};

		static const GUID IID_INetFwAuthorizedApplication = {0xB5E64FFA, 0xC2C5, 0x444E, {0xA3, 0x01, 0xFB, 0x5E, 0x00, 0x01, 0x80, 0x50}};

		static const GUID IID_INetFwRule = {0xAF230D27, 0xBABA, 0x4E42, {0xAC, 0xED, 0xF5, 0x24, 0xF2, 0x2C, 0xFC, 0xE2}};
		...
		_FX BOOLEAN SbieDll_IsOpenClsid(REFCLSID rclsid, ULONG clsctx, const WCHAR* BoxName)
		{
			static const GUID CLSID_WinMgmt = {0x8BC3F05E, 0xD86B, 0x11D0, {0xA0, 0x75, 0x00, 0xC0, 0x4F, 0xB6, 0x88, 0x20}};

			static const GUID CLSID_NetworkListManager = {0xA47979D2, 0xC419, 0x11D9, {0xA5, 0xB4, 0x00, 0x11, 0x85, 0xAD, 0x2B, 0x89}};

			static const GUID CLSID_ShellServiceHostBrokerProvider = {0x3480A401, 0xBDE9, 0x4407, {0xBC, 0x02, 0x79, 0x8A, 0x86, 0x6A, 0xC0, 0x51}};

			static const GUID CLSID_WinInetCache = {0x0358B920, 0x0AC7, 0x461F, {0x98, 0xF4, 0x58, 0xE3, 0x2C, 0xD8, 0x91, 0x48}};
			...
			if (clsctx & CLSCTX_LOCAL_SERVER)
			{
				...
				if (memcmp(rclsid, &CLSID_WinMgmt, sizeof(GUID)) == 0 || memcmp(rclsid, &CLSID_NetworkListManager, sizeof(GUID)) == 0 || memcmp(rclsid, &CLSID_ShellServiceHostBrokerProvider, sizeof(GUID)) == 0 || ((Dll_OsBuild >= 10240) && memcmp(rclsid, &CLSID_WinInetCache, sizeof(GUID)) == 0))
				{
					return TRUE;
				}
				...
				if (Com_NumOpenClsids == -1)
				{
					...
					if (Dll_BoxName)
					{
						status = SbieApi_QueryConfAsIs(NULL, L"ClsidTrace", 0, buf, 90 * sizeof(WCHAR));
						if (buf[0] && buf[0] != L'.')
						{
							Com_TraceFlag = TRUE;
						}
					...
					while (1)
					{
						status = SbieApi_QueryConfAsIs(BoxName, setting, index, buf, 90 * sizeof(WCHAR));
					...
					if (Com_NumOpenClsids)
					{
						for (index = 0; index < Com_NumOpenClsids; ++index)
						{
							...
							if (NT_SUCCESS(status))
							{
								...
								if ((!__sys_IIDFromString) || __sys_IIDFromString(buf, guid) != 0)
								{
		...
		_FX BOOLEAN Com_IsFirewallClsid(REFCLSID rclsid, const WCHAR* BoxName)
		{
			...
			static const GUID CLSID_Firewall[7] = {{0x0CA545C6, 0x37AD, 0x4A6C, {0xBF, 0x92, 0x9F, 0x76, 0x10, 0x06, 0x7E, 0xF5}}, {0x2C5BC43E, 0x3369, 0x4C33, {0xAB, 0x0C, 0xBE, 0x94, 0x69, 0x67, 0x7A, 0xF4}}, {0x304CE942, 0x6E39, 0x40D8, {0x94, 0x3A, 0xB9, 0x13, 0xC4, 0x0C, 0x9C, 0xD4}}, {0x9D745ED8, 0xC514, 0x4D1D, {0xBF, 0x42, 0x75, 0x1F, 0xED, 0x2D, 0x5A, 0xC7}}, {0xCC19079B, 0x8272, 0x4D73, {0xBB, 0x70, 0xCD, 0xB5, 0x33, 0x52, 0x7B, 0x61}}, {0xE2B3C97F, 0x6AE1, 0x41AC, {0x81, 0x7A, 0xF6, 0xF9, 0x21, 0x66, 0xD7, 0xDD}}, {0xEC9846B3, 0x2762, 0x4A6B, {0xA2, 0x14, 0x6A, 0xCB, 0x60, 0x34, 0x62, 0xD2}}};
			ULONG i;
		...
		{
			static const UCHAR EventSystem[16] = {0xa2, 0xfb, 0x14, 0x4e, 0x22, 0x2e, 0xd1, 0x11, 0x99, 0x64, 0x00, 0xc0, 0x4f, 0xbb, 0xb3, 0x45};

			static const UCHAR EventSystemTier2[16] = {0x66, 0xf7, 0xe1, 0x1b, 0x36, 0x55, 0xd1, 0x11, 0xb7, 0x26, 0x00, 0xc0, 0x4f, 0xb9, 0x26, 0xaf};
		...
		_FX HRESULT Com_CoGetClassObject(REFCLSID rclsid, ULONG clsctx, void* pServerInfo, REFIID riid, void** ppv)
		{
		...
		_FX HRESULT Com_CoGetObject(const WCHAR* pszName, void* pBindOptions, REFIID riid, void** ppv)
		{
		...
		_FX HRESULT Com_CoCreateInstance(REFCLSID rclsid, void* pUnkOuter, ULONG clsctx, REFIID riid, void** ppv)
		{
			...
			if (SbieDll_IsOpenClsid(rclsid, clsctx, NULL))
			{
				...
				if (SUCCEEDED(hr))
				{
					hr = IClassFactory_CreateInstance(pFactory, pUnkOuter, riid, ppv);
			...
			if (SUCCEEDED(hr) && Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER && memcmp(riid, &IID_IContextMenu, sizeof(GUID)) == 0)
			{
		...
		_FX HRESULT Com_CoCreateInstanceEx(REFCLSID rclsid, void* pUnkOuter, ULONG clsctx, void* pServerInfo, ULONG cmq, MULTI_QI* pmqs)
		{
			...
			{
				static const UCHAR BitsClsid[16] = {0x4b, 0xd3, 0x91, 0x49, 0xa1, 0x80, 0x91, 0x42, 0x83, 0xb6, 0x33, 0x28, 0x36, 0x6b, 0x90, 0x97};
			...
			if (SbieDll_IsOpenClsid(rclsid, clsctx, NULL))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					for (i = 0; i < cmq; ++i)
					{
						...
						mqi->hr = IClassFactory_CreateInstance(pFactory, pUnkOuter, riid, ppv);
						if (FAILED(mqi->hr))
						{
							++bad;
						}
			...
			{
				hr = __sys_CoCreateInstanceEx(rclsid, pUnkOuter, clsctx, pServerInfo, cmq, pmqs);
			}
		...
		_FX BOOLEAN Com_Hook_CoUnmarshalInterface_W8(UCHAR * code)
		{
			...
			P_CoUnmarshalInterface_W10
				CoUnmarshalInterface_W10
					= (P_CoUnmarshalInterface_W10)GetProcAddress(GetModuleHandle(L"combase.dll"), "CoUnmarshalInterface");
					...
					if (Dll_OsBuild >= 15002)
					{
						if (code[0x18] == 0xe9)
						{
							...
							P_CoUnmarshalInterface_W8 CoUnmarshalInterface_W8 = (P_CoUnmarshalInterface_W8)jump_target;
					...
					{ // call
						...
						P_CoUnmarshalInterface_W81 CoUnmarshalInterface_W81 = (P_CoUnmarshalInterface_W81)jump_target;
		...
		_FX HRESULT __fastcall Com_CoUnmarshalInterface_W8(ULONG_PTR StreamAddr, ULONG64 zero, REFIID riid, void** ppv)
		{
			const HRESULT HR_OR_INVALID_OXID = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, OR_INVALID_OXID);
			HRESULT hr;
			LARGE_INTEGER posl;
			ULARGE_INTEGER posu;
			ULONG_PTR riid_arg;
        ...
        mov ppv_arg, eax
			...
			return Com_CoUnmarshalInterface_Common(pStream, (REFIID)riid_arg, (void**)ppv_arg, &posl);
		}
		...
		_FX HRESULT __fastcall Com_CoUnmarshalInterface_W81(ULONG_PTR StreamAddr, ULONG zero, REFIID riid, void** ppv)
		{
			const HRESULT HR_OR_INVALID_OXID = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, OR_INVALID_OXID);
			HRESULT hr;
			LARGE_INTEGER posl;
		...
		_FX HRESULT Com_CoUnmarshalInterface_W10(ULONG_PTR StreamAddr, REFIID riid, void** ppv)
		{
			const HRESULT HR_OR_INVALID_OXID = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, OR_INVALID_OXID);
			HRESULT hr;
			LARGE_INTEGER posl;
		...
		_FX HRESULT Com_CoUnmarshalInterface(IStream * pStream, REFIID riid, void** ppv)
		{
			const HRESULT HR_OR_INVALID_OXID = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, OR_INVALID_OXID);
			HRESULT hr;
			LARGE_INTEGER posl;
		...
		_FX HRESULT Com_CoUnmarshalInterface_Common(IStream * pStream, REFIID riid, void** ppv, LARGE_INTEGER* seekpos)
		{
			...
			req = (COM_UNMARSHAL_INTERFACE_REQ*)Com_Alloc(128 + COM_MAX_UNMARSHAL_BUF_LEN);
			if (!req)
			{
		...
		static HRESULT Com_CoMarshalInterface(IStream * pStream, REFIID riid, IUnknown * pUnknown, DWORD dwDestContext, void* pvDestContext, DWORD mshlflags)
		{
			static const GUID IID_ISearchNotifyInlineSite = {0xB5702E61, 0xE75C, 0x4B64, {0x82, 0xA1, 0x6C, 0xB4, 0xF8, 0x32, 0xFC, 0xCF}};
			HRESULT hr;
			...
			if (dwDestContext == MSHCTX_INPROC || memcmp(riid, &IID_ISearchNotifyInlineSite, sizeof(GUID)) != 0)
			{
				hr = __sys_CoMarshalInterface(pStream, riid, pUnknown, dwDestContext, pvDestContext, mshlflags);
			}
			else
			{
				//
				...
				{
					ULONG count = 0;
					hr          = IStream_Write(pStream, rpl->Buffer, rpl->BufferLength, &count);
				}
		...
		_FX BOOLEAN Com_Init_ComBase(HMODULE module)
		{
			...
			if (!SbieDll_IsOpenCOM())
			{
				...
				if (Dll_OsBuild >= 8400)
				{
					if (!Com_Hook_CoUnmarshalInterface_W8((UCHAR*)CoUnmarshalInterface))
					{
						return FALSE;
					}
				}
		...
		_FX HRESULT Com_CreateProxyStubFactoryBuffer(REFIID riid, IPSFactoryBuffer * *pFactory, HMODULE * hProxyDll, ITypeInfo * *pTypeInfo)
		{
			static const GUID IID_PSOAInterface = {0x00020424, 0x0000, 0x0000, {0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46}};
			HRESULT hr;
			LONG rc;
			CLSID ProxyClsid;
			ULONG buf_len;
			...
			if ((!__sys_CoGetPSClsid) || (!__sys_StringFromGUID2) || (!__sys_RegOpenKeyExW) || (!__sys_RegCloseKey))
			{
			...
			*hProxyDll = NULL;
			...
			{
				static const GUID CLSID_PSFactoryBuffer = {0x00000320, 0x0000, 0x0000, {0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46}};
			...
			if (rc == 0)
			{
				...
				KEY_VALUE_PARTIAL_INFORMATION* info = (KEY_VALUE_PARTIAL_INFORMATION*)path;

				RtlInitUnicodeString(&ValueName, L"");

				rc = NtQueryValueKey(hkey, &ValueName, KeyValuePartialInformation, info, buf_len - 8, &len);
			...
			DllGetClassObject = (LPFNGETCLASSOBJECT)GetProcAddress(*hProxyDll, "DllGetClassObject");
		...
		_FX HRESULT Com_CreateTypeInfo(REFIID riid, ITypeInfo * *pTypeInfo)
		{
			...
			if ((!__sys_LoadTypeLibEx) || (!__sys_CreateProxyFromTypeInfo) || (!__sys_CreateStubFromTypeInfo))
			{
			...
			if (rc == 0)
			{
				...
				if (rc == 0)
				{
					...
					rc = NtQueryValueKey(hkey, &ValueName, KeyValuePartialInformation, info, sizeof(info_space), &len);
		...
		_FX HRESULT SbieDll_ComCreateProxy(REFIID riid, void* pUnkOuter, void* pChannel, void** ppUnknown)
		{
			HRESULT hr;
			...
			hr = Com_CreateProxyStubFactoryBuffer(riid, &pFactory, &hProxyDll, &pTypeInfo);
			if (SUCCEEDED(hr))
			{
				//
				...
				if (pTypeInfo)
				{
					hr = __sys_CreateProxyFromTypeInfo(pTypeInfo, pUnkOuter, riid, &pProxy, &pUnknown);
				}
				else
				{
					hr = IPSFactoryBuffer_CreateProxy(pFactory, pUnkOuter, riid, &pProxy, &pUnknown);
				}
		...
		_FX HRESULT Com_CreateProxy(REFIID riid, ULONG objidx, void** ppUnknown)
		{
			HRESULT hr;
			IUnknown* pUnkOuter;
			...
			if (SUCCEEDED(hr))
			{
				...
				else
				{
					...
					if (SUCCEEDED(hr))
					{
						hr = SbieDll_ComCreateProxy(riid, pUnkOuter, pChannel, ppUnknown);
						...
						if (SUCCEEDED(hr))
						{
							...
							if (memcmp(riid, &IID_IConnectionPoint, sizeof(GUID)) == 0)
							{
								Com_IConnectionPoint_Hook(*ppUnknown);
							}
							else if (memcmp(riid, &IID_INetFwAuthorizedApplication, sizeof(GUID)) == 0)
							{
								Net_Firewall_Hook(1, *ppUnknown);
							}
							else if (memcmp(riid, &IID_INetFwRule, sizeof(GUID)) == 0)
							{
		...
		_FX HRESULT SbieDll_ComCreateStub(REFIID riid, void* pUnknown, void** ppStub, void** ppChannel)
		{
			HRESULT hr;
			IPSFactoryBuffer* pFactory;
			ITypeInfo* pTypeInfo;
		...
		_FX HRESULT Com_IUnknown_New(ULONG objidx, ULONG methods, ULONG flags, COM_IUNKNOWN * *ppv)
		{
			COM_IUNKNOWN* This;
			ULONG len;
		...
		_FX HRESULT Com_IClassFactory_QueryInterface(COM_IUNKNOWN * This, REFIID riid, void** ppv)
		{
			HRESULT hr;

			if (memcmp(riid, &IID_IUnknown, sizeof(GUID)) == 0 || memcmp(riid, &IID_IClassFactory, sizeof(GUID)) == 0)
			{
		...
		_FX HRESULT Com_IClassFactory_CreateInstance(COM_IUNKNOWN * This, IUnknown * pUnkOuter, REFIID riid, void** ppv)
		{
		...
		_FX HRESULT Com_IClassFactory_LockServer(COM_IUNKNOWN * This, BOOL fLock)
		{
		...
		_FX HRESULT Com_IClassFactory_New(REFCLSID rclsid, const WCHAR* StringGUID, void** ppv)
		{
			COM_IUNKNOWN* This;
			ULONG len;
		...
		_FX HRESULT Com_OuterIUnknown_QueryInterface(COM_IUNKNOWN * This, REFIID riid, void** ppv)
		{
			...
			if (memcmp(riid, &IID_IUnknown, sizeof(GUID)) == 0 || memcmp(riid, &This->Guid, sizeof(GUID)) == 0)
			{
		...
		_FX HRESULT Com_OuterIUnknown_New(ULONG objidx, REFIID iid, void** ppv)
		{
			...
			hr = Com_IUnknown_New(objidx, 3, FLAG_REMOTE_REF | FLAG_PPROXY_AT_VTBL3, &This);
			if (SUCCEEDED(hr))
			{
		...
		_FX HRESULT Com_IRpcChannelBuffer_QueryInterface(COM_IUNKNOWN * This, REFIID riid, void** ppv)
		{
			HRESULT hr;

			if (memcmp(riid, &IID_IUnknown, sizeof(GUID)) == 0 || memcmp(riid, &IID_IRpcChannelBuffer, sizeof(GUID)) == 0)
			{
				IUnknown_AddRef((IUnknown*)This);
				*ppv = This;
				hr   = S_OK;
		...
		_FX HRESULT Com_IRpcChannelBuffer_GetBuffer(COM_IUNKNOWN * This, COM_RPC_MESSAGE * pMessage, REFIID riid)
		{
			HRESULT hr       = S_OK;
		...
		_FX HRESULT Com_IRpcChannelBuffer_FreeBuffer(COM_IUNKNOWN * This, COM_RPC_MESSAGE * pMessage)
		{
		...
		_FX HRESULT Com_IRpcChannelBuffer_GetDestCtx(COM_IUNKNOWN * This, ULONG * pdwDestContext, void** ppvDestContext)
		{
		...
		_FX HRESULT Com_IRpcChannelBuffer_SendReceive(COM_IUNKNOWN * This, COM_RPC_MESSAGE * pMessage, ULONG * pStatus)
		{
			...
			if (SUCCEEDED(hr))
			{
				IRpcChannelBuffer_FreeBuffer((IRpcChannelBuffer*)This, (RPCOLEMESSAGE*)pMessage);

				pMessage->DataRepresentation = rpl->DataRepresentation;
				pMessage->BufferLength       = rpl->BufferLength;
				hr = IRpcChannelBuffer_GetBuffer((IRpcChannelBuffer*)This, (RPCOLEMESSAGE*)pMessage, NULL);
				if (SUCCEEDED(hr))
				{
		...
		_FX HRESULT Com_IRpcChannelBuffer_New(ULONG objidx, REFIID iid, void** ppv)
		{
			HRESULT hr;
			COM_IUNKNOWN* This;
		...
		_FX HRESULT Com_IMarshal_QueryInterface(COM_IUNKNOWN * This, REFIID riid, void** ppv)
		{
			HRESULT hr;

			if (memcmp(riid, &IID_IUnknown, sizeof(GUID)) == 0 || memcmp(riid, &IID_IMarshal, sizeof(GUID)) == 0)
			{
				IUnknown_AddRef((IUnknown*)This);
				*ppv = This;
				hr   = S_OK;
		...
		_FX HRESULT Com_IMarshal_GetUnmarshalClass(COM_IUNKNOWN * This, REFIID riid, void* pv, ULONG dwDestContext, void* pvDestContext, ULONG mshlflags, CLSID* pCid)
		{
		...
		_FX HRESULT Com_IMarshal_GetMarshalSizeMax(COM_IUNKNOWN * This, REFIID riid, void* pv, ULONG dwDestContext, void* pvDestContext, ULONG mshlflags, ULONG* pSize)
		{
			*pSize = 1024;
			return S_OK;
		}
		...
		_FX HRESULT Com_IMarshal_MarshalInterface(COM_IUNKNOWN * This, IStream * pStm, REFIID riid, void* pv, ULONG dwDestContext, void* pvDestContext, ULONG mshlflags)
		{
		...
		_FX HRESULT Com_IMarshal_UnmarshalInterface(COM_IUNKNOWN * This, IStream * pStm, REFIID riid, void** pv)
		{
		...
		_FX HRESULT Com_IMarshal_ReleaseMarshalData(COM_IUNKNOWN * This, IStream * pStm)
		{
		...
		_FX HRESULT Com_IMarshal_DisconnectObject(COM_IUNKNOWN * This, ULONG dwReserved)
		{
		...
		_FX HRESULT Com_IClientSecurity_QueryInterface(COM_IUNKNOWN * This, REFIID riid, void** ppv)
		{
			HRESULT hr;

			if (memcmp(riid, &IID_IUnknown, sizeof(GUID)) == 0 || memcmp(riid, &IID_IClientSecurity, sizeof(GUID)) == 0)
			{
				IUnknown_AddRef((IUnknown*)This);
				*ppv = This;
				hr   = S_OK;
		...
		_FX HRESULT Com_IClientSecurity_QueryBlanket(COM_IUNKNOWN * This, IUnknown * pProxy, ULONG * pAuthnSvc, ULONG * pAuthzSvc, WCHAR * *pServerPrincName, ULONG * pAuthnLevel, ULONG * pImpLevel, RPC_AUTH_IDENTITY_HANDLE * pAuthInfo, ULONG * pCapabilities)
		{
		...
		_FX HRESULT Com_IClientSecurity_SetBlanket(COM_IUNKNOWN * This, IUnknown * pProxy, ULONG dwAuthnSvc, ULONG dwAuthzSvc, WCHAR * pServerPrincName, ULONG dwAuthnLevel, ULONG dwImpLevel, RPC_AUTH_IDENTITY_HANDLE pAuthInfo, ULONG dwCapabilities)
		{
		...
		_FX HRESULT Com_IClientSecurity_CopyProxy(COM_IUNKNOWN * This, IUnknown * pProxy, IUnknown * *ppCopy)
		{
		...
		_FX HRESULT Com_IRpcOptions_QueryInterface(COM_IUNKNOWN * This, REFIID riid, void** ppv)
		{
			HRESULT hr;

			if (memcmp(riid, &IID_IUnknown, sizeof(GUID)) == 0 || memcmp(riid, &IID_IRpcOptions, sizeof(GUID)) == 0)
			{
				IUnknown_AddRef((IUnknown*)This);
				*ppv = This;
				hr   = S_OK;
		...
		_FX HRESULT Com_IRpcOptions_Set(COM_IUNKNOWN * This, IUnknown * pPrx, DWORD prop, ULONG_PTR value)
		{
			if (prop != COMBND_RPCTIMEOUT)
			{
				return E_INVALIDARG;
			}
			if (value != RPC_C_BINDING_INFINITE_TIMEOUT && value != RPC_C_BINDING_MIN_TIMEOUT && value != RPC_C_BINDING_DEFAULT_TIMEOUT && value != RPC_C_BINDING_MAX_TIMEOUT)
			{
				return E_INVALIDARG;
			}
		...
		_FX HRESULT Com_IRpcOptions_Query(COM_IUNKNOWN * This, IUnknown * pPrx, DWORD prop, ULONG_PTR * value)
		{
			if (prop == COMBND_RPCTIMEOUT)
			{
		...
		_FX HRESULT Com_IConnectionPoint_Advise(void* pUnknown, void* pSink, ULONG* pCookie)
		{
			if ((!pSink) || (!pCookie))
			{
				return E_POINTER;
		...
		_FX void Com_Trace_Guid(WCHAR * text, REFGUID guid, WCHAR * subkey)
		{
			WCHAR* ptr;
			HKEY hkey;
			LONG rc;
			...
			*ptr = L' ';
			...
			if (rc == 0)
			{
				...
				rc = NtQueryValueKey(hkey, &ValueName, KeyValuePartialInformation, info, 128, &len);
		...
		_FX void Com_Trace(const WCHAR* TraceType, REFCLSID rclsid, REFIID riid, ULONG ProcNum, HRESULT hr)
		{
			WCHAR* text;
			WCHAR* ptr;
