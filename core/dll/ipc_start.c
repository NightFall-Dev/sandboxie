...
_FX BOOLEAN Ipc_StartServer(const WCHAR* TruePath, BOOLEAN Async)
{
		...
		else
		{
			...
			if (service == _rpcss)
			{
				...
				else
				{
					...
					if (!SbieDll_RunSandboxed(L"*THREAD*", fullpath, homedir, 0, &si, &pi))
					{
						errnum = GetLastError();
					}
					else
	...
	if (Async)
	{
		...
		hThread = CreateThread(NULL, 0, Ipc_StartServer_Thread, (void*)args, CREATE_SUSPENDED, &idThread);
		if (hThread)
		{
	...
	else
	{
		...
		while (1)
		{
			ULONG rc = WaitForMultipleObjects(NumWaitHandles, WaitHandles, FALSE, 30 * 1000);
			...
			{
				if (GetExitCodeProcess(hServerProcess, &rc) && rc != 0 && rc != STILL_ACTIVE)
				{
					...
					{
						const WCHAR* FormatToUse = (rc & 0x80000000) ? _formatHex : _format;
		...
		if (service == _rpcss && (!SbieDll_IsOpenCOM()))
		{
			...
			if (hKey)
			{
				...
				while (1)
				{
					...
					if (WAIT_OBJECT_0 == WaitForSingleObject(hEvent, 30 * 1000))
					{
						break;
					}
...
{
	if (Dll_ImageType == DLL_IMAGE_SANDBOXIE_RPCSS || Dll_ImageType == DLL_IMAGE_SANDBOXIE_DCOMLAUNCH || Dll_RestrictedToken)
	{
		return TRUE;
	}
