...
_FX NTSTATUS SbieApi_Ioctl(ULONG64* parms)
{
	...
	if (SbieApi_DeviceHandle == INVALID_HANDLE_VALUE)
	{
		...
		status = NtOpenFile(&SbieApi_DeviceHandle, FILE_GENERIC_READ, &objattrs, &MyIoStatusBlock, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, 0);
	...
	else
	{
		...
		status = NtDeviceIoControlFile(SbieApi_DeviceHandle, NULL, NULL, NULL, &MyIoStatusBlock, API_SBIEDRV_CTLCODE, parms, sizeof(ULONG64) * 8, NULL, 0);
	}
...
_FX LONG SbieApi_GetVersion(WCHAR* out_version) // WCHAR [16]
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_GetWork(ULONG SessionId, void* Buffer, ULONG* Length)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_Log(ULONG msgid, const WCHAR* format, ...)
{
	ULONG status;
	va_list va_args;
...
_FX LONG SbieApi_LogEx(ULONG session_id, ULONG msgid, const WCHAR* format, ...)
{
	ULONG status;
	va_list va_args;
	va_start(va_args, format);
...
_FX LONG SbieApi_vLogEx(ULONG session_id, ULONG msgid, const WCHAR* format, va_list va_args)
{
	NTSTATUS status;
...
_FX LONG SbieApi_GetHomePath(WCHAR* NtPath, ULONG NtPathMaxLen, WCHAR* DosPath, ULONG DosPathMaxLen)
{
	NTSTATUS status;
...
_FX LONG SbieApi_QueryProcess(HANDLE ProcessId, WCHAR* out_box_name_wchar34, WCHAR* out_image_name_wchar96, WCHAR* out_sid_wchar96, ULONG* out_session_id)
{
	return SbieApi_QueryProcessEx2(ProcessId, 96, out_box_name_wchar34, out_image_name_wchar96, out_sid_wchar96, out_session_id, NULL);
}
...
_FX LONG SbieApi_QueryProcessEx(HANDLE ProcessId, ULONG image_name_len_in_wchars, WCHAR* out_box_name_wchar34, WCHAR* out_image_name_wcharXXX, WCHAR* out_sid_wchar96, ULONG* out_session_id)
{
	return SbieApi_QueryProcessEx2(ProcessId, image_name_len_in_wchars, out_box_name_wchar34, out_image_name_wcharXXX, out_sid_wchar96, out_session_id, NULL);
}
...
_FX LONG SbieApi_QueryProcessEx2(HANDLE ProcessId, ULONG image_name_len_in_wchars, WCHAR* out_box_name_wchar34, WCHAR* out_image_name_wcharXXX, WCHAR* out_sid_wchar96, ULONG* out_session_id, ULONG64* out_create_time)
{
	NTSTATUS status;
	...
	{
		ImageName.Length        = 0;
		ImageName.MaximumLength = (USHORT)(sizeof(WCHAR) * image_name_len_in_wchars);
...
_FX ULONG64 SbieApi_QueryProcessInfo(HANDLE ProcessId, ULONG info_type)
{
	NTSTATUS status;
...
_FX LONG SbieApi_QueryBoxPath(const WCHAR* box_name, // WCHAR [34]
    WCHAR* out_file_path, WCHAR* out_key_path, WCHAR* out_ipc_path, ULONG* inout_file_path_len, ULONG* inout_key_path_len, ULONG* inout_ipc_path_len)
{
	NTSTATUS status;
...
_FX LONG SbieApi_QueryProcessPath(HANDLE ProcessId, WCHAR* out_file_path, WCHAR* out_key_path, WCHAR* out_ipc_path, ULONG* inout_file_path_len, ULONG* inout_key_path_len, ULONG* inout_ipc_path_len)
{
	NTSTATUS status;
	__declspec(align(8)) UNICODE_STRING64 FilePath;
	__declspec(align(8)) UNICODE_STRING64 KeyPath;
...
_FX LONG SbieApi_QueryPathList(ULONG path_code, ULONG* path_len, WCHAR* path_str, HANDLE process_id)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_EnumProcessEx(const WCHAR* box_name, // WCHAR [34]
...
_FX LONG SbieApi_DisableForceProcess(ULONG* NewState, ULONG* OldState)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_RenameFile(HANDLE FileHandle, const WCHAR* TargetDir, const WCHAR* TargetName, BOOLEAN ReplaceIfExists)
{
	NTSTATUS status;
...
_FX LONG SbieApi_GetFileName(HANDLE FileHandle, ULONG NameLen, WCHAR* NameBuf)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_OpenFile(HANDLE* FileHandle, const WCHAR* Path)
{
	NTSTATUS status;
...
_FX LONG SbieApi_CheckInternetAccess(HANDLE ProcessId, const WCHAR* DeviceName32, BOOLEAN IssueMessage)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_GetBlockedDll(WCHAR* DllNameBuf, ULONG DllNameLen)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_CreateDirOrLink(const WCHAR* ObjectName, const WCHAR* TargetName)
{
	NTSTATUS status;
...
_FX LONG SbieApi_DuplicateObject(HANDLE* TargetHandle, HANDLE OtherProcessHandle, HANDLE SourceHandle, ACCESS_MASK DesiredAccess, ULONG Options)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 ResultHandle;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_OpenProcess(HANDLE* ProcessHandle, HANDLE ProcessId)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 ResultHandle;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_OpenDeviceMap(HANDLE* DirectoryHandle)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 ResultHandle;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_QuerySymbolicLink(WCHAR* NameBuf, ULONG NameLen)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_QueryConf(const WCHAR* section_name, // WCHAR [66]
...
_FX BOOLEAN SbieApi_QueryConfBool(const WCHAR* section_name, // WCHAR [66]
    const WCHAR* setting_name,                               // WCHAR [66]
    BOOLEAN def)
{
	WCHAR value[16];
	*value = L'\0';
	SbieApi_QueryConfAsIs(section_name, setting_name, 0, value, sizeof(value));
	if (*value == 'y' || *value == 'Y')
	{
		return TRUE;
	}
...
_FX LONG SbieApi_EnumBoxes(LONG index, // initialize to -1
...
_FX LONG SbieApi_EnumBoxesEx(LONG index, // initialize to -1
    WCHAR* box_name,                     // WCHAR [34]
    BOOLEAN return_all_sections)
{
	...
	while (1)
	{
		...
		if (return_all_sections || (SbieApi_IsBoxEnabled(box_name) == STATUS_SUCCESS))
		{
			return index;
		}
	}
...
_FX LONG SbieApi_SetUserName(const WCHAR* SidString, const WCHAR* UserName)
{
	NTSTATUS status;
...
_FX LONG SbieApi_MonitorControl(ULONG* NewState, ULONG* OldState)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_MonitorPut(USHORT Type, const WCHAR* Name)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_MonitorPut2(USHORT Type, const WCHAR* Name, BOOLEAN bCheckObjectExists)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_MonitorGet(USHORT* Type,
...
_FX LONG SbieApi_GetUnmountHive(WCHAR* path) // WCHAR [256]
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_IsBoxEnabled(const WCHAR* box_name) // WCHAR [34]
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_OpenKey(HANDLE* KeyHandle, const WCHAR* Path)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 ResultHandle;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
...
_FX LONG SbieApi_SetLowLabelKey(const WCHAR* Path)
{
	NTSTATUS status;
	__declspec(align(8)) ULONG64 parms[API_NUM_ARGS];
