...
static NTSTATUS File_InitUsers_2(HANDLE hProfileKey, WCHAR* buf1, WCHAR* buf2, const ULONG buf_len, ULONG* errlvl);

static NTSTATUS File_InitUsers_3(HANDLE hkey, UNICODE_STRING* uni, WCHAR* buf, ULONG buf_len);

static BOOLEAN File_InitUsersFromEnvironment(WCHAR* buf, ULONG buf_len);

static WCHAR* File_AllocAndInitEnvironment_2(ULONG len, WCHAR** p_optr, BOOLEAN add_dev_map);

static void File_AdjustDrives(ULONG path_drive_index, BOOLEAN subst, const WCHAR* path);
...
_FX BOOLEAN File_Init(void)
{
	...
	RtlGetFullPathName_UEx = GetProcAddress(Dll_Ntdll, "RtlGetFullPathName_UEx");
	...
	if (1)
	{
		void* DefineDosDeviceW = GetProcAddress(Dll_KernelBase ? Dll_KernelBase : Dll_Kernel32, "DefineDosDeviceW");
		if (DefineDosDeviceW)
		{
	...
	if (Dll_OsBuild >= 6000)
	{
		void* GetFinalPathNameByHandleW = GetProcAddress(Dll_KernelBase ? Dll_KernelBase : Dll_Kernel32, "GetFinalPathNameByHandleW");
	...
	{
		void* GetVolumeInformationW = GetProcAddress(Dll_KernelBase ? Dll_KernelBase : Dll_Kernel32, "GetVolumeInformationW");
	...
	{
		void* WriteProcessMemory = GetProcAddress(Dll_KernelBase ? Dll_KernelBase : Dll_Kernel32, "WriteProcessMemory");
...
_FX void File_InitPathList(void)
{
	...
	handle = 0;
	NtOpenFile(&handle, FILE_READ_DATA, &objattrs, &MyIoStatusBlock, FILE_SHARE_VALID_FLAGS, 0);
	if (handle)
	{
		NtClose(handle);
	}
	...
	if (path[0])
	{
		...
		File_HomeNtPath    = Dll_Alloc((File_HomeNtPathLen + 1) * sizeof(WCHAR));
	...
	for (drive = 0; drive < 26; ++drive)
	{
		...
		if (!(DriveMask & (1 << drive)))
		{
			...
			if (file_drive)
			{
				...
				if (path && wmemcmp(path, File_BQQB, 4) == 0 && path[4] && path[5] == L':' && path[6] == L'\\')
				{
		...
		if (HaveSymlinkTarget)
		{
			...
			path = File_GetName_TranslateSymlinks(TlsData, path, objname.Length, &translated);
			if (path)
			{
				...
				file_drive_len     = sizeof(FILE_DRIVE) + (path_len + 1) * sizeof(WCHAR);
...
_FX void File_InitLinks(THREAD_DATA* TlsData)
{
	...
	status = NtCreateFile(&hMountMgr, FILE_GENERIC_EXECUTE, &objattrs, &IoStatusBlock, NULL, FILE_ATTRIBUTE_NORMAL, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE, NULL, 0);
	...
	status = NtDeviceIoControlFile(hMountMgr, NULL, NULL, NULL, &IoStatusBlock, IOCTL_MOUNTMGR_QUERY_POINTS, &Input1, sizeof(MOUNTMGR_MOUNT_POINT), Output1, 8192);
	...
	for (index1 = 0; index1 < Output1->NumberOfMountPoints; ++index1)
	{
		...
		WCHAR* DeviceName                = (WCHAR*)((UCHAR*)Output1 + MountPoint->DeviceNameOffset);
		ULONG DeviceNameLen              = MountPoint->DeviceNameLength / sizeof(WCHAR);
		WCHAR* VolumeName                = (WCHAR*)((UCHAR*)Output1 + MountPoint->SymbolicLinkNameOffset);
		...
		status = NtDeviceIoControlFile(hMountMgr, NULL, NULL, NULL, &IoStatusBlock, IOCTL_MOUNTMGR_QUERY_DOS_VOLUME_PATHS, Input2, 98, Output2, 8192);
	...
	if (Dll_BoxName)
	{
		WCHAR* TruePath = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, (Dll_BoxFilePathLen + 1) * sizeof(WCHAR));
		wmemcpy(TruePath, Dll_BoxFilePath, Dll_BoxFilePathLen + 1);

		if (TruePath)
		{
			BOOLEAN converted = File_GetName_ConvertLinks(TlsData, &TruePath, FALSE);
			if (converted)
			{
...
_FX void File_AdjustDrives(ULONG path_drive_index, BOOLEAN subst, const WCHAR* path)
{
	FILE_DRIVE* file_drive;
	...
	for (drive_index = 0; drive_index < 26; ++drive_index)
	{
		...
		if (wcslen(file_drive_path) >= path_len && (file_drive_path[path_len] == L'\0' || file_drive_path[path_len] == L'\\') && _wcsnicmp(file_drive_path, path, path_len) == 0)
		{
			WCHAR* new_path;
			ULONG alloc_len    = sizeof(FILE_DRIVE) + (path_len + 8) * sizeof(WCHAR);
...
_FX void File_InitWow64(void)
{
	...
	pGetSystemWow64Directory = (P_GetSystemWow64Directory)GetProcAddress(Dll_Kernel32, "GetSystemWow64DirectoryW");
	...
	*path = L'\0';
	...
	{
		File_Wow64DisableWow64FsRedirection = GetProcAddress(Dll_Kernel32, "Wow64DisableWow64FsRedirection");
		File_Wow64RevertWow64FsRedirection  = GetProcAddress(Dll_Kernel32, "Wow64RevertWow64FsRedirection");
	}
}
#endif WOW64_FS_REDIR
...
{
	static const WCHAR* _ProfileList = L"\\Registry\\Machine\\Software"
...
_FX NTSTATUS File_InitUsers_2(HANDLE hProfileKey, WCHAR* buf1, WCHAR* buf2, const ULONG buf_len, ULONG* errlvl)
{
	NTSTATUS status;
	FILE_DRIVE* drive;
	ULONG len1, len2;
	...
	InitializeObjectAttributes(&objattrs, &uni, OBJ_CASE_INSENSITIVE, hProfileKey, NULL);
	...
	if (NT_SUCCESS(status))
	{
		...
		File_PublicUser    = Dll_Alloc((File_PublicUserLen + 1) * sizeof(WCHAR));
...
_FX NTSTATUS File_InitUsers_3(HANDLE hkey, UNICODE_STRING* uni, WCHAR* buf, ULONG buf_len)
{
	...
	NTSTATUS status = NtQueryValueKey(hkey, uni, KeyValuePartialInformation, info, buf_len - 16, &len);
...
_FX void* File_AllocAndInitEnvironment(void* Environment, BOOLEAN IsUnicode, BOOLEAN AddDeviceMap, ULONG* OutLengthInBytes)
{
	ULONG len, len1, i;
...
_FX WCHAR* File_AllocAndInitEnvironment_2(ULONG len, WCHAR** p_optr, BOOLEAN add_dev_map)
{
	WCHAR *env, *optr;
	WCHAR devmap[96];

	len += (wcslen(File_AllUsers_EnvVar) + 1 + File_AllUsersLen + 1 + wcslen(File_CurrentUser_EnvVar) + 1 + File_CurrentUserLen + 1) * sizeof(WCHAR);

	if (File_PublicUserLen)
	{
		len += (wcslen(File_PublicUser_EnvVar) + 1 + File_PublicUserLen + 1) * sizeof(WCHAR);
	}
	...
	if (add_dev_map)
	{
		...
		len += (wcslen(File_DeviceMap_EnvVar) + 1 + wcslen(devmap) + 1) * sizeof(WCHAR);
	}
...
_FX void File_InitCopyLimit(void)
{
	...
	if (Dll_ImageType == DLL_IMAGE_SANDBOXIE_CRYPTO || Dll_ImageType == DLL_IMAGE_SANDBOXIE_WUAU || Dll_ImageType == DLL_IMAGE_WUAUCLT || Dll_ImageType == DLL_IMAGE_TRUSTED_INSTALLER)
	{
	...
	status = SbieApi_QueryConfAsIs(NULL, _CopyLimitKb, 0, str, sizeof(str) - sizeof(WCHAR));
	if (NT_SUCCESS(status))
	{
		ULONG num = _wtoi(str);
		if (num)
...
_FX WCHAR* File_TranslateDosToNtPath(const WCHAR* DosPath)
{
	...
	if (DosPath && DosPath[0] && DosPath[1])
	{
		...
		else if (DosPath[1] == L':' && (DosPath[2] == L'\\' || DosPath[2] == L'\0'))
		{
			//
			// drive-letter path
			//
...
_FX void File_GetSetDeviceMap(WCHAR* DeviceMap96)
{
	...
	if (*DeviceMap96)
	{
		...
		status = NtOpenDirectoryObject(&info.Set.DirectoryHandle, DIRECTORY_TRAVERSE, &objattrs);
		...
		{
			status = NtSetInformationProcess(NtCurrentProcess(), ProcessDeviceMap, &info, sizeof(info.Set));
			...
			if (status == STATUS_INFO_LENGTH_MISMATCH && Dll_IsWow64)
			{
				...
				req.DirectoryHandle = (ULONG64)(ULONG_PTR)info.Set.DirectoryHandle;
				rpl                 = SbieDll_CallServer(&req.h);
				if (!rpl)
				{
	...
	else
	{
		...
		else
		{
			...
			{
				UNICODE_STRING* uni = &((OBJECT_NAME_INFORMATION*)dirname)->ObjectName;
