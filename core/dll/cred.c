...
static WCHAR* Cred_GetName(const WCHAR* DomainName, const WCHAR* TargetName, ULONG Type);

static void* Cred_Serialize(CREDENTIALW* cred, ULONG* out_len, BOOLEAN OverrideLastWriteTime);
...
static BOOL Cred_CredReadW(void* TargetName, ULONG Type, ULONG Flags, void** ppCredential);
static BOOL Cred_CredReadA(void* TargetName, ULONG Type, ULONG Flags, void** ppCredential);

static BOOL Cred_CredWriteDomainCredentialsW(void* pTargetInfo, void* pCredential, ULONG Flags);
static BOOL Cred_CredWriteDomainCredentialsA(void* pTargetInfo, void* pCredential, ULONG Flags);

static BOOL Cred_CredReadDomainCredentialsW(void* pTargetInfo, ULONG Flags, ULONG* pCount, void*** ppCredentials);
static BOOL Cred_CredReadDomainCredentialsA(void* pTargetInfo, ULONG Flags, ULONG* pCount, void*** ppCredentials);

static BOOL Cred_CredGetTargetInfoW(void* pTargetName, ULONG Flags, void** pTargetInfo);
static BOOL Cred_CredGetTargetInfoA(void* pTargetName, ULONG Flags, void** pTargetInfo);

static BOOL Cred_CredRenameW(void* OldTargetName, void* NewTargetName, ULONG Type, ULONG Flags);
static BOOL Cred_CredRenameA(void* OldTargetName, void* NewTargetName, ULONG Type, ULONG Flags);
...
static BOOL Cred_CredEnumerateW(void* pFilter, ULONG Flags, ULONG* pCount, void*** ppCredentials);
static BOOL Cred_CredEnumerateA(void* pFilter, ULONG Flags, ULONG* pCount, void*** ppCredentials);
...
DEFINE_GUID(GUID_SANDBOXIE, 0xBE6FB014L, 0x7308, 0x47F5, 0x85, 0x16, 0x80, 0x2E, 0xBF, 0x98, 0x7F, 0xD2);
...
#define SBIEDLL_HOOK_CRED(proc)                                                                  \
	*(ULONG_PTR*)&__sys_##proc = (ULONG_PTR)Ldr_GetProcAddrNew(DllName_advapi32, L#proc, #proc); \
	...
	{                                                                                            \
		*(ULONG_PTR*)&__sys_##proc = (ULONG_PTR)SbieDll_Hook(#proc, __sys_##proc, Cred_##proc);  \
		if (!__sys_##proc)                                                                       \
			return FALSE;                                                                        \
	}
...
_FX BOOLEAN Cred_PreparePStore(void)
{
	...
	if (module)
	{
		Cred_CoTaskMemFree = (P_CoTaskMemFree)GetProcAddress(module, "CoTaskMemFree");
...
_FX WCHAR* Cred_GetName(const WCHAR* DomainName, const WCHAR* TargetName, ULONG Type)
{
	ULONG len;
	WCHAR* name;
...
_FX void* Cred_Serialize(CREDENTIALW* cred, ULONG* out_len, BOOLEAN OverrideLastWriteTime)
{
	ULONG len;
	ULONG i;
	void* buffer;
	...
	*(ULONG*)ptr = cred->AttributeCount;
	...
	for (i = 0; i < cred->AttributeCount; ++i)
	{
		...
		*(ULONG*)ptr = cred->Attributes[i].ValueSize;
		...
		{
			memcpy(ptr, cred->Attributes[i].Value, cred->Attributes[i].ValueSize);
...
_FX BOOLEAN Cred_UnserializeHelper(void* in_cred, CREDENTIALW* cred)
{
	...
	if (cred->AttributeCount)
	{
		...
		ptr2             = (WCHAR*)((UCHAR*)ptr2 + cred->AttributeCount * sizeof(CREDENTIAL_ATTRIBUTEW));
	...
	*pCount = 0;
	...
	for (j = 0; j < i; ++j)
	{
		...
		if (ok)
		{
			...
			if (ok)
			{
				if (newCred->LastWritten.dwLowDateTime == 0 && newCred->LastWritten.dwHighDateTime == 0)
				{
...
_FX BOOL Cred_WriteItem(const WCHAR* name, void* data, ULONG len)
{
	...
	hr = IPStore_WriteItem(Cred_PStore, PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &GUID_SANDBOXIE, name, len, data, NULL, 0, 0);
	...
	if (FAILED(hr))
	{
		...
		hr                     = IPStore_CreateType(Cred_PStore, PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &typeinfo, 0);

		hr = IPStore_CreateSubtype(Cred_PStore, PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &GUID_SANDBOXIE, &typeinfo, NULL, 0);
...
_FX BOOL Cred_CredReadW(void* TargetName, ULONG Type, ULONG Flags, void** ppCredential)
{
	HRESULT hr;
	WCHAR* name;
	UCHAR* mrshcred;
	...
	hr = IPStore_ReadItem(Cred_PStore, PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &GUID_SANDBOXIE, name, &mrshcred_len, &mrshcred, NULL, 0);
...
_FX BOOL Cred_CredWriteDomainCredentialsW(void* pTargetInfo, void* pCredential, ULONG Flags)
{
	CREDENTIAL_TARGET_INFORMATIONW* TargetInfo = (CREDENTIAL_TARGET_INFORMATIONW*)pTargetInfo;
	CREDENTIALW* cred                          = (CREDENTIALW*)pCredential;
	void* mrshcred;
	ULONG mrshcred_len;
	ULONG i;
	...
	for (i = 0; i < 5 && ok; ++i)
	{
		...
		if (DomainName)
		{
			WCHAR* name = Cred_GetName(DomainName, TargetInfo->TargetName, 0);
...
_FX BOOL Cred_CredReadDomainCredentialsW(void* pTargetInfo, ULONG Flags, ULONG* pCount, void*** ppCredentials)
{
	CREDENTIAL_TARGET_INFORMATIONW* TargetInfo = (CREDENTIAL_TARGET_INFORMATIONW*)pTargetInfo;
	HRESULT hr;
	ULONG i;
	...
	if (!ok)
	{
		return __sys_CredReadDomainCredentialsW(pTargetInfo, Flags, pCount, ppCredentials);
	}
...
_FX BOOL Cred_CredEnumerateW(void* pFilter, ULONG Flags, ULONG* pCount, void*** ppCredentials)
{
	HRESULT hr;
	...
	hr = IPStore_EnumItems(Cred_PStore, PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &GUID_SANDBOXIE, 0, &pEnum);
	...
	if (num_mrshcreds)
	{
		if (ok)
		{
			...
			for (i = 0; i < *pCount; ++i)
			{
				...
				{
					mrshcreds[num_mrshcreds] = Cred_Serialize(in_creds[i], &len, FALSE);
					++num_mrshcreds;
				}
			}
...
_FX BOOL Cred_CredRenameW(void* OldTargetName, void* NewTargetName, ULONG Type, ULONG Flags)
{
...
_FX BOOL Cred_CredWriteDomainCredentialsA(void* pTargetInfo, void* pCredential, ULONG Flags)
{
...
_FX BOOL Cred_CredRenameA(void* OldTargetName, void* NewTargetName, ULONG Type, ULONG Flags)
{
...
_FX BOOL Cred_CredReadA(void* TargetName, ULONG Type, ULONG Flags, void** ppCredential)
{
...
_FX BOOL Cred_CredReadDomainCredentialsA(void* pTargetInfo, ULONG Flags, ULONG* pCount, void*** ppCredentials)
{
	SbieApi_Log(2205, L"CredReadDomainCredentialsA");
	return __sys_CredReadDomainCredentialsA(pTargetInfo, Flags, pCount, ppCredentials);
}
...
_FX BOOL Cred_CredEnumerateA(void* pFilter, ULONG Flags, ULONG* pCount, void*** ppCredentials)
{
