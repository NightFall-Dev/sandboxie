...
static ULONG RpcRt_RpcBindingInqAuthClientEx(void* ClientBinding, void* Privs, void** ServerPrincName, ULONG* AuthnLevel, ULONG* AuthnSvc, ULONG* AuthzSvc, ULONG Flags);

static ULONG RpcRt_RpcBindingFromStringBindingW(const WCHAR* StringBinding, void** OutBinding);

static RPC_STATUS NsiRpc_NsiRpcRegisterChangeNotification(LPVOID p1, LPVOID p2, LPVOID p3, LPVOID p4, LPVOID p5, LPVOID p6, LPVOID p7);

static RPC_STATUS RpcRt_RpcBindingCreateW(RPC_BINDING_HANDLE_TEMPLATE_V1_W* Template, RPC_BINDING_HANDLE_SECURITY_V1_W* Security, RPC_BINDING_HANDLE_OPTIONS_V1* Options, RPC_BINDING_HANDLE* Binding);
...
typedef ULONG (*P_RpcBindingInqAuthClientEx)(void* ClientBinding, void* Privs, void** ServerPrincName, ULONG* AuthnLevel, ULONG* AuthnSvc, ULONG* AuthzSvc, ULONG Flags);

typedef ULONG (*P_RpcBindingFromStringBindingW)(const WCHAR* StringBinding, void** OutBinding);

typedef RPC_STATUS (*P_NsiRpcRegisterChangeNotification)(LPVOID p1, LPVOID p2, LPVOID p3, LPVOID p4, LPVOID p5, LPVOID p6, LPVOID p7);

typedef RPC_STATUS (*P_RpcBindingCreateW)(RPC_BINDING_HANDLE_TEMPLATE_V1_W* Template, RPC_BINDING_HANDLE_SECURITY_V1_W* Security, RPC_BINDING_HANDLE_OPTIONS_V1* Options, RPC_BINDING_HANDLE* Binding);
...
_FX BOOLEAN RpcRt_Init(HMODULE module)
{
	...
	RpcBindingInqAuthClientEx = (P_RpcBindingInqAuthClientEx)Ldr_GetProcAddrNew(DllName_rpcrt4, L"RpcBindingInqAuthClientExW", "RpcBindingInqAuthClientExW");
	...
	if (Dll_OsBuild >= 6000) // Vista
	{
		...
		RpcBindingFromStringBindingW = (P_RpcBindingFromStringBindingW)Ldr_GetProcAddrNew(DllName_rpcrt4, L"RpcBindingFromStringBindingW", "RpcBindingFromStringBindingW");

		SBIEDLL_HOOK(RpcRt_, RpcBindingFromStringBindingW);

		RpcBindingCreateW = (P_RpcBindingCreateW)Ldr_GetProcAddrNew(DllName_rpcrt4, L"RpcBindingCreateW", "RpcBindingCreateW");
...
_FX BOOLEAN NsiRpc_Init(HMODULE module)
{
	...
	NsiRpcRegisterChangeNotification = (P_NsiRpcRegisterChangeNotification)Ldr_GetProcAddrNew(DllName_winnsi, L"NsiRpcRegisterChangeNotification", "NsiRpcRegisterChangeNotification");
...
_FX ULONG RpcRt_RpcBindingInqAuthClientEx(void* ClientBinding, void* Privs, void** ServerPrincName, ULONG* AuthnLevel, ULONG* AuthnSvc, ULONG* AuthzSvc, ULONG Flags)
{
	ULONG rc = __sys_RpcBindingInqAuthClientEx(ClientBinding, Privs, ServerPrincName, AuthnLevel, AuthnSvc, AuthzSvc, Flags);
	...
	if (rc == RPC_S_INVALID_AUTH_IDENTITY && AuthnLevel && !(ServerPrincName || AuthnSvc || AuthzSvc || Flags))
	{
		rc          = RPC_S_OK;
...
HANDLE FindServicePidByName(WCHAR* pSvcName)
{
	...
	SERVICE_QUERY_RPL* rpl = (SERVICE_QUERY_RPL*)Scm_QueryServiceByName(pSvcName, TRUE, FALSE);
	if (rpl)
	{
...
_FX ULONG RpcRt_RpcBindingFromStringBindingW(const WCHAR* StringBinding, void** OutBinding)
{
	static const WCHAR* _old = L"ncalrpc:[,Security=Impersonation Dynamic False]";
	...
	if (_wcsicmp(StringBinding, _old) == 0)
	{
		...
		if (WinSpool && (RetAddr > WinSpool) && (RetAddr < WinSpool + 0x1FFFF))
		{
			...
			if (hPidSpooler)
			{
				...
				if (SbieApi_CallThree(API_GET_SPOOLER_PORT, (ULONG_PTR)hPidSpooler, (ULONG_PTR)L"spoolsv.exe", (ULONG_PTR)g_Ipc_DynamicPortNames[SPOOLER_PORT]) == STATUS_SUCCESS)
				{
			...
			if (GetModuleInformation(GetCurrentProcess(), (HANDLE)hWinHttp, &modinfo, sizeof(MODULEINFO)))
			{
				...
				if (pRetAddr < hWinHttp + modinfo.SizeOfImage)
				{
					...
					if (hPidWpad)
					{
						...
						if (SbieApi_CallThree(API_GET_WPAD_PORT, (ULONG_PTR)hPidWpad, (ULONG_PTR)L"svchost.exe", (ULONG_PTR)g_Ipc_DynamicPortNames[WPAD_PORT]) == STATUS_SUCCESS)
						{
...
WCHAR* GetSCardLpcPortName()
{
	...
	if (rpl)
	{
		...
		SbieApi_CallTwo(API_SET_SMART_CARD_PORT, (ULONG_PTR)wstrTempPortName, (ULONG_PTR)g_Ipc_DynamicPortNames[SMART_CARD_PORT]);
...
DEFINE_GUID(EMPTY_UUID, 0x00000000, 0x0000, 0x0000, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);

// {906B0CE0-C70B-1067-B317-00DD010662DA}
DEFINE_GUID(MSDTC_UUID, 0x906B0CE0, 0xC70B, 0x1067, 0xB3, 0x17, 0x00, 0xDD, 0x01, 0x06, 0x62, 0xDA);

_FX RPC_STATUS RpcRt_RpcBindingCreateW(__in RPC_BINDING_HANDLE_TEMPLATE_V1_W* Template, __in_opt RPC_BINDING_HANDLE_SECURITY_V1_W* Security, __in_opt RPC_BINDING_HANDLE_OPTIONS_V1* Options, __out RPC_BINDING_HANDLE* Binding)
{
	if ((memcmp(&Template->ObjectUuid, &MSDTC_UUID, sizeof(GUID)) == 0) && RPC_PROTSEQ_LRPC == Template->ProtocolSequence
	    && !Template->StringEndpoint)
	{
		Template->StringEndpoint = (unsigned short*)L"samss lpc";
	}

	else if ((memcmp(&Template->ObjectUuid, &EMPTY_UUID, sizeof(GUID)) == 0) && RPC_PROTSEQ_LRPC == Template->ProtocolSequence
	    && !Template->StringEndpoint)
	{
				...
				if (pRetAddr < hResourcePolicyClient + modinfo.SizeOfImage)
				{
					...
					if (rpl)
					{
						...
						if (rpl_status == 0)
						{
							...
							// Param 1 is dynamic port name (e.g. "LRPC-f760d5b40689a98168")
							// Param 2 will return the port name with "\RPC Control\" prepended
							SbieApi_CallTwo(API_SET_GAME_CONFIG_STORE_PORT, (ULONG_PTR)wstrTempPortName, (ULONG_PTR)g_Ipc_DynamicPortNames[GAME_CONFIG_STORE_PORT]);
						}
						Dll_Free(rpl);
					}
				}
	...
	if (SbieApi_QueryConf(NULL, L"IpcTrace", 0, wsTraceOptions, sizeof(wsTraceOptions)) == STATUS_SUCCESS)
	{
		...
		Sbie_swprintf(msg,
		    L"SBIE p=%06d t=%06d RpcBindingCreateW Endpoint = '%s', UUID = %s, status = 0x%X\n",
		    GetCurrentProcessId(),
		    GetCurrentThreadId(),
