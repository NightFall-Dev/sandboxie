...
IPStoreImpl::IPStoreImpl(void* ptrCoTaskMemAlloc)
{
	...
	if (!section)
	{
		section = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 8, _SectionName);
		if (section)
		{
	...
	if (section)
	{
		global_timestamp = (__int64*)MapViewOfFile(section, FILE_MAP_ALL_ACCESS, 0, 0, 8);
		if (section_created)
		{
...
void* IPStoreImpl::open_stream(bool forWrite)
{
	...
	handle = CreateFile(filename, desired_access, share_mode, NULL, create_disp, file_attrs, NULL);

	if ((handle == INVALID_HANDLE_VALUE) && (GetLastError() == 5) && forWrite)
	{
...
bool IPStoreImpl::read_type(void* _stream)
{
	...
	if (Stream_Read_Bytes(stream, sizeof(GUID), (UCHAR*)&type->type_id) != STATUS_SUCCESS)
	{
		return false;
	}
	int name_len;
...
bool IPStoreImpl::read_subtype(void* _stream, PsType* type)
{
	...
	if (Stream_Read_Bytes(stream, sizeof(GUID), (UCHAR*)&subtype->subtype_id) != STATUS_SUCCESS)
	{
		return false;
	}
	int name_len;
...
bool IPStoreImpl::write_type(void* _stream, const PsType* type)
{
	...
	if (Stream_Write_Bytes(stream, sizeof(GUID), (UCHAR*)&type->type_id) != STATUS_SUCCESS)
	{
		return false;
	}
...
bool IPStoreImpl::write_subtype(void* _stream, const PsSubtype* subtype)
{
	...
	if (Stream_Write_Bytes(stream, sizeof(GUID), (UCHAR*)&subtype->subtype_id) != STATUS_SUCCESS)
	{
		return false;
	}
...
IPStoreImpl::PsType* IPStoreImpl::create_type(const GUID* pType, const PST_TYPEINFO* pInfo, DWORD dwFlags)
{
	PsType* type = HeapAllocType(PsType);
...
IPStoreImpl::PsSubtype* IPStoreImpl::create_subtype(IPStoreImpl::PsType* type, const GUID* pSubtype, const PST_TYPEINFO* pInfo, DWORD dwFlags)
{
	PsSubtype* subtype = HeapAllocType(PsSubtype);
...
IPStoreImpl::PsItem* IPStoreImpl::create_item(IPStoreImpl::PsSubtype* subtype, IPStoreImpl::PsItem* item, const WCHAR* szItemName, DWORD cbData, void* pbData)
{
	if (!item)
	{
...
IPStoreImpl::PsType* IPStoreImpl::find_type(const GUID* guid)
{
	...
	PSTORE_GET_TYPE_INFO_RPL* rpl = (PSTORE_GET_TYPE_INFO_RPL*)SbieDll_CallServer(&req.h);
...
IPStoreImpl::PsSubtype* IPStoreImpl::find_subtype(IPStoreImpl::PsType* type, const GUID* guid)
{
	...
	PSTORE_GET_SUBTYPE_INFO_RPL* rpl = (PSTORE_GET_SUBTYPE_INFO_RPL*)SbieDll_CallServer(&req.h);
...
IPStoreImpl::PsItem* IPStoreImpl::find_item(IPStoreImpl::PsType* type, IPStoreImpl::PsSubtype* subtype, const WCHAR* name)
{
	...
	ULONG req_len             = sizeof(PSTORE_READ_ITEM_REQ) + (wcslen(name) + 1) * sizeof(WCHAR);
	...
	PSTORE_READ_ITEM_RPL* rpl = (PSTORE_READ_ITEM_RPL*)SbieDll_CallServer(&req->h);
    ...
    /* [in] */ DWORD dwFlags)
{
	...
	*cbData = item->value_len;
	if (item->value_len)
	{
		*ppbData = (BYTE*)((P_CoTaskMemAlloc)CoTaskMemAlloc)(item->value_len);
    ...
    /* [in] */ IEnumPStoreItems __RPC_FAR * __RPC_FAR * ppenum)
{
	...
	*ppenum = IEnumPStoreItemsImpl::CreateEnumItem(this, Key, pItemType, pItemSubtype);
	if (!*ppenum)
	{
		return E_OUTOFMEMORY;
	}
...
ALIGNED HRESULT IPStoreImpl::QueryInterface(REFIID iid, void** ppvObject)
{
	this->AddRef();
	*ppvObject = this;
	return S_OK;
