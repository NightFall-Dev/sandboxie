...
static NTSTATUS SysInfo_NtQuerySystemInformation(SYSTEM_INFORMATION_CLASS SystemInformationClass, void* Buffer, ULONG BufferLength, ULONG* ReturnLength);

static void SysInfo_DiscardProcesses(SYSTEM_PROCESS_INFORMATION* buf);

static BOOL SysInfo_SetLocaleInfoW(LCID Locale, LCTYPE LCType, void* lpLCData);

static BOOL SysInfo_SetLocaleInfoA(LCID Locale, LCTYPE LCType, void* lpLCData);

static NTSTATUS SysInfo_NtTraceEvent(HANDLE TraceHandle, ULONG Flags, ULONG FieldSize, PVOID Fields);

static NTSTATUS SysInfo_NtDeleteWnfStateData(ULONG_PTR UnknownParameter1, ULONG_PTR UnknownParameter2);

static NTSTATUS SysInfo_NtCreateJobObject(HANDLE* JobHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes);

static NTSTATUS SysInfo_NtAssignProcessToJobObject(HANDLE JobHandle, HANDLE ProcessHandle);

static NTSTATUS SysInfo_NtSetInformationJobObject(HANDLE JobHandle, JOBOBJECTINFOCLASS JobObjectInformationClass, void* JobObjectInformtion, ULONG JobObjectInformtionLength);

static void SysInfo_JobCallbackData_Set(HANDLE ProcessHandle, void* JobPortInformation);
...
_FX BOOLEAN SysInfo_Init(void)
{
	...
	if (Dll_OsBuild >= 8400)
	{
		...
		void* NtDeleteWnfStateData = GetProcAddress(Dll_Ntdll, "NtDeleteWnfStateData");
...
_FX NTSTATUS SysInfo_NtQuerySystemInformation(SYSTEM_INFORMATION_CLASS SystemInformationClass, void* Buffer, ULONG BufferLength, ULONG* ReturnLength)
{
	NTSTATUS status;

	status = __sys_NtQuerySystemInformation(SystemInformationClass, Buffer, BufferLength, ReturnLength);
...
_FX NTSTATUS SysInfo_NtTraceEvent(HANDLE TraceHandle, ULONG Flags, ULONG FieldSize, PVOID Fields)
{
...
_FX void* SysInfo_QueryProcesses(ULONG* out_len)
{
	...
	for (i = 0; i < 5; ++i)
	{
		...
		status = NtQuerySystemInformation(SystemProcessInformation, info, len, &len);
...
_FX NTSTATUS SysInfo_NtCreateJobObject(HANDLE* JobHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes)
{
	...
	while (1)
	{
		...
		Sbie_swprintf(jobname, L"%s\\%s_DummyJob_%s_%d", Dll_BoxIpcPath, SBIE, Dll_ImageName, _JobCounter);
		...
		status = __sys_NtCreateJobObject(JobHandle, DesiredAccess, &objattrs);
...
_FX NTSTATUS SysInfo_NtAssignProcessToJobObject(HANDLE JobHandle, HANDLE ProcessHandle)
{
	HANDLE DuplicatedProcessHandle;

	if (0 == NtDuplicateObject(NtCurrentProcess(), ProcessHandle, NtCurrentProcess(), &DuplicatedProcessHandle, PROCESS_QUERY_INFORMATION | SYNCHRONIZE, 0, 0))
	{
...
_FX NTSTATUS SysInfo_NtSetInformationJobObject(HANDLE JobHandle, JOBOBJECTINFOCLASS JobObjectInformationClass, void* JobObjectInformtion, ULONG JobObjectInformtionLength)
{
	NTSTATUS status = __sys_NtSetInformationJobObject(JobHandle, JobObjectInformationClass, JobObjectInformtion, JobObjectInformtionLength);

	if (NT_SUCCESS(status) && JobObjectInformationClass == JobObjectAssociateCompletionPortInformation)
	{
...
_FX void SysInfo_JobCallbackData_Set(HANDLE ProcessHandle, void* JobPortInformation)
{
	...
	if ((Handles[3] & 3) == 3)
	{
		HANDLE ThreadHandle = CreateThread(NULL, 0, SysInfo_JobCallbackData_Thread, Handles, 0, NULL);
...
_FX ULONG SysInfo_JobCallbackData_Thread(void* xHandles)
{
	...
	if (0 != NtQueryInformationProcess(ProcessHandle, ProcessBasicInformation, &info, sizeof(PROCESS_BASIC_INFORMATION), &len))
	{
		info.UniqueProcessId = 0;
	}

	PostQueuedCompletionStatus(CompletionPort, JOB_OBJECT_MSG_NEW_PROCESS, CompletionKey, (LPOVERLAPPED)info.UniqueProcessId);
	...
	if (WaitForSingleObject(ProcessHandle, INFINITE) == 0)
	{
		...
		PostQueuedCompletionStatus(CompletionPort, JOB_OBJECT_MSG_EXIT_PROCESS, CompletionKey, (LPOVERLAPPED)info.UniqueProcessId);
	}
