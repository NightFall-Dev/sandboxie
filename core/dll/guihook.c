...
static HHOOK Gui_SetWindowsHookExA(int idHook, HOOKPROC lpfn, HINSTANCE hMod, DWORD dwThreadId);

static HHOOK Gui_SetWindowsHookExW(int idHook, HOOKPROC lpfn, HINSTANCE hMod, DWORD dwThreadId);

static HHOOK Gui_SetWindowsHookEx(int idHook, HOOKPROC lpfn, HINSTANCE hMod, BOOLEAN IsUnicode);
...
{
	const ACCESS_MASK DesiredAccess = (Dll_OsBuild >= 6000 ? THREAD_QUERY_LIMITED_INFORMATION : THREAD_QUERY_INFORMATION);
	NTSTATUS status;
	HANDLE ThreadHandle;
	CLIENT_ID ClientId;
	...
	status                 = NtOpenThread(&ThreadHandle, DesiredAccess, &objattrs, &ClientId);
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtQueryInformationThread(ThreadHandle, ThreadBasicInformation, &ThreadBasicInfo, sizeof(ThreadBasicInfo), &len);
...
_FX HHOOK Gui_SetWindowsHookExA(int idHook, HOOKPROC lpfn, HINSTANCE hMod, DWORD dwThreadId)
{
	HHOOK hhook;

	if (idHook == WH_JOURNALRECORD || idHook == WH_JOURNALPLAYBACK || idHook == WH_KEYBOARD_LL || idHook == WH_MOUSE_LL || Gui_IsThreadInThisProcess(dwThreadId))
	{
		//
...
_FX HHOOK Gui_SetWindowsHookExW(int idHook, HOOKPROC lpfn, HINSTANCE hMod, DWORD dwThreadId)
{
	//
...
_FX HHOOK Gui_SetWindowsHookEx(int idHook, HOOKPROC lpfn, HINSTANCE hMod, BOOLEAN IsUnicode)
{
	GUI_HOOK* ghk;
	LONG_PTR* threads;
	LONG_PTR i;
...
_FX BOOLEAN Gui_HookThread(GUI_HOOK* ghk, ULONG_PTR idThread)
{
	...
	if (hThread)
	{
		b = GetThreadTimes(hThread, &CreationTime, &ExitTime, &KernelTime, &UserTime);
	...
	while (thd)
	{
		if (thd->idThread == idThread && thd->CreationTime.dwLowDateTime == CreationTime.dwLowDateTime
	...
	if (ghk->IsUnicode)
	{
		thd->hhk = __sys_SetWindowsHookExW(ghk->idHook, ghk->lpfn, ghk->hMod, (ULONG)idThread);
	}
	else
	{
		thd->hhk = __sys_SetWindowsHookExA(ghk->idHook, ghk->lpfn, ghk->hMod, (ULONG)idThread);
	}

#ifdef DEBUG_HOOKS
	{
		WCHAR msg[256];
		Sbie_swprintf(msg, L"2 SetWindowsHookEx%c idHook=%d lpfn=%08X hMod=%08X dwThreadId=%d ; hhook=%08X error=%d\n", ghk->IsUnicode ? L'W' : L'A', ghk->idHook, ghk->lpfn, ghk->hMod, idThread, thd->hhk, GetLastError());
...
_FX LRESULT Gui_ApplyWinHooks(ULONG_PTR idThread)
{
	...
	else
	{
		...
		{
			__sys_EnumWindows(Gui_HookNotifyThreads, (LPARAM)GetCurrentThreadId());
		}
		else
		{
