...
static HANDLE OpenExplorerKey(HANDLE ParentKey, const WCHAR* SubkeyName, ULONG* error);
...
_FX UCHAR GetSetCustomLevel(UCHAR SetLevel)
{
	...
	if (!SetLevel)
	{
		...
		status = Key_OpenOrCreateIfBoxed(&AutoExecHKey, KEY_ALL_ACCESS | Wow64, &objattrs);
		...
		else if (NT_SUCCESS(status))
		{
			...
			status = NtQueryValueKey(AutoExecHKey, &uni, KeyValuePartialInformation, &value_info, sizeof(value_info), &len);
	...
	else if (AutoExecHKey)
	{
		...
		status = NtSetValueKey(AutoExecHKey, &uni, 0, REG_BINARY, &value_info.Data, 1);
...
_FX BOOLEAN Custom_CreateRegLinks(void)
{
	...
	status = NtCreateKey(&hkey1, KEY_ALL_ACCESS | Wow64, &objattrs, 0, NULL, REG_OPTION_CREATE_LINK, NULL);
	...
	status = NtCreateKey(&hkey2, KEY_ALL_ACCESS | Wow64, &objattrs, 0, NULL, 0, NULL);
	...
	status = NtSetValueKey(hkey1, &uni, 0, REG_LINK, path, wcslen(path) * sizeof(WCHAR));
...
_FX BOOLEAN DisableDCOM(void)
{
	...
	RtlInitUnicodeString(&objname, L"\\registry\\machine\\software\\microsoft\\ole");

	status = Key_OpenIfBoxed(&handle, KEY_ALL_ACCESS | Wow64, &objattrs);
	if (!NT_SUCCESS(status))
	{
		if (status != STATUS_BAD_INITIAL_PC && status != STATUS_OBJECT_NAME_NOT_FOUND)
		{
...
_FX BOOLEAN DisableRecycleBin(void)
{
	...
	else if (hkey != INVALID_HANDLE_VALUE)
	{
		...
		status = NtSetValueKey(hkey, &uni, 0, REG_DWORD, (BYTE*)&one, sizeof(one));
		if (!NT_SUCCESS(status))
		{
			error = 0x22;
		}
...
DisableRecycleBinSkipXP:
	...
	if (hkey && hkey != INVALID_HANDLE_VALUE)
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			while (1)
			{
				status = NtEnumerateKey(hkey2, index, KeyBasicInformation, &u, sizeof(u), &len);
				...
				status = Key_OpenIfBoxed(&hkey3, KEY_ALL_ACCESS | Wow64, &objattrs);
				if (NT_SUCCESS(status))
				{
					RtlInitUnicodeString(&uni, L"NukeOnDelete");
					status = NtSetValueKey(hkey3, &uni, 0, REG_DWORD, (BYTE*)&one, sizeof(one));
...
_FX BOOLEAN Custom_EnableBrowseNewProcess(void)
{
	...
	else if (hkey != INVALID_HANDLE_VALUE)
	{
		...
		status = NtSetValueKey(hkey, &uni, 0, REG_SZ, (BYTE*)&yes, sizeof(yes));
		if (!NT_SUCCESS(status))
		{
			error = 0x22;
		}
...
{
	static const WCHAR* _prefix = L"\\registry\\machine\\software\\classes\\clsid\\";
	NTSTATUS status;
	OBJECT_ATTRIBUTES objattrs;
	UNICODE_STRING objname;
	HANDLE handle;
...
_FX HANDLE OpenExplorerKey(HANDLE ParentKey, const WCHAR* SubkeyName, ULONG* error)
{
	static const WCHAR* _Explorer = L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer";
	...
	InitializeObjectAttributes(&objattrs, &uni, OBJ_CASE_INSENSITIVE, HKey_Root, NULL);
	...
	InitializeObjectAttributes(&objattrs, &uni, OBJ_CASE_INSENSITIVE, HKey_Explorer, NULL);
...
_FX void DeleteShellAssocKeys(ULONG Wow64)
{
	...
	static WCHAR* subkeys[] = {L"Machine\\Software\\Classes", L"User\\Current\\Software\\Classes", NULL};
	NTSTATUS status;
	HANDLE hkey;
	WCHAR path[128];
	UNICODE_STRING uni;
...
_FX void AutoExec(void)
{
	...
	while (1)
	{
		status = SbieApi_QueryConfAsIs(NULL, L"AutoExec", index, buf1, buf_len - 16);
		if (status != 0)
		{
			break;
		}

		len = ExpandEnvironmentStrings(buf1, buf2, buf_len / sizeof(WCHAR) - 16);
		...
		if (AutoExecHKey)
		{
			status = NtQueryValueKey(AutoExecHKey, &uni, KeyValueBasicInformation, &basic_info, sizeof(basic_info), &len);
		}
		else
		{
...
_FX BOOLEAN SbieDll_ExpandAndRunProgram(const WCHAR* Command)
{
	...
	ok = CreateProcess(NULL, cmdline, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
...
_FX BOOLEAN Custom_MsgPlusLive(HMODULE module)
{
	...
	for (i = 0; i < 2; ++i)
	{
		...
		{
			wcscat(unibuf,
			    L"Yuna Software\\Messenger Plus!\\"
		...
		if (NT_SUCCESS(status))
		{
			...
			status = NtSetValueKey(hKey, &uni, 0, REG_DWORD, &zero, sizeof(ULONG));
...
_FX BOOLEAN Custom_SilverlightAgCore(HMODULE module)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtSetValueKey(hKey, &uni, 0, REG_DWORD, &two, sizeof(ULONG));
...
{
	const WCHAR* ProductNames[] = {L"Excel", L"Word", L"PowerPoint", NULL};
	const WCHAR* ValueNames[] = {L"DisableAttachmentsInPV", L"DisableInternetFilesInPV", L"DisableUnsafeLocationsInPV", NULL};
	...
	for (ProductIndex = 0; ProductNames[ProductIndex]; ++ProductIndex)
	{
		...
		if (status == STATUS_OBJECT_NAME_NOT_FOUND)
		{
			status = NtCreateKey(&hKey, KEY_ALL_ACCESS | Wow64, &objattrs, 0, NULL, 0, NULL);
		}
		...
		if (NT_SUCCESS(status))
		{
			for (ValueIndex = 0; ValueNames[ValueIndex]; ++ValueIndex)
			{
				...
				NtSetValueKey(hKey, &uni, 0, REG_DWORD, (BYTE*)&one, sizeof(one));
			}
	...
	if (1)
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			NtSetValueKey(hKey, &uni, 0, REG_DWORD, (BYTE*)&zero, sizeof(zero));
...
static NTSTATUS Custom_Avast_SnxHk_LdrGetProcedureAddress(HANDLE ModuleHandle, ANSI_STRING* ProcName, ULONG ProcNum, ULONG_PTR* Address);
...
_FX NTSTATUS Custom_Avast_SnxHk_LdrGetProcedureAddress(HANDLE ModuleHandle, ANSI_STRING* ProcName, ULONG ProcNum, ULONG_PTR* Address)
{
	NTSTATUS status = __sys_LdrGetProcedureAddress(ModuleHandle, ProcName, ProcNum, Address);

	if (NT_SUCCESS(status) && ModuleHandle == Dll_Ntdll && ProcName->Length == 21 && memcmp(ProcName->Buffer, "NtDeviceIoControlFile", 21) == 0)
	{
		static UCHAR* code = 0;
		if (!code)
		{
...
_FX void Custom_ComServer(void)
{
	...
	if (Dll_ImageType != DLL_IMAGE_INTERNET_EXPLORER && Dll_ImageType != DLL_IMAGE_WINDOWS_MEDIA_PLAYER && Dll_ImageType != DLL_IMAGE_NULLSOFT_WINAMP && Dll_ImageType != DLL_IMAGE_PANDORA_KMPLAYER)
	{
		return;
	}
	...
	if (!(wcsstr(cmdline, L"/Embedding") || wcsstr(cmdline, L"-Embedding")))
	{
		return;
	}
	...
	if (Proc_ImpersonateSelf(TRUE))
	{
		...
		BOOL ok = SbieDll_RunSandboxed(L"*THREAD*", L"*COMSRV*", L"", 0, &StartupInfo, &ProcessInformation);
...
_FX void Custom_Load_UxTheme(void)
{
	...
	if (Dll_ChromeSandbox)
	{
		...
		P_SystemParametersInfo SystemParametersInfo = Ldr_GetProcAddrNew(DllName_user32, L"SystemParametersInfoW", "SystemParametersInfoW");
