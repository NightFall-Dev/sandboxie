...
static BOOL Gui_UnregisterClassA(const UCHAR* lpClassName, HINSTANCE hInstance);

static BOOL Gui_UnregisterClassW(const WCHAR* lpClassName, HINSTANCE hInstance);

static ATOM Gui_GetClassInfoA(HINSTANCE hInstance, const UCHAR* lpClassName, WNDCLASSA* wcout);

static ATOM Gui_GetClassInfoW(HINSTANCE hInstance, const WCHAR* lpClassName, WNDCLASSW* wcout);

static ATOM Gui_GetClassInfoExA(HINSTANCE hInstance, const UCHAR* lpClassName, WNDCLASSEXA* wcout);

static ATOM Gui_GetClassInfoExW(HINSTANCE hInstance, const WCHAR* lpClassName, WNDCLASSEXW* wcout);

static int Gui_GetClassNameA(HWND hWnd, UCHAR* lpClassName, int nMaxCount);

static int Gui_GetClassNameW(HWND hWnd, WCHAR* lpClassName, int nMaxCount);

static ULONG Gui_GetClassName2(HWND hWnd, void* clsnm, ULONG maxlen, BOOLEAN unicode);
...
_FX BOOLEAN Gui_InitClass(void)
{
	...
	len            = (wcslen(Sandbox) + 1 + wcslen(Dll_BoxName) + 1 + 1) * sizeof(WCHAR);
	...
	WideCharToMultiByte(CP_ACP, 0, Gui_BoxPrefixW, wcslen(Gui_BoxPrefixW), Gui_BoxPrefixA, len, NULL, NULL);
	...
	else
	{
		...
		{
			WCHAR buf[10];
			LONG rc = SbieApi_QueryConfAsIs(NULL, L"OpenWinClass", len, buf, 8 * sizeof(WCHAR));
			++len;
			if (rc == 0)
			{
	...
	if (Gui_RenameClasses)
	{
		...
		while (1)
		{
			NTSTATUS status = SbieApi_QueryConf(NULL, L"NoRenameWinClass", index, name, 120 * sizeof(WCHAR));
			...
			entry = Dll_Alloc(sizeof(GUI_NO_RENAME_WIN_CLASS) + (name_len + 1) * sizeof(WCHAR));
	...
	if ((!Gui_OpenAllWinClasses) && (!Gui_RenameClasses) && Dll_ImageType == DLL_IMAGE_GOOGLE_CHROME && _wcsicmp(Dll_ImageName, L"maxthon.exe") == 0)
	{
...
_FX void Gui_Hook_CREATESTRUCT_Handler(void)
{
	...
	if (KernelCallbackTable)
	{
		...
		if (*funcptr && VirtualProtect(funcptr, sizeof(ULONG_PTR), PAGE_READWRITE, &prot))
		{
			__sys_CREATESTRUCT_Handler = (P_CREATESTRUCT_Handler)InterlockedExchangePointer((void**)funcptr, (void*)Gui_CREATESTRUCT_Handler);
...
_FX ATOM Gui_RegisterClassExW(WNDCLASSEXW* wcin)
{
	...
	if (_wcsicmp(instName, Gui_comctl32) != 0 && _wcsicmp(instName, Gui_riched20) != 0)
	{
...
_FX BOOL Gui_UnregisterClassW(const WCHAR* lpClassName, HINSTANCE hInstance)
{
	WCHAR* clsnm;
	BOOL ok;
...
_FX BOOL Gui_UnregisterClassA(const UCHAR* lpClassName, HINSTANCE hInstance)
{
	UCHAR* clsnm;
	BOOL ok;
...
_FX ATOM Gui_GetClassInfoExW(HINSTANCE hInstance, const WCHAR* lpClassName, WNDCLASSEXW* wcout)
{
	WCHAR* clsnm;
	ATOM atom;
...
_FX ATOM Gui_GetClassInfoExA(HINSTANCE hInstance, const UCHAR* lpClassName, WNDCLASSEXA* wcout)
{
	UCHAR* clsnm;
	ATOM atom;
...
_FX ATOM Gui_GetClassInfoW(HINSTANCE hInstance, const WCHAR* lpClassName, WNDCLASSW* wcout)
{
	WCHAR* clsnm;
	ATOM atom;
...
_FX ATOM Gui_GetClassInfoA(HINSTANCE hInstance, const UCHAR* lpClassName, WNDCLASSA* wcout)
{
	UCHAR* clsnm;
	ATOM atom;
...
_FX int Gui_GetClassNameW(HWND hWnd, WCHAR* lpClassName, int nMaxCount)
{
...
_FX int Gui_GetClassNameA(HWND hWnd, UCHAR* lpClassName, int nMaxCount)
{
...
_FX ULONG Gui_GetClassName2(HWND hWnd, void* clsnm, ULONG maxlen, BOOLEAN unicode)
{
	...
	rpl = Gui_CallProxy(&req, sizeof(GUI_GET_CLASS_NAME_REQ), sizeof(GUI_GET_CLASS_NAME_RPL));
	if (!rpl)
	{
		result = 0;
	}
...
{
	static const WCHAR* NamesC[] = {L"CHECKLIST_ACLUI",
	    L"CLIPBRDWNDCLASS",
	    L"CicLoaderWndClass",
	...
	static const WCHAR* NamesD[] = {L"DDEMLMom", L"DDEMLEvent", L"DDEMLAnsiClient", L"DDEMLAnsiServer", L"DDEMLUnicodeClient", L"DDEMLUnicodeServer", NULL};

	static const WCHAR* NamesM[] = {L"mdiclient", L"msctls_hotkey", L"msctls_hotkey32", L"msctls_progress", L"msctls_progress32", L"msctls_statusbar", L"msctls_statusbar32", L"msctls_trackbar", L"msctls_trackbar32", L"msctls_updown", L"msctls_updown32", L"MSCTFIME Composition", L"MSCTFIME UI", NULL};

	static const WCHAR* NamesO[] = {L"OleDdeWndClass", L"OleMainThreadWndClass", L"OLE2UIresimage", L"OLE2UIiconbox", NULL};

	static const WCHAR* NamesR[] = {L"ReBarWindow32",
	    L"REComboBox20W",
	    L"REListBox20W",
	    L"RICHEDIT"
	    L"RichEdit20A",
	...
	static const WCHAR* NamesS[] = {L"Static", L"ScrollBar", L"SysAnimate32", L"SysCredential", L"SysDateTimePick32", L"SysHeader32", L"SysIPAddress32", L"SysLink", L"SysListView32", L"SysMonthCal32", L"SysPager", L"SysTabControl32", L"SysTreeView32", NULL};

	static const WCHAR* NamesT[] = {L"ToolbarWindow32", L"tooltips_class", L"tooltips_class32", NULL};

	static const WCHAR* NamesW[] = {L"WorkerA", L"WorkerW", NULL};

	static const WCHAR* Names_[] = {L"#32768", L"#32770", L"AtlAxWin", L"Button", L"Edit", L"ListBox", L"IME", L"NativeFontCtl", L"PrintTray_Notify_WndClass", NULL};
...
_FX BOOLEAN Gui_IsWindowAccessible(HWND hWnd)
{
	...
	else if (hWnd)
	{
		...
		{
			NTSTATUS status;
			HANDLE hProcess = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, (ULONG)idProcess);
			if (!hProcess)
			{
			...
			{
				ULONG len;
				status = NtQueryInformationProcess(hProcess, ProcessImageFileName, clsnm, sizeof(clsnm) - 8, &len);
...
_FX ULONG_PTR Gui_CREATESTRUCT_Handler(ULONG_PTR* args)
{
	...
	if (clsnm)
	{
		if (wmemcmp((WCHAR*)clsnm, Gui_BoxPrefixW, Gui_BoxPrefix_Len) == 0)
		{
			clsnm += Gui_BoxPrefix_Len * sizeof(WCHAR);
			*pclsnm = clsnm;
		}
		else if (memcmp((UCHAR*)clsnm, Gui_BoxPrefixA, Gui_BoxPrefix_Len) == 0)
		{
...
_FX void Gui_CREATESTRUCT_Restore(LPARAM lParam)
{
	...
	__try
	{
		...
		if (clsnm)
		{
			...
			if (wmemcmp((WCHAR*)clsnmW, Gui_BoxPrefixW, Gui_BoxPrefix_Len) == 0)
			{
				*pclsnm = clsnmW;
			}
			else if (memcmp((UCHAR*)clsnmA, Gui_BoxPrefixA, Gui_BoxPrefix_Len) == 0)
			{
				*pclsnm = clsnmA;
			}
		}
