...
_FX BOOLEAN SbieDll_IsWow64(void)
{
	...
	if (!init)
	{
		P_IsWow64Process __sys_IsWow64Process = (P_IsWow64Process)GetProcAddress(Dll_Kernel32, "IsWow64Process");
...
_FX BOOLEAN SbieDll_ConnectPort(BOOLEAN Silent)
{
	...
	if (!data->PortHandle)
	{
		...
		status = NtConnectPort(&data->PortHandle, &PortName, &QoS, NULL, NULL, &data->MaxDataLen, NULL, NULL);
...
_FX MSG_HEADER* SbieDll_CallServer(MSG_HEADER* req)
{
	...
	if (!data->PortHandle)
	{
		BOOLEAN Silent = (req->msgid == MSGID_SBIE_INI_GET_VERSION || req->msgid == MSGID_SBIE_INI_GET_USER || req->msgid == MSGID_PROCESS_CHECK_INIT_COMPLETE);
	...
	while (1)
	{
		ULONG buf_len_plus_msg = (ULONG)(ULONG_PTR)(buf - (UCHAR*)rpl) + msg->u1.s1.DataLength;
...
_FX ULONG SbieDll_QueueCreate(const WCHAR* QueueName, HANDLE* out_EventHandle)
{
	NTSTATUS status;
...
_FX ULONG SbieDll_QueueGetReq(const WCHAR* QueueName, ULONG* out_ClientPid, ULONG* out_ClientTid, ULONG* out_RequestId, void** out_DataPtr, ULONG* out_DataLen)
{
	NTSTATUS status;
...
_FX ULONG SbieDll_QueuePutRpl(const WCHAR* QueueName, ULONG RequestId, void* DataPtr, ULONG DataLen)
{
	NTSTATUS status;
	ULONG req_len;
...
_FX ULONG SbieDll_QueuePutReq(const WCHAR* QueueName, void* DataPtr, ULONG DataLen, ULONG* out_RequestId, HANDLE* out_EventHandle)
{
	NTSTATUS status;
	ULONG req_len;
...
_FX ULONG SbieDll_QueueGetRpl(const WCHAR* QueueName, ULONG RequestId, void** out_DataPtr, ULONG* out_DataLen)
{
	NTSTATUS status;
...
_FX ULONG SbieDll_UpdateConf(WCHAR OpCode, const WCHAR* Password, const WCHAR* Section, const WCHAR* Setting, const WCHAR* Value)
{
...
_FX BOOL SbieDll_RunSandboxed(const WCHAR* box_name, const WCHAR* cmd, const WCHAR* dir, ULONG creation_flags, STARTUPINFO* si, PROCESS_INFORMATION* pi)
{
	...
	req_len = sizeof(PROCESS_RUN_SANDBOXED_REQ) + (cmd_len + dir_len + env_len + 8) * sizeof(WCHAR);
