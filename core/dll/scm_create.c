...
static SC_HANDLE Scm_CreateServiceW(SC_HANDLE hSCManager, WCHAR* lpServiceName, WCHAR* lpDisplayName, ULONG dwDesiredAccess, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, WCHAR* lpBinaryPathName, WCHAR* lpLoadOrderGroup, WCHAR* lpdwTagId, WCHAR* lpDependencies, WCHAR* lpServiceStartName, WCHAR* lpPassword);

static SC_HANDLE Scm_CreateServiceA(SC_HANDLE hSCManager, UCHAR* lpServiceName, UCHAR* lpDisplayName, ULONG dwDesiredAccess, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, UCHAR* lpBinaryPathName, UCHAR* lpLoadOrderGroup, UCHAR* lpdwTagId, UCHAR* lpDependencies, UCHAR* lpServiceStartName, UCHAR* lpPassword);

static BOOL Scm_ChangeServiceConfigW(SC_HANDLE hService, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, WCHAR* lpBinaryPathName, WCHAR* lpLoadOrderGroup, WCHAR* lpdwTagId, WCHAR* lpDependencies, WCHAR* lpServiceStartName, WCHAR* lpPassword, WCHAR* lpDisplayName);

static BOOL Scm_ChangeServiceConfigA(SC_HANDLE hService, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, UCHAR* lpBinaryPathName, UCHAR* lpLoadOrderGroup, UCHAR* lpdwTagId, UCHAR* lpDependencies, UCHAR* lpServiceStartName, UCHAR* lpPassword, UCHAR* lpDisplayName);

static BOOL Scm_ChangeServiceConfig2W(SC_HANDLE hService, ULONG dwInfoLevel, void* lpInfo);

static BOOL Scm_ChangeServiceConfig2A(SC_HANDLE hService, ULONG dwInfoLevel, void* lpInfo);

static BOOL Scm_DeleteService(SC_HANDLE hService);

static ULONG Scm_StartBoxedService2(const WCHAR* name, SERVICE_QUERY_RPL* qrpl);

static BOOL Scm_StartServiceW(SC_HANDLE hService, DWORD dwNumServiceArgs, void* lpServiceArgVector);

static BOOL Scm_StartServiceA(SC_HANDLE hService, DWORD dwNumServiceArgs, void* lpServiceArgVector);

static ULONG Scm_ServiceMainThread(ULONG_PTR* args);

static BOOL Scm_StartServiceCtrlDispatcherX(const void* ServiceTable, BOOLEAN IsUnicode);

static BOOL Scm_StartServiceCtrlDispatcherW(const SERVICE_TABLE_ENTRYW* ServiceTable);

static BOOL Scm_StartServiceCtrlDispatcherA(const SERVICE_TABLE_ENTRYA* ServiceTable);

static SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerW(WCHAR* ServiceName, LPHANDLER_FUNCTION HandlerProc);

static SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerA(UCHAR* ServiceName, LPHANDLER_FUNCTION HandlerProc);

static SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerExW(WCHAR* ServiceName, LPHANDLER_FUNCTION_EX HandlerProc, void* Context);

static SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerExA(UCHAR* ServiceName, LPHANDLER_FUNCTION_EX HandlerProc, void* Context);

#ifdef _WIN64

static SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerHelp(UCHAR* ServiceName, ULONG_PTR HandlerProc, void* Context, ULONG Flags);

#endif _WIN64

static BOOL Scm_SetServiceStatus_Internal(HKEY ServiceKeyHandle, SERVICE_STATUS_HANDLE hServiceStatus, LPSERVICE_STATUS lpServiceStatus, BOOLEAN SetProcessId);

static BOOL Scm_SetServiceStatus(SERVICE_STATUS_HANDLE hServiceStatus, LPSERVICE_STATUS lpServiceStatus);

static BOOL Scm_ControlService(SC_HANDLE hService, DWORD dwControl, SERVICE_STATUS* lpServiceStatus);
...
_FX NTSTATUS Scm_AddBoxedService(const WCHAR* ServiceName)
{
	...
	else
	{
		...
		status = NtSetValueKey(hkey, &uni, 0, REG_MULTI_SZ, names2, len * sizeof(WCHAR));
...
_FX SC_HANDLE Scm_CreateServiceW(SC_HANDLE hSCManager, WCHAR* lpServiceName, WCHAR* lpDisplayName, ULONG dwDesiredAccess, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, WCHAR* lpBinaryPathName, WCHAR* lpLoadOrderGroup, WCHAR* lpdwTagId, WCHAR* lpDependencies, WCHAR* lpServiceStartName, WCHAR* lpPassword)
{
	NTSTATUS status;
	SC_HANDLE hService;
	HANDLE hkey;
	...
	hService = Scm_OpenServiceW(hSCManager, lpServiceName, SERVICE_QUERY_STATUS);
	...
	status = NtSetValueKey(hkey, &uni, 0, REG_DWORD, &dwServiceType, sizeof(ULONG));
	...
	status = NtSetValueKey(hkey, &uni, 0, REG_DWORD, &dwStartType, sizeof(ULONG));
	if (!NT_SUCCESS(status))
	{
		goto abort;
	}
	...
	status = NtSetValueKey(hkey, &uni, 0, REG_DWORD, &dwErrorControl, sizeof(ULONG));
	if (!NT_SUCCESS(status))
	{
		goto abort;
	}
	...
	if (lpDisplayName)
	{
		...
		status = NtSetValueKey(hkey, &uni, 0, REG_SZ, lpDisplayName, (wcslen(lpDisplayName) + 1) * sizeof(WCHAR));
		if (!NT_SUCCESS(status))
		{
			goto abort;
		}
	...
	if (lpBinaryPathName)
	{
		...
		status = NtSetValueKey(hkey, &uni, 0, REG_SZ, lpBinaryPathName, (wcslen(lpBinaryPathName) + 1) * sizeof(WCHAR));
		if (!NT_SUCCESS(status))
		{
			goto abort;
		}
	...
	if (lpServiceStartName)
	{
		...
		status = NtSetValueKey(hkey, &uni, 0, REG_SZ, lpServiceStartName, (wcslen(lpServiceStartName) + 1) * sizeof(WCHAR));
		if (!NT_SUCCESS(status))
		{
			goto abort;
		}
	...
	name          = Dll_Alloc(sizeof(ULONG) + (wcslen(lpServiceName) + 1) * sizeof(WCHAR));
	*(ULONG*)name = tzuk;
...
_FX SC_HANDLE Scm_CreateServiceA(SC_HANDLE hSCManager, UCHAR* lpServiceName, UCHAR* lpDisplayName, ULONG dwDesiredAccess, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, UCHAR* lpBinaryPathName, UCHAR* lpLoadOrderGroup, UCHAR* lpdwTagId, UCHAR* lpDependencies, UCHAR* lpServiceStartName, UCHAR* lpPassword)
{
	NTSTATUS status;
...
#define FREE_UNI(x)  \
	...
	{
		handle = Scm_CreateServiceW(hSCManager,
...
_FX BOOL Scm_ChangeServiceConfigW(SC_HANDLE hService, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, WCHAR* lpBinaryPathName, WCHAR* lpLoadOrderGroup, WCHAR* lpdwTagId, WCHAR* lpDependencies, WCHAR* lpServiceStartName, WCHAR* lpPassword, WCHAR* lpDisplayName)
{
	NTSTATUS status;
	WCHAR* ServiceName;
	HANDLE hkey;
...
_FX BOOL Scm_ChangeServiceConfigA(SC_HANDLE hService, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, UCHAR* lpBinaryPathName, UCHAR* lpLoadOrderGroup, UCHAR* lpdwTagId, UCHAR* lpDependencies, UCHAR* lpServiceStartName, UCHAR* lpPassword, UCHAR* lpDisplayName)
{
	NTSTATUS status;
	UNICODE_STRING lpBinaryPathNameW, lpLoadOrderGroupW;
	UNICODE_STRING lpdwTagIdW, lpDependenciesW;
...
#define FREE_UNI(x)  \
	...
	{
		ok = Scm_ChangeServiceConfigW(hService,
		    dwServiceType,
		    dwStartType,
		    dwErrorControl,
		    lpBinaryPathNameW.Buffer,
...
_FX BOOL Scm_ChangeServiceConfig2W(SC_HANDLE hService, ULONG dwInfoLevel, void* lpInfo)
{
	WCHAR* ServiceName;
...
_FX BOOL Scm_ChangeServiceConfig2A(SC_HANDLE hService, ULONG dwInfoLevel, void* lpInfo)
{
...
_FX BOOL Scm_DeleteService(SC_HANDLE hService)
{
	...
	++len;
	RtlInitUnicodeString(&uni, L"SandboxedServices");
	status = NtSetValueKey(hkey2, &uni, 0, REG_MULTI_SZ, names, len * sizeof(WCHAR));
...
_FX BOOL SbieDll_StartBoxedService(const WCHAR* ServiceName, BOOLEAN WithAdd)
{
	...
	if (rpl->service_status.dwCurrentState != SERVICE_STOPPED && rpl->service_status.dwCurrentState != SERVICE_START_PENDING)
	{
		Dll_Free(rpl);
	...
	Scm_SetServiceStatus_Internal(hkey, HANDLE_SERVICE_STATUS, &ss, FALSE);
...
_FX ULONG Scm_StartBoxedService2(const WCHAR* name, SERVICE_QUERY_RPL* qrpl)
{
	...
	if (_wcsicmp(name, _bits) == 0 || _wcsicmp(name, _wuauserv) == 0 || _wcsicmp(name, Scm_CryptSvc) == 0)
	{
...
_FX BOOL Scm_StartServiceW(SC_HANDLE hService, DWORD dwNumServiceArgs, void* lpServiceArgVector)
{
	union {
	...
	u.req.h.length = sizeof(SERVICE_START_REQ) + (u.req.name_len + 1) * sizeof(WCHAR);
...
_FX BOOL Scm_StartServiceA(SC_HANDLE hService, DWORD dwNumServiceArgs, void* lpServiceArgVector)
{
...
_FX ULONG Scm_ServiceMainThread(ULONG_PTR* args)
{
	...
	while (1)
	{
		HANDLE hEvent = OpenEvent(EVENT_MODIFY_STATE, FALSE, _MsiServerInUseEventName);
...
_FX BOOL Scm_StartServiceCtrlDispatcherX(const void* ServiceTable, BOOLEAN IsUnicode)
{
	static const WCHAR* ServiceName_EnvVar = L"00000000_" SBIE L"_SERVICE_NAME";
	WCHAR* ServiceName;
	WCHAR* Buffer;
	UNICODE_STRING uni;
	void* args[3];
	...
	status = NtNotifyChangeKey(Scm_ServiceKey, hEvent, NULL, NULL, &iosb, REG_NOTIFY_CHANGE_LAST_SET, FALSE, Buffer, 1024, TRUE);
	...
	while (!Scm_Stopped)
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			status = NtQueryValueKey(Scm_ServiceKey, &uni, KeyValuePartialInformation, &u.info, sizeof(u), &len);
			...
			if (NT_SUCCESS(status) && u.info.Type == REG_DWORD && u.info.DataLength == 4)
			{
				...
				{
					Scm_HandlerEx(*(ULONG*)u.info.Data, 0, NULL, Scm_HandlerContext);
				}
			}

			status = NtDeleteValueKey(Scm_ServiceKey, &uni);
			if (NT_SUCCESS(status))
			{ // if key was actually deleted, we need to reissue the reg notification
				status = NtNotifyChangeKey(Scm_ServiceKey, hEvent, NULL, NULL, &iosb, REG_NOTIFY_CHANGE_LAST_SET, FALSE, Buffer, 1024, TRUE);
			}
		}
	}
...
_FX BOOL Scm_StartServiceCtrlDispatcherW(const SERVICE_TABLE_ENTRYW* ServiceTable)
{
...
_FX BOOL Scm_StartServiceCtrlDispatcherA(const SERVICE_TABLE_ENTRYA* ServiceTable)
{
...
_FX SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerW(WCHAR* ServiceName, LPHANDLER_FUNCTION HandlerProc)
{
...
_FX SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerA(UCHAR* ServiceName, LPHANDLER_FUNCTION HandlerProc)
{
	Scm_Handler = HandlerProc;
	return (SERVICE_STATUS_HANDLE)HANDLE_SERVICE_STATUS;
}
...
_FX SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerExW(WCHAR* ServiceName, LPHANDLER_FUNCTION_EX HandlerProc, void* Context)
{
...
_FX SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerExA(UCHAR* ServiceName, LPHANDLER_FUNCTION_EX HandlerProc, void* Context)
{
	Scm_HandlerEx      = HandlerProc;
	Scm_HandlerContext = Context;
	return (SERVICE_STATUS_HANDLE)HANDLE_SERVICE_STATUS;
...
_FX SERVICE_STATUS_HANDLE Scm_RegisterServiceCtrlHandlerHelp(UCHAR* ServiceName, ULONG_PTR HandlerProc, void* Context, ULONG Flags)
{
	if (Flags == 0)
	{
...
_FX BOOL Scm_SetServiceStatus_Internal(HKEY ServiceKeyHandle, SERVICE_STATUS_HANDLE hServiceStatus, LPSERVICE_STATUS lpServiceStatus, BOOLEAN SetProcessId)
{
	UNICODE_STRING uni;
	ULONG val;
...
#define MySetValueKey() NtSetValueKey(ServiceKeyHandle, &uni, 0, REG_DWORD, (BYTE*)&val, sizeof(ULONG))
...
_FX BOOL Scm_SetServiceStatus(SERVICE_STATUS_HANDLE hServiceStatus, LPSERVICE_STATUS lpServiceStatus)
{
	return Scm_SetServiceStatus_Internal(Scm_ServiceKey, hServiceStatus, lpServiceStatus, TRUE);
}
...
_FX BOOL Scm_ControlService(SC_HANDLE hService, DWORD dwControl, SERVICE_STATUS* lpServiceStatus)
{
	NTSTATUS status;
	WCHAR* ServiceName;
	HANDLE hkey;
	...
	RtlInitUnicodeString(&uni, SBIE L"_ControlCode");
	val    = dwControl;
	status = NtSetValueKey(hkey, &uni, 0, REG_DWORD, (BYTE*)&val, sizeof(ULONG));
	if (!NT_SUCCESS(status))
	{
		NtClose(hkey);
		SetLastError(ERROR_SERVICE_REQUEST_TIMEOUT);
	...
	for (retries = 0; retries < 40; ++retries)
	{
		...
		status = NtQueryValueKey(hkey, &uni, KeyValuePartialInformation, &u.info, sizeof(u), &val);
