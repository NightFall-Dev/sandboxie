...
static UINT Gui_DdeInitializeW(ULONG_PTR pidInst, ULONG_PTR pfnCallback, ULONG afCmd, ULONG ulRes);

static UINT Gui_DdeInitializeA(ULONG_PTR pidInst, ULONG_PTR pfnCallback, ULONG afCmd, ULONG ulRes);
...
typedef BOOL (*P_UnpackDDElParam)(UINT msg, LPARAM lParam, PUINT_PTR uiLo, PUINT_PTR uiHi);
...
{
	__sys_PackDDElParam   = Ldr_GetProcAddrNew(DllName_user32, L"PackDDElParam", "PackDDElParam");
	__sys_UnpackDDElParam = Ldr_GetProcAddrNew(DllName_user32, L"UnpackDDElParam", "UnpackDDElParam");
...
_FX UINT Gui_DdeInitializeW(ULONG_PTR pidInst, ULONG_PTR pfnCallback, ULONG afCmd, ULONG ulRes)
{
...
_FX UINT Gui_DdeInitializeA(ULONG_PTR pidInst, ULONG_PTR pfnCallback, ULONG afCmd, ULONG ulRes)
{
...
_FX BOOLEAN Gui_DDE_COPYDATA_Received(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	WCHAR prop_name[64];
	HWND hClientWnd;
	HGLOBAL hGlobal;
	...
	if (cds->cbData == (Gui_DDE_ACK_Len + 1) * sizeof(WCHAR) && wcscmp(cds->lpData, Gui_DDE_ACK) == 0)
	{
		const ULONG_PTR _DDEACK_fAck = 0x8000;

		if (TlsData->gui_dde_client_hwnd == (HWND)-1 && (!TlsData->gui_dde_proxy_hwnd) && (WPARAM)hWnd == TlsData->gui_dde_post_wparam)
		{
		...
		lParam = __sys_PackDDElParam(WM_DDE_ACK, _DDEACK_fAck, (UINT_PTR)hGlobal);
	...
	if (cds->cbData == (Gui_DDE_TERM_Len + 1) * sizeof(WCHAR) && wcscmp(cds->lpData, Gui_DDE_TERM) == 0)
	{
	...
	if (cds->cbData > (Gui_DDE_REQ_Len + 1) * sizeof(WCHAR) && wcsncmp(cds->lpData, Gui_DDE_REQ, Gui_DDE_REQ_Len) == 0)
	{
	...
	if (cds->cbData >= sizeof(ULONG) && _wcsicmp(Dll_ImageName, L"opera.exe") == 0)
	{
		...
		{
			hGlobal = GlobalAlloc(GMEM_DDESHARE | GMEM_FIXED, cds->cbData * sizeof(WCHAR));
			if (hGlobal)
			{
				...
				if (pGlobal)
				{
					...
					__sys_PostMessageW(hWnd, WM_DDE_EXECUTE, (WPARAM)hClientWnd, (LPARAM)hGlobal);
					return TRUE;
				}
			}
...
_FX BOOLEAN Gui_DDE_Post_In_Box(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	//
	...
	else if (uMsg == WM_DDE_ACK)
	{
		if (hWnd == TlsData->gui_dde_proxy_hwnd)
		{
			...
			//
			__sys_PostMessageW((HWND)wParam, WM_DDE_TERMINATE, (WPARAM)hWnd, 0);
			return TRUE;
		}
...
_FX LRESULT Gui_DDE_DATA_Posting(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	if (TlsData->gui_dde_proxy_hwnd && TlsData->gui_dde_client_hwnd && TlsData->gui_dde_proxy_hwnd == (HWND)hWnd)
	{
		ULONG_PTR lo, hi;
