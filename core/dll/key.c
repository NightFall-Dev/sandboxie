...
#define KEY_DENIED_ACCESS \
	~(STANDARD_RIGHTS_READ | GENERIC_READ | SYNCHRONIZE | KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS | KEY_NOTIFY | KEY_WOW64_32KEY | KEY_WOW64_64KEY)
...
static NTSTATUS Key_GetName(HANDLE RootDirectory, UNICODE_STRING* ObjectName, WCHAR** OutTruePath, WCHAR** OutCopyPath, BOOLEAN* OutIsBoxedPath);

static NTSTATUS Key_FixNameWow64(ACCESS_MASK DesiredAccess, WCHAR** OutTruePath, WCHAR** OutCopyPath);

static NTSTATUS Key_FixNameWow64_2(WCHAR** OutTruePath, WCHAR** OutCopyPath);

static BOOLEAN Key_FixNameWow64_3(WCHAR** OutPath);

static ACCESS_MASK Key_GetWow64Flag(const WCHAR* TruePath, ACCESS_MASK DesiredAccess);

static NTSTATUS Key_NtOpenKey(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes);

static NTSTATUS Key_NtOpenKeyImpl(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes);

static NTSTATUS Key_NtOpenKeyEx(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG OpenOptions);

static NTSTATUS Key_NtCreateKey(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG TitleIndex, UNICODE_STRING* Class, ULONG CreateOptions, ULONG* Disposition);

static NTSTATUS Key_NtCreateKeyImpl(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG TitleIndex, UNICODE_STRING* Class, ULONG CreateOptions, ULONG* Disposition);

static NTSTATUS Key_CreatePath(OBJECT_ATTRIBUTES* objattrs, PSECURITY_DESCRIPTOR* sd);

static NTSTATUS Key_CreatePath_Key(HANDLE* out_handle, OBJECT_ATTRIBUTES* objattrs, ULONG* out_disp);
...
static NTSTATUS Key_NtDeleteValueKey(HANDLE KeyHandle, UNICODE_STRING* ValueName);

static NTSTATUS Key_NtSetValueKey(HANDLE KeyHandle, UNICODE_STRING* ValueName, ULONG TitleIndex, ULONG Type, void* Data, ULONG DataSize);

static NTSTATUS Key_NtQueryKey(HANDLE KeyHandle, KEY_INFORMATION_CLASS KeyInformationClass, void* KeyInformation, ULONG Length, ULONG* ResultLength);

static NTSTATUS Key_NtQueryKeyImpl(HANDLE KeyHandle, KEY_INFORMATION_CLASS KeyInformationClass, void* KeyInformation, ULONG Length, ULONG* ResultLength);

static NTSTATUS Key_NtEnumerateKey(HANDLE KeyHandle, ULONG Index, KEY_INFORMATION_CLASS KeyInformationClass, void* KeyInformation, ULONG Length, ULONG* ResultLength);

static NTSTATUS Key_NtEnumerateKeyFake(const WCHAR* KeyPath, ULONG KeyPathLen, KEY_INFORMATION_CLASS KeyInformationClass, void* KeyInformation, ULONG Length, ULONG* ResultLength, LARGE_INTEGER* LastWriteTime);

static NTSTATUS Key_NtQueryValueKey(HANDLE KeyHandle, UNICODE_STRING* ValueName, KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass, void* KeyValueInformation, ULONG Length, ULONG* ResultLength);

static NTSTATUS Key_NtQueryValueKeyFakeForInternetExplorer(const WCHAR* TruePath, const WCHAR* ValueNameBuf, ULONG ValueNameLen, void* OutputBuf, ULONG OutputLen, ULONG* ResultLength);

static NTSTATUS Key_NtQueryValueKeyFakeForAcrobatReader(const WCHAR* TruePath, const WCHAR* ValueNameBuf, ULONG ValueNameLen, void* OutputBuf, ULONG OutputLen, ULONG* ResultLength);

static NTSTATUS Key_NtQueryValueKeyFakeForCreateProcess(const WCHAR* TruePath, const WCHAR* ValueNameBuf, ULONG ValueNameLen, void* OutputBuf, ULONG OutputLen, ULONG* ResultLength);

static NTSTATUS Key_NtEnumerateValueKey(HANDLE KeyHandle, ULONG Index, KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass, void* KeyValueInformation, ULONG Length, ULONG* ResultLength);

static BOOLEAN Key_CheckDeletedValue(KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass, void* info);

static NTSTATUS Key_NtQueryMultipleValueKey(HANDLE KeyHandle, PKEY_VALUE_ENTRY ValueEntries, ULONG EntryCount, void* ValueBuffer, ULONG* Length, ULONG* ResultLength);

static NTSTATUS Key_MarkUnDeleted(HANDLE KeyHandle);

static NTSTATUS Key_NtNotifyChangeKey(HANDLE KeyHandle, HANDLE Event OPTIONAL, PIO_APC_ROUTINE ApcRoutine OPTIONAL, PVOID ApcContext OPTIONAL, PIO_STATUS_BLOCK IoStatusBlock, ULONG CompletionFilter, BOOLEAN WatchTree, PVOID Buffer, ULONG BufferSize, BOOLEAN Asynchronous);

static NTSTATUS Key_NtNotifyChangeMultipleKeys(HANDLE MasterKeyHandle, ULONG Count, OBJECT_ATTRIBUTES* SlaveObjects, HANDLE Event OPTIONAL, PIO_APC_ROUTINE ApcRoutine OPTIONAL, PVOID ApcContext OPTIONAL, PIO_STATUS_BLOCK IoStatusBlock, ULONG CompletionFilter, BOOLEAN WatchTree, PVOID Buffer, ULONG BufferSize, BOOLEAN Asynchronous);

static NTSTATUS Key_NtRenameKey(HANDLE KeyHandle, UNICODE_STRING* ReplacementName);

static NTSTATUS Key_NtSaveKey(HANDLE KeyHandle, HANDLE FileHandle);

static NTSTATUS Key_NtLoadKey(OBJECT_ATTRIBUTES* TargetObjectAttributes, OBJECT_ATTRIBUTES* SourceObjectAttributes);
...
_FX BOOLEAN Key_Init(void)
{
	...
	Key_CurrentUserLen = Key_RegistryLen + 6 /* \user\ */
...
_FX NTSTATUS Key_GetName(HANDLE RootDirectory, UNICODE_STRING* ObjectName, WCHAR** OutTruePath, WCHAR** OutCopyPath, BOOLEAN* OutIsBoxedPath)
{
	static const WCHAR* _bfe        = L"\\registry\\machine\\system\\CurrentControlSet\\services\\bfe";
	static const WCHAR* _ControlSet = L"\\registry\\machine\\system\\ControlSet"; // 35
	static const WCHAR* _SideBySide = L"\\registry\\machine\\software"            // 70
	...
	*OutCopyPath = NULL;
	...
	if (RootDirectory)
	{
		...
		status = __sys_NtQueryKey(RootDirectory, KeyNameInformation, name, length, &length);
		...
		*OutTruePath = ((KEY_NAME_INFORMATION*)name)->Name;
		...
		name = (*OutTruePath) + ((KEY_NAME_INFORMATION*)name)->NameLength / sizeof(WCHAR);
	...
	if (length < Key_RegistryLen || _wcsnicmp(*OutTruePath, Key_Registry, Key_RegistryLen) != 0)
	{
		return STATUS_OBJECT_PATH_SYNTAX_BAD;
	}

	if ((*OutTruePath)[Key_RegistryLen] != L'\\' && (*OutTruePath)[Key_RegistryLen] != L'\0')
	{
		return STATUS_OBJECT_PATH_SYNTAX_BAD;
	}
...
check_sandbox_prefix:
	...
	if (length >= Dll_BoxKeyPathLen && 0 == Dll_NlsStrCmp(*OutTruePath, Dll_BoxKeyPath, Dll_BoxKeyPathLen))
	{
	...
	if (length >= Key_RegistryLen + Key_UserCurrentLen && _wcsnicmp(*OutTruePath + Key_RegistryLen, Key_UserCurrent, Key_UserCurrentLen) == 0)
	{
		...
		name2  = Dll_GetTlsNameBuffer(TlsData, COPY_NAME_BUFFER, length * sizeof(WCHAR));
		wmemcpy(name2, Key_CurrentUser, Key_CurrentUserLen);
		wmemcpy(name2 + Key_CurrentUserLen, *OutTruePath + prefix_len, suffix_len);

		*OutTruePath = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, length * sizeof(WCHAR));
	...
	name = Dll_GetTlsNameBuffer(TlsData, COPY_NAME_BUFFER, Dll_BoxKeyPathLen + length);
	...
	*OutCopyPath = name;
	...
	if (length >= Key_CurrentUserLen && _wcsnicmp(*OutTruePath, Key_CurrentUser, Key_CurrentUserLen) == 0)
	{
	...
	else if (length >= 35 && _wcsnicmp(*OutTruePath, _ControlSet, 35) == 0)
	{
		//
	...
	else if (Dll_OsBuild >= 6000 && Dll_OsBuild < 7600 && length >= 70 && _wcsnicmp(*OutTruePath, _SideBySide, 70) == 0)
	{
		//
...
_FX NTSTATUS Key_FixNameWow64(ACCESS_MASK DesiredAccess, WCHAR** OutTruePath, WCHAR** OutCopyPath)
{
	NTSTATUS status;
...
_FX NTSTATUS Key_FixNameWow64_2(WCHAR** OutTruePath, WCHAR** OutCopyPath)
{
	...
	else
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			status = Key_GetName(NULL, &objname, OutTruePath, OutCopyPath, NULL);
...
_FX ACCESS_MASK Key_GetWow64Flag(const WCHAR* TruePath, ACCESS_MASK DesiredAccess)
{
	if (Dll_IsWin64)
	{
		//
...
_FX NTSTATUS Key_NtOpenKey(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes)
{
...
_FX NTSTATUS Key_NtOpenKeyImpl(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes)
{
	//
	...
	NTSTATUS status = Key_NtCreateKeyImpl(KeyHandle, DesiredAccess, ObjectAttributes, 0, NULL, tzuk, NULL);
	return status;
}
...
_FX NTSTATUS Key_NtOpenKeyEx(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG OpenOptions)
{
	NTSTATUS status;
	...
	status = Key_NtCreateKeyImpl(KeyHandle, DesiredAccess, ObjectAttributes, 0, NULL, tzuk, NULL);
...
_FX NTSTATUS Key_NtCreateKey(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG TitleIndex, UNICODE_STRING* Class, ULONG CreateOptions, ULONG* Disposition)
{
	NTSTATUS status = Key_NtCreateKeyImpl(KeyHandle, DesiredAccess, ObjectAttributes, TitleIndex, Class, CreateOptions, Disposition);
...
_FX NTSTATUS Key_NtCreateKeyImpl(HANDLE* KeyHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG TitleIndex, UNICODE_STRING* Class, ULONG CreateOptions, ULONG* Disposition)
{
	ULONG LastError;
	...
	if (TlsData->key_NtCreateKey_lock)
	{
		...
		{
			return __sys_NtCreateKey(KeyHandle, DesiredAccess, ObjectAttributes, TitleIndex, Class, CreateOptions, Disposition);
		}
	}
	...
	__try
	{
		...
		status = Key_GetName(ObjectAttributes->RootDirectory, ObjectAttributes->ObjectName, &TruePath, &CopyPath, NULL);
		if (!NT_SUCCESS(status))
		{
			__leave;
		}
		...
		if (TlsData->proc_create_process && Sxs_KeyCallback(TruePath, KeyHandle))
		{
			__leave;
		}
		...
		{
#define KEY_READ_WOW64 (KEY_READ | (DesiredAccess & (KEY_WOW64_32KEY | KEY_WOW64_64KEY)))
			...
			if (CreateOptions == tzuk)
			{
				...
				{
					status = __sys_NtOpenKey(KeyHandle, KEY_READ_WOW64, &objattrs);
				}
			}
			else
			{
				status = __sys_NtCreateKey(KeyHandle, DesiredAccess, &objattrs, TitleIndex, Class, CreateOptions, Disposition);

				if (status == STATUS_ACCESS_DENIED && DesiredAccess == MAXIMUM_ALLOWED)
				{
					status = __sys_NtCreateKey(KeyHandle, KEY_READ_WOW64, &objattrs, TitleIndex, Class, CreateOptions, Disposition);
				}
			}
...
#undef KEY_READ_WOW64
		...
		if (Dll_IsWin64)
		{
			if (_wcsnicmp(TruePath, Key_ClickToRun, Key_ClickToRunLen) != 0 && _wcsnicmp(TruePath, Key_ClickToRun_v16, Key_ClickToRunLen_v16) != 0)
			{
		...
		if (CreateOptions == tzuk)
		{
			status = __sys_NtOpenKey(KeyHandle, DesiredAccess | KEY_READ, &objattrs);
		}
		else
		{
			CreateOptions &= ~REG_OPTION_BACKUP_RESTORE;

			status = __sys_NtCreateKey(KeyHandle, DesiredAccess | KEY_READ, &objattrs, TitleIndex, Class, CreateOptions, Disposition);
		...
		if (status == STATUS_ACCESS_DENIED && Secure_IsRestrictedToken(TRUE))
		{
			...
			if (NT_SUCCESS(status2))
			{
				if (CreateOptions == tzuk)
				{
					status2 = __sys_NtOpenKey(KeyHandle, DesiredAccess | KEY_READ, &objattrs);
				}
				else
				{
					status2 = __sys_NtCreateKey(KeyHandle, DesiredAccess | KEY_READ, &objattrs, TitleIndex, Class, CreateOptions, Disposition);
		...
		if (NT_SUCCESS(status))
		{
			...
			if (Disposition)
			{
				...
				else if ((*Disposition == REG_CREATED_NEW_KEY) && PATH_NOT_WRITE(mp_flags))
				{
					...
					RtlInitUnicodeString(&objname, TruePath);
					status = __sys_NtOpenKey(&handle, Wow64KeyReadAccess, &objattrs);
	...
	if (Dll_RestrictedToken && status == STATUS_ACCESS_DENIED)
	{
		...
		DesiredAccess |= OriginalDesiredAccess & (KEY_WOW64_32KEY | KEY_WOW64_64KEY);

		status = Key_NtCreateKeyImpl(KeyHandle, DesiredAccess, ObjectAttributes, TitleIndex, Class, CreateOptions, Disposition);
...
_FX NTSTATUS Key_CreatePath(OBJECT_ATTRIBUTES* objattrs, PSECURITY_DESCRIPTOR* sd)
{
	NTSTATUS status;
	HANDLE handle;
	WCHAR* path;
...
_FX NTSTATUS Key_CreatePath_Key(HANDLE* out_handle, OBJECT_ATTRIBUTES* objattrs, ULONG* out_disp)
{
	NTSTATUS status = __sys_NtCreateKey(out_handle, GENERIC_READ, objattrs, 0, NULL, 0, out_disp);

	if (status == STATUS_CHILD_MUST_BE_VOLATILE)
	{
		status = __sys_NtCreateKey(out_handle, GENERIC_READ, objattrs, 0, NULL, REG_OPTION_VOLATILE, out_disp);
	}
...
_FX BOOLEAN Key_CheckDeletedKey(HANDLE KeyHandle)
{
	...
	status = __sys_NtQueryKey(KeyHandle, KeyBasicInformation, &info, sizeof(info), &len);
...
#define COND(ll, ss) (len2 == ll && _wcsicmp(ptr2, ss) == 0)
		...
		len2 = wcslen(ptr2);
		if (COND(8, L"\\machine") || COND(17, L"\\machine\\software") || COND(25, L"\\machine\\software\\classes") || COND(31, L"\\machine\\software\\classes\\clsid") || COND(33, L"\\machine\\software\\classes\\typelib") || COND(35, L"\\machine\\software\\classes\\interface") || COND(5, L"\\user") || COND(13, L"\\user\\current") || COND(22, L"\\user\\current\\software") || COND(30, L"\\user\\current\\software\\classes") || COND(36, L"\\user\\current\\software\\classes\\clsid") || COND(38, L"\\user\\current\\software\\classes\\typelib") || COND(40, L"\\user\\current\\software\\classes\\interface") || 0)
		{
			*ptr = L'\\';
			return FALSE;
		}

		if (Dll_IsWin64 && COND(29, L"\\machine\\software\\wow6432node") || COND(36, L"\\machine\\software\\classes\\wow6432node") || 0)
		{
			*ptr = L'\\';
			return FALSE;
		}
...
_FX NTSTATUS Key_NtDeleteKeyTreeImpl(HANDLE KeyHandle, BOOLEAN DeleteTree)
{
	...
	__try
	{
		...
		status = Key_GetName(handle, &objname, &TruePath, &CopyPath, &IsBoxedPath);
		if (!NT_SUCCESS(status))
		{
			__leave;
		}
		...
		if (!DeleteTree)
		{
			status = Key_NtQueryKeyImpl(handle, KeyFullInformation, info, info_len, &len);
		...
		while (1)
		{
			status = __sys_NtEnumerateKey(handle, 0, KeyBasicInformation, info, info_len, &len);
			...
			Wow64DeleteAccess = Key_GetWow64Flag(info->Name, GENERIC_WRITE | DELETE);
...
_FX NTSTATUS Key_MarkDeletedAndClose(HANDLE KeyHandle)
{
	...
	status = NtSetInformationKey(KeyHandle, KeyWriteTimeInformation, &kwti, sizeof(KEY_WRITE_TIME_INFORMATION));
...
_FX NTSTATUS Key_MarkUnDeleted(HANDLE KeyHandle)
{
	...
	if (status == STATUS_ACCESS_DENIED)
	{
		...
		RtlInitUnicodeString(&objname, L"");
		InitializeObjectAttributes(&objattrs, &objname, OBJ_CASE_INSENSITIVE, KeyHandle, NULL);

		status = NtOpenKey(&handle, KEY_SET_VALUE, &objattrs);

		if (NT_SUCCESS(status))
		{
			status = NtSetInformationKey(handle, KeyWriteTimeInformation, &kwti, sizeof(KEY_WRITE_TIME_INFORMATION));
...
_FX NTSTATUS Key_NtDeleteValueKey(HANDLE KeyHandle, UNICODE_STRING* ValueName)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	NTSTATUS status;
	...
	if (NT_SUCCESS(status))
	{
		...
		else
		{
			...
			status = Key_NtQueryValueKey(KeyHandle, ValueName, KeyValueBasicInformation, &u.info, sizeof(u), &len);

			if (status == STATUS_SUCCESS || status == STATUS_BUFFER_OVERFLOW)
			{
				status = NtSetValueKey(KeyHandle, ValueName, 0, tzuk, NULL, 0);
			}
		}
	}
...
_FX NTSTATUS Key_NtSetValueKey(HANDLE KeyHandle, UNICODE_STRING* ValueName, ULONG TitleIndex, ULONG Type, void* Data, ULONG DataSize)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	if (ValueName && ValueName->Length && ValueName->Buffer)
	{
		...
		if (Dll_ImageType == DLL_IMAGE_TRUSTED_INSTALLER && uni.Length == 20 && _wcsnicmp(uni.Buffer, L"StoreDirty", 10) == 0)
		{
			return STATUS_SUCCESS;
		}
	}
	...
	status = __sys_NtSetValueKey(KeyHandle, &uni, 0, Type, Data, DataSize);
	...
	if (status == STATUS_ACCESS_DENIED)
	{
		...
		if (NT_SUCCESS(status))
		{
			status = __sys_NtSetValueKey(handle, &uni, 0, Type, Data, DataSize);
...
_FX NTSTATUS Key_NtQueryKey(HANDLE KeyHandle, KEY_INFORMATION_CLASS KeyInformationClass, void* KeyInformation, ULONG Length, ULONG* ResultLength)
{
	NTSTATUS status = Key_NtQueryKeyImpl(KeyHandle, KeyInformationClass, KeyInformation, Length, ResultLength);
...
_FX NTSTATUS Key_NtQueryKeyImpl(HANDLE KeyHandle, KEY_INFORMATION_CLASS KeyInformationClass, void* KeyInformation, ULONG Length, ULONG* ResultLength)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (KeyInformationClass == KeyBasicInformation || KeyInformationClass == KeyNodeInformation || KeyInformationClass == KeyFlagsInformation)
		{
			status = __sys_NtQueryKey(KeyHandle, KeyInformationClass, KeyInformation, Length, ResultLength);

			if (KeyInformationClass != KeyFlagsInformation && (NT_SUCCESS(status) || status == STATUS_BUFFER_OVERFLOW) && IS_DELETE_MARK((LARGE_INTEGER*)KeyInformation))
			{
		...
		if (KeyInformationClass == KeyHandleTagsInformation && __sys_NtOpenKeyEx)
		{
			if (Length != sizeof(ULONG))
			{
		...
		if (!NT_SUCCESS(status))
		{
			if (status == STATUS_BAD_INITIAL_PC)
			{
				...
				if ((NT_SUCCESS(status) || status == STATUS_BUFFER_OVERFLOW) && IS_DELETE_MARK((LARGE_INTEGER*)KeyInformation))
				{
					status = STATUS_KEY_DELETED;
				}
			}
		...
		{
			KEY_FULL_INFORMATION* info = (KEY_FULL_INFORMATION*)KeyInformation;
		...
		else if (KeyInformationClass == KeyCachedInformation)
		{
			...
			KEY_CACHED_INFORMATION* info = (KEY_CACHED_INFORMATION*)KeyInformation;
...
_FX NTSTATUS Key_NtEnumerateKey(HANDLE KeyHandle, ULONG Index, KEY_INFORMATION_CLASS KeyInformationClass, void* KeyInformation, ULONG Length, ULONG* ResultLength)
{
	...
	__try
	{
		...
		if (KeyInformationClass != KeyBasicInformation && KeyInformationClass != KeyNodeInformation && KeyInformationClass != KeyFullInformation)
		{
			status = STATUS_INVALID_PARAMETER;
			__leave;
		}
		...
		if (!NT_SUCCESS(status))
		{
			if (status == STATUS_BAD_INITIAL_PC)
			{
				status = __sys_NtEnumerateKey(KeyHandle, Index, KeyInformationClass, KeyInformation, Length, ResultLength);
		...
		if (subkey)
		{
			len = merge->name_len + sizeof(WCHAR)   // backspace
			    + subkey->name_len + sizeof(WCHAR); // null
			...
			memcpy(SubkeyPath + merge->name_len / sizeof(WCHAR) + 1, subkey->name, subkey->name_len + sizeof(WCHAR));

			SubkeyPathLen = (merge->name_len + sizeof(WCHAR) + subkey->name_len) / sizeof(WCHAR);
		...
		if (merge->name_len == _Registry_User_Len * sizeof(WCHAR) && _wcsnicmp(merge->name, _Registry_User, _Registry_User_Len) == 0)
		{
			//
		...
		else if (SubkeyPathLen == Key_System_Len && _wcsnicmp(SubkeyPath, Key_System, Key_System_Len) == 0)
		{
			//
		...
		else
		{
			if ((KeyInformationClass == KeyBasicInformation || KeyInformationClass == KeyNodeInformation))
			{
				...
				else
				{
					...
					if (SubkeyPathLen > _Registry_User_Len + 2 && SubkeyPath[_Registry_User_Len] == L'\\' && (SubkeyPath[_Registry_User_Len + 1] == L'S' || SubkeyPath[_Registry_User_Len + 1] == L's') && SubkeyPath[_Registry_User_Len + 2] == L'-' && _wcsnicmp(SubkeyPath, _Registry_User, _Registry_User_Len) == 0)
					{
						WCHAR* backslash = wcschr(SubkeyPath + _Registry_User_Len + 1, L'\\');
						if (backslash && _wcsicmp(backslash, L"\\Software\\Classes") == 0)
						{
							status = STATUS_ACCESS_DENIED;
						}
					}
			...
			if (NT_SUCCESS(status))
			{
				...
				status = Key_NtOpenKeyImpl(&SubkeyHandle, Key_GetWow64Flag(SubkeyPath, KEY_READ), &objattrs);
			}
		}

		if (NT_SUCCESS(status))
		{
			status = Key_NtQueryKeyImpl(SubkeyHandle, KeyInformationClass, KeyInformation, Length, ResultLength);
		...
		else if (status == STATUS_ACCESS_DENIED)
		{
			...
			status = Key_NtEnumerateKeyFake(SubkeyPath, SubkeyPathLen, KeyInformationClass, KeyInformation, Length, ResultLength, &SubkeyLastWriteTime);
		}
...
_FX NTSTATUS Key_NtEnumerateKeyFake(const WCHAR* KeyPath, ULONG KeyPathLen, KEY_INFORMATION_CLASS KeyInformationClass, void* KeyInformation, ULONG Length, ULONG* ResultLength, LARGE_INTEGER* LastWriteTime)
{
	NTSTATUS status;
	ULONG MinLength;
	ULONG* NameLength;
	...
	__try
	{
		...
		else
		{
			...
			subkey_name_len = (KeyPathLen - (subkey_name - KeyPath)) * sizeof(WCHAR);
		}
...
_FX NTSTATUS Key_NtQueryValueKey(HANDLE KeyHandle, UNICODE_STRING* ValueName, KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass, void* KeyValueInformation, ULONG Length, ULONG* ResultLength)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (KeyValueInformationClass == KeyValuePartialInformation && KeyValueInformation && ResultLength)
		{
			if (Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER)
			{
				status = Key_NtQueryValueKeyFakeForInternetExplorer(TruePath, ValueNameBuf, ValueNameLen1, KeyValueInformation, Length, ResultLength);
			...
			if (Dll_ImageType == DLL_IMAGE_ACROBAT_READER || Dll_ImageType == DLL_IMAGE_PLUGIN_CONTAINER || Dll_ImageType == DLL_IMAGE_GOOGLE_CHROME || Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER)
			{ // AcroPDF.dll

				status = Key_NtQueryValueKeyFakeForAcrobatReader(TruePath, ValueNameBuf, ValueNameLen1, KeyValueInformation, Length, ResultLength);
			...
			{
				status = Key_NtQueryValueKeyFakeForCreateProcess(TruePath, ValueNameBuf, ValueNameLen1, KeyValueInformation, Length, ResultLength);
		...
		if (!NT_SUCCESS(status))
		{
			if (status == STATUS_BAD_INITIAL_PC)
			{
				status = __sys_NtQueryValueKey(KeyHandle, ValueName, KeyValueInformationClass, KeyValueInformation, Length, ResultLength);

				if ((NT_SUCCESS(status) || status == STATUS_BUFFER_OVERFLOW) && Key_CheckDeletedValue(KeyValueInformationClass, KeyValueInformation))
				{
					status = STATUS_OBJECT_NAME_NOT_FOUND;
				}
			}
		...
		while (value)
		{
			if (value->name_len == ValueNameLen2 && _wcsnicmp(value->name, ValueNameBuf, ValueNameLen1) == 0)
			{
				break;
			}
			value = List_Next(value);
		...
		if (value)
		{
			status = Key_GetMergedValue(value, KeyValueInformationClass, KeyValueInformation, Length, ResultLength);
		}
		else
		{
			status = STATUS_OBJECT_NAME_NOT_FOUND;
...
_FX NTSTATUS Key_NtQueryValueKeyFakeForInternetExplorer(const WCHAR* TruePath, const WCHAR* ValueNameBuf, ULONG ValueNameLen, void* KeyValueInformation, ULONG Length, ULONG* ResultLength)
{
	ULONG ValueType = 0;
	ULONG ValueData;
	...
	else if (ValueNameLen == 13 && _wcsicmp(ValueNameBuf, L"DetourDialogs") == 0)
	{
		ValueType = REG_DWORD;
	...
	{
		static const WCHAR* _Zones = L"\\Microsoft\\Windows\\CurrentVersion\\"
	...
	else if (ValueNameLen == 27 && _wcsicmp(ValueNameBuf, L"ProtectedModeOffForAllZones") == 0)
	{
		ValueType = REG_DWORD;
	...
	else if (ValueNameLen == 21 && _wcsicmp(ValueNameBuf, L"NoProtectedModeBanner") == 0)
	{
		ValueType = REG_DWORD;
	...
	{
		KEY_VALUE_PARTIAL_INFORMATION* kvpi = (KEY_VALUE_PARTIAL_INFORMATION*)KeyValueInformation;
...
_FX NTSTATUS Key_NtQueryValueKeyFakeForAcrobatReader(const WCHAR* TruePath, const WCHAR* ValueNameBuf, ULONG ValueNameLen, void* KeyValueInformation, ULONG Length, ULONG* ResultLength)
{
	ULONG ValueType = 0;
	ULONG ValueData;
	...
	else if (ValueNameLen == 14 && _wcsicmp(ValueNameBuf, L"bProtectedMode") == 0)
	{
		ValueType = REG_DWORD;
	...
	else if (ValueNameLen == 12 && _wcsicmp(ValueNameBuf, L"iCheckReader") == 0)
	{
		ValueType = REG_DWORD;
...
_FX NTSTATUS Key_NtQueryValueKeyFakeForCreateProcess(const WCHAR* TruePath, const WCHAR* ValueNameBuf, ULONG ValueNameLen, void* KeyValueInformation, ULONG Length, ULONG* ResultLength)
{
	ULONG ValueType = 0;
	ULONG ValueData;
	...
	else if (ValueNameLen == 19 && _wcsicmp(ValueNameBuf, L"AuthenticodeEnabled") == 0)
	{
		ValueType = REG_DWORD;
...
_FX NTSTATUS Key_NtEnumerateValueKey(HANDLE KeyHandle, ULONG Index, KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass, void* KeyValueInformation, ULONG Length, ULONG* ResultLength)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (!NT_SUCCESS(status))
		{
			if (status == STATUS_BAD_INITIAL_PC)
			{
				status = __sys_NtEnumerateValueKey(KeyHandle, Index, KeyValueInformationClass, KeyValueInformation, Length, ResultLength);
...
_FX BOOLEAN Key_CheckDeletedValue(KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass, void* info)
{
	ULONG type;
...
_FX NTSTATUS Key_NtQueryMultipleValueKey(HANDLE KeyHandle, KEY_VALUE_ENTRY* ValueEntries, ULONG EntryCount, void* ValueBuffer, ULONG* BufferLength, ULONG* ResultLength)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (!NT_SUCCESS(status))
		{
			if (status == STATUS_BAD_INITIAL_PC)
			{
				status = __sys_NtQueryMultipleValueKey(KeyHandle, ValueEntries, EntryCount, ValueBuffer, BufferLength, ResultLength);
			}
		...
		while (EntryCount > 0)
		{
			...
			while (value)
			{
				if (value->name_len == ValueNameLen && _wcsicmp(value->name, ValueNameBuf) == 0)
				{
					break;
				}
				value = List_Next(value);
			...
			else
			{
				if (value->data_len)
				{
					memcpy((UCHAR*)ValueBuffer + need_len, value->data_ptr, value->data_len);

					used_len = (used_len + value->data_len + sizeof(ULONG) - 1) & ~(sizeof(ULONG) - 1);
				}
			...
			need_len = (need_len + value->data_len + sizeof(ULONG) - 1) & ~(sizeof(ULONG) - 1);
...
_FX NTSTATUS Key_NtNotifyChangeKey(HANDLE KeyHandle, HANDLE Event OPTIONAL, PIO_APC_ROUTINE ApcRoutine OPTIONAL, PVOID ApcContext OPTIONAL, PIO_STATUS_BLOCK IoStatusBlock, ULONG CompletionFilter, BOOLEAN WatchTree, PVOID Buffer, ULONG BufferSize, BOOLEAN Asynchronous)
{
	return Key_NtNotifyChangeMultipleKeys(KeyHandle, 0, NULL, Event, ApcRoutine, ApcContext, IoStatusBlock, CompletionFilter, WatchTree, Buffer, BufferSize, Asynchronous);
}
...
_FX NTSTATUS Key_NtNotifyChangeMultipleKeys(HANDLE MasterKeyHandle, ULONG Count, OBJECT_ATTRIBUTES* SlaveObjects, HANDLE Event OPTIONAL, PIO_APC_ROUTINE ApcRoutine OPTIONAL, PVOID ApcContext OPTIONAL, PIO_STATUS_BLOCK IoStatusBlock, ULONG CompletionFilter, BOOLEAN WatchTree, PVOID Buffer, ULONG BufferSize, BOOLEAN Asynchronous)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		status = Key_GetName(MasterKeyHandle, &objname, &TruePath, &CopyPath, &IsBoxedPath);
		if (!NT_SUCCESS(status))
		{
			__leave;
		}
		...
		{
			status = Key_GetName(SlaveObjects->RootDirectory, SlaveObjects->ObjectName, &TruePath, &CopyPath, &IsBoxedPath);
...
CallService:
	...
	if (NT_SUCCESS(status))
	{
		status = __sys_NtNotifyChangeMultipleKeys(MasterKeyHandle, Count, SlaveObjects, Event, ApcRoutine, ApcContext, IoStatusBlock, CompletionFilter, WatchTree, Buffer, BufferSize, Asynchronous);
		...
		{
			status = __sys_NtNotifyChangeMultipleKeys(MasterKeyHandle, 0, NULL, Event, ApcRoutine, ApcContext, IoStatusBlock, CompletionFilter, WatchTree, Buffer, BufferSize, Asynchronous);
		}
	}
...
_FX HANDLE Key_GetTrueHandle(HANDLE KeyHandle, BOOLEAN* pIsOpenPath)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		if (!mp_flags)
		{
			...
			status = __sys_NtOpenKey(&handle, Key_GetWow64Flag(TruePath, KEY_READ), &objattrs);
...
_FX NTSTATUS Key_NtRenameKey(HANDLE KeyHandle, UNICODE_STRING* ReplacementName)
{
...
_FX NTSTATUS Key_NtSaveKey(HANDLE KeyHandle, HANDLE FileHandle)
{
...
_FX NTSTATUS Key_NtLoadKey(OBJECT_ATTRIBUTES* TargetObjectAttributes, OBJECT_ATTRIBUTES* SourceObjectAttributes)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		status = NtCreateFile(&FileHandle, FILE_READ_ATTRIBUTES | SYNCHRONIZE, SourceObjectAttributes, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE, NULL, 0);
		...
		status = Key_GetName(TargetObjectAttributes->RootDirectory, TargetObjectAttributes->ObjectName, &TruePath, &CopyPath, NULL);
...
_FX ULONG Key_NtQueryObjectName(UNICODE_STRING* ObjectName, ULONG MaxLen)
{
	...
	if (Len >= (Dll_BoxKeyPathLen + 8) * sizeof(WCHAR) && 0 == Dll_NlsStrCmp(Buf, Dll_BoxKeyPath, Dll_BoxKeyPathLen))
	{
		//
		...
		if (_wcsnicmp(Buf + Dll_BoxKeyPathLen, _Machine, 8) == 0)
		{
			...
			ObjectName->Length        = (USHORT)((9 /* \REGISTRY */ + MoveLen) * sizeof(WCHAR));
		...
		else if (_wcsnicmp(Buf + Dll_BoxKeyPathLen, _UserCurrent, 13) == 0)
		{
			WCHAR* TmpBuf;
