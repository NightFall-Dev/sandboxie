...
_FX BOOLEAN Gui_InitConsole1(void)
{
	...
	if (Dll_ProcessFlags & (SBIE_FLAG_CREATE_CONSOLE_SHOW | SBIE_FLAG_CREATE_CONSOLE_HIDE))
	{
	...
	if (!Gui_ConsoleHwnd)
	{
		...
		if (Dll_KernelBase)
		{
			__sys_AllocConsole = (P_AllocConsole)GetProcAddress(Dll_KernelBase, _AllocConsole);
		}
		else
		{
		...
		{
			__sys_AllocConsole = (P_AllocConsole)GetProcAddress(Dll_Kernel32, _AllocConsole);
		}
...
_FX BOOL Gui_ConnectConsole(ULONG ShowFlag)
{
	...
	status = NtOpenProcessToken(NtCurrentProcess(), TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY, &ProcessToken);
	if (NT_SUCCESS(status))
	{
		...
		rpl = Gui_CallProxy(&req, sizeof(req), sizeof(GUI_CREATE_CONSOLE_RPL));
		if (!rpl)
		{
		...
		else
		{
			...
			if (NT_SUCCESS(status))
			{
				...
				P_AttachConsole AttachConsole = (P_AttachConsole)GetProcAddress(Dll_Kernel32, "AttachConsole");
...
_FX void Gui_InitConsole2(void)
{
	...
	if (Handles[0])
	{
		...
		if (Handles[1])
		{
			Handles[2] = CreateThread(NULL, 0, Gui_ConsoleThread, Handles, 0, NULL);
			if (Handles[2])
			{
...
_FX ULONG Gui_ConsoleThread(void* xHandles)
{
	...
	__sys_GetMessageW = (P_GetMessage)Ldr_GetProcAddrNew(DllName_user32, L"GetMessageW", "GetMessageW");

	if ((!__sys_MsgWaitForMultipleObjects) || (!__sys_PeekMessageW) || (!__sys_GetMessageW) || (!__sys_DispatchMessageW) || (!__sys_RegisterClassW) || (!__sys_CreateWindowExW) || (!__sys_DefWindowProcW))
	{
		return 0;
	}
	...
	hwnd = __sys_CreateWindowExW(0, (void*)atom, L"", WS_OVERLAPPEDWINDOW, 1, 1, 1, 1, NULL, NULL, NULL, NULL);
	if (!hwnd)
	{
		return 0;
	}
	...
	while (1)
	{
		...
		rc = __sys_MsgWaitForMultipleObjects(1, Handles, FALSE, INFINITE, QS_ALLINPUT);
