...
static ULONG_PTR Ldr_LdrResolveDelayLoadedAPI(HMODULE ImageBase, ULONG_PTR DelayImportDescriptor, ULONG_PTR UnknownParameter1, ULONG_PTR UnknownParameter2, ULONG_PTR UnknownParameter3, ULONG_PTR UnknownParameter4);

static NTSTATUS Ldr_LdrQueryImageFileExecutionOptions(UNICODE_STRING* SubKey, const WCHAR* ValueName, ULONG Type, void* Buffer, ULONG BufferSize, ULONG* ReturnedLength);

static ULONG_PTR Ldr_NtApphelpCacheControl(ULONG_PTR Unknown1, ULONG_PTR Unknown2);
...
typedef NTSTATUS (*P_LdrUnloadDll)(HANDLE ModuleHandle);

typedef ULONG_PTR (*P_LdrResolveDelayLoadedAPI)(HMODULE ImageBase, ULONG_PTR DelayImportDescriptor, ULONG_PTR UnknownParameter1, ULONG_PTR UnknownParameter2, ULONG_PTR UnknownParameter3, ULONG_PTR UnknownParameter4);

typedef NTSTATUS (*P_LdrQueryImageFileExecutionOptions)(UNICODE_STRING* SubKey, const WCHAR* ValueName, ULONG Type, void* Buffer, ULONG BufferSize, ULONG* ReturnedLength);

typedef ULONG_PTR (*P_NtApphelpCacheControl)(ULONG_PTR Unknown1, ULONG_PTR Unknown2);
...
static DLL Ldr_Dlls[] = {{L"advapi32.dll", "advapi32.dll", AdvApi_Init, 0},
...
NTSTATUS Ldr_NtAccessCheckByTypeResultList(PSECURITY_DESCRIPTOR SecurityDescriptor, PSID PrincipalSelfSid, HANDLE ClientToken, ACCESS_MASK DesiredAccess, POBJECT_TYPE_LIST ObjectTypeList, ULONG ObjectTypeListLength, PGENERIC_MAPPING GenericMapping, PPRIVILEGE_SET PrivilegeSet, PULONG PrivilegeSetLength, PACCESS_MASK GrantedAccess, PNTSTATUS AccessStatus);

NTSTATUS Ldr_NtAccessCheck(IN PSECURITY_DESCRIPTOR SecurityDescriptor, IN HANDLE ClientToken, IN ACCESS_MASK DesiredAccess, IN PGENERIC_MAPPING GenericMapping OPTIONAL, OUT PPRIVILEGE_SET RequiredPrivilegesBuffer, IN OUT PULONG BufferLength, OUT PACCESS_MASK GrantedAccess, OUT PNTSTATUS AccessStatus);

NTSTATUS Ldr_NtQuerySecurityAttributesToken(IN HANDLE TokenHandle, IN PUNICODE_STRING Attributes, IN ULONG NumberOfAttributes, OUT PVOID Buffer, IN ULONG Length, OUT PULONG ReturnLength);

NTSTATUS Ldr_NtQueryInformationToken(HANDLE TokenHandle, TOKEN_INFORMATION_CLASS TokenInformationClass, void* TokenInformation, ULONG TokenInformationLength, ULONG* ReturnLength);
...
NTSTATUS Ldr_NtTerminateProcess(HANDLE ProcessHandle, NTSTATUS ExitStatus)
{
	...
	if (!ProcessHandle || ProcessHandle == NtCurrentProcess() || GetCurrentProcessId() == GetProcessId(ProcessHandle))
	{
...
_FX BOOLEAN Ldr_Init()
{
	...
	else
	{
		...
		if (Dll_OsBuild >= 8400)
		{
			P_LdrResolveDelayLoadedAPI LdrResolveDelayLoadedAPI = (P_LdrResolveDelayLoadedAPI)GetProcAddress(Dll_Ntdll, "LdrResolveDelayLoadedAPI");

			P_NtApphelpCacheControl NtApphelpCacheControl = (P_NtApphelpCacheControl)GetProcAddress(Dll_Ntdll, "NtApphelpCacheControl");
...
_FX void Ldr_CallDllCallbacks(void)
{
	...
	if (OldList)
	{
		for (iOld = 0; iOld < OldList->ModuleCount; ++iOld)
		{
			...
			for (iNew = 0; iNew < NewList->ModuleCount; ++iNew)
			{
				...
				if (pOld->ImageBaseAddress == pNew->ImageBaseAddress && strcmp(pOld->Path, pNew->Path) == 0)
				{
					found = TRUE;
					break;
				}
	...
	for (iNew = 0; iNew < NewList->ModuleCount; ++iNew)
	{
		...
		if (!found)
		{
			...
			__my_Ldr_CallOneDllCallback(pNew->Path + pNew->NameOffset, pNew->ImageBaseAddress);
...
_FX NTSTATUS Ldr_LdrLoadDll(WCHAR* PathString, ULONG* DllFlags, UNICODE_STRING* ModuleName, HANDLE* ModuleHandle)
{
	//
...
_FX NTSTATUS Ldr_Win10_LdrLoadDll(WCHAR* PathString, ULONG* DllFlags, UNICODE_STRING* ModuleName, HANDLE* ModuleHandle)
{
	//
	// load the DLL
	//
...
_FX ULONG_PTR Ldr_LdrResolveDelayLoadedAPI(HMODULE ImageBase, ULONG_PTR DelayImportDescriptor, ULONG_PTR UnknownParameter1, ULONG_PTR UnknownParameter2, ULONG_PTR UnknownParameter3, ULONG_PTR UnknownParameter4)
{
	//
	// load the DLL and invoke callbacks
	//
	...
	result = __sys_LdrResolveDelayLoadedAPI(ImageBase, DelayImportDescriptor, UnknownParameter1, UnknownParameter2, UnknownParameter3, UnknownParameter4);
...
_FX NTSTATUS Ldr_LdrQueryImageFileExecutionOptions(UNICODE_STRING* SubKey, const WCHAR* ValueName, ULONG Type, void* Buffer, ULONG BufferSize, ULONG* ReturnedLength)
{
	NTSTATUS rc;
	//
	...
	rc = __sys_LdrQueryImageFileExecutionOptions(SubKey, ValueName, Type, Buffer, BufferSize, ReturnedLength);
	return rc;
}
...
_FX ULONG_PTR Ldr_NtApphelpCacheControl(ULONG_PTR Unknown1, ULONG_PTR Unknown2)
{
	ULONG_PTR rc;
	//
...
_FX NTSTATUS Ldr_NtLoadDriver(UNICODE_STRING* RegistryPath)
{
	...
	if (status == STATUS_PRIVILEGE_NOT_HELD)
	{
		...
		if (RegistryPath && (((ULONG)(ULONG_PTR)RegistryPath) & 0xFFFFFF00) != 0xFFFFFF00 && RegistryPath->Buffer)
		{
...
_FX NTSTATUS Ldr_NtQueryInformationToken(HANDLE TokenHandle, TOKEN_INFORMATION_CLASS TokenInformationClass, void* TokenInformation, ULONG TokenInformationLength, ULONG* ReturnLength)
{
	NTSTATUS status      = 0;
	...
	status = __sys_NtQueryInformationToken(hTokenReal ? hTokenReal : TokenHandle, TokenInformationClass, TokenInformation, TokenInformationLength, ReturnLength);
