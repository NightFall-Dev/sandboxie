...
static BOOL Gui_EnumCommon(UCHAR which, ULONG_PTR arg, WNDENUMPROC lpEnumFunc, LPARAM lParam);

static BOOL Gui_EnumChildWindows(HWND hWndParent, WNDENUMPROC lpEnumFunc, LPARAM lParam);

static BOOL Gui_EnumThreadWindows(DWORD dwThreadId, WNDENUMPROC lpEnumFunc, LPARAM lParam);

static BOOL Gui_EnumDesktopWindows(HDESK hDesktop, WNDENUMPROC lpEnumFunc, LPARAM lParam);

static BOOL Gui_EnumDesktopsW(HWINSTA hwinsta, DESKTOPENUMPROC lpEnumFunc, LPARAM lParam);

static BOOL Gui_EnumDesktopsA(HWINSTA hwinsta, DESKTOPENUMPROC lpEnumFunc, LPARAM lParam);

static HDESK Gui_OpenDesktopW(void* lpszDesktop, ULONG dwFlags, BOOL fInherit, ACCESS_MASK dwDesiredAccess);

static HDESK Gui_OpenDesktopA(void* lpszDesktop, ULONG dwFlags, BOOL fInherit, ACCESS_MASK dwDesiredAccess);

static HDESK Gui_CreateDesktopW(void* lpszDesktop, void* lpszDevice, void* DevMode, ULONG dwFlags, ACCESS_MASK dwDesiredAccess, void* SecurityAttributes);

static HDESK Gui_CreateDesktopA(void* lpszDesktop, void* lpszDevice, void* DevMode, ULONG dwFlags, ACCESS_MASK dwDesiredAccess, void* SecurityAttributes);
...
static HWND Gui_FindWindowCommon(ULONG which, HWND hwndParent, HWND hwndChildAfter, const void* lpClassName, const void* lpWindowName);

HWND Gui_FindWindowW(const WCHAR* lpClassName, const WCHAR* lpWindowName);

static HWND Gui_FindWindowA(const UCHAR* lpClassName, const UCHAR* lpWindowName);

static HWND Gui_FindWindowExW(HWND hwndParent, HWND hwndChildAfter, const WCHAR* lpClassName, const WCHAR* lpWindowName);

static HWND Gui_FindWindowExA(HWND hwndParent, HWND hwndChildAfter, const UCHAR* lpClassName, const UCHAR* lpWindowName);
	...
	if (code)
	{
		...
		if (code[_E8_Offset] == 0xE8)
		{
			...
			else
			{
				...
				// 32-bit Windows 8
				if (code[0] == 0xB8 && code[5] == 0xE8 && *(SHORT*)(code + 10) == 0x08C2)
				{
					ok = TRUE;
				}
...
_FX BOOL Gui_EnumProc(HWND hWnd, LPARAM lParam)
{
	...
	if (allow)
	{
		...
		ULONG_PTR result      = ProtectCall2(p->lpEnumFunc, (ULONG_PTR)hWnd, p->lParam);
		return (BOOL)result;
	}
...
_FX BOOL Gui_EnumCommon(UCHAR which, ULONG_PTR arg, WNDENUMPROC lpEnumFunc, LPARAM lParam)
{
	...
	else
	{
		...
		for (i = 0; i < rpl->num_hwnds; ++i)
		{
			...
			ULONG_PTR result = ProtectCall2(lpEnumFunc, (ULONG_PTR)hwnd, lParam);
			if (!result)
			{
				ok = FALSE;
				break;
...
_FX BOOL Gui_EnumChildWindows(HWND hWndParent, WNDENUMPROC lpEnumFunc, LPARAM lParam)
{
...
_FX BOOL Gui_EnumThreadWindows(DWORD dwThreadId, WNDENUMPROC lpEnumFunc, LPARAM lParam)
{
...
_FX BOOL Gui_EnumDesktopWindows(HDESK hDesktop, WNDENUMPROC lpEnumFunc, LPARAM lParam)
{
...
_FX BOOL Gui_EnumDesktopsW(HWINSTA hwinsta, DESKTOPENUMPROC lpEnumFunc, LPARAM lParam)
{
...
_FX BOOL Gui_EnumDesktopsA(HWINSTA hwinsta, DESKTOPENUMPROC lpEnumFunc, LPARAM lParam)
{
...
_FX HDESK Gui_OpenDesktopW(void* lpszDesktop, ULONG dwFlags, BOOL fInherit, ACCESS_MASK dwDesiredAccess)
{
...
_FX HDESK Gui_OpenDesktopA(void* lpszDesktop, ULONG dwFlags, BOOL fInherit, ACCESS_MASK dwDesiredAccess)
{
	return CreateEvent(NULL, FALSE, FALSE, NULL);
}
...
_FX HDESK Gui_CreateDesktopW(void* lpszDesktop, void* lpszDevice, void* DevMode, ULONG dwFlags, ACCESS_MASK dwDesiredAccess, void* SecurityAttributes)
{
	HANDLE rc = 0;
...
_FX HDESK Gui_CreateDesktopA(void* lpszDesktop, void* lpszDevice, void* DevMode, ULONG dwFlags, ACCESS_MASK dwDesiredAccess, void* SecurityAttributes)
{
	HANDLE rc = 0;
...
_FX HWND Gui_FindWindowCommon(ULONG which, HWND hwndParent, HWND hwndChildAfter, const void* lpClassName, const void* lpWindowName)
{
...
_FX HWND Gui_FindWindowW(const WCHAR* lpClassName, const WCHAR* lpWindowName)
{
	WCHAR* clsnm;
	WCHAR* winnm;
	HWND hwndResult;
	...
	if (!hwndResult)
	{
		hwndResult = Gui_FindWindowCommon('fw w', NULL, NULL, lpClassName, lpWindowName);
	}
...
_FX HWND Gui_FindWindowA(const UCHAR* lpClassName, const UCHAR* lpWindowName)
{
	UCHAR* clsnm;
	UCHAR* winnm;
	HWND hwndResult;
	...
	if (!hwndResult)
	{
		hwndResult = Gui_FindWindowCommon('fw a', NULL, NULL, lpClassName, lpWindowName);
	}
...
_FX HWND Gui_FindWindowExW(HWND hwndParent, HWND hwndChildAfter, const WCHAR* lpClassName, const WCHAR* lpWindowName)
{
	WCHAR* clsnm;
	WCHAR* winnm;
	HWND hwndResult;
	...
	hwndResult = __sys_FindWindowExW(hwndParent, hwndChildAfter, clsnm, lpWindowName);
	...
	if (!hwndResult && lpWindowName)
	{
		...
		if (winnm != lpWindowName)
		{
			hwndResult = __sys_FindWindowExW(hwndParent, hwndChildAfter, clsnm, winnm);
			Gui_Free((void*)winnm);
		}
	}
	...
	if (!hwndResult)
	{
		hwndResult = Gui_FindWindowCommon('fwxw', hwndParent, hwndChildAfter, lpClassName, lpWindowName);
	}
...
_FX HWND Gui_FindWindowExA(HWND hwndParent, HWND hwndChildAfter, const UCHAR* lpClassName, const UCHAR* lpWindowName)
{
	UCHAR* clsnm;
	UCHAR* winnm;
	HWND hwndResult;
	...
	hwndResult = __sys_FindWindowExA(hwndParent, hwndChildAfter, clsnm, lpWindowName);
	...
	if (!hwndResult && lpWindowName)
	{
		...
		if (winnm != lpWindowName)
		{
			hwndResult = __sys_FindWindowExA(hwndParent, hwndChildAfter, clsnm, winnm);
			Gui_Free((void*)winnm);
		}
	}
	...
	if (!hwndResult)
	{
		hwndResult = Gui_FindWindowCommon('fwxa', hwndParent, hwndChildAfter, lpClassName, lpWindowName);
	}
