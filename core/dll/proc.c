...
static BOOL Proc_CreateProcessInternalW(HANDLE hToken, const WCHAR* lpApplicationName, WCHAR* lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, ULONG dwCreationFlags, void* lpEnvironment, void* lpCurrentDirectory, LPSTARTUPINFOW lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation, HANDLE* hNewToken);

static BOOL Proc_CreateProcessInternalW_RS5(HANDLE hToken, const WCHAR* lpApplicationName, WCHAR* lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, ULONG dwCreationFlags, void* lpEnvironment, void* lpCurrentDirectory, LPSTARTUPINFOW lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation, HANDLE* hNewToken);

static BOOL Proc_AlternateCreateProcess(const WCHAR* lpApplicationName, WCHAR* lpCommandLine, void* lpCurrentDirectory, LPPROCESS_INFORMATION lpProcessInformation, BOOL* ReturnValue);
...
static void Proc_QuoteCommandLine_XP(THREAD_DATA* TlsData, const WCHAR* CommandLine, const WCHAR* ImagePath);

static _FX void Proc_FixBatchCommandLine(THREAD_DATA* TlsData, const WCHAR* CommandLine, const WCHAR* ImagePath);

static void Proc_StoreImagePath(THREAD_DATA* TlsData, HANDLE FileHandle);

static NTSTATUS Proc_RtlCreateProcessParametersEx(void* ProcessParameters, UNICODE_STRING* ImagePathName, UNICODE_STRING* DllPath, UNICODE_STRING* CurrentDirectory, UNICODE_STRING* CommandLine, void* Environment, UNICODE_STRING* WindowTitle, UNICODE_STRING* DesktopInfo, UNICODE_STRING* ShellInfo, UNICODE_STRING* RuntimeData, void* UnknownParameter11);

static BOOL Proc_CreateProcessWithTokenW(HANDLE hToken, ULONG dwLogonFlags, void* lpApplicationName, void* lpCommandLine, ULONG dwCreationFlags, void* lpEnvironment, void* lpCurrentDirectory, LPSTARTUPINFOW lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation);
...
static BOOLEAN Proc_IsSplWow64(const WCHAR* lpApplicationName, const WCHAR* lpCommandLine, LPPROCESS_INFORMATION lpProcessInformation);

static HRESULT Proc_RunSetupCommandW(HWND hWnd, const WCHAR* szCmdName, const WCHAR* szInfSection, const WCHAR* szDir, const WCHAR* szTitle, HANDLE* phEXE, ULONG dwFlags, void* pvReserved);

static BOOLEAN Proc_IsForcedElevation(const WCHAR* lpCommandLine, const WCHAR* lpCurrentDirectory, LPPROCESS_INFORMATION lpProcessInformation);

static NTSTATUS Proc_NtSetInformationProcess(HANDLE ProcessHandle, ULONG InfoClass, void* Info, ULONG InfoLength);

static NTSTATUS Proc_NtQueryInformationProcess(HANDLE ProcessHandle, PROCESSINFOCLASS ProcessInformationClass, PVOID ProcessInformation, ULONG ProcessInformationLength, PULONG ReturnLength);

static NTSTATUS Proc_NtCreateProcessEx(HANDLE* ProcessHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, HANDLE* ParentProcessHandle, BOOLEAN InheritHandles, HANDLE SectionHandle, HANDLE DebugPort, HANDLE ExceptionPort, ULONG UnknownExtraParameter);

extern NTSTATUS File_GetName(HANDLE RootDirectory, UNICODE_STRING* ObjectName, WCHAR** OutTruePath, WCHAR** OutCopyPath, ULONG* OutFlags);

//---------------------------------------------------------------------------


typedef BOOL (*P_CreateProcessInternal)(HANDLE hToken, const void* lpApplicationName, void* lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, ULONG dwCreationFlags, void* lpEnvironment, void* lpCurrentDirectory, void* lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation, HANDLE* hNewToken);

typedef BOOL (*P_CreateProcessInternal_RS5)(HANDLE hToken, const void* lpApplicationName, void* lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, ULONG dwCreationFlags, void* lpEnvironment, void* lpCurrentDirectory, void* lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation, HANDLE* hNewToken);

typedef BOOL (*P_CreateProcessWithTokenW)(HANDLE hToken, ULONG dwLogonFlags, const void* lpApplicationName, void* lpCommandLine, ULONG dwCreationFlags, void* lpEnvironment, void* lpCurrentDirectory, void* lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation);

typedef NTSTATUS (*P_RtlCreateProcessParametersEx)(void* ProcessParameters, UNICODE_STRING* ImagePathName, UNICODE_STRING* DllPath, UNICODE_STRING* CurrentDirectory, UNICODE_STRING* CommandLine, void* Environment, UNICODE_STRING* WindowTitle, UNICODE_STRING* DesktopInfo, UNICODE_STRING* ShellInfo, UNICODE_STRING* RuntimeData, void* UnknownParameter11);

typedef void (*P_ExitProcess)(UINT ExitCode);

typedef UINT (*P_WinExec)(LPCSTR lpCmdLine, UINT uCmdShow);

typedef HRESULT (*P_RunSetupCommandW)(HWND hWnd, const WCHAR* szCmdName, const WCHAR* szInfSection, const WCHAR* szDir, const WCHAR* szTitle, HANDLE* phEXE, ULONG dwFlags, void* pvReserved);

typedef BOOL (*P_GetTokenInformation)(_In_ HANDLE TokenHandle, _In_ TOKEN_INFORMATION_CLASS TokenInformationClass, _Out_writes_bytes_to_opt_(TokenInformationLength, *ReturnLength) LPVOID TokenInformation, _In_ DWORD TokenInformationLength, _Out_ PDWORD ReturnLength);
...
static P_RtlCreateProcessParametersEx __sys_RtlCreateProcessParametersEx = NULL;
...
_FX BOOLEAN Proc_Init(void)
{
	...
	if (Dll_OsBuild >= 6000)
	{
		P_RtlCreateProcessParametersEx RtlCreateProcessParametersEx = (P_RtlCreateProcessParametersEx)GetProcAddress(Dll_Ntdll, "RtlCreateProcessParametersEx");
	...
	if (Dll_OsBuild >= 7600)
	{
		status = LdrGetProcedureAddress(Dll_KernelBase, &ansi, 0, (void**)&CreateProcessInternalW);
	}
	else
	{
	...
	{
		status = LdrGetProcedureAddress(Dll_Kernel32, &ansi, 0, (void**)&CreateProcessInternalW);
	}
...
_FX BOOLEAN Proc_Init_AdvApi(HMODULE module)
{
	...
	{
		P_CreateProcessWithTokenW CreateProcessWithTokenW = (P_CreateProcessWithTokenW)GetProcAddress(module, "CreateProcessWithTokenW");
			...
			if (pTokenInfo)
			{
				if (__sys_GetTokenInformation(hToken, TokenDefaultDacl, pTokenInfo, dwTokenInfoLen, &dwTokenInfoLenReturn))
				{
					if (pTokenInfo && pTokenInfo->DefaultDacl)
					{
						...
						{
							if (ERROR_SUCCESS
							    != __sys_SetSecurityInfo(hProcess,
							        SE_KERNEL_OBJECT,
							        DACL_SECURITY_INFORMATION,
							        NULL,
							        NULL,
							        pTokenInfo->DefaultDacl,
							        NULL))
							{
...
_FX BOOL Proc_CreateProcessInternalW(HANDLE hToken, const WCHAR* lpApplicationName, WCHAR* lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, ULONG dwCreationFlags, void* lpEnvironment, void* lpCurrentDirectory, LPSTARTUPINFOW lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation, HANDLE* hNewToken)
{
	...
	if (Proc_AlternateCreateProcess(lpApplicationName, lpCommandLine, lpCurrentDirectory, lpProcessInformation, &ok))
	{
		return ok;
	}
	...
	if (Dll_ImageType == DLL_IMAGE_FLASH_PLAYER_SANDBOX || Dll_ImageType == DLL_IMAGE_ACROBAT_READER || Dll_ImageType == DLL_IMAGE_PLUGIN_CONTAINER)
	{
		hToken = NULL;
	}
	...
	lpEnvironment = File_AllocAndInitEnvironment(lpEnvironment, (dwCreationFlags & CREATE_UNICODE_ENVIRONMENT ? TRUE : FALSE), FALSE, NULL);
	...
	ok = __sys_CreateProcessInternalW(NULL, lpApplicationName, lpCommandLine, NULL, NULL, FALSE, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation, hNewToken);
	...
	if (!Proc_CheckMailer(TlsData->proc_image_path, TlsData->proc_image_is_copy))
	{
		ok  = FALSE;
	...
	Proc_QuoteCommandLine_XP(TlsData, lpCommandLine, TlsData->proc_image_path);
	...
	if ((!lpApplicationName) && TlsData->proc_image_path)
	{
		...
		if (dot)
		{
			...
			if (_wcsicmp(dot, L"bat") == 0 || _wcsicmp(dot, L"cmd") == 0)
			{
				...
				{
					Proc_FixBatchCommandLine(TlsData, lpCommandLine, TlsData->proc_image_path);
	...
	ok = __sys_CreateProcessInternalW(NULL, TlsData->proc_image_path, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation, hNewToken);
	...
	if (SaveOwnerProcess)
	{
		...
		if (sd->Control & SE_SELF_RELATIVE)
		{
			((SECURITY_DESCRIPTOR_RELATIVE*)sd)->Owner = (ULONG)(ULONG_PTR)SaveOwnerProcess;
		}
		else
		{
	...
	if (SaveOwnerThread)
	{
		...
		if (sd->Control & SE_SELF_RELATIVE)
		{
			((SECURITY_DESCRIPTOR_RELATIVE*)sd)->Owner = (ULONG)(ULONG_PTR)SaveOwnerThread;
		}
		else
		{
	...
	if (ok)
	{
		...
		if (ok)
		{
			...
			NTSTATUS status = NtSetInformationProcess(lpProcessInformation->hProcess, ProcessAccessToken, NullToken, sizeof(NullToken));
...
finish:
	...
	if ((!ok) && (err == ERROR_ELEVATION_REQUIRED))
	{
		...
		if (SH32_DoRunAs(lpCommandLine, lpCurrentDirectory, lpProcessInformation, &cancelled))
		{
			err = 0;
			ok  = TRUE;
		}
...
_FX BOOL Proc_CreateProcessInternalW_RS5(HANDLE hToken, const WCHAR* lpApplicationName, WCHAR* lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, ULONG dwCreationFlags, void* lpEnvironment, void* lpCurrentDirectory, LPSTARTUPINFOW lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation, HANDLE* hNewToken)
{
	THREAD_DATA* TlsData = Dll_GetTlsData(NULL);
	...
	ok = __sys_CreateProcessInternalW_RS5(NULL, lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation, hNewToken);
...
_FX BOOL Proc_AlternateCreateProcess(const WCHAR* lpApplicationName, WCHAR* lpCommandLine, void* lpCurrentDirectory, LPPROCESS_INFORMATION lpProcessInformation, BOOL* ReturnValue)
{
	...
	if (Proc_IsSplWow64(lpApplicationName, lpCommandLine, lpProcessInformation))
	{
		return FALSE; // continue with CreateProcessInternal
	}

	if (Proc_IsForcedElevation(lpCommandLine, lpCurrentDirectory, lpProcessInformation))
	{
		*ReturnValue = ((GetLastError() == ERROR_CANCELLED) ? FALSE : TRUE);
		return TRUE; // exit CreateProcessInternal
	}

	if (Dll_ImageType == DLL_IMAGE_SANDBOXIE_DCOMLAUNCH && lpApplicationName && wcsstr(lpApplicationName, L"klwtblfs.exe"))
	{
	...
	if (Dll_ImageType == DLL_IMAGE_SANDBOXIE_DCOMLAUNCH && lpCommandLine && wcsstr(lpCommandLine, L"smartscreen.exe"))
	{
		return TRUE; // exit CreateProcessInternal
	}
	return FALSE; // continue with CreateProcessInternal
...
_FX BOOL Proc_CreateProcessInternalW_Impersonate(HANDLE TokenHandle)
{
	...
	if (TokenHandle)
	{
		...
		status = NtDuplicateToken(TokenHandle, TOKEN_IMPERSONATE | TOKEN_QUERY, &objattrs, FALSE, TokenImpersonation, &NewTokenHandle);
	}
	else
	{
		NewTokenHandle = NULL;
	...
	if (NT_SUCCESS(status))
	{
		status = NtSetInformationThread(NtCurrentThread(), ThreadImpersonationToken, &NewTokenHandle, sizeof(HANDLE));
...
_FX WCHAR* Proc_SelectCurrentDirectory(const WCHAR* lpCurrentDirectory)
{
	...
	FileHandle = CreateFileW(lpCurrentDirectory, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);

	if (FileHandle != INVALID_HANDLE_VALUE)
	{
		BOOLEAN IsBoxedPath;
		NTSTATUS status = SbieDll_GetHandlePath(FileHandle, path, &IsBoxedPath);
...
_FX void Proc_QuoteCommandLine_XP(THREAD_DATA* TlsData, const WCHAR* CommandLine, const WCHAR* ImagePath)
{
#ifndef _WIN64
	...
	while (*CommandPtr)
	{
		if (_wcsnicmp(CommandPtr, ImageName, ImageNameLen) == 0 && (iswspace(CommandPtr[ImageNameLen]) || CommandPtr[ImageNameLen] == L'\0'))
		{
...
_FX void Proc_FixBatchCommandLine(THREAD_DATA* TlsData, const WCHAR* CommandLine, const WCHAR* ImagePath)
{
	WCHAR* ptr;
	WCHAR* cmd;
	ULONG len;
...
_FX void Proc_StoreImagePath(THREAD_DATA* TlsData, HANDLE FileHandle)
{
	...
	NTSTATUS status = SbieDll_GetHandlePath(FileHandle, path, &TlsData->proc_image_is_copy);
...
_FX NTSTATUS Proc_RtlCreateProcessParametersEx(void* ProcessParameters, UNICODE_STRING* ImagePathName, UNICODE_STRING* DllPath, UNICODE_STRING* CurrentDirectory, UNICODE_STRING* CommandLine, void* Environment, UNICODE_STRING* WindowTitle, UNICODE_STRING* DesktopInfo, UNICODE_STRING* ShellInfo, UNICODE_STRING* RuntimeData, void* UnknownParameter11)
{
	...
	if (TlsData->proc_create_process_capture_image)
	{
		HANDLE FileHandle = CreateFileW(ImagePathName->Buffer, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
	...
	else
	{
		...
		status = __sys_RtlCreateProcessParametersEx(ProcessParameters, ImagePathName, DllPath, CurrentDirectory, CommandLine, Environment, WindowTitle, DesktopInfo, ShellInfo, RuntimeData, UnknownParameter11);
	}
...
_FX BOOL Proc_CreateProcessWithTokenW(HANDLE hToken, DWORD dwLogonFlags, LPVOID lpApplicationName, LPVOID lpCommandLine, DWORD dwCreationFlags, LPVOID lpEnvironment, LPVOID lpCurrentDirectory, LPSTARTUPINFOW lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation)
{
	STARTUPINFOW si;
	if (!lpStartupInfo)
	{
	...
	return Proc_CreateProcessInternalW(hToken, lpApplicationName, lpCommandLine, NULL, NULL, FALSE, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation, NULL);
}
...
_FX UINT Proc_WinExec(LPCSTR lpCmdLine, UINT uCmdShow)
{
	...
	ok = CreateProcessA(NULL, (char*)lpCmdLine, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
...
_FX BOOLEAN SbieDll_RunFromHome(const WCHAR* pgmName, const WCHAR* pgmArgs, STARTUPINFOW* si, PROCESS_INFORMATION* pi)
{
	ULONG len, i, err;
	WCHAR *path, *ptr;
	BOOL ok, inherit;
	...
	memzero(pi, sizeof(PROCESS_INFORMATION));
	ok  = CreateProcess(NULL, path, NULL, NULL, inherit, creation_flags, NULL, NULL, si, pi);
	err = GetLastError();
...
_FX BOOLEAN Proc_CheckMailer(const WCHAR* ImagePath, BOOLEAN IsBoxedPath)
{
	...
	if (_wcsicmp(imgName, L"thunderbird.exe") == 0 || _wcsicmp(imgName, L"msimn.exe") == 0 || _wcsicmp(imgName, L"outlook.exe") == 0 || _wcsicmp(imgName, L"winmail.exe") == 0 || _wcsicmp(imgName, L"wlmail.exe") == 0 || _wcsicmp(imgName, L"IncMail.exe") == 0 || _wcsicmp(imgName, L"eudora.exe") == 0 || _wcsicmp(imgName, L"thebat.exe") == 0 || 0)
	{
	...
	if (_wcsicmp(imgName, L"rundll32.exe") == 0 || _wcsicmp(imgName, L"opera.exe") == 0 || _wcsicmp(imgName, L"iexplore.exe") == 0 || _wcsicmp(imgName, L"firefox.exe") == 0 || _wcsicmp(imgName, L"chrome.exe") == 0 || 0)
	{
	...
	else
	{
		...
		while (!ok)
		{
			LONG rc = SbieApi_QueryConfAsIs(NULL, L"OpenFilePath", idx, value, 500 * sizeof(WCHAR));
			++idx;
			if (rc == 0)
			{
				tmp = wcschr(value, L',');
		...
		while (!ok)
		{
			LONG rc = SbieApi_QueryConfAsIs(NULL, L"OpenPipePath", idx, value, 500 * sizeof(WCHAR));
			++idx;
			if (rc == 0)
			{
				tmp = wcschr(value, L',');
...
_FX BOOLEAN Proc_IsSplWow64(const WCHAR* lpApplicationName, const WCHAR* lpCommandLine, LPPROCESS_INFORMATION lpProcessInformation)
{
	//
...
_FX BOOLEAN Proc_Init_AdvPack(HMODULE module)
{
	...
	if (module && Dll_OsBuild >= 6000 && Secure_IsInternetExplorerTabProcess)
	{
		P_RunSetupCommandW RunSetupCommandW = (P_RunSetupCommandW)GetProcAddress(module, "RunSetupCommandW");
		if (RunSetupCommandW)
		{
...
_FX HRESULT Proc_RunSetupCommandW(HWND hWnd, const WCHAR* szCmdName, const WCHAR* szInfSection, const WCHAR* szDir, const WCHAR* szTitle, HANDLE* phEXE, ULONG dwFlags, void* pvReserved)
{
	THREAD_DATA* TlsData = Dll_GetTlsData(NULL);
	HRESULT hr;
	TlsData->proc_create_process_force_elevate = TRUE;
	hr = __sys_RunSetupCommandW(hWnd, szCmdName, szInfSection, szDir, szTitle, phEXE, dwFlags, pvReserved);
...
_FX BOOLEAN Proc_IsForcedElevation(const WCHAR* lpCommandLine, const WCHAR* lpCurrentDirectory, LPPROCESS_INFORMATION lpProcessInformation)
{
	THREAD_DATA* TlsData = Dll_GetTlsData(NULL);
	WCHAR *cmd, *dir;
	ULONG len;
...
{
	RTL_USER_PROCESS_PARAMETERS* ProcessParms = (RTL_USER_PROCESS_PARAMETERS*)
#ifdef _WIN64
...
_FX NTSTATUS Proc_NtSetInformationProcess(HANDLE ProcessHandle, ULONG InfoClass, void* Info, ULONG InfoLength)
{
	NTSTATUS status;
	...
	if (InfoClass == ProcessAccessToken && ProcessHandle == Proc_LastCreatedProcessHandle)
	{
		status = STATUS_SUCCESS;
	}
	else
	{
		status = __sys_NtSetInformationProcess(ProcessHandle, InfoClass, Info, InfoLength);
...
_FX NTSTATUS Proc_NtQueryInformationProcess(HANDLE ProcessHandle, PROCESSINFOCLASS ProcessInformationClass, PVOID ProcessInformation, ULONG ProcessInformationLength, PULONG ReturnLength)
{
	NTSTATUS status;

	status = __sys_NtQueryInformationProcess(ProcessHandle, ProcessInformationClass, ProcessInformation, ProcessInformationLength, ReturnLength);

	if ((ProcessInformationClass == ProcessImageFileName) && (NT_SUCCESS(status)))
	{ // caller requesting image file name?
...
_FX NTSTATUS Proc_NtCreateProcessEx(HANDLE* ProcessHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, HANDLE* ParentProcessHandle, BOOLEAN InheritHandles, HANDLE SectionHandle, HANDLE DebugPort, HANDLE ExceptionPort, ULONG UnknownExtraParameter)
{
	//
...
_FX void Proc_RestartProcessOutOfPcaJob(void)
{
	...
	FileHandle = CreateFile(Directory, FILE_GENERIC_READ, FILE_SHARE_VALID_FLAGS, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
	...
	if (FileHandle != INVALID_HANDLE_VALUE)
	{
		...
		NTSTATUS status       = SbieDll_GetHandlePath(FileHandle, BoxedDirectory, &is_copy);

		if (NT_SUCCESS(status) && SbieDll_TranslateNtToDosPath(BoxedDirectory))
		{
	...
	ok = SbieDll_RunSandboxed(L"*THREAD*", CommandLine, Directory, 0, &StartupInfo, &ProcessInformation);
