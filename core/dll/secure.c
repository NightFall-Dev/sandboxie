...
static NTSTATUS Secure_NtOpenProcess(HANDLE* ProcessHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, CLIENT_ID* ClientId);

static NTSTATUS Secure_NtOpenThread(HANDLE* ThreadHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, CLIENT_ID* ClientId);

static NTSTATUS Secure_NtDuplicateObject(HANDLE SourceProcessHandle, HANDLE SourceHandle, HANDLE TargetProcessHandle, HANDLE* TargetHandle, ACCESS_MASK DesiredAccess, ULONG HandleAttributes, ULONG Options);

static NTSTATUS Secure_NtQuerySecurityObject(HANDLE Handle, SECURITY_INFORMATION SecurityInformation, PSECURITY_DESCRIPTOR SecurityDescriptor, ULONG Length, ULONG* LengthNeeded);

static NTSTATUS Secure_NtSetSecurityObject(HANDLE Handle, SECURITY_INFORMATION SecurityInformation, SECURITY_DESCRIPTOR* SecurityDescriptor);

static NTSTATUS Secure_NtQueryInformationToken(HANDLE TokenHandle, TOKEN_INFORMATION_CLASS TokenInformationClass, void* TokenInformation, ULONG TokenInformationLength, ULONG* ReturnLength);

static NTSTATUS Secure_NtSetInformationToken(HANDLE TokenHandle, TOKEN_INFORMATION_CLASS TokenInformationClass, void* TokenInformation, ULONG TokenInformationLength);

static NTSTATUS Secure_NtAdjustPrivilegesToken(HANDLE TokenHandle, BOOLEAN DisableAllPrivileges, TOKEN_PRIVILEGES* NewState, ULONG BufferLength, TOKEN_PRIVILEGES* PreviousState, ULONG* ReturnLength);
...
{
	typedef (*P_RtlAddMandatoryAce)(PACL Acl, ULONG AceRevision, ULONG AceFlags, PSID Sid, ULONG AceType, ULONG MandatoryPolicy);
	...
	static UCHAR LowLabelSid[12] = {
	    ...
	    0,
	    0,
	    16, // SECURITY_MANDATORY_LABEL_AUTHORITY
	    0x00,
	    0x10 // SECURITY_MANDATORY_LOW_RID
	};

	void* UserSid = NULL;

#define MyAddAccessAllowedAce(dacl, owner) \
	RtlAddAccessAllowedAceEx((dacl), ACL_REVISION, CONTAINER_INHERIT_ACE | OBJECT_INHERIT_ACE | INHERITED_ACE, GENERIC_ALL, (owner))
...
#define MyAllocAndInitSD(sd) \
	...
	pRtlAddMandatoryAce = (P_RtlAddMandatoryAce)GetProcAddress(Dll_Ntdll, "RtlAddMandatoryAce");
	if (pRtlAddMandatoryAce)
	{
		...
		pRtlAddMandatoryAce(MyAcl, ACL_REVISION, 0, LowLabelSid, SYSTEM_MANDATORY_LABEL_ACE_TYPE, SYSTEM_MANDATORY_LABEL_NO_WRITE_UP);
...
_FX BOOLEAN Secure_Init(void)
{
	...
	if (Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER)
	{
		...
		if (CmdLine && wcsstr(CmdLine, L"SCODEF:") && wcsstr(CmdLine, L"CREDAT:"))
		{
	...
	RtlQueryElevationFlags = GetProcAddress(Dll_Ntdll, "RtlQueryElevationFlags");

	if (RtlQueryElevationFlags)
	{
		BOOLEAN ShouldFakeRunningAsAdmin = Dll_ImageType == DLL_IMAGE_SANDBOXIE_SBIESVC || Dll_ImageType == DLL_IMAGE_SANDBOXIE_RPCSS || Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER || (_wcsicmp(Dll_ImageName, L"SynTPEnh.exe") == 0) || (_wcsicmp(Dll_ImageName, L"SynTPHelper.exe") == 0);
		...
		if (ShouldFakeRunningAsAdmin)
		{
			...
			if (Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER)
			{
				if (Secure_IsInternetExplorerTabProcess || (SH_GetInternetExplorerVersion() >= 10) || (Dll_ProcessFlags & SBIE_FLAG_RIGHTS_DROPPED))
				{
...
_FX NTSTATUS Secure_NtOpenProcess(HANDLE* ProcessHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, CLIENT_ID* ClientId)
{
	NTSTATUS status;
	...
	if (ClientId)
	{
		ULONG64 OtherProcessFlags = SbieApi_QueryProcessInfo(ClientId->UniqueProcess, 0);
		...
		if (!OtherProcessFlags)
		{
			...
			{
				DesiredAccess |= STANDARD_RIGHTS_READ | PROCESS_QUERY_INFORMATION | SYNCHRONIZE;
			}

			DesiredAccess &= STANDARD_RIGHTS_READ | SYNCHRONIZE | PROCESS_VM_READ | PROCESS_QUERY_INFORMATION | PROCESS_QUERY_LIMITED_INFORMATION;
		}
	}
	...
	status = __sys_NtOpenProcess(ProcessHandle, DesiredAccess, ObjectAttributes, ClientId);

	if (status == STATUS_ACCESS_DENIED)
	{
		if (DesiredAccess == PROCESS_QUERY_INFORMATION || DesiredAccess == PROCESS_QUERY_LIMITED_INFORMATION)
		{
			//
			...
			return SbieApi_OpenProcess(ProcessHandle, ClientId->UniqueProcess);
		}
...
_FX NTSTATUS Secure_NtOpenThread(HANDLE* ThreadHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, CLIENT_ID* ClientId)
{
	NTSTATUS status;

	status = __sys_NtOpenThread(ThreadHandle, DesiredAccess, ObjectAttributes, ClientId);

	if (status == STATUS_ACCESS_DENIED)
	{
		if (DesiredAccess & MAXIMUM_ALLOWED)
		{
			DesiredAccess |= STANDARD_RIGHTS_READ | THREAD_QUERY_INFORMATION | SYNCHRONIZE;
		}

		DesiredAccess &= STANDARD_RIGHTS_READ | THREAD_GET_CONTEXT | THREAD_QUERY_INFORMATION | SYNCHRONIZE;
...
_FX NTSTATUS Secure_NtDuplicateObject(HANDLE SourceProcessHandle, HANDLE SourceHandle, HANDLE TargetProcessHandle, HANDLE* TargetHandle, ACCESS_MASK DesiredAccess, ULONG HandleAttributes, ULONG Options)
{
	NTSTATUS status;
	...
	if (SourceProcessHandle == NtCurrentProcess() && (!(Options & DUPLICATE_SAME_ACCESS)) && Obj_GetObjectType(SourceHandle) == OBJ_TYPE_PROCESS)
	{
		if (DesiredAccess & (PROCESS_CREATE_THREAD | PROCESS_VM_OPERATION | PROCESS_VM_WRITE))
		{
			HANDLE OtherProcessId     = (HANDLE)(ULONG_PTR)GetProcessId(SourceHandle);
			ULONG64 OtherProcessFlags = SbieApi_QueryProcessInfo(OtherProcessId, 0);
			if (OtherProcessFlags)
			{
			...
			if (!OtherProcessFlags)
			{
				DesiredAccess &= ~(PROCESS_CREATE_THREAD | PROCESS_VM_OPERATION | PROCESS_VM_WRITE);
			}
		}
	}
	...
	status = __sys_NtDuplicateObject(SourceProcessHandle, SourceHandle, TargetProcessHandle, TargetHandle, DesiredAccess, HandleAttributes, Options & ~DUPLICATE_CLOSE_SOURCE);
	...
	if (NT_SUCCESS(status))
	{
		if (Options & DUPLICATE_CLOSE_SOURCE)
		{
			...
			__sys_NtDuplicateObject(SourceProcessHandle, SourceHandle, NULL, NULL, DesiredAccess, HandleAttributes, DUPLICATE_CLOSE_SOURCE);
		}
	...
	else if (status == STATUS_ACCESS_DENIED)
	{
		...
		if (OtherProcessHandle)
		{
			...
			status = SbieApi_DuplicateObject(TargetHandle, OtherProcessHandle, SourceHandle, DesiredAccess, Options);
		}
	}
...
_FX NTSTATUS Secure_NtQuerySecurityObject(HANDLE Handle, SECURITY_INFORMATION SecurityInformation, PSECURITY_DESCRIPTOR SecurityDescriptor, ULONG Length, ULONG* LengthNeeded)
{
	NTSTATUS status;
	HANDLE handle2     = NULL;
	...
	if (IsOpenPath)
	{
		...
		status = __sys_NtQuerySecurityObject(Handle, SecurityInformation, SecurityDescriptor, Length, LengthNeeded);
	...
	status = __sys_NtQuerySecurityObject(handle2, SecurityInformation, SecurityDescriptor, Length, LengthNeeded);

	if (status == STATUS_ACCESS_DENIED)
	{
		const ULONG SACL_INFO = (SACL_SECURITY_INFORMATION | PROTECTED_SACL_SECURITY_INFORMATION | UNPROTECTED_SACL_SECURITY_INFORMATION);
...
_FX NTSTATUS Secure_NtSetSecurityObject(HANDLE Handle, SECURITY_INFORMATION SecurityInformation, SECURITY_DESCRIPTOR* SecurityDescriptor)
{
	NTSTATUS status;
	HANDLE handle2          = NULL;
	BOOLEAN IsOpenPath      = FALSE;
	...
	if (IsOpenPath || IsUnnamedObject)
	{
		...
		status = __sys_NtSetSecurityObject(Handle, SecurityInformation, SecurityDescriptor);
	...
	if (Dll_OsBuild >= 6000 && (SecurityInformation & LABEL_SECURITY_INFORMATION))
	{
		__sys_NtSetSecurityObject(Handle, LABEL_SECURITY_INFORMATION, Secure_EveryoneSD);
...
_FX NTSTATUS Secure_NtQueryInformationToken(HANDLE TokenHandle, TOKEN_INFORMATION_CLASS TokenInformationClass, void* TokenInformation, ULONG TokenInformationLength, ULONG* ReturnLength)
{
	NTSTATUS status      = 0;
	THREAD_DATA* TlsData = NULL;


	status = __sys_NtQueryInformationToken(TokenHandle, TokenInformationClass, TokenInformation, TokenInformationLength, ReturnLength);
...
#include "poppack.h"
			...
			{
				TOKEN_INTEGRITY_LEVEL* Info = (TOKEN_INTEGRITY_LEVEL*)TokenInformation;

				Info->Pointer    = (ULONG_PTR)TokenInformation + sizeof(ULONG_PTR) * 2;
				Info->Sixty      = 0x60;
				Info->OneOhOne   = 0x101;
...
static NTSTATUS Secure_NtSetInformationToken(HANDLE TokenHandle, TOKEN_INFORMATION_CLASS TokenInformationClass, void* TokenInformation, ULONG TokenInformationLength)
{
	NTSTATUS status = __sys_NtSetInformationToken(TokenHandle, TokenInformationClass, TokenInformation, TokenInformationLength);

	if ((!NT_SUCCESS(status)) && (TokenInformationClass == TokenSessionId || TokenInformationClass == TokenIntegrityLevel))
	{
...
_FX NTSTATUS Secure_NtAdjustPrivilegesToken(HANDLE TokenHandle, BOOLEAN DisableAllPrivileges, TOKEN_PRIVILEGES* NewState, ULONG BufferLength, TOKEN_PRIVILEGES* PreviousState, ULONG* ReturnLength)
{
	ULONG status = __sys_NtAdjustPrivilegesToken(TokenHandle, DisableAllPrivileges, NewState, BufferLength, PreviousState, ReturnLength);
...
_FX BOOLEAN Secure_IsRestrictedToken(BOOLEAN CheckThreadToken)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtQueryInformationToken(hToken, TokenRestrictedSids, sids, len, &len);
...
_FX BOOLEAN Secure_IsLocalSystemToken(BOOLEAN CheckThreadToken)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtQueryInformationToken(hToken, TokenUser, user, len, &len);
...
_FX BOOLEAN Secure_IsSameBox(HANDLE idProcess)
{
	WCHAR boxname[48];
	ULONG session_id;
	NTSTATUS status = SbieApi_QueryProcess(idProcess, boxname, NULL, NULL, &session_id);
	if (!NT_SUCCESS(status))
	{
		return FALSE;
	}
...
extern ULONG_PTR __cdecl Secure_Ndr64AsyncClientCall(void* pProxyInfo, ULONG nProcNum, void* pReturnValue, ...);

#else

extern ULONG_PTR __cdecl Secure_NdrAsyncClientCall(void* pStubDescriptor, void* pFormat, ULONG_PTR* Args);

#endif _WIN64

BOOLEAN __cdecl Secure_CheckElevation(void* ReturnAddressFromNdrAsyncClientCall, void* pStubDescriptor, void* pFormat, SECURE_UAC_ARGS* Args);

ULONG_PTR __cdecl Secure_HandleElevation(void* pStubDescriptor, void* pFormat, SECURE_UAC_ARGS* Args);

static ULONG_PTR Secure_RpcAsyncCompleteCall(RPC_ASYNC_STATE* AsyncState, void* Reply);


//---------------------------------------------------------------------------


typedef ULONG_PTR(__cdecl* P_NdrAsyncClientCall)(void* pStubDescriptor, void* pFormat, ULONG_PTR* Args);

typedef ULONG_PTR(__cdecl* P_Ndr64AsyncClientCall)(void* pProxyInfo, ULONG nProcNum, void* pReturnValue, ...);

typedef ULONG_PTR (*P_RpcAsyncCompleteCall)(RPC_ASYNC_STATE* AsyncState, void* Reply);
...
_FX BOOLEAN Secure_Init_Elevation(HMODULE module)
{
	...
	RpcAsyncCompleteCall = (P_RpcAsyncCompleteCall)Ldr_GetProcAddrNew(DllName_rpcrt4, L"RpcAsyncCompleteCall", "RpcAsyncCompleteCall");

#ifdef _WIN64

	Ndr64AsyncClientCall = (P_Ndr64AsyncClientCall)Ldr_GetProcAddrNew(DllName_rpcrt4, L"Ndr64AsyncClientCall", "Ndr64AsyncClientCall");

	SBIEDLL_HOOK(Secure_, Ndr64AsyncClientCall);

#else !_WIN64

	NdrAsyncClientCall = (P_NdrAsyncClientCall)Ldr_GetProcAddrNew(DllName_rpcrt4, L"NdrAsyncClientCall", "NdrAsyncClientCall");
...
ALIGNED BOOLEAN __cdecl Secure_CheckElevation(void* ReturnAddressFromNdrAsyncClientCall, void* pStubDescriptor, void* pFormat, SECURE_UAC_ARGS* Args)
{
	static UCHAR elevation_binding_1[16] = {0x9A, 0xF9, 0x1E, 0x20, 0xA0, 0x7F, 0x4C, 0x44, 0x93, 0x99, 0x19, 0xBA, 0x84, 0xF1, 0x2A, 0x1A};
	static UCHAR elevation_binding_2_Vista[16] = {0x7D, 0xCE, 0x54, 0x5F, 0x79, 0x5B, 0x75, 0x41, 0x85, 0x84, 0xCB, 0x65, 0x31, 0x3A, 0x0E, 0x98};
	static UCHAR elevation_binding_2_Win7[16] = {0x23, 0x05, 0x7A, 0xFD, 0x70, 0xDC, 0xDD, 0x43, 0x9B, 0x2E, 0x9C, 0x5E, 0xD4, 0x82, 0x25, 0xB1};
...
#define IS_BINDING_GUID(n) (0 == memcmp(ptr + OFFSET_OF_BINDING_GUID, elevation_binding_##n, 16))
		...
		else if (IS_BINDING_GUID(2_Vista) || IS_BINDING_GUID(2_Win7))
		{
			...
			for (i = 3; i < 16; ++i)
			{
				if ((ULONG)Args->u.UnknownParameters[i] == (ULONG)-1)
				{
					...
					Secure_Elevation_ResultHandle = (HANDLE*)Args->u.UnknownParameters[i + 1];
...
ALIGNED ULONG_PTR __cdecl Secure_HandleElevation(void* pStubDescriptor, void* pFormat, SECURE_UAC_ARGS* Args)
{
	...
	pkt_len = sizeof(SECURE_UAC_PACKET) + (app_len + cmd_len + dir_len) * sizeof(WCHAR) + sizeof(ULONG);
...
ALIGNED ULONG_PTR Secure_RpcAsyncCompleteCall(RPC_ASYNC_STATE* AsyncState, void* Reply)
{
	...
	*Secure_Elevation_ResultHandle = (HANDLE)(ULONG_PTR)Secure_Elevation_Packet->hResult;
