...
static void Crypt_InitPromptData(COM_CRYPT_PROTECT_DATA_REQ* req, CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct);

static BOOL Crypt_CryptUnprotectData(DATA_BLOB* pDataIn, LPWSTR* ppszDataDescr, DATA_BLOB* pOptionalEntropy, PVOID pvReserved, CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct, DWORD dwFlags, DATA_BLOB* pDataOut);

static BOOL Crypt_CryptProtectData(DATA_BLOB* pDataIn, LPCWSTR szDataDescr, DATA_BLOB* pOptionalEntropy, PVOID pvReserved, CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct, DWORD dwFlags, DATA_BLOB* pDataOut);

static BOOL Crypt_CertGetCertificateChain(ULONG_PTR hChainEngine, ULONG_PTR pCertContext, ULONG_PTR pTime, ULONG_PTR hAdditionalStore, ULONG_PTR pChainPara, ULONG dwFlags, ULONG_PTR pvReserved, ULONG_PTR ppChainContext);
...
typedef BOOL (*P_CryptUnprotectData)(DATA_BLOB* pDataIn, LPWSTR* ppszDataDescr, DATA_BLOB* pOptionalEntropy, PVOID pvReserved, CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct, DWORD dwFlags, DATA_BLOB* pDataOut);

typedef BOOL (*P_CryptProtectData)(DATA_BLOB* pDataIn, LPCWSTR szDataDescr, DATA_BLOB* pOptionalEntropy, PVOID pvReserved, CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct, DWORD dwFlags, DATA_BLOB* pDataOut);

typedef BOOL (*P_CertGetCertificateChain)(ULONG_PTR hChainEngine, ULONG_PTR pCertContext, ULONG_PTR pTime, ULONG_PTR hAdditionalStore, ULONG_PTR pChainPara, ULONG dwFlags, ULONG_PTR pvReserved, ULONG_PTR ppChainContext);
...
_FX void Crypt_InitPromptData(COM_CRYPT_PROTECT_DATA_REQ* req, CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct)
{
	if (pPromptStruct && pPromptStruct->cbSize == sizeof(CRYPTPROTECT_PROMPTSTRUCT))
	{
		...
		{
			ULONG len = (wcslen(pPromptStruct->szPrompt) + 1) * sizeof(WCHAR);
...
_FX BOOL Crypt_CryptUnprotectData(DATA_BLOB* pDataIn, LPWSTR* ppszDataDescr, DATA_BLOB* pOptionalEntropy, PVOID pvReserved, CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct, DWORD dwFlags, DATA_BLOB* pDataOut)
{
	...
	{
		BOOL ok = __sys_CryptUnprotectData(pDataIn, ppszDataDescr, pOptionalEntropy, pvReserved, pPromptStruct, dwFlags, pDataOut);
	...
	req_len = sizeof(COM_CRYPT_PROTECT_DATA_REQ) + pDataIn->cbData + entropy_len;
	...
	rpl = (COM_CRYPT_PROTECT_DATA_RPL*)SbieDll_CallServer((MSG_HEADER*)req);
	Dll_Free(req);
		...
		else
		{
			...
			if (ppszDataDescr)
			{
				...
				*ppszDataDescr  = LocalAlloc(LPTR, descr_len);
				...
				{
					wmemcpy(*ppszDataDescr, (WCHAR*)(rpl->data + rpl->data_len), rpl->descr_len);
...
_FX BOOL Crypt_CryptProtectData(DATA_BLOB* pDataIn, LPCWSTR szDataDescr, DATA_BLOB* pOptionalEntropy, PVOID pvReserved, CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct, DWORD dwFlags, DATA_BLOB* pDataOut)
{
	COM_CRYPT_PROTECT_DATA_REQ* req;
	COM_CRYPT_PROTECT_DATA_RPL* rpl;
	ULONG req_len;
	...
	if (!Crypt_CallSbieSvc)
	{
		BOOL ok = __sys_CryptProtectData(pDataIn, szDataDescr, pOptionalEntropy, pvReserved, pPromptStruct, dwFlags, pDataOut);
		if (ok || GetLastError() != RPC_S_SERVER_UNAVAILABLE)
		{
			return ok;
		}
	...
	req_len = sizeof(COM_CRYPT_PROTECT_DATA_REQ) + pDataIn->cbData + entropy_len + (descr_len + 1) * sizeof(WCHAR);
...
_FX BOOL Crypt_CertGetCertificateChain(ULONG_PTR hChainEngine, ULONG_PTR pCertContext, ULONG_PTR pTime, ULONG_PTR hAdditionalStore, ULONG_PTR pChainPara, ULONG dwFlags, ULONG_PTR pvReserved, ULONG_PTR ppChainContext)
{
	//
	...
	return __sys_CertGetCertificateChain(hChainEngine, pCertContext, pTime, hAdditionalStore, pChainPara, dwFlags, pvReserved, ppChainContext);
}
...
_FX BOOLEAN Crypt_Init(HMODULE module)
{
	...
	CertGetCertificateChain = GetProcAddress(module, "CertGetCertificateChain");

	if ((!CryptProtectData) && (Dll_OsBuild >= 8400) && (Dll_ImageType == DLL_IMAGE_MOZILLA_FIREFOX) && GetModuleHandle(L"UMEngx86.dll"))
	{
