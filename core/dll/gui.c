...
P_CreateDialogIndirectParam __sys_CreateDialogIndirectParamA        = NULL;
P_CreateDialogIndirectParam __sys_CreateDialogIndirectParamW        = NULL;
P_CreateDialogIndirectParamAorW __sys_CreateDialogIndirectParamAorW = NULL;
...
P_DialogBoxIndirectParamAorW __sys_DialogBoxIndirectParamAorW = NULL;

P_LoadString __sys_LoadStringW = NULL;

P_RegisterDeviceNotification __sys_RegisterDeviceNotificationA    = NULL;
P_RegisterDeviceNotification __sys_RegisterDeviceNotificationW    = NULL;
P_UnregisterDeviceNotification __sys_UnregisterDeviceNotification = NULL;
...
static BOOL Gui_UserHandleGrantAccess(HANDLE hUserHandle, HANDLE hJob, BOOL bGrant);

static HWND Gui_CreateDummyParentWindow(void);

static HWND Gui_CreateWindowExA(DWORD dwExStyle, UCHAR* lpClassName, void* lpWindowName, DWORD dwStyle, int x, int y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam);

static HWND Gui_CreateWindowExW(DWORD dwExStyle, void* lpClassName, void* lpWindowName, DWORD dwStyle, int x, int y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam);

static BOOLEAN Gui_CanForwardMsg(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT Gui_DefWindowProcA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT Gui_DefWindowProcW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
static BOOL Gui_MoveWindow(HWND hWnd, int x, int y, int w, int h, BOOL bRepaint);

static BOOL Gui_SetWindowPos(HWND hWnd, HWND hWndInsertAfter, int x, int y, int w, int h, UINT flags);

static int Gui_MessageBoxW(HWND hWnd, LPCWSTR lpText, LPCWSTR lpCaption, UINT uType);

static int Gui_MessageBoxExW(HWND hWnd, LPCWSTR lpText, LPCWSTR lpCaption, UINT uType, WORD wLanguageId);

static BOOL Gui_EndTask(HWND hWnd, BOOL fShutDown, BOOL fForce);

static BOOL Gui_ConsoleControl(ULONG ctlcode, ULONG* data, ULONG_PTR unknown);

static ULONG_PTR Gui_RegisterDeviceNotificationA(HANDLE hRecipient, LPVOID NotificationFilter, DWORD Flags);

static ULONG_PTR Gui_RegisterDeviceNotificationW(HANDLE hRecipient, LPVOID NotificationFilter, DWORD Flags);

static BOOL Gui_UnregisterDeviceNotification(ULONG_PTR Handle);

static int Gui_MapWindowPoints(HWND hWndFrom, HWND hWndTo, LPPOINT lpPoints, UINT cPoints);
...
#define GUI_IMPORT_(base, suffix)                                               \
	{                                                                           \
		...
		__sys_##base##suffix = (P_##base)GetProcAddress(module, #base #suffix); \
...
_FX BOOLEAN Gui_Init3(void)
{
	...
	if (__sys_RegisterDeviceNotificationA == __sys_RegisterDeviceNotificationW)
	{
...
_FX void Gui_InitWindows7(void)
{
	if (Dll_KernelBase)
	{
		...
		for (i = 0; i < 2; ++i)
		{
			...
			if (*(UCHAR*)SourceFunc == 0x48 && *(USHORT*)((UCHAR*)SourceFunc + 1) == 0x25FF)
			{
			...
			*pSourceFunc = (ULONG_PTR)SbieDll_Hook(FuncName, (void*)(*pSourceFunc), DetourFunc);
		}
	}
}
...
//
typedef BOOL(WINAPI* pGetProcessMitigationPolicy)(HANDLE hProcess, PROCESS_MITIGATION_POLICY MitigationPolicy, PVOID lpBuffer, SIZE_T dwLength);
...
_FX BOOLEAN Gui_ConnectToWindowStationAndDesktop(HMODULE User32)
{
	...
	else
	{
		...
		GUI_GET_WINDOW_STATION_RPL* rpl = Gui_CallProxyEx(&req, sizeof(ULONG), sizeof(*rpl), FALSE);
		...
		else
		{
			...
			if (!__sys_SetThreadDesktop)
			{
				...
				__sys_SetThreadDesktop = (P_SetThreadDesktop)GetProcAddress(User32, "SetThreadDesktop");
			}
			...
			else
			{
				...
				memcpy(&SaveDesktopName, &ProcessParms->DesktopName, sizeof(UNICODE_STRING));

				RtlInitUnicodeString(&ProcessParms->DesktopName, rpl->name);
				...
				if (Dll_IsWow64)
				{
					...
					DesktopName64        = (UNICODE_STRING64*)(ProcessParms64 + 0xC0);

					memcpy(&SaveDesktopName64, DesktopName64, sizeof(UNICODE_STRING64));

					DesktopName64->Length        = ProcessParms->DesktopName.Length;
					DesktopName64->MaximumLength = ProcessParms->DesktopName.MaximumLength;
					DesktopName64->Buffer        = (ULONG)ProcessParms->DesktopName.Buffer;
				}
#endif !_WIN64
				...
				{
					rc = __sys_NtSetInformationThread(NtCurrentThread(), ThreadImpersonationToken, &rc, sizeof(rc));
				}
				else
				{
					rc = NtSetInformationThread(NtCurrentThread(), ThreadImpersonationToken, &rc, sizeof(rc));
				}
				...
				else if (!_SetProcessWindowStation((HWINSTA)rpl->hwinsta))
				{
					errlvl = 5;
					rc     = GetLastError();
				}
				...
				memcpy(&ProcessParms->DesktopName, &SaveDesktopName, sizeof(UNICODE_STRING));
#ifndef _WIN64
				if (Dll_IsWow64)
				{
					memcpy(DesktopName64, &SaveDesktopName64, sizeof(UNICODE_STRING64));
				}
#endif !_WIN64
			}
...
_FX BOOL Gui_UserHandleGrantAccess(HANDLE hUserHandle, HANDLE hJob, BOOL bGrant)
{
	SetLastError(ERROR_SUCCESS);
	return TRUE;
}
...
_FX BOOLEAN Gui_IsSameBox(HWND hwnd, ULONG_PTR* out_idProcess, ULONG_PTR* out_idThread)
{
	...
	if ((!idThread) || (!idProcess))
	{
		...
		if (((ULONG_PTR)hwnd & 0xFFFF0000) == 0 && (!out_idProcess) && (!out_idThread))
		{
			//
	...
	status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)idProcess, boxname, NULL, NULL, &session_id);
...
_FX HWND Gui_CreateDummyParentWindow(void)
{
	...
	if (!_atom)
	{
		...
		Sbie_swprintf(clsnm, L"%s-DUMMY-%d-%d", SBIE, Dll_ProcessId, GetTickCount());
	...
	_hwnd = __sys_CreateWindowExW(0, (void*)_atom, L"", 0, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, Dll_Instance, NULL);
	return _hwnd;
}
...
_FX HWND Gui_CreateWindowExW(DWORD dwExStyle, void* lpClassName, void* lpWindowName, DWORD dwStyle, int x, int y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam)
{
...
*/
	...
	if ((!Gui_DisableTitle) && lpWindowName && (dwStyle & WS_CAPTION) == WS_CAPTION && ((!hWndParent) || (dwStyle & WS_CHILD) == 0))
	{
	...
	if (hWndParent && (hWndParent != HWND_MESSAGE) && (!__sys_IsWindow(hWndParent)))
	{
		if (dwStyle & WS_CHILD)
		{
	...
	hwndResult = __sys_CreateWindowExW(dwExStyle, clsnm, new_WindowName, dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);

	if ((!hwndResult) && (clsnm != lpClassName))
	{
		ULONG LastError = GetLastError();
		if (LastError == ERROR_CANNOT_FIND_WND_CLASS || LastError == ERROR_INVALID_WINDOW_HANDLE)
		{
			hwndResult = __sys_CreateWindowExW(dwExStyle, lpClassName, new_WindowName, dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);
		}
	}
...
_FX HWND Gui_CreateWindowExA(DWORD dwExStyle, UCHAR* lpClassName, void* lpWindowName, DWORD dwStyle, int x, int y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam)
{
	THREAD_DATA* TlsData = Dll_GetTlsData(NULL);
	void* new_WindowName;
	UCHAR* clsnm;
	...
	hwndResult = __sys_CreateWindowExA(dwExStyle, clsnm, new_WindowName, dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);
	...
	if ((!hwndResult) && (clsnm != lpClassName))
	{
		...
		if (LastError == ERROR_CANNOT_FIND_WND_CLASS || LastError == ERROR_INVALID_WINDOW_HANDLE)
		{
			hwndResult = __sys_CreateWindowExA(dwExStyle, lpClassName, new_WindowName, dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);
		}
	}
...
_FX BOOLEAN Gui_CanForwardMsg(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == WM_NULL)
	{
		if (wParam == tzuk)
...
_FX LRESULT Gui_WindowProcW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	WNDPROC wndproc;
	LRESULT lResult;
	LPARAM new_lParam;
...
_FX LRESULT Gui_WindowProcA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	WNDPROC wndproc;
	LRESULT lResult;
	LPARAM new_lParam;
...
_FX LRESULT Gui_DefWindowProcW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
_FX LRESULT Gui_DefWindowProcA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LPARAM new_lParam = lParam;
	LRESULT lResult;
...
_FX ULONG_PTR Gui_ActivateKeyboardLayout(ULONG_PTR hkl, UINT flags)
{
	...
	return __sys_DefWindowProcW(__sys_GetFocus(), WM_INPUTLANGCHANGEREQUEST, INPUTLANGCHANGE_SYSCHARSET, hkl);
}
...
_FX BOOL Gui_MoveWindow(HWND hWnd, int x, int y, int w, int h, BOOL bRepaint)
{
...
_FX BOOL Gui_SetWindowPos(HWND hWnd, HWND hWndInsertAfter, int x, int y, int w, int h, UINT flags)
{
	...
	if ((!Gui_DisableTitle) && (flags & SWP_FRAMECHANGED))
	{
		...
		if (InterlockedIncrement(&SetWindowPosDepth) == 1)
		{
			if (Gui_ShouldCreateTitle(hWnd))
			{
				...
				{
					__sys_SendMessageTimeoutW(hWnd, WM_SETTEXT, 0, (LPARAM)title, SMTO_BLOCK | SMTO_ABORTIFHUNG, 1000, &lResult1);
				}
				Dll_Free(title);
			}
...
static BOOL MsgBoxShouldContinue(UINT uType)
{
	...
	if (TlsData && TlsData->gui_should_suppress_msgbox && (uType & 0xF) == MB_OK)
	{
		return FALSE;
	}
	return TRUE;
...
_FX int Gui_MessageBoxW(HWND hWnd, LPCWSTR lpText, LPCWSTR lpCaption, UINT uType)
{
...
_FX int Gui_MessageBoxExW(HWND hWnd, LPCWSTR lpText, LPCWSTR lpCaption, UINT uType, WORD wLanguageId)
{
	if (!MsgBoxShouldContinue(uType))
	{
		return IDOK;
...
{
	LRESULT lResult1;
	LRESULT lResult2 = __sys_SendMessageTimeoutW(hWnd, WM_CLOSE, 0, 0, SMTO_BLOCK | SMTO_ABORTIFHUNG, 1000, &lResult1);
	SetLastError(ERROR_SUCCESS);
	return TRUE;
}
...
_FX ULONG_PTR Gui_RegisterDeviceNotificationA(HANDLE hRecipient, LPVOID NotificationFilter, DWORD Flags)
{
	SetLastError(0);
...
_FX ULONG_PTR Gui_RegisterDeviceNotificationW(HANDLE hRecipient, LPVOID NotificationFilter, DWORD Flags)
{
	SetLastError(0);
	return (ULONG_PTR)0x12345678;
}
...
_FX void Gui_AllowSetForegroundWindow(void)
{
	...
	P_AllowSetForegroundWindow pAllowSetForegroundWindow = (P_AllowSetForegroundWindow)Ldr_GetProcAddrNew(DllName_user32, L"AllowSetForegroundWindow", "AllowSetForegroundWindow");
...
int Gui_MapWindowPoints(HWND hWndFrom, HWND hWndTo, LPPOINT lpPoints, UINT cPoints)
{
	...
	retval = __sys_MapWindowPoints(hWndFrom, hWndTo, lpPoints, cPoints);
	if (retval)
	{
		return retval;
	}
...
_FX void* Gui_CallProxyEx(void* req, ULONG req_len, ULONG rpl_min_len, BOOLEAN msgwait)
{
		...
		if (msgwait)
		{
			...
			if (__sys_MsgWaitForMultipleObjects && __sys_PeekMessageW)
			{
				while (1)
				{
					...
					{
#ifdef _WIN64
						status = __sys_MsgWaitForMultipleObjects(1, &event, FALSE, (10 * 1000), QS_SENDMESSAGE);
#else !_WIN64
						...
						extern Gui_MsgWaitForMultipleObjects(ULONG a, HANDLE * b, ULONG c, ULONG d, ULONG e);
						status = Gui_MsgWaitForMultipleObjects(1, &event, FALSE, (10 * 1000), QS_SENDMESSAGE);
#endif _WIN64
					}

					if (status != 1)
					{ // status 1 means message queue
						if (status == -1)
						{
