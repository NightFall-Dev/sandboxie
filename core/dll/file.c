...
#define FILE_DENIED_ACCESS \
	~(STANDARD_RIGHTS_READ | GENERIC_READ | SYNCHRONIZE | READ_CONTROL | FILE_READ_DATA | FILE_READ_EA | FILE_READ_ATTRIBUTES | GENERIC_EXECUTE | FILE_EXECUTE)

#define DIRECTORY_JUNCTION_ACCESS \
	(GENERIC_ALL | GENERIC_WRITE | MAXIMUM_ALLOWED | FILE_APPEND_DATA | FILE_WRITE_DATA | FILE_WRITE_ATTRIBUTES)
...
#define IS_DELETE_MARK(p_large_integer) \
	((p_large_integer)->HighPart == DELETE_MARK_HIGH && (p_large_integer)->LowPart == DELETE_MARK_LOW)
...
#define OBJECT_ATTRIBUTES_ATTRIBUTES (ObjectAttributes ? ObjectAttributes->Attributes | OBJ_CASE_INSENSITIVE : 0)
...
NTSTATUS File_GetName(HANDLE RootDirectory, UNICODE_STRING* ObjectName, WCHAR** OutTruePath, WCHAR** OutCopyPath, ULONG* OutFlags);

static WCHAR* File_GetName_TranslateSymlinks(THREAD_DATA* TlsData, const WCHAR* objname_buf, ULONG objname_len, BOOLEAN* translated);

static WCHAR* File_GetName_ExpandShortNames(THREAD_DATA* TlsData, WCHAR* Path);

static BOOLEAN File_GetName_ConvertLinks(THREAD_DATA* TlsData, WCHAR** OutTruePath, BOOLEAN ConvertWow64Link);

static void File_GetName_FixTruePrefix(THREAD_DATA* TlsData, WCHAR** OutTruePath, ULONG* InOutLength, ULONG old_prefix_len, const WCHAR* new_prefix, ULONG new_prefix_len);

#ifdef WOW64_FS_REDIR
static ULONG File_GetName_SkipWow64Link(const WCHAR* name);
#endif WOW64_FS_REDIR

static NTSTATUS File_GetName_FromFileId(OBJECT_ATTRIBUTES* ObjectAttributes, WCHAR** OutTruePath, WCHAR** OutCopyPath);
...
static NTSTATUS File_NtOpenFile(HANDLE* FileHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, IO_STATUS_BLOCK* IoStatusBlock, ULONG ShareAccess, ULONG OpenOptions);

static NTSTATUS File_NtCreateFile(HANDLE* FileHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, IO_STATUS_BLOCK* IoStatusBlock, LARGE_INTEGER* AllocationSize, ULONG FileAttributes, ULONG ShareAccess, ULONG CreateDisposition, ULONG CreateOptions, void* EaBuffer, ULONG EaLength);

static NTSTATUS File_NtCreateFileImpl(HANDLE* FileHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, IO_STATUS_BLOCK* IoStatusBlock, LARGE_INTEGER* AllocationSize, ULONG FileAttributes, ULONG ShareAccess, ULONG CreateDisposition, ULONG CreateOptions, void* EaBuffer, ULONG EaLength);

static NTSTATUS File_CheckCreateParameters(ACCESS_MASK DesiredAccess, ULONG CreateDisposition, ULONG CreateOptions, ULONG FileType);

static NTSTATUS File_GetFileType(OBJECT_ATTRIBUTES* ObjectAttributes, BOOLEAN IsWritePath, ULONG* FileType, BOOLEAN* IsEmpty);

static BOOLEAN File_CheckDeletedParent(WCHAR* CopyPath);

static NTSTATUS File_CreatePath(WCHAR* TruePath, WCHAR* CopyPath);

static NTSTATUS File_CreatePath_2(HANDLE* handle, OBJECT_ATTRIBUTES* objattrs, IO_STATUS_BLOCK* IoStatusBlock, ULONG FileAttributes);

static NTSTATUS File_MigrateFile(const WCHAR* TruePath, const WCHAR* CopyPath, BOOLEAN IsWritePath, BOOLEAN WithContents);

static const WCHAR* File_MigrateFile_ShouldBypass(const WCHAR* TruePath);

static NTSTATUS File_CopyShortName(const WCHAR* TruePath, const WCHAR* CopyPath);

static BOOLEAN File_AdjustShortName(const WCHAR* TruePath, const WCHAR* CopyPath, HANDLE FileHandle);
...
static NTSTATUS File_QueryFullAttributesDirectoryFile(const WCHAR* TruePath, FILE_NETWORK_OPEN_INFORMATION* FileInformation);

static ULONG File_CheckDepthForIsWritePath(const WCHAR* TruePath);

static NTSTATUS File_NtQueryAttributesFile(OBJECT_ATTRIBUTES* ObjectAttributes, FILE_BASIC_INFORMATION* FileInformation);

static NTSTATUS File_NtQueryFullAttributesFile(OBJECT_ATTRIBUTES* ObjectAttributes, FILE_NETWORK_OPEN_INFORMATION* FileInformation);

static NTSTATUS File_NtQueryFullAttributesFileImpl(OBJECT_ATTRIBUTES* ObjectAttributes, FILE_NETWORK_OPEN_INFORMATION* FileInformation);

static NTSTATUS File_NtQueryInformationFile(HANDLE FileHandle, IO_STATUS_BLOCK* IoStatusBlock, void* FileInformation, ULONG Length, FILE_INFORMATION_CLASS FileInformationClass);

static ULONG File_GetFinalPathNameByHandleW(HANDLE hFile, WCHAR* lpszFilePath, ULONG cchFilePath, ULONG dwFlags);

static WCHAR* File_GetFinalPathNameByHandleW_2(WCHAR* TruePath, ULONG dwFlags);

static WCHAR* File_GetFinalPathNameByHandleW_3(WCHAR* TruePath, ULONG TruePath_len);

static NTSTATUS File_NtSetInformationFile(HANDLE FileHandle, IO_STATUS_BLOCK* IoStatusBlock, void* FileInformation, ULONG Length, FILE_INFORMATION_CLASS FileInformationClass);

static NTSTATUS File_SetAttributes(HANDLE FileHandle, const WCHAR* CopyPath, FILE_BASIC_INFORMATION* Information);

static NTSTATUS File_SetDisposition(HANDLE FileHandle, IO_STATUS_BLOCK* IoStatusBlock, void* FileInformation, ULONG Length);
...
static NTSTATUS File_RenameFile(HANDLE FileHandle, FILE_RENAME_INFORMATION* info);

static BOOLEAN File_RecordRecover(HANDLE FileHandle, const WCHAR* TruePath);

static NTSTATUS File_SetReparsePoint(HANDLE FileHandle, UCHAR* Data, ULONG DataLen);
...
static P_NtQueryVolumeInformationFile __sys_NtQueryVolumeInformationFile = NULL;
...
_FX NTSTATUS File_GetName(HANDLE RootDirectory, UNICODE_STRING* ObjectName, WCHAR** OutTruePath, WCHAR** OutCopyPath, ULONG* OutFlags)
{
	...
	*OutCopyPath = NULL;
	...
	if (RootDirectory)
	{
		...
		length = 256;
		name   = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, length + objname_len);
		...
		if (uni->Buffer && convert_wow64_link)
		{
			...
			if (length >= sys32len && _wcsnicmp(name, File_Wow64FileLink->src, sys32len) == 0 && (name[sys32len] == L'\\' || name[sys32len] == L'\0'))
			{
	...
	else if (objname_len)
	{
		...
		{
			if (objname_buf[0] == L'\\' && objname_buf[1] == L'?' && objname_buf[2] == L'?' && objname_buf[3] == L'\\' && objname_buf[5] == L':')
			{
		...
		if (drive)
		{
			...
			name = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, objname_len + (drive->len + 1) * sizeof(WCHAR));
		...
		else
		{
			...
			if (drive)
			{
				name = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, objname_len + sizeof(WCHAR));
		...
		else
		{
			...
			*OutTruePath = File_GetName_TranslateSymlinks(TlsData, objname_buf, objname_len, &translated);
			...
			translated |= File_GetName_ConvertLinks(TlsData, OutTruePath, convert_wow64_link);
	...
	if (length < _DeviceLen || _wcsnicmp(*OutTruePath, File_Mup, _DeviceLen) != 0)
	{
		return STATUS_OBJECT_PATH_SYNTAX_BAD;
	}
...
check_sandbox_prefix:
	...
	if (length >= Dll_BoxFilePathLen && 0 == Dll_NlsStrCmp(*OutTruePath, Dll_BoxFilePath, Dll_BoxFilePathLen))
	{
	...
	if (File_AltBoxPath && length >= File_AltBoxPathLen && 0 == Dll_NlsStrCmp(*OutTruePath, File_AltBoxPath, File_AltBoxPathLen))
	{
	...
	if (length >= (_DriveLen - 1) && _wcsnicmp(*OutTruePath, _Drive, _DriveLen - 1) == 0)
	{
		name = (*OutTruePath);
		...
		File_GetName_FixTruePrefix(TlsData, OutTruePath, &length, _DriveLen + 1 /* drive letter */, drive->path, drive->len);
	...
	else if (length >= _UserLen && _wcsnicmp(*OutTruePath, _User, _UserLen) == 0)
	{
		if (File_AllUsersLen && length >= _UserAllLen && _wcsnicmp(*OutTruePath, _UserAll, _UserAllLen) == 0)
		{
			File_GetName_FixTruePrefix(TlsData, OutTruePath, &length, _UserAllLen, File_AllUsers, File_AllUsersLen);
		}
		else if (File_CurrentUserLen && length >= _UserCurrentLen && _wcsnicmp(*OutTruePath, _UserCurrent, _UserCurrentLen) == 0)
		{
			File_GetName_FixTruePrefix(TlsData, OutTruePath, &length, _UserCurrentLen, File_CurrentUser, File_CurrentUserLen);
		}
		else if (File_PublicUserLen && length >= _UserPublicLen && _wcsnicmp(*OutTruePath, _UserPublic, _UserPublicLen) == 0)
		{
			File_GetName_FixTruePrefix(TlsData, OutTruePath, &length, _UserPublicLen, File_PublicUser, File_PublicUserLen);
		}
		else
		{
			//
			...
			name = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, (Dll_BoxFilePathLen + length + 1) * sizeof(WCHAR));
	...
	else if (length >= _ShareLen && _wcsnicmp(*OutTruePath, _Share, _ShareLen) == 0)
	{
		File_GetName_FixTruePrefix(TlsData, OutTruePath, &length, _ShareLen, File_Mup, File_MupLen);
	...
	if (convert_wow64_link)
	{
		...
		if (length >= File_Wow64SysNativeLen && 0 == _wcsnicmp(name, File_Wow64SysNative, File_Wow64SysNativeLen) && (name[File_Wow64SysNativeLen] == L'\\' || name[File_Wow64SysNativeLen] == L'\0') && (!File_GetName_SkipWow64Link(L"")))
		{
			name = *OutTruePath;
			...
			File_GetName_FixTruePrefix(TlsData,
			    ...
			    File_Wow64FileLink->src,
			    File_Wow64FileLink->src_len);
	...
	if (TruePath)
	{
		...
		if (length >= Dll_BoxFilePathLen && 0 == Dll_NlsStrCmp(TruePath, Dll_BoxFilePath, Dll_BoxFilePathLen))
		{
			is_boxed_path = TRUE;
		}
		else if (File_AltBoxPath && length >= File_AltBoxPathLen && 0 == Dll_NlsStrCmp(TruePath, File_AltBoxPath, File_AltBoxPathLen))
		{
			is_boxed_path = TRUE;
		}
		if (is_boxed_path)
		{
			name = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, (length + 1) * sizeof(WCHAR));
	...
	name = Dll_GetTlsNameBuffer(TlsData, COPY_NAME_BUFFER, Dll_BoxFilePathLen + length);
	...
	*OutCopyPath = name;
	...
	else if (length >= File_MupLen && _wcsnicmp(TruePath, File_Mup, File_MupLen) == 0)
	{
	...
	else if (File_AllUsersLen && length >= File_AllUsersLen && 0 == Dll_NlsStrCmp(TruePath, File_AllUsers, File_AllUsersLen))
	{
	...
	else if (File_CurrentUserLen && length >= File_CurrentUserLen && 0 == Dll_NlsStrCmp(TruePath, File_CurrentUser, File_CurrentUserLen))
	{
	...
	else if (File_PublicUserLen && length >= File_PublicUserLen && 0 == Dll_NlsStrCmp(TruePath, File_PublicUser, File_PublicUserLen))
	{
...
_FX WCHAR* File_GetName_TranslateSymlinks(THREAD_DATA* TlsData, const WCHAR* objname_buf, ULONG objname_len, BOOLEAN* translated)
{
	NTSTATUS status;
	HANDLE handle;
	...
	while (1)
	{
		...
		if (status == STATUS_ACCESS_DENIED && objname.Length <= 1020 * sizeof(WCHAR))
		{
			//
			...
			if (NT_SUCCESS(status))
			{
				...
				name = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, path_len + suffix_len + sizeof(WCHAR));
		...
		} while (objname.Length && objname_buf[objname.Length / sizeof(WCHAR)] != L'\\');
...
copy_suffix:
	...
	else
	{
		...
		*name2 = L'\0';
		...
		name2 = Dll_GetTlsNameBuffer(TlsData, COPY_NAME_BUFFER, path_len + suffix_len + sizeof(WCHAR));

		memcpy(name2, name, path_len + suffix_len + sizeof(WCHAR));

		name = File_GetName_TranslateSymlinks(TlsData, name2, path_len + suffix_len, translated);
		if (name)
		{
			*translated = TRUE;
		}
...
_FX WCHAR* File_GetName_ExpandShortNames(THREAD_DATA* TlsData, WCHAR* Path)
{
	NTSTATUS status;
	...
	for (index = 0; Path[index] != 0;)
	{
		...
		status = __sys_NtCreateFile(&handle,
		...
		if (!NT_SUCCESS(status))
		{
			if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_OBJECT_PATH_NOT_FOUND || status == STATUS_NOT_A_DIRECTORY)
			{
				status = STATUS_SUCCESS;
			}
		...
		status = __sys_NtQueryDirectoryFile(handle,
		    NULL,
		    NULL,
...
_FX BOOLEAN File_GetName_ConvertLinks(THREAD_DATA* TlsData, WCHAR** OutTruePath, BOOLEAN ConvertWow64Link)
{
	WCHAR* name;
	ULONG name_len;
	FILE_LINK* link;
	...
	while (link)
	{
		...
		if (name_len >= src_len && (name[src_len] == L'\\' || name[src_len] == L'\0') && _wcsnicmp(name, link->src, src_len) == 0)
		{
#ifdef WOW64_FS_REDIR
			if (link == File_Wow64FileLink)
			{
				ULONG skip = (!ConvertWow64Link) ? 1 : File_GetName_SkipWow64Link(name + src_len);
				if (skip)
				{
					link = List_Next(link);
					continue;
			...
			File_GetName_FixTruePrefix(TlsData, &name, &name_len, src_len, link->dst, link->dst_len);
...
_FX void File_GetName_FixTruePrefix(THREAD_DATA* TlsData, WCHAR** OutTruePath, ULONG* InOutLength, ULONG old_prefix_len, const WCHAR* new_prefix, ULONG new_prefix_len)
{
	//
	...
	WCHAR* nm        = Dll_GetTlsNameBuffer(TlsData, COPY_NAME_BUFFER, len * sizeof(WCHAR));
	wmemcpy(nm, new_prefix, new_prefix_len);
	wmemcpy(nm + new_prefix_len, *OutTruePath + old_prefix_len, suffix_len);

	*OutTruePath = Dll_GetTlsNameBuffer(TlsData, TRUE_NAME_BUFFER, len * sizeof(WCHAR));
...
_FX ULONG File_GetName_SkipWow64Link(const WCHAR* name)
{
	...
	if ((!x) && *name)
	{
		...
		static const WCHAR* _specialcases[] = {L"driverstore", // only on Windows 7
		    L"catroot",
		    L"catroot2",
		    L"drivers\\etc",
		    L"logfiles",
		...
		for (i = 0; _specialcases[i]; ++i)
		{
			...
			if (_wcsnicmp(name, _specialcases[i], n) == 0 && (name[n] == L'\\' || name[n] == L'\0'))
			{
				x = 1;
				break;
			}
...
_FX NTSTATUS File_GetName_FromFileId(OBJECT_ATTRIBUTES* ObjectAttributes, WCHAR** OutTruePath, WCHAR** OutCopyPath)
{
	NTSTATUS status;
	...
	if (1)
	{
		...
		status      = SbieDll_GetHandlePath(ObjectAttributes->RootDirectory, path, &IsBoxedPath);
		if (IsBoxedPath && (NT_SUCCESS(status) || (status == STATUS_BAD_INITIAL_PC)))
		{
			status = SbieDll_GetHandlePath(ObjectAttributes->RootDirectory, path, NULL);
			if (NT_SUCCESS(status))
			{
				HANDLE hTrueRoot;
				...
				status = __sys_NtCreateFile(&hTrueRoot, SYNCHRONIZE | FILE_READ_ATTRIBUTES, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
				...
				if (NT_SUCCESS(status))
				{
					...
					status = __sys_NtCreateFile(&FileHandle, SYNCHRONIZE | FILE_READ_ATTRIBUTES, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_OPEN_BY_FILE_ID | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	...
	if (!FileHandle)
	{
		status = __sys_NtCreateFile(&FileHandle, SYNCHRONIZE | FILE_READ_ATTRIBUTES, ObjectAttributes, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_OPEN_BY_FILE_ID | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
		...
		if (!NT_SUCCESS(status))
		{
			...
			memcpy(&FileId, ObjectAttributes->ObjectName->Buffer, ObjectAttributes->ObjectName->Length);
			...
			InitializeObjectAttributes(&objattrs, &objname, OBJ_CASE_INSENSITIVE, ObjectAttributes->RootDirectory, NULL);
	...
	if (FileHandle)
	{
		status = File_GetName(FileHandle, NULL, OutTruePath, OutCopyPath, NULL);
...
_FX ULONG File_MatchPath2(const WCHAR* path, ULONG* FileFlags, BOOLEAN bCheckObjectExists, BOOLEAN bMonitorLog)
{
	...
	if (File_HomeNtPathLen)
	{
		...
		if (path_len >= File_HomeNtPathLen && (path[File_HomeNtPathLen] == L'\\' || path[File_HomeNtPathLen] == L'\0') && 0 == Dll_NlsStrCmp(path, File_HomeNtPath, File_HomeNtPathLen))
		{
...
_FX NTSTATUS File_NtOpenFile(HANDLE* FileHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, IO_STATUS_BLOCK* IoStatusBlock, ULONG ShareAccess, ULONG OpenOptions)
{
	NTSTATUS status = File_NtCreateFileImpl(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock, NULL, 0, ShareAccess, FILE_OPEN, OpenOptions, NULL, 0);
...
_FX NTSTATUS File_NtCreateFile(HANDLE* FileHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, IO_STATUS_BLOCK* IoStatusBlock, LARGE_INTEGER* AllocationSize, ULONG FileAttributes, ULONG ShareAccess, ULONG CreateDisposition, ULONG CreateOptions, void* EaBuffer, ULONG EaLength)
{
	NTSTATUS status = File_NtCreateFileImpl(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock, AllocationSize, FileAttributes, ShareAccess, CreateDisposition, CreateOptions, EaBuffer, EaLength);
...
_FX NTSTATUS File_NtCreateFileImpl(HANDLE* FileHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, IO_STATUS_BLOCK* IoStatusBlock, LARGE_INTEGER* AllocationSize, ULONG FileAttributes, ULONG ShareAccess, ULONG CreateDisposition, ULONG CreateOptions, void* EaBuffer, ULONG EaLength)
{
	ULONG LastError;
	...
	{
		return __sys_NtCreateFile(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock, AllocationSize, FileAttributes, ShareAccess, CreateDisposition, CreateOptions, EaBuffer, EaLength);
	}
	...
	__try
	{
		...
		InitializeObjectAttributes(&objattrs, &objname, OBJECT_ATTRIBUTES_ATTRIBUTES, NULL, Secure_NormalSD);
		...
		{
			status    = File_GetName_FromFileId(ObjectAttributes, &TruePath, &CopyPath);
			FileFlags = 0;
		...
		{
			status = File_GetName(ObjectAttributes->RootDirectory, ObjectAttributes->ObjectName, &TruePath, &CopyPath, &FileFlags);
		}
	...
	ReparseLoop:
		...
		if (!NT_SUCCESS(status))
		{
			...
			if (status == STATUS_BAD_INITIAL_PC)
			{
				...
				if (pipe_type)
				{
					status = File_NtCreateFilePipe(FileHandle,
					    DesiredAccess,
					    &objattrs,
				...
				else
				{
					...
					status = __sys_NtCreateFile(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock, AllocationSize, FileAttributes, ShareAccess, CreateDisposition, CreateOptions, EaBuffer, EaLength);
		...
		if (TlsData->proc_create_process && Sxs_FileCallback(TruePath, FileHandle))
		{
			__leave;
		}
        ...
        */
			...
			status = __sys_NtCreateFile(FileHandle, DesiredAccess, &objattrs, IoStatusBlock, AllocationSize, FileAttributes, ShareAccess, CreateDisposition, CreateOptions, EaBuffer, EaLength);

			if (status == STATUS_ACCESS_DENIED && (FileFlags & FGN_REPARSED_OPEN_PATH))
			{
				//
				...
				if (ReparsedPath)
				{
					WCHAR* ReparsedPath2 = File_FixPermLinksForMatchPath(ReparsedPath);
					if (ReparsedPath2)
					{
			...
			if (status == STATUS_ACCESS_DENIED && DesiredAccess == MAXIMUM_ALLOWED)
			{
				//
				// if we can't get maximum access, try read-only access
				//

				status = __sys_NtCreateFile(FileHandle, FILE_GENERIC_READ, &objattrs, IoStatusBlock, AllocationSize, FileAttributes, ShareAccess, CreateDisposition, CreateOptions, EaBuffer, EaLength);
			}
		...
		if (Dll_ImageType == DLL_IMAGE_OFFICE_OUTLOOK && wcsstr(TruePath, L"\\OICE_"))
		{
		...
		status = File_CheckCreateParameters(DesiredAccess, CreateDisposition, CreateOptions, -1);
		if (!NT_SUCCESS(status))
		{
			__leave;
		}
		...
		{
			const ULONG DesiredAccess2 = (DesiredAccess & FILE_DENIED_ACCESS) & ~(FILE_WRITE_ATTRIBUTES | DELETE);
			if (DesiredAccess2 || (CreateDisposition != FILE_OPEN && CreateDisposition != FILE_OPEN_IF))
			{
		...
		else if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_OBJECT_PATH_NOT_FOUND)
		{
			...
			if ((FileType & TYPE_REPARSE_POINT) && (CreateOptions & FILE_OPEN_REPARSE_POINT) == 0 && (!AlreadyReparsed))
			{
				//
			...
			if (FileType & TYPE_FILE)
			{
				...
				if (dot)
				{
					static const WCHAR* _ReadOnlyFileTypes = L".url.avi.wma.wmv.mpg.mp3.mp4";
			...
			if (FileType && (CreateDisposition == FILE_OPEN || CreateDisposition == FILE_OPEN_IF))
			{
				//
				...
				if ((DesiredAccess & FILE_DENIED_ACCESS) == FILE_WRITE_ATTRIBUTES)
				{
					//
				...
				if (((DesiredAccess & FILE_DENIED_ACCESS) == DELETE) && ((CreateOptions & FILE_DELETE_ON_CLOSE) == 0))
				{
				...
				if ((DesiredAccess & FILE_DENIED_ACCESS) == 0)
				{
					...
					if (Dll_ImageType != DLL_IMAGE_SANDBOXIE_CRYPTO && Dll_ImageType != DLL_IMAGE_SANDBOXIE_WUAU && Dll_ImageType != DLL_IMAGE_DLLHOST_WININET_CACHE)
					{
						__leave;
					}
		...
		status = File_CheckCreateParameters(DesiredAccess, CreateDisposition, CreateOptions, FileType);

		if (status == STATUS_OBJECT_NAME_NOT_FOUND && (!HaveCopyParent) && (!HaveTrueParent))
		{
			//
		...
		if (status == STATUS_FILE_IS_A_DIRECTORY && TruePathColon && CopyPathColon && (CreateOptions & FILE_NON_DIRECTORY_FILE) && (FileType & TYPE_DIRECTORY))
		{
			//
		...
		if (Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER && (TlsData->file_dont_strip_write_access == 0))
		{
			if ((!DeleteOnClose) && (!HaveCopyFile) && FileType && (CreateDisposition == FILE_OPEN) && PATH_NOT_WRITE(mp_flags))
			{
				RtlInitUnicodeString(&objname, TruePath);
		...
		if ((!HaveCopyFile) && FileType)
		{
			...
			if (CreateDisposition == FILE_OPEN || CreateDisposition == FILE_OPEN_IF || TruePathColon)
			{
				...
				if (FileType & TYPE_FILE)
				{
					...
					if (DeleteOnClose && ((DesiredAccess & FILE_DENIED_ACCESS) == DELETE))
					{
						WithContents = FALSE;
					}
					...
					if (WithContents)
					{
						...
						if (dot && _wcsicmp(dot + 1, L"db") == 0)
						{
							...
							ptr = wcsstr(TempPath, L"\\microsoft\\windows\\explorer\\");
							if (ptr && (wcscmp(ptr + 28, L"thumbcache_") == 0 || wcscmp(ptr + 28, L"iconcache_") == 0))
							{
								WithContents = FALSE;
							}
				...
				status = File_MigrateFile(TruePath, CopyPath, IsWritePath, WithContents);
			...
			{
				status = File_MigrateFile(TruePath, CopyPath, IsWritePath, FALSE);
			}
		...
		if (HaveCopyFile && (FileType & TYPE_DELETED) && (CreateDisposition != FILE_OPEN))
		{
			RtlInitUnicodeString(&objname, CopyPath);
		...
		{
			if ((CreateOptions & FILE_DIRECTORY_FILE) || (FileType & TYPE_DIRECTORY) && (!TruePathColon) && (!CopyPathColon))
			{
		...
		status = __sys_NtCreateFile(FileHandle, DesiredAccess | FILE_READ_ATTRIBUTES, &objattrs, IoStatusBlock, AllocationSize, FileAttributes, ShareAccess, CreateDisposition, CreateOptions, EaBuffer, EaLength);
		...
		if (NT_SUCCESS(status))
		{
			...
			else
			{
				...
				if (NT_SUCCESS(status))
				{
					...
					{
						if (FileType == 0 || IsEmptyCopyFile || (FileType & TYPE_DELETED))
						{
							//
					...
					if ((!IsRecover) && (FileType == 0 || (FileType & TYPE_DELETED)))
					{
						BOOLEAN adjusted = File_AdjustShortName(TruePath, CopyPath, *FileHandle);
						if (adjusted)
						{
							//
							...
							status = __sys_NtCreateFile(FileHandle, DesiredAccess | FILE_READ_ATTRIBUTES, &objattrs, IoStatusBlock, AllocationSize, FileAttributes, ShareAccess, FILE_OPEN, CreateOptions, EaBuffer, EaLength);
...
_FX NTSTATUS File_CheckCreateParameters(ACCESS_MASK DesiredAccess, ULONG CreateDisposition, ULONG CreateOptions, ULONG FileType)
{
	BOOLEAN FileExists;
	...
	if ((CreateOptions & (FILE_SYNCHRONOUS_IO_ALERT | FILE_SYNCHRONOUS_IO_NONALERT)) && ((DesiredAccess & SYNCHRONIZE) == 0))
	{
	...
	if ((CreateOptions & (FILE_DIRECTORY_FILE | FILE_NON_DIRECTORY_FILE)) == (FILE_DIRECTORY_FILE | FILE_NON_DIRECTORY_FILE))
	{
		return STATUS_INVALID_PARAMETER;
	}
	...
	if ((!FileExists) && (CreateDisposition == FILE_OPEN || CreateDisposition == FILE_OVERWRITE))
	{
	...
	if ((FileType & TYPE_DIRECTORY) && (CreateDisposition == FILE_SUPERSEDE || CreateDisposition == FILE_OVERWRITE || CreateDisposition == FILE_OVERWRITE_IF))
	{
	...
	{
		if ((FileType & TYPE_DIRECTORY) && (CreateOptions & FILE_NON_DIRECTORY_FILE))
		{
			return STATUS_FILE_IS_A_DIRECTORY;
		}
		else
	...
	{
		if ((CreateOptions & FILE_DIRECTORY_FILE) && (FileType & TYPE_FILE))
		{
			return STATUS_NOT_A_DIRECTORY;
		}

		else if ((CreateOptions & FILE_NON_DIRECTORY_FILE) && (FileType & TYPE_DIRECTORY))
		{
			return STATUS_FILE_IS_A_DIRECTORY;
		}
	}
	...
	if (FileSystem && (CreateDisposition == FILE_SUPERSEDE || CreateDisposition == FILE_OVERWRITE || CreateDisposition == FILE_OVERWRITE_IF))
	{
		FileReadOnly = TRUE;
	}
	...
	if (FileReadOnly && (CreateDisposition == FILE_OPEN || CreateDisposition == FILE_OPEN_IF))
	{
		//
		...
		const ACCESS_MASK AllowedAccess = FILE_WRITE_ATTRIBUTES | DELETE | ACCESS_SYSTEM_SECURITY | WRITE_OWNER | WRITE_DAC;
	...
	if (FileReadOnly && ((DesiredAccess & FILE_DENIED_ACCESS) || (CreateDisposition == FILE_SUPERSEDE || CreateDisposition == FILE_OVERWRITE || CreateDisposition == FILE_OVERWRITE_IF)))
	{
		if (CreateOptions & FILE_DELETE_ON_CLOSE)
		{
			return STATUS_CANNOT_DELETE;
...
_FX NTSTATUS File_GetFileType(OBJECT_ATTRIBUTES* ObjectAttributes, BOOLEAN IsWritePath, ULONG* FileType, BOOLEAN* IsEmpty)
{
	NTSTATUS status;
	...
	*FileType = 0;
	...
	if (IsWritePath)
	{
		status = File_QueryFullAttributesDirectoryFile(ObjectAttributes->ObjectName->Buffer, &info);
	}
	else
	{
...
_FX NTSTATUS File_CreatePath(WCHAR* TruePath, WCHAR* CopyPath)
{
	...
	while (1)
	{
		...
		*sep     = L'\0';
		...
		status = __sys_NtCreateFile(&handle, FILE_READ_ATTRIBUTES | DELETE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN_IF, FILE_DIRECTORY_FILE, NULL, 0);
		...
		*sep                  = savechar;
		...
		if (NT_SUCCESS(status))
		{
			status = __sys_NtQueryInformationFile(handle, &IoStatusBlock, &basic_info, sizeof(FILE_BASIC_INFORMATION), FileBasicInformation);

			NtClose(handle);

			if (NT_SUCCESS(status) && IS_DELETE_MARK(&basic_info.CreationTime))
			{
				return STATUS_OBJECT_PATH_NOT_FOUND;
			}
	...
	*sep2 = savechar2;
	...
	while (1)
	{
		...
		*sep     = L'\0';
		...
		status = __sys_NtCreateFile(&handle, FILE_READ_ATTRIBUTES | DELETE, &objattrs, &IoStatusBlock, NULL, FILE_ATTRIBUTE_NORMAL, FILE_SHARE_VALID_FLAGS, FILE_OPEN_IF, FILE_DIRECTORY_FILE, NULL, 0);
...
_FX NTSTATUS File_MigrateFile(const WCHAR* TruePath, const WCHAR* CopyPath, BOOLEAN IsWritePath, BOOLEAN WithContents)
{
	NTSTATUS status;
	...
	status = __sys_NtCreateFile(&TrueHandle, FILE_GENERIC_READ, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	...
	if (status == STATUS_SHARING_VIOLATION)
	{
		...
		if (!NT_SUCCESS(status))
		{
			...
			status = __sys_NtCreateFile(&TrueHandle, FILE_READ_ATTRIBUTES | SYNCHRONIZE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
		}
	}
	...
	status = __sys_NtQueryInformationFile(TrueHandle, &IoStatusBlock, &open_info, sizeof(FILE_NETWORK_OPEN_INFORMATION), FileNetworkOpenInformation);
	...
	if (WithContents)
	{
		...
		if (open_info.EndOfFile.HighPart != 0 || file_size > (File_CopyLimitKb * 1024))
		{
			const WCHAR* TruePathName = File_MigrateFile_ShouldBypass(TruePath);
			...
			if (TruePathName)
			{
				...
				if (!File_CopyLimitSilent)
				{
					...
					WCHAR* text           = Dll_AllocTemp((TruePathNameLen + 64) * sizeof(WCHAR));
					Sbie_swprintf(text, L"%s [%s / %d]", TruePathName, Dll_BoxName, file_size);
	...
	status = __sys_NtCreateFile(&CopyHandle, DesiredAccess, &objattrs, &IoStatusBlock, NULL, FILE_ATTRIBUTE_NORMAL, FILE_SHARE_VALID_FLAGS, FILE_CREATE, FILE_SYNCHRONOUS_IO_NONALERT | CreateOptions, NULL, 0);
	...
	if (file_size)
	{
		...
		while (file_size > 0)
		{
			ULONG buffer_size = (file_size > PAGE_SIZE) ? PAGE_SIZE : file_size;

			status = NtReadFile(TrueHandle, NULL, NULL, NULL, &IoStatusBlock, buffer, buffer_size, NULL, NULL);
			...
			if (NT_SUCCESS(status))
			{
				...
				status = NtWriteFile(CopyHandle, NULL, NULL, NULL, &IoStatusBlock, buffer, buffer_size, NULL, NULL);
			}
...
{
	static const WCHAR* _names[] = {// firefox
	    L"places.sqlite",
	    L"xul.mfl",
...
_FX NTSTATUS File_CopyShortName(const WCHAR* TruePath, const WCHAR* CopyPath)
{
	NTSTATUS status;
	HANDLE handle;
	OBJECT_ATTRIBUTES objattrs;
	...
	status = __sys_NtQueryDirectoryFile(handle, NULL, NULL, NULL, &IoStatusBlock, &info.dir, sizeof(info), FileBothDirectoryInformation, TRUE, &objname, FALSE);
	...
	if (status == STATUS_BUFFER_OVERFLOW)
	{
		...
		if (info.dir.NextEntryOffset == tzuk)
		{
			...
			status = __sys_NtQueryDirectoryFile(handle, NULL, NULL, NULL, &IoStatusBlock, dir2, 1024, FileBothDirectoryInformation, TRUE, &objname, FALSE);
	...
	if (NT_SUCCESS(status) && info.dir.ShortNameLength && info.dir.ShortNameLength <= 12 * sizeof(WCHAR))
	{
		...
		if (req)
		{
			...
			memcpy(req->info.FileName, info.dir.ShortName, req->info.FileNameLength);
	...
	if (NT_SUCCESS(status) && info.dir.FileAttributes & FILE_ATTRIBUTE_DIRECTORY)
	{
...
_FX BOOLEAN File_AdjustShortName(const WCHAR* TruePath, const WCHAR* CopyPath, HANDLE FileHandle)
{
	NTSTATUS status;
	HANDLE handle;
	OBJECT_ATTRIBUTES objattrs;
	...
	if (status == STATUS_NOT_A_DIRECTORY)
	{
		status = __sys_NtCreateFile(&handle, FILE_GENERIC_READ, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	}
	...
	if (NT_SUCCESS(status))
	{
		...
		if (req)
		{
			...
			memzero(&req->info, sizeof(req->info));
			Sbie_swprintf(req->info.FileName, L"SB~%05X.%03X", ticks >> 12, ticks & 0xFFF);
...
_FX NTSTATUS File_SetCreateTime(HANDLE FileHandle, const WCHAR* CopyPath)
{
	...
	status = __sys_NtQueryInformationFile(FileHandle, &IoStatusBlock, &info, sizeof(FILE_BASIC_INFORMATION), FileBasicInformation);
...
_FX NTSTATUS File_QueryFullAttributesDirectoryFile(const WCHAR* TruePath, FILE_NETWORK_OPEN_INFORMATION* FileInformation)
{
	...
	if (NT_SUCCESS(status))
	{
		status = __sys_NtQueryInformationFile(FileHandle, &MyIoStatusBlock, FileInformation, sizeof(FILE_BASIC_INFORMATION), FileBasicInformation);
		...
		if (NT_SUCCESS(status))
		{
			...
			const FILE_BASIC_INFORMATION* BasicInfo = (FILE_BASIC_INFORMATION*)FileInformation;
...
_FX NTSTATUS File_NtQueryAttributesFile(OBJECT_ATTRIBUTES* ObjectAttributes, FILE_BASIC_INFORMATION* FileInformation)
{
	NTSTATUS status;
	FILE_NETWORK_OPEN_INFORMATION info;
...
_FX NTSTATUS File_NtQueryFullAttributesFile(OBJECT_ATTRIBUTES* ObjectAttributes, FILE_NETWORK_OPEN_INFORMATION* FileInformation)
{
...
_FX NTSTATUS File_NtQueryFullAttributesFileImpl(OBJECT_ATTRIBUTES* ObjectAttributes, FILE_NETWORK_OPEN_INFORMATION* FileInformation)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	{
		if (ObjectAttributes && ObjectAttributes->ObjectName && ObjectAttributes->ObjectName->Length == 36
		    && ObjectAttributes->ObjectName->Buffer && _wcsicmp(ObjectAttributes->ObjectName->Buffer + 7, L"autorun.inf") == 0)
		{
			return STATUS_OBJECT_NAME_NOT_FOUND;
		}
	}
	...
	__try
	{
		...
		if (PATH_IS_CLOSED(mp_flags))
		{
			...
			status = File_QueryFullAttributesDirectoryFile(TruePath, FileInformation);
	...
	if (FileAttrs != -1                                // if successful
	    ...
	    && (FileFlags & FGN_TRAILING_BACKSLASH))
	{ // but trailing b.s
...
_FX NTSTATUS File_NtQueryInformationFile(HANDLE FileHandle, IO_STATUS_BLOCK* IoStatusBlock, void* FileInformation, ULONG Length, FILE_INFORMATION_CLASS FileInformationClass)
{
	NTSTATUS status;
	ULONG LastError;
	THREAD_DATA* TlsData;
	...
	if (FileInformationClass == FileNetworkPhysicalNameInformation)
	{
		...
		if (TruePath)
		{
			...
			status2 = __sys_NtCreateFile(&FileHandleTrue, FILE_READ_ATTRIBUTES | SYNCHRONIZE, &objattrs, &IoStatusBlock2, NULL, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN, FILE_OPEN_FOR_BACKUP_INTENT | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);

			if (NT_SUCCESS(status2))
			{
				status = __sys_NtQueryInformationFile(FileHandleTrue, IoStatusBlock, FileInformation, Length, FileInformationClass);
	...
	else if (FileInformationClass != FileNameInformation)
	{
		...
		status = __sys_NtQueryInformationFile(FileHandle, IoStatusBlock, FileInformation, Length, FileInformationClass);
		...
		if (FileInformationClass == FileInternalInformation && NT_SUCCESS(status))
		{
			FileId = &((FILE_INTERNAL_INFORMATION*)FileInformation)->IndexNumber;
		}
		else if (FileInformationClass == FileAllInformation && (NT_SUCCESS(status) || status == STATUS_BUFFER_OVERFLOW))
		{
			FileId = &(((FILE_ALL_INFORMATION*)FileInformation)->InternalInformation.IndexNumber);
		}
		...
		if (FileId && FileId->QuadPart)
		{
			BOOLEAN IsBoxedPath;
			WCHAR* path      = Dll_AllocTemp(8192);
			NTSTATUS status2 = SbieDll_GetHandlePath(FileHandle, path, &IsBoxedPath);
			if (IsBoxedPath && (NT_SUCCESS(status2) || (status2 == STATUS_BAD_INITIAL_PC)))
			{
				FileId->LowPart ^= 0xFFFFFFFF;
				FileId->HighPart ^= 0xFFFFFFFF;
			}
	...
	__try
	{
		...
		else
		{
			...
			ULONG dummy_len;
			const FILE_DRIVE* file_drive = File_GetDriveForUncPath(TruePath, wcslen(TruePath), &dummy_len);
...
_FX ULONG File_GetFinalPathNameByHandleW(HANDLE hFile, WCHAR* lpszFilePath, ULONG cchFilePath, ULONG dwFlags)
{
	NTSTATUS status;
	ULONG rc;
	ULONG err;
	...
	if (!IsBoxedPath)
	{
		rc  = __sys_GetFinalPathNameByHandleW(hFile, lpszFilePath, cchFilePath, dwFlags);
		err = GetLastError();
	}
...
_FX WCHAR* File_GetFinalPathNameByHandleW_3(WCHAR* TruePath, ULONG TruePath_len)
{
	const FILE_DRIVE* file_drive;
	const FILE_LINK* file_link;
	WCHAR* path;
	...
	status = __sys_NtCreateFile(&hFile, FILE_READ_ATTRIBUTES | SYNCHRONIZE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	...
	status = __sys_GetFinalPathNameByHandleW(hFile, path, MAX_PATH, 1 /* VOLUME_NAME_GUID */);
...
_FX ULONG File_NtQueryObjectName(UNICODE_STRING* ObjectName, ULONG MaxLen)
{
	...
	if (Len >= Dll_BoxFilePathLen * sizeof(WCHAR) && 0 == Dll_NlsStrCmp(Buf, Dll_BoxFilePath, Dll_BoxFilePathLen))
	{
		ULONG NewPathLen;
		WCHAR* NewPath;
...
_FX NTSTATUS File_NtSetInformationFile(HANDLE FileHandle, IO_STATUS_BLOCK* IoStatusBlock, void* FileInformation, ULONG Length, FILE_INFORMATION_CLASS FileInformationClass)
{
	...
	else if (FileInformationClass == FileDispositionInformation || FileInformationClass == FileDispositionInformationEx)
	{
		...
		{
			status = File_SetDisposition(FileHandle, IoStatusBlock, FileInformation, Length);
		}
	...
	else if ((FileInformationClass == FilePipeInformation || FileInformationClass == FileCompletionInformation || FileInformationClass == FileIoCompletionNotificationInformation) && ((ULONG_PTR)FileHandle & PROXY_PIPE_MASK) == PROXY_PIPE_MASK)
	{
		FillIoStatusBlock = FALSE;

		status = File_SetProxyPipe(FileHandle, IoStatusBlock, FileInformation, Length, FileInformationClass);
	...
	else
	{
		...
		status = __sys_NtSetInformationFile(FileHandle, IoStatusBlock, FileInformation, Length, FileInformationClass);

		if ((FileInformationClass == FileLinkInformation || FileInformationClass == FileHardLinkFullIdInformation) && (!NT_SUCCESS(status)))
		{
			//
...
_FX NTSTATUS File_SetAttributes(HANDLE FileHandle, const WCHAR* CopyPath, FILE_BASIC_INFORMATION* Information)
{
	THREAD_DATA* TlsData = Dll_GetTlsData(NULL);
	...
	status = __sys_NtSetInformationFile(FileHandle, &IoStatusBlock, Information, sizeof(FILE_BASIC_INFORMATION), FileBasicInformation);
	...
	if (Information->CreationTime.QuadPart == 0 && Information->LastAccessTime.QuadPart != 0
	    && Information->LastWriteTime.QuadPart == 0 && Information->ChangeTime.QuadPart == 0 && Information->FileAttributes == 0)
	{
		return STATUS_SUCCESS;
	}
	...
	if (Information->CreationTime.QuadPart == -1 && Information->LastAccessTime.QuadPart == -1
	    && Information->LastWriteTime.QuadPart == -1 && Information->ChangeTime.QuadPart == -1 && Information->FileAttributes == 0)
	{
		return STATUS_SUCCESS;
	}
	...
	__try
	{
		...
		if (!CopyPath)
		{
			...
			status = File_GetName(FileHandle, &objname, &TruePath, (WCHAR**)&CopyPath, NULL);
			...
			status = NtCreateFile(&FileHandle, FILE_WRITE_ATTRIBUTES | SYNCHRONIZE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN_IF, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
			...
			if (status == STATUS_ACCESS_DENIED)
			{
				...
				status2 = File_GetName(FileHandle, &objname, &TruePath, &CopyPath2, NULL);
...
_FX NTSTATUS File_SetDisposition(HANDLE FileHandle, IO_STATUS_BLOCK* IoStatusBlock, void* FileInformation, ULONG Length)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	if (PATH_IS_OPEN(mp_flags))
	{
		status = __sys_NtSetInformationFile(FileHandle, IoStatusBlock, FileInformation, Length, FileDispositionInformation);
	}
	else if (NT_SUCCESS(status))
	{
		OBJECT_ATTRIBUTES objattrs;

		InitializeObjectAttributes(&objattrs, &uni, OBJ_CASE_INSENSITIVE, FileHandle, NULL);
...
_FX NTSTATUS File_NtDeleteFileImpl(OBJECT_ATTRIBUTES* ObjectAttributes)
{
	...
	status = File_NtCreateFileImpl(&handle, DELETE, ObjectAttributes, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_DELETE_ON_CLOSE, NULL, 0);
...
_FX NTSTATUS File_RenameFile(HANDLE FileHandle, FILE_RENAME_INFORMATION* info)
{
	THREAD_DATA* TlsData = Dll_GetTlsData(NULL);
	...
	__try
	{
		...
		status = NtCreateFile(&SourceHandle, FILE_GENERIC_WRITE | DELETE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
		...
		if (status == STATUS_SHARING_VIOLATION || status == STATUS_ACCESS_DENIED)
		{
			...
			status = NtCreateFile(&SourceHandle, SYNCHRONIZE | DELETE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
		...
		status = File_GetName(SourceHandle, &objname, &TruePath, &CopyPath, &SourceFlags);
		...
		status = File_GetName(info->RootDirectory, &objname, &TruePath, &CopyPath, &TargetFlags);
		...
		*TargetFileName = L'\0';
		...
		status = NtCreateFile(&TargetHandle, FILE_GENERIC_WRITE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_DIRECTORY_FILE, NULL, 0);
		...
		else if (status == STATUS_ACCESS_DENIED)
		{
			...
			*TargetCopyPtr = save_char_copy;
			...
			if (status == STATUS_ACCESS_DENIED)
			{
				status = NtCreateFile(&TargetHandle, FILE_GENERIC_READ, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_DIRECTORY_FILE, NULL, 0);
			}
		}
		...
		*TargetFileName = save_char;
		...
		info2_len = sizeof(FILE_RENAME_INFORMATION) + wcslen(TargetFileName) * sizeof(WCHAR);
	...
	issue_rename:
		...
		status = __sys_NtSetInformationFile(SourceHandle, &IoStatusBlock, info2, info2_len, FileRenameInformation);
		...
		status = __sys_NtCreateFile(&TargetHandle, FILE_READ_ATTRIBUTES | SYNCHRONIZE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	...
	after_rename:
		...
		if (SourceFlags & FGN_IS_BOXED_PATH)
		{
			...
			if (NT_SUCCESS(status2))
			{
				...
				status2 = __sys_NtCreateFile(&handle2, FILE_GENERIC_READ, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_CREATE, FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE, NULL, 0);
...
_FX HANDLE File_GetTrueHandle(HANDLE FileHandle, BOOLEAN* pIsOpenPath)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		if (!mp_flags)
		{
			...
			status = __sys_NtCreateFile(&handle, FILE_GENERIC_READ, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
...
_FX ULONG SbieDll_GetHandlePath(HANDLE FileHandle, WCHAR* OutWchar8192, BOOLEAN* IsBoxedPath)
{
	THREAD_DATA* TlsData = Dll_GetTlsData(NULL);
	...
	status = File_GetName(FileHandle, NULL, &TruePath, &CopyPath, &FileFlags);
...
_FX BOOLEAN SbieDll_TranslateNtToDosPath(WCHAR* path)
{
	...
	{ // if not sandboxed
		...
		InitializeCriticalSectionAndSpinCount(File_DrivesAndLinks_CritSec, 1000);
		SbieDll_GetDrivePath(0); // initialize drives
	}
	...
	if (NtPath)
	{
		if (_wcsnicmp(NtPath, Dll_BoxFilePath, Dll_BoxFilePathLen) == 0)
		{
			...
			if (status == STATUS_BAD_INITIAL_PC)
			{
				...
				WCHAR* BackslashPath = Dll_AllocTemp((len + 2) * sizeof(WCHAR));
	...
	if (wParam == DBT_DEVICEARRIVAL || wParam == DBT_DEVICEREMOVECOMPLETE)
	{
		...
		if (hdr->dbch_devicetype == DBT_DEVTYP_VOLUME)
		{
			...
			if (!(vol->dbcv_flags & DBTF_MEDIA))
			{
				...
				if ((wParam != LastWParam) || (ThisDriveMask != LastDriveMask) || ((ThisTickCount - LastTickCount) > 500))
				{
