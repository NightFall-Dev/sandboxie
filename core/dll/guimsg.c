...
static LRESULT Gui_SendMessageA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT Gui_SendMessageW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT Gui_SendMessageTimeoutA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult);

static LRESULT Gui_SendMessageTimeoutW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult);

static LRESULT Gui_SendNotifyMessageA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT Gui_SendNotifyMessageW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT Gui_PostMessageA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT Gui_PostMessageW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

static LRESULT Gui_SendPostMessageCommon(ULONG which, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult);

static LRESULT Gui_SendPostMessageMulti(ULONG which, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult);

static BOOL Gui_SendPostMessageMultiCallback(HWND hWnd, LPARAM lParam);

static LRESULT Gui_SendCopyData(ULONG which, HWND hWnd, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult);

static BOOL Gui_PostThreadMessageA(ULONG idThread, UINT uMsg, WPARAM wParam, LPARAM lParam);

static BOOL Gui_PostThreadMessageW(ULONG idThread, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
static BOOLEAN Gui_DoStatusBarMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT* plResult);

static int Gui_LoadStringW(HINSTANCE hInstance, UINT uID, void* lpBuffer, int nBufferMax);
...
_FX BOOLEAN Gui_InitMsg(void)
{
	...
	if (!Gui_OpenAllWinClasses)
	{
		...
		if (Dll_OsBuild >= 6000)
		{
			__sys_GetProcessIdOfThread = (P_GetProcessIdOfThread)GetProcAddress(Dll_Kernel32, "GetProcessIdOfThread");
...
_FX LRESULT Gui_SendMessageA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lResult;
	...
	lResult = Gui_SendPostMessageCommon('sm a', hWnd, uMsg, wParam, lParam, 0, 0, NULL);
...
_FX LRESULT Gui_SendMessageW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lResult;
	...
	if ((ULONG_PTR)hWnd != XFF4 && (ULONG_PTR)hWnd != XFF8)
	{
		if (uMsg == SB_SETTEXTW)
		{
			BOOL handled = Gui_DoStatusBarMessage(hWnd, uMsg, wParam, lParam, &lResult);
			if (handled)
			{
				return lResult;
			}
	...
	lResult = Gui_SendPostMessageCommon('sm w', hWnd, uMsg, wParam, lParam, 0, 0, NULL);
...
_FX LRESULT Gui_SendMessageTimeoutA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult)
{
	LRESULT lResult;

	lResult = Gui_SendPostMessageCommon('smta', hWnd, uMsg, wParam, lParam, fuFlags, uTimeout, lpdwResult);
...
_FX LRESULT Gui_SendMessageTimeoutW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult)
{
	LRESULT lResult;

	lResult = Gui_SendPostMessageCommon('smtw', hWnd, uMsg, wParam, lParam, fuFlags, uTimeout, lpdwResult);

	if ((ULONG_PTR)hWnd != XFF4 && (ULONG_PTR)hWnd != XFF8)
	{
		if ((!lResult) && Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER && uMsg == WM_NULL && wParam == 0 && lParam == 0 && (fuFlags & SMTO_ABORTIFHUNG))
		{
			//
...
_FX LRESULT Gui_SendNotifyMessageA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	return Gui_SendPostMessageCommon('snma', hWnd, uMsg, wParam, lParam, 0, 0, NULL);
}
...
_FX LRESULT Gui_SendNotifyMessageW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	return Gui_SendPostMessageCommon('snmw', hWnd, uMsg, wParam, lParam, 0, 0, NULL);
}
...
_FX LRESULT Gui_PostMessageA(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lResult;
	...
	{
		lResult = Gui_SendPostMessageCommon('pm a', hWnd, uMsg, wParam, lParam, 0, 0, NULL);
	}
	...
	if ((ULONG_PTR)hWnd != XFF4 && (ULONG_PTR)hWnd != XFF8)
	{
		...
		if ((!lResult) && (uMsg == WM_DROPFILES || uMsg == WM_DDE_ACK || uMsg == WM_DDE_TERMINATE))
		{
			SetLastError(0);
			lResult = 1;
		}
...
_FX LRESULT Gui_PostMessageW(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lResult;
	...
	{
		lResult = Gui_SendPostMessageCommon('pm w', hWnd, uMsg, wParam, lParam, 0, 0, NULL);
	}
...
_FX LRESULT Gui_SendPostMessageCommon(ULONG which, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult)
{
	...
	if ((ULONG_PTR)hWnd == XFF4 || (ULONG_PTR)hWnd == XFF8)
	{
		return Gui_SendPostMessageMulti(which, hWnd, uMsg, wParam, lParam, fuFlags, uTimeout, lpdwResult);
	}
	...
	{
		if (hWnd && (which == 'pm w' || which == 'pm a') && (uMsg >= WM_DDE_FIRST && uMsg <= WM_DDE_LAST))
		{
		...
		{
			return __sys_SendMessageTimeoutW(hWnd, uMsg, wParam, lParam, fuFlags, uTimeout, lpdwResult);
		}
		else if (which == 'smta')
		{
			return __sys_SendMessageTimeoutA(hWnd, uMsg, wParam, lParam, fuFlags, uTimeout, lpdwResult);
		}
	...
	if (uMsg == WM_COPYDATA)
	{
		return Gui_SendCopyData(which, hWnd, wParam, lParam, fuFlags, uTimeout, lpdwResult);
	}
...
_FX LRESULT Gui_SendPostMessageMulti(ULONG which, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult)
{
...
_FX BOOL Gui_SendPostMessageMultiCallback(HWND hWnd, LPARAM lParam)
{
	...
	Gui_SendPostMessageCommon(parm->which, hWnd, parm->uMsg, parm->wParam, parm->lParam, parm->fuFlags, parm->uTimeout, parm->lpdwResult);
...
_FX LRESULT Gui_SendCopyData(ULONG which, HWND hWnd, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout, PDWORD_PTR lpdwResult)
{
	COPYDATASTRUCT* cds;
	...
	if (which != 'sm w' && which != 'sm a' && which != 'smtw' && which != 'smta')
	{
...
_FX BOOL Gui_PostThreadMessageA(ULONG idThread, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
_FX BOOL Gui_PostThreadMessageW(ULONG idThread, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (Gui_PostThreadMessage_Check(idThread, uMsg))
	{
...
_FX BOOLEAN Gui_PostThreadMessage_Check(ULONG idThread, UINT uMsg)
{
	...
	ThreadHandle = OpenThread(THREAD_QUERY_LIMITED_INFORMATION, FALSE, idThread);
	if (!ThreadHandle)
	{
		goto fail;
	}
	...
	status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)ProcessId, name, NULL, NULL, &session_id);
...
fail:
	...
	if (Dll_OsBuild >= 8400 && __sys_GetClipboardFormatNameW((ATOM)uMsg, name, 40) && _wcsicmp(name, L"MSUIM.Msg.LangBarModal") == 0)
	{
		//
...
_FX BOOLEAN Gui_DoStatusBarMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT* plResult)
{
	WCHAR clsnm[32];
...
_FX int Gui_LoadStringW(HINSTANCE hInstance, UINT uID, void* lpBuffer, int nBufferMax)
{
	//
	...
	if (uID == 12940 && nBufferMax >= 0x80 && hInstance == GetModuleHandle(L"mshtml.dll"))
	{
