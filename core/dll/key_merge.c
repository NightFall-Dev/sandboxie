...
static NTSTATUS Key_Merge(HANDLE KeyHandle, const WCHAR* TruePath, const WCHAR* CopyPath, BOOLEAN want_subkeys, BOOLEAN want_values, KEY_MERGE** out_merge);

static NTSTATUS Key_OpenForMerge(HANDLE KeyHandle, const WCHAR* TruePath, const WCHAR* CopyPath, KEY_MERGE** out_TrueMerge, HANDLE* out_CopyHandle);

static BOOLEAN Key_ShouldNotMerge(const WCHAR* TruePath, const WCHAR* CopyPath);

static NTSTATUS Key_MergeCache(HANDLE TrueHandle, LARGE_INTEGER* LastWriteTime, const WCHAR* TruePath, KEY_MERGE** out_TrueMerge);
...
static NTSTATUS Key_MergeSubkeys(KEY_MERGE* merge, KEY_MERGE* TrueMerge, HANDLE CopyHandle);

static NTSTATUS Key_MergeValues(KEY_MERGE* merge, KEY_MERGE* TrueMerge, HANDLE CopyHandle);

static void Key_MergeFree(KEY_MERGE* merge, BOOLEAN FreeMergeItself);

static NTSTATUS Key_GetMergedValue(KEY_MERGE_VALUE* value, KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass, void* KeyValueInformation, ULONG Length, ULONG* ResultLength);

static void Key_DiscardMergeByHandle(THREAD_DATA* TlsData, HANDLE KeyHandle, BOOLEAN Recurse);
...
_FX NTSTATUS Key_Merge(HANDLE KeyHandle, const WCHAR* TruePath, const WCHAR* CopyPath, BOOLEAN want_subkeys, BOOLEAN want_values, KEY_MERGE** out_merge)
{
	NTSTATUS status;
	ULONG ticks_now;
	ULONG TruePath_len;
	...
	while (merge)
	{
		...
		else if (merge->name_len == TruePath_len && _wcsnicmp(merge->name, TruePath, TruePath_len / sizeof(WCHAR)) == 0)
		{
			//
	...
	if ((want_subkeys && (!merge->subkeys_merged)) || (want_values && (!merge->values_merged)))
	{
		status = Key_OpenForMerge(KeyHandle, TruePath, CopyPath, &TrueMerge, &CopyHandle);
...
_FX NTSTATUS Key_OpenForMerge(HANDLE KeyHandle, const WCHAR* TruePath, const WCHAR* CopyPath, KEY_MERGE** out_TrueMerge, HANDLE* out_CopyHandle)
{
	NTSTATUS status;
	...
	*out_CopyHandle = NULL;
	...
	if (NT_SUCCESS(status))
	{
		...
		status = __sys_NtQueryKey(*out_CopyHandle, KeyBasicInformation, &info, sizeof(KEY_BASIC_INFORMATION), &len);
	...
	else if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_OBJECT_PATH_NOT_FOUND || status == STATUS_ACCESS_DENIED)
	{
		//
	...
	{
		status = __sys_NtOpenKey(&TrueHandle, Key_GetWow64Flag(TruePath, KEY_READ), &objattrs);
	}
	...
	if (NT_SUCCESS(status))
	{
		...
		status = __sys_NtQueryKey(TrueHandle, KeyBasicInformation, &info, sizeof(KEY_BASIC_INFORMATION), &len);

		if (NT_SUCCESS(status) || status == STATUS_BUFFER_OVERFLOW)
		{
			status = Key_MergeCache(TrueHandle, &info.LastWriteTime, TruePath, out_TrueMerge);
		}
...
_FX BOOLEAN Key_ShouldNotMerge(const WCHAR* TruePath, const WCHAR* CopyPath)
{
	...
	static const WCHAR* _Domains      = L"Software\\Microsoft\\Windows\\CurrentVersion"
	...
	ptr = TruePath;
	if (_wcsnicmp(ptr, Key_System, 17) == 0)
	{ // \registry\machine
		ptr += 17 + 1;
		hklm = TRUE;
	}
	...
	if ((hklm && HaveHklmDomainsKey == (BOOLEAN)-1) || ((!hklm) && HaveHkcuDomainsKey == (BOOLEAN)-1))
	{
		WCHAR* ptr;
		MSG_HEADER* rpl;
		...
		req_len = sizeof(FILE_CHECK_KEY_EXISTS_REQ) + path_len * sizeof(WCHAR);
		...
		*ptr = L'\\';
		...
		if (rpl)
		{
			if (rpl->status == STATUS_OBJECT_NAME_NOT_FOUND || rpl->status == STATUS_OBJECT_PATH_NOT_FOUND)
			{
				if (hklm)
				{
	...
	if ((hklm && HaveHklmDomainsKey == FALSE) || ((!hklm) && HaveHkcuDomainsKey == FALSE))
	{
		return TRUE;
	}
...
_FX NTSTATUS Key_MergeCache(HANDLE TrueHandle, LARGE_INTEGER* LastWriteTime, const WCHAR* TruePath, KEY_MERGE** out_TrueMerge)
{
	NTSTATUS status;
	ULONG ticks_now;
	ULONG TruePath_len;
...
_FX NTSTATUS Key_MergeCacheSubkeys(KEY_MERGE* merge, HANDLE TrueHandle)
{
	...
	while (1)
	{
		status = __sys_NtEnumerateKey(TrueHandle, index, KeyNodeInformation, info, info_len, &len);
		...
		subkey->TitleOrClass = (info->TitleIndex || info->ClassOffset != -1 || info->ClassLength);
...
_FX NTSTATUS Key_MergeCacheValues(KEY_MERGE* merge, HANDLE TrueHandle)
{
	...
	while (1)
	{
		status = __sys_NtEnumerateValueKey(TrueHandle, index, KeyValueFullInformation, info, info_len, &len);
		...
		len   = sizeof(KEY_MERGE_VALUE) + info->NameLength + sizeof(WCHAR) + info->DataLength;
		value = Dll_Alloc(len);
		...
		value->data_ptr  = (UCHAR*)value->name + value->name_len + sizeof(WCHAR);
		value->data_len  = info->DataLength;
		memcpy(value->data_ptr, (UCHAR*)info + info->DataOffset, info->DataLength);
...
_FX NTSTATUS Key_MergeSubkeys(KEY_MERGE* merge, KEY_MERGE* TrueMerge, HANDLE CopyHandle)
{
	NTSTATUS status;
	ULONG info_len;
	ULONG len;
	...
	status = __sys_NtQueryKey(CopyHandle, KeyBasicInformation, info, info_len, &len);
...
TrueHandleFinish:;
	...
	while (1)
	{
		status = __sys_NtEnumerateKey(CopyHandle, index, KeyNodeInformation, info, info_len, &len);
...
_FX NTSTATUS Key_MergeValues(KEY_MERGE* merge, KEY_MERGE* TrueMerge, HANDLE CopyHandle)
{
	NTSTATUS status;
	ULONG info_len;
	ULONG len;
	...
	while (value2)
	{
		len   = sizeof(KEY_MERGE_VALUE) + value2->name_len + sizeof(WCHAR) + value2->data_len;
		value = Dll_Alloc(len);
...
TrueHandleFinish:;
	...
	while (1)
	{
		status = __sys_NtEnumerateValueKey(CopyHandle, index, KeyValueFullInformation, info, info_len, &len);
		...
		value->data_type = info->Type;
		value->data_ptr  = (UCHAR*)value->name + info->NameLength + sizeof(WCHAR);
		value->data_len  = info->DataLength;
		memcpy(value->data_ptr, (UCHAR*)info + info->DataOffset, value->data_len);
...
_FX void Key_MergeFree(KEY_MERGE* merge, BOOLEAN FreeMergeItself)
{
	while (1)
	{
...
_FX NTSTATUS Key_GetMergedValue(KEY_MERGE_VALUE* value, KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass, void* KeyValueInformation, ULONG Length, ULONG* ResultLength)
{
	NTSTATUS status;
	...
	__try
	{
		...
		{
			info_min_len = FIELD_OFFSET(KEY_VALUE_PARTIAL_INFORMATION_ALIGN64, Data);
			info_max_len = info_min_len + value->data_len;
		}
		else
		{
		...
		*(ULONG*)info_buf = 0; // reset TitleIndex
		...
		if (KeyValueInformationClass == KeyValueBasicInformation)
		{
			KEY_VALUE_BASIC_INFORMATION* info = (KEY_VALUE_BASIC_INFORMATION*)info_buf;
		...
		{
			KEY_VALUE_FULL_INFORMATION* info = (KEY_VALUE_FULL_INFORMATION*)info_buf;

			info->Type       = value->data_type;
			info->DataOffset = FIELD_OFFSET(KEY_VALUE_FULL_INFORMATION, Name) + value->name_len;
			info->DataLength = value->data_len;
			info->NameLength = value->name_len;
			memcpy(info->Name, value->name, value->name_len);
			memcpy(info_buf + info->DataOffset, value->data_ptr, value->data_len);
		}
		else if (KeyValueInformationClass == KeyValuePartialInformation)
		{
		...
		{
			KEY_VALUE_PARTIAL_INFORMATION_ALIGN64* info = (KEY_VALUE_PARTIAL_INFORMATION_ALIGN64*)info_buf;
...
_FX void Key_DiscardMergeByPath(const WCHAR* TruePath, BOOLEAN Recurse)
{
	...
	while (merge)
	{
		...
		if (merge->name_len == TruePath_len && _wcsnicmp(merge->name, TruePath, TruePath_len / sizeof(WCHAR)) == 0)
		{
			if (Recurse)
			{
...
_FX void Key_DiscardMergeByHandle(THREAD_DATA* TlsData, HANDLE KeyHandle, BOOLEAN Recurse)
{
	NTSTATUS status;
	UNICODE_STRING objname;
	WCHAR* TruePath;
