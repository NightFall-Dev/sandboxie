...
static WCHAR* SbieDll_AssocQueryCommandInternal(const WCHAR* subj, const WCHAR* verb);

static ULONG SH32_SHChangeNotifyRegister(HWND hwnd, int fSources, LONG fEvents, UINT wMsg, int cEntries, SHChangeNotifyEntry* pfsne);

static ULONG SH32_SHOpenFolderAndSelectItems(LPCITEMIDLIST pidlFolder, UINT cidl, void* apidl, ULONG dwFlags);

static BOOLEAN SH32_SHOpenFolderAndSelectItems_2(LPCITEMIDLIST pidlParent, LPCITEMIDLIST pidlChild, WCHAR* path);

static ULONG SH32_GetPrivateProfileStringW(void* lpAppName, void* lpKeyName, void* lpDefault, void* lpReturnedString, ULONG nSize, void* lpFileName);

static HRESULT SH32_SHGetFolderLocation(HWND hwndOwner, int nFolder, HANDLE hToken, DWORD dwReserved, void** ppidl);

static NTSTATUS SH32_LdrGetDllHandleEx(ULONG_PTR One, ULONG_PTR Unknown1, ULONG_PTR Unknown2, UNICODE_STRING* DllName, ULONG_PTR* hModule);
...
typedef BOOL (*P_ShellExecuteEx)(void* lpExecInfo);

typedef ULONG (*P_SHChangeNotifyRegister)(HWND hwnd, int fSources, LONG fEvents, UINT wMsg, int cEntries, SHChangeNotifyEntry* pfsne);

typedef ULONG (*P_SHOpenFolderAndSelectItems)(LPCITEMIDLIST pidlFolder, UINT cidl, void* apidl, ULONG dwFlags);

typedef BOOL (*P_SHGetPathFromIDList)(LPCITEMIDLIST pidl, void* pszPath);

typedef ULONG (*P_GetPrivateProfileString)(void* lpAppName, void* lpKeyName, void* lpDefault, void* lpReturnedString, ULONG nSize, void* lpFileName);

typedef HRESULT (*P_SHGetFolderLocation)(HWND hwndOwner, int nFolder, HANDLE hToken, DWORD dwReserved, void** ppidl);
...
static P_SHOpenFolderAndSelectItems __sys_SHOpenFolderAndSelectItems = NULL;
...
_FX BOOLEAN SbieDll_IsDirectory(const WCHAR* PathW)
{
	...
	if (len2 == 2 || len2 == 3)
	{
		...
		WCHAR ch1 = PathW[1];
		WCHAR ch2 = PathW[2];
		if (ch0 >= L'a' && ch0 <= L'z' && ch1 == L':' && (ch2 == L'\\' || ch2 == L'\0'))
		{
			return TRUE;
		}
	}
	...
	else if (status == STATUS_SUCCESS && (openinfo.FileAttributes & FILE_ATTRIBUTE_DIRECTORY))
	{
		isdir = TRUE;
	}
...
_FX BOOL SH32_ShellExecuteExW(SHELLEXECUTEINFOW* lpExecInfo)
{
	...
	if ((!verb) || (!verb[0]) || is_explore_verb || _wcsicmp(verb, L"open") == 0)
	{
		if (lpExecInfo->fMask & SEE_MASK_IDLIST)
		{
			path     = HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, (MAX_PATH + 8) * sizeof(WCHAR));
			FreePath = TRUE;
	...
	if (CallSystem)
	{
		...
		if (!b)
		{
			...
			if ((err == ERROR_FILE_NOT_FOUND) && FreePath && pArgs && (!lpExecInfo->lpParameters))
			{
	...
	{
		b = SH_OpenFolder(path, (is_explore_verb ? L'e' : 0), &lpExecInfo->hProcess);
...
_FX ULONG SH32_SHChangeNotifyRegister(HWND hwnd, int fSources, LONG fEvents, UINT wMsg, int cEntries, SHChangeNotifyEntry* pfsne)
{
	ULONG rc;
	...
	rc = __sys_SHChangeNotifyRegister(hwnd, fSources, fEvents, wMsg, cEntries, pfsne);
...
_FX ULONG SH32_SHOpenFolderAndSelectItems(LPCITEMIDLIST pidlFolder, UINT cidl, void* apidl, ULONG dwFlags)
{
	HRESULT hr = __sys_SHOpenFolderAndSelectItems(pidlFolder, cidl, apidl, dwFlags);
...
_FX BOOLEAN SH32_SHOpenFolderAndSelectItems_2(LPCITEMIDLIST pidlParent, LPCITEMIDLIST pidlChild, WCHAR* path)
{
	typedef BOOL (*P_ILIsParent)(LPCITEMIDLIST pidl1, LPCITEMIDLIST pidl2, BOOL fImmediate);
	typedef LPITEMIDLIST (*P_ILCombine)(LPCITEMIDLIST pidl1, LPCITEMIDLIST pid2);
	typedef void (*P_SHFree)(void* pv);

	BOOLEAN result = FALSE;

	P_ILIsParent pILIsParent = (P_ILIsParent)Ldr_GetProcAddrNew(DllName_shell32, L"ILIsParent", "ILIsParent");

	P_ILCombine pILCombine = (P_ILCombine)Ldr_GetProcAddrNew(DllName_shell32, L"ILCombine", "ILCombine");

	P_SHFree pSHFree = (P_SHFree)Ldr_GetProcAddrNew(DllName_shell32, L"SHFree", "SHFree");

	if (pILIsParent && pILCombine && pSHFree)
	{
		LPCITEMIDLIST pidl;
		if (pidlParent && pidlChild && pILIsParent(pidlParent, pidlChild, FALSE))
		{
...
_FX BOOL SH32_DoRunAs(const WCHAR* CmdLine, const WCHAR* WorkDir, PROCESS_INFORMATION* pi, BOOL* cancelled)
{
	THREAD_DATA* TlsData = Dll_GetTlsData(NULL);
	WCHAR* arg;
	...
	if (shell32)
	{
		...
		if (pShellExecuteEx)
		{
			if (pShellExecuteEx(&shex) && ((int)(ULONG_PTR)shex.hInstApp > 32))
			{
...
_FX ULONG SH32_GetPrivateProfileStringW(void* lpAppName, void* lpKeyName, void* lpDefault, void* lpReturnedString, ULONG nSize, void* lpFileName)
{
	ULONG rv = __sys_GetPrivateProfileStringW(lpAppName, lpKeyName, lpDefault, lpReturnedString, nSize, lpFileName);
	...
	if (rv == 12)
	{
		...
		if (AppNameW && 0 == _wcsicmp(AppNameW, L"boot") && KeyNameW && 0 == _wcsicmp(KeyNameW, L"shell") && FileNameW && 0 == _wcsicmp(FileNameW, L"system.ini") && RetStrW && 0 == _wcsicmp(RetStrW, L"explorer.exe"))
		{
			*(WCHAR*)(RetStrW + 0) = L'x';
			*(WCHAR*)(RetStrW + 1) = L'\0';
			rv                     = 1;
...
_FX HRESULT SH32_SHGetFolderLocation(HWND hwndOwner, int nFolder, HANDLE hToken, DWORD dwReserved, void** ppidl)
{
	HRESULT hr = __sys_SHGetFolderLocation(hwndOwner, nFolder, hToken, dwReserved, ppidl);
	...
	if (hr == 0x80070002 && nFolder == CSIDL_PERSONAL)
	{
		...
		hr = __sys_SHGetFolderLocation(hwndOwner, nFolder | CSIDL_FLAG_CREATE, hToken, dwReserved, ppidl);
	}
...
_FX NTSTATUS SH32_LdrGetDllHandleEx(ULONG_PTR One, ULONG_PTR Unknown1, ULONG_PTR Unknown2, UNICODE_STRING* DllName, ULONG_PTR* hModule)
{
	//
	...
	if (DllName && DllName->Buffer)
	{
		...
		if (DllName->Length == len * sizeof(WCHAR) && _wcsnicmp(DllName->Buffer, _DllName, len) == 0)
		{
			*hModule = 0;
	...
	return __sys_LdrGetDllHandleEx(One, Unknown1, Unknown2, DllName, hModule);
}
...
_FX BOOLEAN SH32_Init(HMODULE module)
{
	...
	__sys_SHGetPathFromIDListW = (P_SHGetPathFromIDList)GetProcAddress(module, "SHGetPathFromIDListW");

	//
	// intercept entry points
	//

	ShellExecuteExW = (P_ShellExecuteEx)GetProcAddress(module, "ShellExecuteExW");

	SHChangeNotifyRegister = (P_SHChangeNotifyRegister)GetProcAddress(module, "SHChangeNotifyRegister");

	SHGetItemFromObject = GetProcAddress(module, "SHGetItemFromObject");

	SHOpenFolderAndSelectItems = (P_SHOpenFolderAndSelectItems)GetProcAddress(module, "SHOpenFolderAndSelectItems");
	...
	if (Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER || Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER)
	{
		WCHAR buf[8];
		SbieApi_QueryConfAsIs(NULL, L"NoAutoExitExplorer", 0, buf, sizeof(buf));
		if (!buf[0])
		{
	...
	if (Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER)
	{
		...
		{
			__sys_LdrGetDllHandleEx = (P_LdrGetDllHandleEx)GetProcAddress(Dll_Ntdll, "LdrGetDllHandleEx");
		}

		if (__sys_LdrGetDllHandleEx)
		{
			*(ULONG_PTR*)&__sys_LdrGetDllHandleEx = (ULONG_PTR)SbieDll_Hook("LdrGetDllHandleEx", __sys_LdrGetDllHandleEx, SH32_LdrGetDllHandleEx);
		}
		...
		if (1)
		{
			...
			GetPrivateProfileStringW = (P_GetPrivateProfileString)GetProcAddress(Dll_Kernel32, "GetPrivateProfileStringW");
		...
		if (Dll_OsBuild >= 6000)
		{
			Key_DeleteValueFromCLSID(L"clsid", L"ceff45ee-c862-41de-aee2-a022c81eda92", L"AppId");
		}
		...
		if (Dll_OsBuild >= 7600)
		{
			...
			SHGetFolderLocation = (P_SHGetFolderLocation)GetProcAddress(module, "SHGetFolderLocation");
...
_FX WCHAR* SbieDll_AssocQueryCommandInternal(const WCHAR* subj, const WCHAR* verb)
{
	NTSTATUS status;
	OBJECT_ATTRIBUTES objattrs;
	UNICODE_STRING objname;
...
retry:
	...
	InitializeObjectAttributes(&objattrs, &objname, OBJ_CASE_INSENSITIVE, hkey, NULL);
	...
	if (!NT_SUCCESS(status))
	{
		...
		status = NtQueryValueKey(hkey, &objname, KeyValuePartialInformation, kvpi, kvpi_len - 32, &len);
	...
	if ((!NT_SUCCESS(status)) || kvpi->DataLength == 0 || (kvpi->Type != REG_SZ && kvpi->Type != REG_EXPAND_SZ))
	{
		goto finish;
	}
...
_FX ULONG SH_GetInternetExplorerVersion(void)
{
	...
	RtlInitUnicodeString(&objname, L"\\REGISTRY\\MACHINE\\SOFTWARE\\Microsoft\\Internet Explorer");
	InitializeObjectAttributes(&objattrs, &objname, OBJ_CASE_INSENSITIVE, NULL, NULL);
	...
	if (NT_SUCCESS(status))
	{
		ULONG len;
		ULONG kvpi_space[64];
		KEY_VALUE_PARTIAL_INFORMATION* kvpi = (KEY_VALUE_PARTIAL_INFORMATION*)&kvpi_space;

		RtlInitUnicodeString(&objname, L"svcVersion"); // for IE 10
		memzero(kvpi_space, sizeof(kvpi_space));

		status = NtQueryValueKey(handle, &objname, KeyValuePartialInformation, kvpi, sizeof(kvpi_space) - 4, &len);
...
_FX ULONG SH_WindowMonitorThread(void* lpParameter)
{
	...
	const ULONG timeout = (Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER) ? 2500 : 5000;
...
_FX ULONG SH_WindowMonitorCount(void)
{
	...
	while (1)
	{
		...
		else if (Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER && Dll_ProcessId == (ULONG_PTR)info_ptr->InheritedFromProcessId)
		{
			WCHAR image[112];
			status = SbieApi_QueryProcess(info_ptr->UniqueProcessId, NULL, image, NULL, NULL);
...
_FX BOOL SH_WindowMonitorEnum(HWND hWnd, LPARAM lParam)
{
	...
	if (Gui_IsWindowVisible(hWnd))
	{
		...
		while (1)
		{
			if (info_ptr->HandleCount && (ULONG)(ULONG_PTR)info_ptr->UniqueProcessId == pid)
			{
				++args[1];
			}
...
typedef void (*P_RouteTheCall)(void* Unknown1, HINSTANCE hInstance, const UCHAR* Path, ULONG Unknown2);


static ULONG_PTR __sys_RouteTheCall = 0;


_FX void SH32_RouteTheCall(void* Unknown1, HINSTANCE hInstance, const UCHAR* Path, ULONG Unknown2)
{
	ANSI_STRING ansi;
	UNICODE_STRING uni;
	WCHAR *iptr, *optr;
...
typedef HRESULT (*P_SetWindowThemeAttribute)(HWND hwnd, ULONG eAttribute, void* pvAttribute, ULONG cbAttribute);


static ULONG_PTR __sys_SetWindowThemeAttribute = 0;


_FX HRESULT SH32_SetWindowThemeAttribute(HWND hwnd, ULONG eAttribute, void* pvAttribute, ULONG cbAttribute)
{
	return S_OK;
}
...
_FX BOOLEAN SH32_Init_UxTheme(HMODULE module)
{
	if (Dll_ImageType == DLL_IMAGE_SHELL_EXPLORER)
	{
		...
		SetWindowThemeAttribute = (P_SetWindowThemeAttribute)GetProcAddress(module, "SetWindowThemeAttribute");
...
_FX HRESULT SH32_IShellExtInit_Initialize(void* pShellExtInit, void* pidlFolder, IDataObject* pDataObject, HKEY hkeyProgID)
{
	typedef HRESULT (*P_Initialize)(void*, void*, IDataObject*, HKEY);
	ULONG_PTR* StubData = Dll_JumpStubData();

	extern IDataObject* Ole_XDataObject_From_IDataObject(IDataObject * pDataObject);

	return ((P_Initialize)StubData[1])(pShellExtInit, pidlFolder, Ole_XDataObject_From_IDataObject(pDataObject), hkeyProgID);
}


_FX HRESULT SH32_IContextMenuHook_QueryInterface(void* pContextMenu, REFIID riid, void** ppv)
{
	...
	if (SUCCEEDED(hr) && memcmp(riid, &IID_IShellExtInit, sizeof(GUID)) == 0)
	{
		void* pShellExtInit = (*ppv);
		SH32_IContextMenu_HookVtbl(pShellExtInit, 3, SH32_IShellExtInit_Initialize);
	}
...
_FX void SH32_IContextMenu_Hook(REFCLSID rclsid, void* pContextMenu)
{
	...
	P_IIDFromString pIIDFromString = (P_IIDFromString)Ldr_GetProcAddrNew(DllName_ole32_or_combase, L"IIDFromString", "IIDFromString");
	if (!pIIDFromString)
	{
		return;
	}
	...
	while (1)
	{
		NTSTATUS status = SbieApi_QueryConfAsIs(NULL, L"IContextMenuClsid", index, buf, 90 * sizeof(WCHAR));
		if (!NT_SUCCESS(status))
		{
			break;
		}
		++index;
		if (pIIDFromString(buf, &guid) == 0 && memcmp(rclsid, &guid, sizeof(GUID)) == 0)
		{
			match = TRUE;
			break;
		}
	...
	if (match)
	{
		SH32_IContextMenu_HookVtbl(pContextMenu, 0, SH32_IContextMenuHook_QueryInterface);
	}
}
