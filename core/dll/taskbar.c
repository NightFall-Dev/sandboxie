...
static HRESULT Taskbar_SetCurrentProcessExplicitAppUserModelID(const WCHAR* AppId);

static HRESULT Taskbar_GetCurrentProcessExplicitAppUserModelID(WCHAR** AppId);
...
static HRESULT Taskbar_Unknown_QueryInterface(IPropertyStore* This, REFIID riid, void** ppv);

static ULONG Taskbar_Unknown_AddRef(IPropertyStore* This);

static ULONG Taskbar_Unknown_Release(IPropertyStore* This);

static HRESULT Taskbar_SHGetPropertyStoreForWindow(HWND hwnd, REFIID riid, void** ppv);

static HRESULT Taskbar_IPropertyStore_GetCount(IPropertyStore* This, DWORD* cProps);

static HRESULT Taskbar_IPropertyStore_GetAt(IPropertyStore* This, DWORD iProp, PROPERTYKEY* pkey);

static HRESULT Taskbar_IPropertyStore_GetValue(IPropertyStore* This, REFPROPERTYKEY key, PROPVARIANT* pv);

static HRESULT Taskbar_IPropertyStore_SetValue(IPropertyStore* This, REFPROPERTYKEY key, REFPROPVARIANT propvar);
...
typedef HRESULT (*P_SetCurrentProcessExplicitAppUserModelID)(const WCHAR* AppId);

typedef HRESULT (*P_GetCurrentProcessExplicitAppUserModelID)(WCHAR** AppId);

typedef HRESULT (*P_SHGetPropertyStoreForWindow)(HWND hwnd, REFIID riid, void** ppv);
...
static P_SetCurrentProcessExplicitAppUserModelID __sys_SetCurrentProcessExplicitAppUserModelID = NULL;

static P_GetCurrentProcessExplicitAppUserModelID __sys_GetCurrentProcessExplicitAppUserModelID = NULL;

static P_SHGetPropertyStoreForWindow __sys_SHGetPropertyStoreForWindow = NULL;
...
static const GUID FOLDERID_Recent = {0xAE50C081, 0xEBD2, 0x438A, {0x86, 0x55, 0x8A, 0x09, 0x2E, 0x34, 0x98, 0x7A}};
...
_FX BOOLEAN Taskbar_Init(HMODULE module)
{
	...
	SbieApi_QueryConf(NULL, L"TaskbarUnique", 0, Version, 14 * sizeof(WCHAR));
	...
	SHGetPropertyStoreForWindow = (P_SHGetPropertyStoreForWindow)GetProcAddress(module, "SHGetPropertyStoreForWindow");
...
{
	P_SetCurrentProcessExplicitAppUserModelID SetCurrentProcessExplicitAppUserModelID;
	P_GetCurrentProcessExplicitAppUserModelID GetCurrentProcessExplicitAppUserModelID;

	SetCurrentProcessExplicitAppUserModelID = (P_SetCurrentProcessExplicitAppUserModelID)GetProcAddress(module, "SetCurrentProcessExplicitAppUserModelID");

	GetCurrentProcessExplicitAppUserModelID = (P_GetCurrentProcessExplicitAppUserModelID)GetProcAddress(module, "GetCurrentProcessExplicitAppUserModelID");
...
_FX BOOLEAN Taskbar_ShouldOverrideAppUserModelId(void)
{
	...
	if (Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER)
	{
		...
		if (0 == NtQueryInformationProcess(NtCurrentProcess(), ProcessBasicInformation, &info, sizeof(PROCESS_BASIC_INFORMATION), &len))
		{
			ULONG64 ParentProcessFlags = SbieApi_QueryProcessInfo((HANDLE)info.InheritedFromUniqueProcessId, 0);
	...
	else if (SHGetKnownFolderPath(&FOLDERID_Recent, KF_FLAG_DONT_VERIFY | KF_FLAG_DONT_UNEXPAND, NULL, &path) != 0)
	{
		path = NULL;
	}
...
_FX HRESULT Taskbar_SetCurrentProcessExplicitAppUserModelID(const WCHAR* AppId)
{
	HRESULT hr;
	WCHAR* NewId;
...
_FX void Taskbar_SetProcessAppUserModelId(void)
{
	...
	//
	//  if(Dll_Windows < 10) {
	SetCurrentProcessExplicitAppUserModelID = (P_SetCurrentProcessExplicitAppUserModelID)Ldr_GetProcAddrNew(DllName_shell32, L"SetCurrentProcessExplicitAppUserModelID", "SetCurrentProcessExplicitAppUserModelID");
...
_FX void Taskbar_SetWindowAppUserModelId(HWND hwnd)
{
	...
	SHGetPropertyStoreForWindow = (P_SHGetPropertyStoreForWindow)Ldr_GetProcAddrNew(DllName_shell32, L"SHGetPropertyStoreForWindow", "SHGetPropertyStoreForWindow");
	...
	hr = SHGetPropertyStoreForWindow(hwnd, &IID_IPropertyStore, &pPropertyStore);
	...
	if (ExeDescr)
	{
		...
		pPropertyStore->lpVtbl->SetValue(pPropertyStore, &PKEY_AppUserModel_RelaunchDisplayNameResource, &v);
		Taskbar_FreeBSTR(v.bstrVal);
	}
	...
	if (ExePath)
	{
		...
		pPropertyStore->lpVtbl->SetValue(pPropertyStore, &PKEY_AppUserModel_RelaunchCommand, &v);
		Taskbar_FreeBSTR(v.bstrVal);
	}
	...
	if (ExePath)
	{
		WCHAR* IconPath = Dll_AllocTemp((wcslen(ExePath) + 8) * sizeof(WCHAR));
		wcscpy(IconPath, ExePath);
		wcscat(IconPath, L",-0");

		v.vt      = VT_BSTR;
		v.bstrVal = Taskbar_AllocBSTR(IconPath);
		pPropertyStore->lpVtbl->SetValue(pPropertyStore, &PKEY_AppUserModel_RelaunchIconResource, &v);
		Taskbar_FreeBSTR(v.bstrVal);
...
_FX void Taskbar_GetProgramName(WCHAR** name, WCHAR** path)
{
	...
	typedef HRESULT (*P_AssocQueryString)(ULONG_PTR flags, ULONG_PTR str, LPCTSTR pszAssoc, LPCTSTR pszExtra, LPTSTR pszOut, DWORD* pcchOut);
	...
	*path = _path;
	...
	AssocQueryString = (P_AssocQueryString)Ldr_GetProcAddrNew(L"shlwapi.dll", L"AssocQueryStringW", "AssocQueryStringW");
	if (!AssocQueryString)
	{
		return;
	}
	...
	hFile = CreateFile(_path, GENERIC_READ, FILE_SHARE_VALID_FLAGS, NULL, OPEN_EXISTING, 0, NULL);
	...
	if (0 != AssocQueryString(ASSOCF_INIT_BYEXENAME, ASSOCSTR_FRIENDLYAPPNAME, _path, NULL, _name, &len))
	{
...
_FX WCHAR* Taskbar_GetStartExeCommand(const WCHAR* args)
{
	...
	SbieApi_QueryConf(NULL, L"TaskbarProgram", 0, command + 1, MAX_PATH * sizeof(WCHAR));
...
_FX WCHAR* Taskbar_GetNameWithBoxPrefix(const WCHAR* name)
{
	...
	SbieApi_QueryConf(NULL, L"TaskbarName", 0, out + 1, MAX_PATH * sizeof(WCHAR));
	wcscat(out, L"] ");
	wcscat(out, name);
...
_FX HRESULT Taskbar_Unknown_QueryInterface(IPropertyStore* This, REFIID riid, void** ppv)
{
...
HRESULT Taskbar_SHGetPropertyStoreForWindow(HWND hwnd, REFIID riid, void** ppv)
{
...
_FX HRESULT Taskbar_IPropertyStore_GetCount(IPropertyStore* This, DWORD* cProps)
{
	TASKBAR_UNKNOWN* pThis = (TASKBAR_UNKNOWN*)This;
	IPropertyStore* pReal  = (IPropertyStore*)pThis->pReal;
...
_FX HRESULT Taskbar_IPropertyStore_GetAt(IPropertyStore* This, DWORD iProp, PROPERTYKEY* pkey)
{
	TASKBAR_UNKNOWN* pThis = (TASKBAR_UNKNOWN*)This;
	IPropertyStore* pReal  = (IPropertyStore*)pThis->pReal;
...
_FX HRESULT Taskbar_IPropertyStore_GetValue(IPropertyStore* This, REFPROPERTYKEY key, PROPVARIANT* pv)
{
	TASKBAR_UNKNOWN* pThis = (TASKBAR_UNKNOWN*)This;
	IPropertyStore* pReal  = (IPropertyStore*)pThis->pReal;
...
_FX HRESULT Taskbar_IPropertyStore_SetValue(IPropertyStore* This, REFPROPERTYKEY key, REFPROPVARIANT propvar)
{
	TASKBAR_UNKNOWN* pThis = (TASKBAR_UNKNOWN*)This;
	IPropertyStore* pReal  = (IPropertyStore*)pThis->pReal;
	...
	else if (memcmp(key, &PKEY_AppUserModel_RelaunchCommand, sizeof(PROPERTYKEY)) == 0)
	{
		if (propvar->vt == VT_BSTR && propvar->bstrVal)
		{
	...
	else if (memcmp(key, &PKEY_AppUserModel_RelaunchDisplayNameResource, sizeof(PROPERTYKEY)) == 0)
	{
		if (propvar->vt == VT_BSTR && propvar->bstrVal)
		{
