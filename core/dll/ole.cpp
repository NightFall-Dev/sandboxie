...
typedef UINT (*P_DragQueryFile)(HDROP hDrop, UINT iFile, WCHAR* lpszFile, UINT cch);
...
typedef HRESULT (*P_SHILCreateFromPath)(void* pszPath, LPITEMIDLIST* ppidl, ULONG* rgflnOut);
...
static BOOLEAN Ole_WriteStreamToFile(IStream* pStream, ULONG StreamSize, WCHAR* Path);
...
extern "C" _FX BOOLEAN Ole_Init(HMODULE module)
{
	...
	ReleaseStgMedium = (P_ReleaseStgMedium)GetProcAddress(module, "ReleaseStgMedium");
...
_FX HRESULT XEnumFormatEtc::Next(ULONG celt, FORMATETC* rgelt, ULONG* pceltFetched)
{
	if (m_eof)
	{
		if (pceltFetched)
...
_FX XDataObject::XDataObject(IDataObject* pDataObject, HDROP hDrop)
{
	...
	if (__shell32)
	{
		pDragQueryFileW = (P_DragQueryFile)GetProcAddress(__shell32, "DragQueryFileW");

		pILCombine = (P_ILCombine)GetProcAddress(__shell32, "ILCombine");

		pILGetSize = (P_ILGetSize)GetProcAddress(__shell32, "ILGetSize");

		pSHGetPathFromIDList = (P_SHGetPathFromIDList)GetProcAddress(__shell32, "SHGetPathFromIDListW");

		pSHILCreateFromPath = (P_SHILCreateFromPath)GetProcAddress(__shell32, "SHILCreateFromPath");

		pSHFree = (P_SHFree)GetProcAddress(__shell32, "SHFree");

		if (!Ole_CF_FileNameA)
		{
			Ole_CF_FileNameA = __sys_RegisterClipboardFormatW(CFSTR_FILENAMEA);
		}
		if (!Ole_CF_FileNameW)
		{
			Ole_CF_FileNameW = __sys_RegisterClipboardFormatW(CFSTR_FILENAMEW);
		}

		if (!Ole_CF_ShellIdList)
		{
			Ole_CF_ShellIdList = __sys_RegisterClipboardFormatW(CFSTR_SHELLIDLIST);
		}
	}
	else
	{
...
_FX HRESULT XDataObject::DAdvise(FORMATETC* pFormatetc, DWORD advf, IAdviseSink* pAdvSink, DWORD* pdwConnection)
{
	if (!m_pDataObject)
	{
...
_FX HRESULT XDataObject::EnumFormatEtc(DWORD dwDirection, IEnumFORMATETC** ppenumFormatetc)
{
	if (!m_pDataObject)
	{
...
_FX HRESULT XDataObject::GetCanonicalFormatEtc(FORMATETC* pFormatetcIn, FORMATETC* pFormatetcOut)
{
	if (!m_pDataObject)
	{
...
_FX HRESULT XDataObject::GetData(FORMATETC* pFormatetc, STGMEDIUM* pmedium)
{
	HRESULT hr;
...
_FX HRESULT XDataObject::GetDataHere(FORMATETC* pFormatetc, STGMEDIUM* pmedium)
{
	if (!m_pDataObject)
	{
		return E_FAIL;
...
_FX HRESULT XDataObject::SetData(FORMATETC* pFormatetc, STGMEDIUM* pmedium, BOOL fRelease)
{
	if (!m_pDataObject)
	{
		return E_NOTIMPL;
...
_FX void XDataObject::PrintFormat(const WCHAR* FuncName, const FORMATETC* pFormatetc)
{
...
_FX HGLOBAL XDataObject::InitFormatHDrop(HGLOBAL hData)
{
	...
	for (UINT i = 0; i < count; ++i)
	{
		...
		HANDLE hFile = CreateFileW(DropName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
_FX HANDLE XDataObject::OpenFileFromHDrop(HGLOBAL hData)
{
	...
	{
		FileName[count] = L'\0';
		hFile = CreateFileW(FileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
	}
...
_FX HGLOBAL XDataObject::InitFormatFileNameA(HGLOBAL hData)
{
	...
	if (m_pDataObject)
	{
		...
		hFile = CreateFileA(FileNameA, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
_FX HGLOBAL XDataObject::InitFormatFileNameW(HGLOBAL hData)
{
	...
	if (m_pDataObject)
	{
		...
		hFile = CreateFileW(FileNameW, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
#define GetPidl(i) (LPCITEMIDLIST)(((LPBYTE)pIdList) + (pIdList)->aoffset[i])
	...
	if ((!pILCombine) || (!pILGetSize) || (!pSHFree) || (!pSHILCreateFromPath) || (!pSHGetPathFromIDList))
	{
		return NULL;
	}
	...
	for (count = 1; count <= pIdList->cidl; ++count)
	{
		...
		if (pidl)
		{
			...
			if (SUCCEEDED(hr))
			{
				hFile = CreateFileW(path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
	...
	hFile = CreateFileW(path, GENERIC_WRITE, FILE_SHARE_VALID_FLAGS, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
	if (hFile == INVALID_HANDLE_VALUE)
	{
		goto finish;
	}
	...
	ULONG new_idlist_len = (pIdList->cidl + 2) * sizeof(UINT) + pILGetSize(pidl);
	for (count = 1; count <= pIdList->cidl; ++count)
	{
	...
	if (SUCCEEDED(hr) && Gui_DropTargetProp_Atom)
	{
		...
		if ((exstyle & WS_EX_ACCEPTFILES) == 0)
		{
			...
			if (!__sys_DragFinish)
			{
				...
				__sys_DragFinish = (P_DragFinish)GetProcAddress(shell32, "DragFinish");
			}
...
_FX HRESULT Ole_RevokeDragDrop(HWND hwnd)
{
	...
	if (SUCCEEDED(hr) && Gui_DropTargetProp_Atom)
	{
		...
		void* pDropTarget = __sys_RemovePropW(hwnd, (LPCWSTR)Gui_DropTargetProp_Atom);
		if (pDropTarget)
		{
			ULONG exstyle = __sys_GetWindowLongW(hwnd, GWL_EXSTYLE);
...
extern "C" _FX BOOLEAN Ole_DoDragDrop(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	if (!Gui_RenameClasses)
	{
		return FALSE;
	...
	if (wParam)
	{
		pDropTarget = (IDropTarget*)__sys_GetPropW(hWnd, (LPCWSTR)Gui_DropTargetProp_Atom);
	}
	...
	ULONG len                = sizeof(XDataObject);
	void* buf                = Dll_Alloc(len);
	XDataObject* xDataObject = new (buf) XDataObject(NULL, (HDROP)wParam);
	...
	if (SUCCEEDED(hr))
	{
		if (Dll_ImageType == DLL_IMAGE_INTERNET_EXPLORER || Dll_ImageType == DLL_IMAGE_MOZILLA_FIREFOX || Dll_ImageType == DLL_IMAGE_GOOGLE_CHROME)
		{
			ULONG_PTR* Args = (ULONG_PTR*)Dll_Alloc(sizeof(ULONG_PTR) * 8);
			...
			if (QueueUserWorkItem(Ole_DoDragDrop_2, Args, WT_EXECUTELONGFUNCTION))
			{
				return TRUE;
			}
...
extern "C" _FX IDataObject* Ole_XDataObject_From_IDataObject(IDataObject* pDataObject)
{
	ULONG len                = sizeof(XDataObject);
	void* buf                = Dll_Alloc(len);
