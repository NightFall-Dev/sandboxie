...
typedef BOOL (*P_GetUserObjectInformation)(HANDLE hObj, int nIndex, void* pvInfo, DWORD nLength, DWORD* lpnLengthNeeded);
...
typedef BOOLEAN (*P_WinStationDisconnect)(HANDLE hServer, ULONG LogonId, ULONG_PTR Unknown);

typedef BOOLEAN (*P_WinStationEnumerate)(HANDLE hServer, void* InfoBuffer, ULONG* InfoCount);

typedef BOOLEAN (*P_WinStationQueryInformation)(HANDLE, ULONG, WINSTATIONINFOCLASS, PVOID, ULONG, PULONG);

typedef BOOLEAN (*P_WinStationIsSessionRemoteable)(HANDLE hServer, ULONG LogonId, BOOLEAN* IsRemote);

typedef BOOLEAN (*P_WinStationNameFromLogonId)(HANDLE hServer, ULONG LogonId, WCHAR* Name);

typedef BOOLEAN (*P_WinStationGetConnectionProperty)(ULONG LogonId, GUID* TypeGuid, WTS_PROPERTY_VALUE** Value);

typedef BOOLEAN (*P_WinStationFreePropertyValue)(WTS_PROPERTY_VALUE* Value);
...
typedef BOOL (*P_WTSEnumerateSessions)(HANDLE hServer, DWORD Reserved, DWORD Version, WTS_SESSION_INFO** ppSessionInfo, DWORD* pCount);

typedef BOOL (*P_WTSEnumerateProcesses)(HANDLE hServer, DWORD Reserved, DWORD Version, WTS_PROCESS_INFO** ppProcessInfo, DWORD* pCount);

typedef void (*P_WTSFreeMemory)(void* pMemory);

typedef BOOL (*P_WTSRegisterSessionNotification)(HWND hWnd, ULONG dwFlags);

typedef BOOL (*P_WTSRegisterSessionNotificationEx)(HANDLE hServer, HWND hWnd, ULONG dwFlags);

typedef BOOL (*P_WTSUnRegisterSessionNotification)(HWND hWnd);

typedef BOOL (*P_WTSUnRegisterSessionNotificationEx)(HANDLE hServer, HWND hWnd);
...
static BOOLEAN Terminal_WinStationDisconnect(HANDLE hServer, ULONG LogonId, ULONG_PTR Unknown);

static BOOLEAN Terminal_WinStationEnumerateW(HANDLE hServer, void* InfoBuffer, ULONG* InfoCount);

static BOOLEAN Terminal_WinStaQueryInformationW(HANDLE hServer, ULONG LogonId, WINSTATIONINFOCLASS InformationClass, void* Information, ULONG InformationLength, PULONG pReturnLength);

static BOOLEAN Terminal_WinStationIsSessionRemoteable(HANDLE hServer, ULONG LogonId, BOOLEAN* IsRemote);

static BOOLEAN Terminal_WinStationNameFromLogonIdW(HANDLE hServer, ULONG LogonId, WCHAR* Name);

static BOOLEAN Terminal_WinStationGetConnectionProperty(ULONG LogonId, GUID* PropType, WTS_PROPERTY_VALUE** Value);

static BOOLEAN Terminal_WinStationFreePropertyValue(WTS_PROPERTY_VALUE* Value);
...
static BOOL Terminal_WTSEnumerateSessionsW(HANDLE hServer, DWORD Reserved, DWORD Version, WTS_SESSION_INFO** ppSessionInfo, DWORD* pCount);

static BOOL Terminal_WTSEnumerateProcessesW(HANDLE hServer, DWORD Reserved, DWORD Version, WTS_PROCESS_INFO** ppProcessInfo, DWORD* pCount);

static void Terminal_WTSFreeMemory(void* pMemory);

static BOOL Terminal_WTSRegisterSessionNotification(HWND hWnd, ULONG dwFlags);

static BOOL Terminal_WTSRegisterSessionNotificationEx(HANDLE hServer, HWND hWnd, ULONG dwFlags);

static BOOL Terminal_WTSUnRegisterSessionNotification(HWND hWnd);

static BOOL Terminal_WTSUnRegisterSessionNotificationEx(HANDLE hServer, HWND hWnd);
...
_FX WCHAR* Terminal_GetWindowStationName(void)
{
	...
	P_GetProcessWindowStation pGetProcessWindowStation = (P_GetProcessWindowStation)Ldr_GetProcAddrNew(DllName_user32, L"GetProcessWindowStation", "GetProcessWindowStation");

	P_GetUserObjectInformation pGetUserObjectInformation = (P_GetUserObjectInformation)Ldr_GetProcAddrNew(DllName_user32, L"GetUserObjectInformationW", "GetUserObjectInformationW");
	...
	if (winsta && pGetUserObjectInformation)
	{
		...
		if (!pGetUserObjectInformation(winsta, UOI_NAME, name, len, &len))
		{
			Dll_Free(name);
			name = NULL;
		}
...
_FX BOOLEAN Terminal_WinStationDisconnect(HANDLE hServer, ULONG LogonId, ULONG_PTR Unknown)
{
	MSG_HEADER req;
	MSG_HEADER* rpl;
	ULONG err;

	if ((hServer != SERVERNAME_CURRENT && hServer != HANDLE_SERVER_FAKE) || (LogonId != LOGONID_CURRENT))
	{
		SetLastError(ERROR_ACCESS_DENIED);
		return FALSE;
	}
...
_FX BOOLEAN Terminal_WinStationEnumerateW(HANDLE hServer, void** InfoBuffer, ULONG* InfoCount)
{
	WCHAR* WinStaName;
...
_FX BOOLEAN Terminal_WinStaQueryInformationW(HANDLE hServer, ULONG LogonId, WINSTATIONINFOCLASS InformationClass, void* Information, ULONG InformationLength, PULONG pReturnLength)
{
...
_FX BOOLEAN Terminal_WinStationIsSessionRemoteable(HANDLE hServer, ULONG LogonId, BOOLEAN* IsRemote)
{
...
_FX BOOLEAN Terminal_WinStationNameFromLogonIdW(HANDLE hServer, ULONG LogonId, WCHAR* Name)
{
...
_FX BOOLEAN Terminal_WinStationGetConnectionProperty(ULONG LogonId, GUID* TypeGuid, WTS_PROPERTY_VALUE** Value)
{
	...
	else
	{
		...
		{
			UCHAR* ptr;
			WTS_PROPERTY_VALUE* value = Terminal_Alloc(sizeof(WTS_PROPERTY_VALUE) + rpl->len);
...
_FX BOOLEAN Terminal_WinStationFreePropertyValue(WTS_PROPERTY_VALUE* Value)
{
...
_FX BOOLEAN Terminal_Init_WinSta(HMODULE module)
{
	...
	WinStationOpenServerW = (P_WinStationOpenServer)GetProcAddress(module, "WinStationOpenServerW");

	WinStationCloseServer = (P_WinStationCloseServer)GetProcAddress(module, "WinStationCloseServer");

	WinStationDisconnect = (P_WinStationDisconnect)GetProcAddress(module, "WinStationDisconnect");

	WinStationEnumerateW = (P_WinStationEnumerate)GetProcAddress(module, "WinStationEnumerateW");

	WinStaQueryInformationW = (P_WinStationQueryInformation)GetProcAddress(module, "WinStationQueryInformationW");

	WinStationIsSessionRemoteable = (P_WinStationIsSessionRemoteable)GetProcAddress(module, "WinStationIsSessionRemoteable");

	WinStationNameFromLogonIdW = (P_WinStationNameFromLogonId)GetProcAddress(module, "WinStationNameFromLogonIdW");

	WinStationGetConnectionProperty = (P_WinStationGetConnectionProperty)GetProcAddress(module, "WinStationGetConnectionProperty");

	WinStationFreePropertyValue = (P_WinStationFreePropertyValue)GetProcAddress(module, "WinStationFreePropertyValue");

	WinStationFreeMemory = (P_WinStationFreeMemory)GetProcAddress(module, "WinStationFreeMemory");
	...
	*pToken = NULL;
	...
	for (i = 1; i <= pids[0]; ++i)
	{
		...
		if (_wcsicmp(image, Ipc_SandboxieRpcSs) == 0)
		{
			...
			if (process)
			{
				NTSTATUS status = NtOpenProcessToken(process, TOKEN_ALL_ACCESS, pToken);
				if (status == 0)
				{
					err = 0;
				}
...
_FX BOOL Terminal_WTSEnumerateSessionsW(HANDLE hServer, DWORD Reserved, DWORD Version, WTS_SESSION_INFO** ppSessionInfo, DWORD* pCount)
{
	WCHAR* WinStaName;
	...
	len = sizeof(WTS_SESSION_INFO) + (wcslen(WinStaName) + 8) * sizeof(WCHAR);
...
_FX BOOL Terminal_WTSEnumerateProcessesW(HANDLE hServer, DWORD Reserved, DWORD Version, WTS_PROCESS_INFO** ppProcessInfo, DWORD* pCount)
{
	...
	while (1)
	{
		...
		{
			hProcess = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, (ULONG)(ULONG_PTR)info_ptr->UniqueProcessId);
		}
		...
		if (hProcess)
		{
			...
			NTSTATUS status = NtQueryInformationProcess(hProcess, ProcessImageFileName, work_ptr, 512, &i);
			...
			if (NT_SUCCESS(status))
			{
				status = NtQueryInformationToken(hToken, TokenUser, work_ptr, 256, &i);
				...
				status = NtQueryInformationToken(hToken, TokenSessionId, &session_id, sizeof(ULONG), &i);
	...
	i = num * sizeof(WTS_PROCESS_INFO) + (ULONG)(work_ptr - work_area);
...
_FX BOOL Terminal_WTSRegisterSessionNotification(HWND hWnd, ULONG dwFlags)
{
	SetLastError(0);
	return TRUE;
}
...
_FX BOOL Terminal_WTSRegisterSessionNotificationEx(HANDLE hServer, HWND hWnd, ULONG dwFlags)
{
	SetLastError(0);
	return TRUE;
}
...
_FX BOOL Terminal_WTSUnRegisterSessionNotification(HWND hWnd)
{
	SetLastError(0);
	return TRUE;
}
...
_FX BOOL Terminal_WTSUnRegisterSessionNotificationEx(HANDLE hServer, HWND hWnd)
{
	SetLastError(0);
	return TRUE;
}
...
_FX BOOLEAN Terminal_Init_WtsApi(HMODULE module)
{
	...
	WTSQueryUserToken      = (P_WTSQueryUserToken)GetProcAddress(module, "WTSQueryUserToken");
	WTSEnumerateSessionsW  = (P_WTSEnumerateSessions)GetProcAddress(module, "WTSEnumerateSessionsW");
	WTSEnumerateProcessesW = (P_WTSEnumerateProcesses)GetProcAddress(module, "WTSEnumerateProcessesW");
	WTSFreeMemory          = (P_WTSFreeMemory)GetProcAddress(module, "WTSFreeMemory");
	WTSRegisterSessionNotification = (P_WTSRegisterSessionNotification)GetProcAddress(module, "WTSRegisterSessionNotification");
	WTSRegisterSessionNotificationEx = (P_WTSRegisterSessionNotificationEx)GetProcAddress(module, "WTSRegisterSessionNotificationEx");
	WTSUnRegisterSessionNotification = (P_WTSUnRegisterSessionNotification)GetProcAddress(module, "WTSUnRegisterSessionNotification");
	WTSUnRegisterSessionNotificationEx = (P_WTSUnRegisterSessionNotificationEx)GetProcAddress(module, "WTSUnRegisterSessionNotificationEx");
