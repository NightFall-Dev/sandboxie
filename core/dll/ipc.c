...
static NTSTATUS Ipc_GetName(HANDLE RootDirectory, UNICODE_STRING* ObjectName, WCHAR** OutTruePath, WCHAR** OutCopyPath, BOOLEAN* OutIsBoxedPath);

static NTSTATUS Ipc_GetName2(OBJECT_ATTRIBUTES* ObjectAttributes, WCHAR** OutTruePath, WCHAR** OutCopyPath);
...
static NTSTATUS Ipc_NtCreatePort(HANDLE* PortHandle, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG MaxConnectInfoLength, ULONG MaxDataLength, ULONG* Reserved);

static NTSTATUS Ipc_NtConnectPort(HANDLE* PortHandle, UNICODE_STRING* PortName, SECURITY_QUALITY_OF_SERVICE* SecurityQos, PLPC_SECTION_OWNER_MEMORY ClientSharedMemory, PLPC_SECTION_MEMORY ServerSharedMemory, ULONG* MaximumMessageLength, void* ConnectionInfo, ULONG* ConnectionInfoLength);

static NTSTATUS Ipc_NtSecureConnectPort(HANDLE* PortHandle, UNICODE_STRING* PortName, SECURITY_QUALITY_OF_SERVICE* SecurityQos, PLPC_SECTION_OWNER_MEMORY ClientSharedMemory, PSID ServerSid, PLPC_SECTION_MEMORY ServerSharedMemory, ULONG* MaximumMessageLength, void* ConnectionInfo, ULONG* ConnectionInfoLength);

static NTSTATUS Ipc_NtAlpcCreatePort(HANDLE* PortHandle, OBJECT_ATTRIBUTES* ObjectAttributes, void* AlpcCreateInfo);

static NTSTATUS Ipc_NtAlpcConnectPort(HANDLE* PortHandle, UNICODE_STRING* PortName, OBJECT_ATTRIBUTES* ObjectAttributes, ALPC_PORT_ATTRIBUTES* AlpcConnectInfo, ULONG ConnectionFlags, PSID ServerSid, void* ConnectionInfo, ULONG* ConnectionInfoLength, void* InMessageBuffer, void* OutMessageBuffer, LARGE_INTEGER* Timeout);

static NTSTATUS Ipc_NtAlpcConnectPortEx(HANDLE* PortHandle, OBJECT_ATTRIBUTES* ObjectAttributes1, OBJECT_ATTRIBUTES* ObjectAttributes2, ALPC_PORT_ATTRIBUTES* AlpcConnectInfo, ULONG ConnectionFlags, SECURITY_DESCRIPTOR* ServerSd, void* ConnectionInfo, ULONG* ConnectionInfoLength, void* InMessageBuffer, void* OutMessageBuffer, LARGE_INTEGER* Timeout);

static NTSTATUS Ipc_NtAlpcQueryInformation(HANDLE* PortHandle, ULONG InformationClass, void* Information, ULONG InformationLength, ULONG_PTR UnknownParameter5);

static NTSTATUS Ipc_NtAlpcQueryInformationMessage(HANDLE* PortHandle, void* RpcMessage, ULONG InformationClass, void* Information, ULONG InformationLength, ULONG_PTR UnknownParameter6);

BOOLEAN Ipc_IsAdminToken(HANDLE hToken);

static NTSTATUS Ipc_ImpersonateSelf(PORT_MESSAGE* PortMsg);

static NTSTATUS Ipc_NtImpersonateClientOfPort(HANDLE PortHandle, PORT_MESSAGE* RequestMessage);

static NTSTATUS Ipc_NtAlpcImpersonateClientOfPort(HANDLE PortHandle, PORT_MESSAGE* RequestMessage, ULONG_PTR UnknownParameter3);

static NTSTATUS Ipc_NtImpersonateAnonymousToken(HANDLE ThreadHandle);

static NTSTATUS Ipc_NtImpersonateThread(HANDLE ServerThreadHandle, HANDLE ClientThreadHandle, PSECURITY_QUALITY_OF_SERVICE SecurityQos);

static BOOLEAN Ipc_IsKnownDllInSandbox(THREAD_DATA* TlsData, const WCHAR* TruePath);

static NTSTATUS Ipc_ConnectProxyPort(HANDLE* PortHandle, const WCHAR* TruePath, ULONG AlpcConnectionFlags, void* ConnectionInfo, ULONG* ConnectionInfoLength, void* MaximumMessageLengthOrAlpcInfo, void* ShouldBeNull1, void* ShouldBeNull2, void* ShouldBeNull3);

static NTSTATUS Ipc_NtRequestWaitReplyPort(HANDLE PortHandle, PORT_MESSAGE* RequestMsg, PORT_MESSAGE* ReplyMsg);

static NTSTATUS Ipc_NtAlpcSendWaitReceivePort(HANDLE PortHandle, ULONG SendFlags, PORT_MESSAGE* SendMsg, ALPC_MESSAGE_VIEW* SendView, PORT_MESSAGE* ReceiveMsg, ULONG* ReceiveMsgSize, ALPC_MESSAGE_VIEW* ReceiveView, LARGE_INTEGER* Timeout);


//---------------------------------------------------------------------------


static NTSTATUS Ipc_NtCreateEvent(HANDLE* EventHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, EVENT_TYPE EventType, BOOLEAN InitialState);

static NTSTATUS Ipc_NtOpenEvent(HANDLE* EventHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes);


//---------------------------------------------------------------------------


static NTSTATUS Ipc_NtCreateMutant(HANDLE* MutantHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, BOOLEAN InitialOwner);

static NTSTATUS Ipc_NtOpenMutant(HANDLE* MutantHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes);


//---------------------------------------------------------------------------


static NTSTATUS Ipc_NtCreateSemaphore(HANDLE* SemaphoreHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG InitialCount, ULONG MaximumCount);

static NTSTATUS Ipc_NtOpenSemaphore(HANDLE* SemaphoreHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes);


//---------------------------------------------------------------------------


static NTSTATUS Ipc_NtCreateSection(HANDLE* SectionHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, LARGE_INTEGER* MaximumSize, ULONG PageAttributes, ULONG SectionAttributes, HANDLE FileHandle);

static NTSTATUS Ipc_NtOpenSection(HANDLE* SectionHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes);
...
static P_NtAlpcQueryInformationMessage __sys_NtAlpcQueryInformationMessage = NULL;
static P_NtImpersonateClientOfPort __sys_NtImpersonateClientOfPort         = NULL;
static P_NtAlpcImpersonateClientOfPort __sys_NtAlpcImpersonateClientOfPort = NULL;
...
static P_NtImpersonateAnonymousToken __sys_NtImpersonateAnonymousToken     = NULL;
...
_FX NTSTATUS Ipc_GetName(HANDLE RootDirectory, UNICODE_STRING* ObjectName, WCHAR** OutTruePath, WCHAR** OutCopyPath, BOOLEAN* OutIsBoxedPath)
{
	...
	*OutCopyPath = NULL;
	...
	if (RootDirectory)
	{
		...
		*OutTruePath = ((OBJECT_NAME_INFORMATION*)name)->ObjectName.Buffer;
		...
		name = (*OutTruePath) + ((OBJECT_NAME_INFORMATION*)name)->ObjectName.Length / sizeof(WCHAR);
...
check_sandbox_prefix:
	...
	if (length >= Dll_BoxIpcPathLen && 0 == Dll_NlsStrCmp(*OutTruePath, Dll_BoxIpcPath, Dll_BoxIpcPathLen))
	{
	...
	name = Dll_GetTlsNameBuffer(TlsData, COPY_NAME_BUFFER, Dll_BoxIpcPathLen + length);
...
_FX BOOLEAN Ipc_GetName_AdjustSplWow64Path(WCHAR* TruePath, BOOLEAN adj)
{
	...
	if (nameStart && (0 == _wcsnicmp(nameStart + 1, L"SplWow64_", 9) || 0 == _wcsnicmp(nameStart + 1, L"UmpdProxy_", 10) || 0 == _wcsnicmp(nameStart + 1, L"WinSpl64to32Mutex_", 18)))
	{
		...
		if (name && name[1])
		{
			...
			else if (Dll_OsBuild >= 7601 && name[2] == L'0' && name[3] == L'0' && name[4] == L'0' && name[5] == L'\0')
			{
				...
				if (Dll_OsBuild >= 8400)
				{
					...
					if (authid)
					{
						...
						if (authid)
						{
							...
							if ((name > authid) && (name - authid) >= (3 * sizeof(WCHAR)))
							{
...
_FX NTSTATUS Ipc_GetName2(OBJECT_ATTRIBUTES* ObjectAttributes, WCHAR** OutTruePath, WCHAR** OutCopyPath)
{
	NTSTATUS status;
	...
	status = Ipc_GetName(RootDirectory, ObjectName, OutTruePath, OutCopyPath, NULL);
...
_FX void Ipc_AdjustPortPath(UNICODE_STRING* ObjectName)
{
	...
	*BackslashPtr = L'\0';
	...
	if (NT_SUCCESS(status))
	{
		ULONG length                  = sizeof(OBJECT_NAME_INFORMATION) + (ParentLength + 8) * sizeof(WCHAR);
		OBJECT_NAME_INFORMATION* name = Dll_AllocTemp(length);

		status = Obj_GetObjectName(handle, name, &length);

		if (NT_SUCCESS(status) && name->ObjectName.Length >= ParentLength * sizeof(WCHAR)
...
_FX NTSTATUS Ipc_NtCreatePort(HANDLE* PortHandle, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG MaxConnectInfoLength, ULONG MaxDataLength, ULONG* Reserved)
{
	ULONG LastError;
	...
	__try
	{
		...
		if (PATH_IS_OPEN(mp_flags))
		{
			...
			status = __sys_NtCreatePort(PortHandle, &objattrs, MaxConnectInfoLength, MaxDataLength, Reserved);
...
_FX NTSTATUS Ipc_NtConnectPort(HANDLE* PortHandle, UNICODE_STRING* PortName, SECURITY_QUALITY_OF_SERVICE* SecurityQos, PLPC_SECTION_OWNER_MEMORY ClientSharedMemory, PLPC_SECTION_MEMORY ServerSharedMemory, ULONG* MaximumMessageLength, void* ConnectionInfo, ULONG* ConnectionInfoLength)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		status = Ipc_ConnectProxyPort(PortHandle, TruePath, -1, ConnectionInfo, ConnectionInfoLength, MaximumMessageLength, ClientSharedMemory, NULL, ServerSharedMemory);
		...
		status = __sys_NtConnectPort(PortHandle, &objname, SecurityQos, ClientSharedMemory, ServerSharedMemory, MaximumMessageLength, ConnectionInfo, ConnectionInfoLength);
...
_FX NTSTATUS Ipc_NtSecureConnectPort(HANDLE* PortHandle, UNICODE_STRING* PortName, SECURITY_QUALITY_OF_SERVICE* SecurityQos, PLPC_SECTION_OWNER_MEMORY ClientSharedMemory, PSID ServerSid, PLPC_SECTION_MEMORY ServerSharedMemory, ULONG* MaximumMessageLength, void* ConnectionInfo, ULONG* ConnectionInfoLength)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		status = Ipc_ConnectProxyPort(PortHandle, TruePath, -1, ConnectionInfo, ConnectionInfoLength, MaximumMessageLength, ClientSharedMemory, ServerSid, ServerSharedMemory);
		...
		status = __sys_NtSecureConnectPort(PortHandle, &objname, SecurityQos, ClientSharedMemory, ServerSid, ServerSharedMemory, MaximumMessageLength, ConnectionInfo, ConnectionInfoLength);
...
_FX NTSTATUS Ipc_NtAlpcCreatePort(HANDLE* PortHandle, OBJECT_ATTRIBUTES* ObjectAttributes, void* AlpcCreateInfo)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (PATH_IS_OPEN(mp_flags))
		{
			...
			status = __sys_NtAlpcCreatePort(PortHandle, &objattrs, AlpcCreateInfo);
...
_FX NTSTATUS Ipc_NtAlpcConnectPort(HANDLE* PortHandle, UNICODE_STRING* PortName, OBJECT_ATTRIBUTES* ObjectAttributes, ALPC_PORT_ATTRIBUTES* AlpcConnectInfo, ULONG ConnectionFlags, PSID ServerSid, void* ConnectionInfo, ULONG* ConnectionInfoLength, void* InMessageBuffer, void* OutMessageBuffer, LARGE_INTEGER* Timeout)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		{
			if (g_Ipc_DynamicPortNames[i] && *g_Ipc_DynamicPortNames[i] && (_wcsicmp(TruePath, g_Ipc_DynamicPortNames[i]) == 0))
			{
		...
		status = Ipc_ConnectProxyPort(PortHandle, TruePath, ConnectionFlags, ConnectionInfo, ConnectionInfoLength, AlpcConnectInfo, NULL, ServerSid, NULL);
		...
		status = __sys_NtAlpcConnectPort(PortHandle, &objname, ObjectAttributes, AlpcConnectInfo, ConnectionFlags, ServerSid, ConnectionInfo, ConnectionInfoLength, InMessageBuffer, OutMessageBuffer, Timeout);
...
_FX NTSTATUS Ipc_NtAlpcConnectPortEx(HANDLE* PortHandle, OBJECT_ATTRIBUTES* ObjectAttributes1, OBJECT_ATTRIBUTES* ObjectAttributes2, ALPC_PORT_ATTRIBUTES* AlpcConnectInfo, ULONG ConnectionFlags, SECURITY_DESCRIPTOR* ServerSd, void* ConnectionInfo, ULONG* ConnectionInfoLength, void* InMessageBuffer, void* OutMessageBuffer, LARGE_INTEGER* Timeout)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		status = Ipc_GetName(NULL, ObjectAttributes1->ObjectName, &TruePath, &CopyPath, NULL);
		if (!NT_SUCCESS(status))
		{
			__leave;
		}
		...
		status = Ipc_ConnectProxyPort(PortHandle, TruePath, ConnectionFlags, ConnectionInfo, ConnectionInfoLength, AlpcConnectInfo, NULL, ServerSd, NULL);
		...
		status = __sys_NtAlpcConnectPortEx(PortHandle, &objattrs, ObjectAttributes2, AlpcConnectInfo, ConnectionFlags, ServerSd, ConnectionInfo, ConnectionInfoLength, InMessageBuffer, OutMessageBuffer, Timeout);
	...
	OpenTruePath:
		...
		if (Dll_OsBuild >= 8400 && ServerSd && _wcsnicmp(TruePath, L"\\RPC Control\\splwow64_", 12) == 0)
		{
			//
...
_FX NTSTATUS Ipc_NtAlpcQueryInformation(HANDLE* PortHandle, ULONG InformationClass, void* Information, ULONG InformationLength, ULONG_PTR UnknownParameter5)
{
	NTSTATUS status;

	status = __sys_NtAlpcQueryInformation(PortHandle, InformationClass, Information, InformationLength, UnknownParameter5);
...
_FX NTSTATUS Ipc_NtAlpcQueryInformationMessage(HANDLE* PortHandle, void* RpcMessage, ULONG InformationClass, void* Information, ULONG InformationLength, ULONG_PTR UnknownParameter6)
{
	NTSTATUS status;

	status = __sys_NtAlpcQueryInformationMessage(PortHandle, RpcMessage, InformationClass, Information, InformationLength, UnknownParameter6);
...
_FX BOOLEAN Ipc_IsAdminToken(HANDLE hToken)
{
	static UCHAR AdministratorsSid[16] = {
	    ...
	    0,
	    0,
	    0, // SubAuthority 1 - SECURITY_BUILTIN_DOMAIN_RID
	    0x20,
	    2,
	    0,
	    0 // SubAuthority 2 - DOMAIN_ALIAS_RID_ADMINS
	};
	...
	P_CheckTokenMembership pCheckTokenMembership = (P_CheckTokenMembership)Ldr_GetProcAddrNew((Dll_KernelBase ? DllName_kernelbase : DllName_advapi32), L"CheckTokenMembership", "CheckTokenMembership");
...
_FX NTSTATUS Ipc_ImpersonateSelf(PORT_MESSAGE* PortMsg)
{
	...
	status = NtOpenThreadToken(NtCurrentThread(), TOKEN_QUERY, FALSE, &hOldToken);
	...
	else
	{
		...
		status = NtQueryInformationToken(hOldToken, TokenImpersonationLevel, &ImpLevel, sizeof(ImpLevel), &len);
		...
		if (PortMsg && (ImpLevel >= SecurityImpersonation) && (!(Dll_ProcessFlags & SBIE_FLAG_IMAGE_FROM_SBIE_DIR)) && (!Ipc_IsAdminToken(hOldToken)))
		{
			//
			...
			{
				WCHAR ImageName[96];
				SbieApi_QueryProcess(OtherProcessId, NULL, ImageName, NULL, NULL);
	...
	status = NtOpenProcessToken(NtCurrentProcess(), TOKEN_DUPLICATE, &hPriToken);
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtDuplicateToken(hPriToken, TOKEN_IMPERSONATE | TOKEN_QUERY, &objattrs, FALSE, TokenImpersonation, &hNewToken);
	...
	if (!NT_SUCCESS(status))
	{
		if (hOldToken)
		{
			NtSetInformationThread(NtCurrentThread(), ThreadImpersonationToken, &hOldToken, sizeof(HANDLE));
		}
	}
...
_FX NTSTATUS Ipc_NtImpersonateClientOfPort(HANDLE PortHandle, PORT_MESSAGE* RequestMessage)
{
	//
	...
	NTSTATUS status = __sys_NtImpersonateClientOfPort(PortHandle, RequestMessage);
...
_FX NTSTATUS Ipc_NtAlpcImpersonateClientOfPort(HANDLE PortHandle, PORT_MESSAGE* RequestMessage, ULONG_PTR UnknownParameter3)
{
	//
	...
	NTSTATUS status = __sys_NtAlpcImpersonateClientOfPort(PortHandle, RequestMessage, UnknownParameter3);
...
_FX NTSTATUS Ipc_NtImpersonateThread(HANDLE ServerThreadHandle, HANDLE ClientThreadHandle, PSECURITY_QUALITY_OF_SERVICE SecurityQos)
{
...
_FX NTSTATUS Ipc_NtCreateEvent(HANDLE* EventHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, EVENT_TYPE EventType, BOOLEAN InitialState)
{
	ULONG LastError;
	THREAD_DATA* TlsData;
	...
	{
		return __sys_NtCreateEvent(EventHandle, DesiredAccess, ObjectAttributes, EventType, InitialState);
	}
	...
	__try
	{
		...
		if (!TruePath)
		{
			status = __sys_NtCreateEvent(EventHandle, DesiredAccess, ObjectAttributes, EventType, InitialState);
		...
		if (PATH_IS_OPEN(mp_flags))
		{
			...
			status = __sys_NtCreateEvent(EventHandle, DesiredAccess, &objattrs, EventType, InitialState);
...
_FX NTSTATUS Ipc_NtOpenEvent(HANDLE* EventHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (!TruePath)
		{
			status = __sys_NtOpenEvent(EventHandle, DesiredAccess, ObjectAttributes);
		...
		status = __sys_NtOpenEvent(EventHandle, DesiredAccess, &objattrs);
...
_FX NTSTATUS Ipc_NtCreateMutant(HANDLE* MutantHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, BOOLEAN InitialOwner)
{
	ULONG LastError;
	THREAD_DATA* TlsData;
	...
	if ((!ObjectAttributes) || (!ObjectAttributes->ObjectName))
	{
		return __sys_NtCreateMutant(MutantHandle, DesiredAccess, ObjectAttributes, InitialOwner);
	}
	...
	__try
	{
		...
		if (!TruePath)
		{
			status = __sys_NtCreateMutant(MutantHandle, DesiredAccess, ObjectAttributes, InitialOwner);
		...
		if (PATH_IS_OPEN(mp_flags))
		{
			...
			status = __sys_NtCreateMutant(MutantHandle, DesiredAccess, &objattrs, InitialOwner);
...
_FX NTSTATUS Ipc_NtOpenMutant(HANDLE* MutantHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (!TruePath)
		{
			status = __sys_NtOpenMutant(MutantHandle, DesiredAccess, ObjectAttributes);
		...
		status = __sys_NtOpenMutant(MutantHandle, DesiredAccess, &objattrs);
...
_FX NTSTATUS Ipc_NtCreateSemaphore(HANDLE* SemaphoreHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, ULONG InitialCount, ULONG MaximumCount)
{
	ULONG LastError;
	THREAD_DATA* TlsData;
	...
	if ((!ObjectAttributes) || (!ObjectAttributes->ObjectName))
	{
		return __sys_NtCreateSemaphore(SemaphoreHandle, DesiredAccess, ObjectAttributes, InitialCount, MaximumCount);
	}
	...
	__try
	{
		...
		if (!TruePath)
		{
			status = __sys_NtCreateSemaphore(SemaphoreHandle, DesiredAccess, ObjectAttributes, InitialCount, MaximumCount);
		...
		if (PATH_IS_OPEN(mp_flags))
		{
			...
			status = __sys_NtCreateSemaphore(SemaphoreHandle, DesiredAccess, &objattrs, InitialCount, MaximumCount);
...
_FX NTSTATUS Ipc_NtOpenSemaphore(HANDLE* SemaphoreHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (!TruePath)
		{
			status = __sys_NtOpenSemaphore(SemaphoreHandle, DesiredAccess, ObjectAttributes);
		...
		status = __sys_NtOpenSemaphore(SemaphoreHandle, DesiredAccess, &objattrs);
...
_FX NTSTATUS Ipc_NtCreateSection(HANDLE* SectionHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes, LARGE_INTEGER* MaximumSize, ULONG PageAttributes, ULONG SectionAttributes, HANDLE FileHandle)
{
	ULONG LastError;
	THREAD_DATA* TlsData;
	...
	if ((!ObjectAttributes) || (!ObjectAttributes->ObjectName))
	{
		if ((!MaximumSize) && (PageAttributes == PAGE_EXECUTE) && (SectionAttributes == SEC_IMAGE))
		{
			//
		...
		return __sys_NtCreateSection(SectionHandle, DesiredAccess, ObjectAttributes, MaximumSize, PageAttributes, SectionAttributes, FileHandle);
	}
	...
	__try
	{
		...
		if (!TruePath)
		{
			status = __sys_NtCreateSection(SectionHandle, DesiredAccess, ObjectAttributes, MaximumSize, PageAttributes, SectionAttributes, FileHandle);
		...
		if (PATH_IS_OPEN(mp_flags))
		{
			...
			status = __sys_NtCreateSection(SectionHandle, DesiredAccess, &objattrs, MaximumSize, PageAttributes, SectionAttributes, FileHandle);
...
_FX NTSTATUS Ipc_NtOpenSection(HANDLE* SectionHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes)
{
	ULONG LastError;
	THREAD_DATA* TlsData = Dll_GetTlsData(&LastError);
	...
	__try
	{
		...
		if (!TruePath)
		{
			status = __sys_NtOpenSection(SectionHandle, DesiredAccess, ObjectAttributes);
		...
		status = __sys_NtOpenSection(SectionHandle, DesiredAccess, &objattrs);
...
_FX BOOLEAN Ipc_IsKnownDllInSandbox(THREAD_DATA* TlsData, const WCHAR* TruePath)
{
	...
	status = NtCreateFile(&handle, FILE_GENERIC_READ, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
...
_FX NTSTATUS Ipc_ConnectProxyPort(HANDLE* PortHandle, const WCHAR* TruePath, ULONG AlpcConnectionFlags, void* ConnectionInfo, ULONG* ConnectionInfoLength, void* MaximumMessageLengthOrAlpcInfo, void* ShouldBeNull1, void* ShouldBeNull2, void* ShouldBeNull3)
{
	NTSTATUS status;
	...
	if (_wcsicmp(TruePath, L"\\RPC Control\\ntsvcs") != 0 && _wcsicmp(TruePath, L"\\RPC Control\\plugplay") != 0)
	{
	...
	if (AlpcConnectionFlags != -1)
	{
		...
		if (AlpcConnectionFlags != 0x20000)
		{ // sync-connection flag??
			err = 0x12;
			goto finish;
		}
		alpc_info = (ALPC_PORT_ATTRIBUTES*)MaximumMessageLengthOrAlpcInfo;
		if (alpc_info->Flags != 0x10000)
		{ // can-impersonate flag??
			err = 0x13;
			goto finish;
		}
...
_FX NTSTATUS Ipc_NtRequestWaitReplyPort(HANDLE PortHandle, PORT_MESSAGE* RequestMsg, PORT_MESSAGE* ReplyMsg)
{
	...
	if (!handle)
	{
		return __sys_NtRequestWaitReplyPort(PortHandle, RequestMsg, ReplyMsg);
	}
	...
	if (RequestMsg->u2.s2.DataInfoOffset)
	{
		...
		info = (PORT_DATA_INFO*)((UCHAR*)RequestMsg + RequestMsg->u2.s2.DataInfoOffset);
	...
	else
	{
		...
		else
		{
			...
			msg->ClientId.UniqueProcess = (HANDLE)(ULONG_PTR)GetCurrentProcessId();
			msg->ClientId.UniqueThread  = (HANDLE)(ULONG_PTR)GetCurrentThreadId();
			...
			if (msg->u2.s2.DataInfoOffset)
			{
				...
				PORT_DATA_INFO* info2 = (PORT_DATA_INFO*)((UCHAR*)msg + msg->u2.s2.DataInfoOffset);
				if (!info)
				{
					err = 0x23;
					goto finish;
				}
				if ((info2->NumDataInfo != info->NumDataInfo) || (info2->BufferLen > info->BufferLen))
				{
					err = 0x24;
					goto finish;
				}
...
_FX NTSTATUS Ipc_NtAlpcSendWaitReceivePort(HANDLE PortHandle, ULONG SendFlags, PORT_MESSAGE* SendMsg, ALPC_MESSAGE_VIEW* SendView, PORT_MESSAGE* ReceiveMsg, ULONG* ReceiveMsgSize, ALPC_MESSAGE_VIEW* ReceiveView, LARGE_INTEGER* Timeout)
{
	...
	if (!handle)
	{
		return __sys_NtAlpcSendWaitReceivePort(PortHandle, SendFlags, SendMsg, SendView, ReceiveMsg, ReceiveMsgSize, ReceiveView, Timeout);
	}
	...
	if ((!SendMsg) || (!SendView) || (!ReceiveMsg) || (!ReceiveView) || (!ReceiveMsgSize))
	{
		SendFlags &= ~0x40000000; // turn off WOW64 flag
		if (SendFlags == 0 && SendView && SendView->SendFlags == 0x40000000 && SendView->ReceiveFlags == SendView->SendFlags
	...
	req_len = sizeof(NAMED_PIPE_ALPC_REQUEST_REQ) + SendMsg->u1.s1.TotalLength;
	req     = Dll_AllocTemp(req_len);
		...
		else
		{
			...
			*ReceiveMsgSize = rpl->msg_len;
			...
			if (ReceiveView->ReceiveFlags & 0x40000000)
			{
				...
				UCHAR* ViewBase = (UCHAR*)VirtualAlloc(NULL, ViewSize, MEM_RESERVE | MEM_COMMIT | MEM_TOP_DOWN, PAGE_READWRITE);
				...
				{
					UCHAR* OldViewBase = InterlockedExchangePointer((void**)&Ipc_ProxyViews[FileIndex], ViewBase);

					ReceiveView->u.s2.ViewAttrs = MEM_MAPPED;
					ReceiveView->u.s2.ViewBase  = (ULONG_PTR)ViewBase;
					ReceiveView->u.s2.ViewSize  = ViewSize;

					memcpy(ViewBase, &rpl->data[(rpl->msg_len + 7) & (~7)], ViewSize);
...
_FX ULONG Ipc_NtQueryObjectName(UNICODE_STRING* ObjectName, ULONG MaxLen)
{
	...
	if (Len >= Dll_BoxIpcPathLen * sizeof(WCHAR) && 0 == Dll_NlsStrCmp(Buf, Dll_BoxIpcPath, Dll_BoxIpcPathLen))
	{
