...
static SC_HANDLE Scm_OpenSCManagerA(UCHAR* lpMachineName, UCHAR* lpDatabaseName, DWORD dwDesiredAccess);

static SC_HANDLE Scm_OpenSCManagerW(WCHAR* lpMachineName, WCHAR* lpDatabaseName, DWORD dwDesiredAccess);

static SC_HANDLE Scm_OpenServiceA(SC_HANDLE hSCManager, const UCHAR* lpServiceName, DWORD dwDesiredAccess);

static SC_HANDLE Scm_OpenServiceW(SC_HANDLE hSCManager, const WCHAR* lpServiceName, DWORD dwDesiredAccess);
...
typedef SC_HANDLE (*P_OpenSCManager)(void* lpMachineName, void* lpDatabaseName, DWORD dwDesiredAccess);

typedef SC_HANDLE (*P_OpenService)(SC_HANDLE hSCManager, void* lpServiceName, DWORD dwDesiredAccess);

typedef BOOL (*P_CloseServiceHandle)(SC_HANDLE hSCObject);

typedef BOOL (*P_QueryServiceStatus)(SC_HANDLE hSCObject, SERVICE_STATUS* lpServiceStatus);
...
typedef BOOL (*P_QueryServiceConfig)(SC_HANDLE hService, void* lpServiceConfig, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

typedef BOOL (*P_QueryServiceConfig2)(SC_HANDLE hService, DWORD InfoLevel, LPBYTE lpBuffer, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

typedef BOOL (*P_EnumServicesStatus)(SC_HANDLE hSCManager, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle);

typedef BOOL (*P_EnumServicesStatusEx)(SC_HANDLE hSCManager, SC_ENUM_TYPE InfoLevel, DWORD dwServiceType, DWORD dwServiceState, void* lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned, LPDWORD lpResumeHandle, void* GroupName);

typedef BOOL (*P_QueryServiceLockStatus)(SC_HANDLE hService, void* lpLockStatus, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

typedef BOOL (*P_GetServiceDisplayName)(SC_HANDLE hSCManager, WCHAR* lpServiceName, WCHAR* lpDisplayName, LPDWORD lpcchBuffer);

typedef BOOL (*P_GetServiceKeyName)(SC_HANDLE hSCManager, WCHAR* lpDisplayName, WCHAR* lpServiceName, LPDWORD lpcchBuffer);

typedef BOOL (*P_EnumDependentServices)(SC_HANDLE hService, DWORD dwServiceState, LPENUM_SERVICE_STATUS lpServices, DWORD cbBufSize, LPDWORD pcbBytesNeeded, LPDWORD lpServicesReturned);

typedef BOOL (*P_QueryServiceObjectSecurity)(SC_HANDLE hService, SECURITY_INFORMATION dwSecurityInformation, PSECURITY_DESCRIPTOR lpSecurityDescriptor, DWORD cbBufSize, LPDWORD pcbBytesNeeded);

typedef BOOL (*P_SetServiceObjectSecurity)(SC_HANDLE hService, SECURITY_INFORMATION dwSecurityInformation, PSECURITY_DESCRIPTOR lpSecurityDescriptor);
...
typedef BOOL (*P_CreateService)(SC_HANDLE hSCManager, void* lpServiceName, void* lpDisplayName, ULONG dwDesiredAccess, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, void* lpBinaryPathName, void* lpLoadOrderGroup, void* lpdwTagId, void* lpDependencies, void* lpServiceStartName, void* lpPassword);

typedef BOOL (*P_ChangeServiceConfig)(SC_HANDLE hService, ULONG dwServiceType, ULONG dwStartType, ULONG dwErrorControl, void* lpBinaryPathName, void* lpLoadOrderGroup, void* lpdwTagId, void* lpDependencies, void* lpServiceStartName, void* lpPassword, void* lpDisplayName);

typedef BOOL (*P_ChangeServiceConfig2)(SC_HANDLE hService, ULONG dwInfoLevel, void* lpInfo);

typedef BOOL (*P_DeleteService)(SC_HANDLE hService);

typedef BOOL (*P_StartService)(SC_HANDLE hService, DWORD dwNumServiceArgs, void* lpServiceArgVector);

typedef BOOL (*P_StartServiceCtrlDispatcher)(const void* ServiceTable);

typedef SERVICE_STATUS_HANDLE (*P_RegisterServiceCtrlHandler)(void* ServiceName, void* HandlerProc);

typedef SERVICE_STATUS_HANDLE (*P_RegisterServiceCtrlHandlerEx)(void* ServiceName, void* HandlerProc, void* Context);

typedef BOOL (*P_SetServiceStatus)(SERVICE_STATUS_HANDLE hServiceStatus, SERVICE_STATUS* ServiceStatus);

typedef BOOL (*P_ControlService)(SC_HANDLE hService, DWORD dwControl, SERVICE_STATUS* lpServiceStatus);

typedef BOOL (*P_ControlServiceEx)(SC_HANDLE hService, DWORD dwControl, ULONG dwInfoLevel, void* pControlParams);
...
typedef BOOL (*P_ReportEvent)(HANDLE hEventLog, WORD wType, WORD wCategory, DWORD dwEventID, PSID lpUserSid, WORD wNumStrings, DWORD dwDataSize, void* Strings, void* RawData);

typedef BOOL (*P_CloseEventLog)(HANDLE hEventLog);
...
static P_QueryServiceObjectSecurity __sys_QueryServiceObjectSecurity = NULL;

static P_SetServiceObjectSecurity __sys_SetServiceObjectSecurity = NULL;
...
static P_StartServiceCtrlDispatcher __sys_StartServiceCtrlDispatcherW = NULL;
static P_StartServiceCtrlDispatcher __sys_StartServiceCtrlDispatcherA = NULL;

static P_RegisterServiceCtrlHandler __sys_RegisterServiceCtrlHandlerW = NULL;
static P_RegisterServiceCtrlHandler __sys_RegisterServiceCtrlHandlerA = NULL;

static P_RegisterServiceCtrlHandlerEx __sys_RegisterServiceCtrlHandlerExW = NULL;
static P_RegisterServiceCtrlHandlerEx __sys_RegisterServiceCtrlHandlerExA = NULL;
...
static const WCHAR* Scm_ServicesKeyPath = L"\\REGISTRY\\MACHINE\\SYSTEM\\CurrentControlSet\\Services\\";
...
#define SCM_IMPORT_XX(base, suffix)                                                                             \
	{                                                                                                           \
		...
		__sys_##base##suffix  = (P_##base)Ldr_GetProcAddrNew(DllName_advapi32, L#base L#suffix, #base #suffix); \
	}

#define SCM_IMPORT_(m, base, suffix)                                                             \
	{                                                                                            \
		const UCHAR* ProcName = #base #suffix;                                                   \
		__sys_##base##suffix  = (P_##base)Ldr_GetProcAddrNew(m, L#base L#suffix, #base #suffix); \
...
#define SBIEDLL_HOOK_SCM(proc)                                                             \
	*(ULONG_PTR*)&__sys_##proc = (ULONG_PTR)SbieDll_Hook(#proc, __sys_##proc, Scm_##proc); \
	if (!__sys_##proc)                                                                     \
		return FALSE;
...
BOOLEAN Scm_HookRegisterServiceCtrlHandler(void)
{
	...
	if (memcmp(__sys_RegisterServiceCtrlHandlerW, PrologW, 7) == 0 && memcmp(__sys_RegisterServiceCtrlHandlerExW, PrologExW, 7) == 0)
	{
		ULONG64 AddrW = (ULONG64)(ULONG_PTR)__sys_RegisterServiceCtrlHandlerW + 3 + 3 + 5 + // xor, xor, jmp
		    *(LONG*)((UCHAR*)__sys_RegisterServiceCtrlHandlerW + 7);
		ULONG64 AddrExW = (ULONG64)(ULONG_PTR)__sys_RegisterServiceCtrlHandlerExW + 6 + 5 + // mov, jmp
...
_FX SC_HANDLE Scm_OpenSCManagerW(WCHAR* lpMachineName, WCHAR* lpDatabaseName, DWORD dwDesiredAccess)
{
...
_FX SC_HANDLE Scm_OpenSCManagerA(UCHAR* lpMachineName, UCHAR* lpDatabaseName, DWORD dwDesiredAccess)
{
...
_FX SC_HANDLE Scm_OpenServiceW(SC_HANDLE hSCManager, const WCHAR* lpServiceName, DWORD dwDesiredAccess)
{
	WCHAR* name;
	BOOLEAN ok = FALSE;
	...
	{
		SERVICE_QUERY_RPL* rpl = (SERVICE_QUERY_RPL*)Scm_QueryServiceByName(lpServiceName, FALSE, FALSE);
		if (rpl)
		{
			Dll_Free(rpl);
			ok = TRUE;
...
_FX SC_HANDLE Scm_OpenServiceA(SC_HANDLE hSCManager, const UCHAR* lpServiceName, DWORD dwDesiredAccess)
{
	SC_HANDLE h;
	ANSI_STRING ansi;
	UNICODE_STRING uni;
...
_FX BOOLEAN Scm_IsBoxedService(const WCHAR* ServiceName)
{
	...
	if (!found)
	{
		if (_wcsicmp(ServiceName, Scm_MsiServer) == 0 || _wcsicmp(ServiceName, _TrustedInstaller) == 0 || _wcsicmp(ServiceName, _bits) == 0 || _wcsicmp(ServiceName, _wuauserv) == 0 || _wcsicmp(ServiceName, Scm_CryptSvc) == 0)
		{
			found = TRUE;
		}
	}
...
_FX WCHAR* Scm_GetBoxedServices(void)
{
	...
	if ((!NT_SUCCESS(status)) || u.info.Type != REG_MULTI_SZ || u.info.DataLength == 0)
	{
		goto finish;
	}
...
_FX WCHAR* Scm_GetAllServices(void)
{
	...
	while (1)
	{
		...
		for (work_ptr = true_names; *work_ptr; work_ptr += wcslen(work_ptr) + 1)
		{
			if (*work_ptr == L'*')
			{
				continue;
		...
		if (copy_names)
		{
			for (work_ptr = copy_names; *work_ptr; work_ptr += wcslen(work_ptr) + 1)
			{
				if (*work_ptr == L'*')
				{
					continue;
		...
		if (last_ptr && *small_ptr == last_ptr0 && _wcsicmp(small_ptr + 1, last_ptr + 1) == 0)
		{
			*small_ptr = L'*';
			continue;
		}
...
_FX HANDLE Scm_OpenKeyForService(const WCHAR* ServiceName, BOOLEAN ForWrite)
{
	...
	{
		ULONG disp;
		status = NtCreateKey(&handle, KEY_ALL_ACCESS, &objattrs, 0, NULL, 0, &disp);
	}
	else
	{
