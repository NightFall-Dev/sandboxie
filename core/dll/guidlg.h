...
void* GUI_CreateDialogTemplate(DLGTMPL* tmpl)
{
	DLGTMPL1* newTmpl;
	UCHAR *ptr, *out;
	WCHAR* old_clsnm[256];
	...
	{
		int ofs = (ULONG)((UCHAR*)&tmpl->typeface - (UCHAR*)&tmpl->pointsize);
		ptr += ofs;
	...
	size += ptr - (UCHAR*)tmpl + wcslen(new_winnm) * sizeof(WCHAR) + sizeof(DWORD) * 4 * tmpl->cDlgItems;
	alloc_size = size + 8;
