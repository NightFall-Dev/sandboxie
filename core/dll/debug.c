...
static BOOL Debug_WaitForDebugEvent(LPDEBUG_EVENT lpDebugEvent, DWORD dwMilliseconds);
...
static ULONG_PTR Debug_GetProcAddress(HMODULE hModule, const UCHAR* ProcName);
	...
	#ifdef HIDE_SBIEDLL
static NTSTATUS Debug_LdrGetDllHandle(WCHAR* DllPath, ULONG* DllCharacteristics, UNICODE_STRING* DllName, ULONG_PTR* DllHandle);
	#endif
...
typedef BOOL (*P_WaitForDebugEvent)(LPDEBUG_EVENT lpDebugEvent, DWORD dwMilliseconds);
...
static P_GetProcAddress __sys_GetProcAddress = NULL;
	...
	#ifdef HIDE_SBIEDLL
typedef NTSTATUS (*P_LdrGetDllHandle)(WCHAR* DllPath, ULONG* DllCharacteristics, UNICODE_STRING* DllName, ULONG_PTR* DllHandle);
static P_LdrGetDllHandle __sys_LdrGetDllHandle = NULL;
__declspec(dllimport) NTSTATUS LdrGetDllHandle(WCHAR* DllPath, ULONG* DllCharacteristics, UNICODE_STRING* DllName, ULONG_PTR* DllHandle);
	#endif
...
_FX int Debug_Init(void)
{
	...
	RtlSetLastWin32Error = (P_RtlSetLastWin32Error)GetProcAddress(Dll_Ntdll, "RtlSetLastWin32Error");
	...
	OutputDebugStringW = (P_OutputDebugString)GetProcAddress(Dll_Kernel32, "OutputDebugStringW");
	OutputDebugStringA = (P_OutputDebugString)GetProcAddress(Dll_Kernel32, "OutputDebugStringA");
	...
	DebugActiveProcess = (P_DebugActiveProcess)GetProcAddress(Dll_Kernel32, "DebugActiveProcess");

	WaitForDebugEvent = (P_WaitForDebugEvent)GetProcAddress(Dll_Kernel32, "WaitForDebugEvent");
...
ALIGNED BOOL Debug_WaitForDebugEvent(LPDEBUG_EVENT lpDebugEvent, DWORD dwMilliseconds)
{
	WCHAR txt[256];
	BOOL ok;
	ULONG err;
	...
	Sbie_swprintf(txt, L"Debug Event %d on Process Id %d\n", ok ? lpDebugEvent->dwDebugEventCode : 0, ok ? lpDebugEvent->dwProcessId : 0);
	OutputDebugString(txt);
...
//---------------------------------------------------------------------------
	...
	#ifdef HIDE_SBIEDLL
_FX NTSTATUS Debug_LdrGetDllHandle(WCHAR* DllPath, ULONG* DllCharacteristics, UNICODE_STRING* DllName, ULONG_PTR* DllHandle)
{
	NTSTATUS status = __sys_LdrGetDllHandle(DllPath, DllCharacteristics, DllName, DllHandle);
	if (Dll_InitComplete)
	{
		if (NT_SUCCESS(status) && DllHandle && *DllHandle == (ULONG_PTR)Dll_Instance)
		{
			*DllHandle = 0;
