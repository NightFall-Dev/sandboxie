...
bool DriverAssist::InjectLow_InitHelper(ULONG* errlvl)
{
	...
	*errlvl = 0x22;
	...
	if (nt_hdrs->FileHeader.NumberOfSections < 2)
	{
		return false;
	}
...
void DriverAssist::InjectLow(void* _msg)
{
	...
	ULONG_PTR tramp_remote_addr =                      // calculate address in remote process
	    (ULONG_PTR)remote_addr + m_sbielow_data_offset // offset of args area
	    + FIELD_OFFSET(SBIELOW_DATA, LdrInitializeThunk_tramp);

	if (!InjectLow_BuildTramp(_msg, lowdata.long_diff, lowdata.LdrInitializeThunk_tramp, tramp_remote_addr))
	{
		errlvl = 0x44;
		goto finish;
	}
	...
	if (!msg->bHostInject)
	{
		if (!GuiServer::GetInstance()->InitProcess(hProcess, msg->process_id, msg->session_id, msg->add_to_job))
		{
			errlvl = 0x88;
			goto finish;
		}
...
finish:
	...
	if (errlvl)
	{
		...
		SbieApi_LogEx(msg->session_id, 2335, L"%S [%02X / %d]", msg->process_name, errlvl, err);
	}
...
HANDLE DriverAssist::InjectLow_OpenProcess(void* _msg)
{
	...
	const ULONG _DesiredAccess = PROCESS_DUP_HANDLE | PROCESS_TERMINATE | PROCESS_SUSPEND_RESUME | PROCESS_SET_INFORMATION | PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE;
...
HANDLE DriverAssist::InjectLow_SendHandle(HANDLE hProcess)
{
	...
	status = NtOpenFile(&HandleLocal, FILE_GENERIC_READ, &objattrs, &MyIoStatusBlock, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, 0);
	...
	if (NT_SUCCESS(status))
	{
		...
		BOOL ok = DuplicateHandle(NtCurrentProcess(), HandleLocal, hProcess, &HandleRemote, 0, FALSE, DUPLICATE_SAME_ACCESS);
...
void* DriverAssist::InjectLow_CopyCode(HANDLE hProcess, BOOLEAN iswow64, UCHAR* code, ULONG code_len)
{
	...
	if (remote_addr)
	{
		...
		SIZE_T len2 = 0;
		BOOL vm_ok  = WriteProcessMemory(hProcess, remote_addr, m_sbielow_ptr, len1, &len2);
		...
		if (vm_ok && len1 == len2)
		{
            ...
            */
			vm_ok = ReadProcessMemory(hProcess, (void*)m_LdrInitializeThunk, code, len1, &len2);
...
bool DriverAssist::InjectLow_BuildTramp(void* _msg, BOOLEAN long_diff, UCHAR* code, ULONG_PTR addr)
{
...
#undef IS_1BYTE
	...
	if (!long_diff)
	{
		if (Dll_Windows >= 10)
		{
			code[offset]               = 0x48;
			code[offset + 1]           = 0xE9;
			*(ULONG*)&code[offset + 2] = (ULONG)(m_LdrInitializeThunk + offset - (addr + offset + 6));
		}
		else
		{
			code[offset]               = 0xe9;
			*(ULONG*)&code[offset + 1] = (ULONG)(m_LdrInitializeThunk + offset - (addr + offset + 5));
		}
	}
	else
	{
...
void* DriverAssist::InjectLow_CopySyscalls(HANDLE hProcess)
{
	...
	if (remote_addr)
	{
		...
		SIZE_T len2 = 0;
		BOOL vm_ok  = WriteProcessMemory(hProcess, remote_addr, m_syscall_data, len1, &len2);
		if (vm_ok && len1 == len2)
		{
			return remote_addr;
		}
...
bool DriverAssist::InjectLow_CopyData(HANDLE hProcess, void* remote_addr, void* local_data)
{
	//
	...
	SIZE_T len2     = 0;
	BOOL vm_ok      = WriteProcessMemory(hProcess, data_addr, local_data, len1, &len2);
	if (vm_ok && len1 == len2)
	{
		ULONG protect;
		vm_ok = VirtualProtectEx(hProcess, remote_addr, m_sbielow_len, PAGE_EXECUTE_READ, &protect);
		if (vm_ok)
		{
			return true;
		}
...
void* DriverAssist::InjectLow_getPage(HANDLE hProcess, void* remote_addr)
{
    ...
    */
	...
	{ //not hot patch area: This is a hack
		...
		{
			if (*((ULONG_PTR*)&myBuffer[i]) == 0x9090909090909090 || *((ULONG_PTR*)&myBuffer[i]) == 0xcccccccccccccccc)
			{
