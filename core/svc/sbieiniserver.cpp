...
MSG_HEADER* SbieIniServer::Handler2(MSG_HEADER* msg)
{
	...
	if (msg->msgid == MSGID_SBIE_INI_SET_PASSWORD || msg->msgid == MSGID_SBIE_INI_TEST_PASSWORD)
	{
	...
	if (msg->msgid == MSGID_SBIE_INI_SET_SETTING || msg->msgid == MSGID_SBIE_INI_ADD_SETTING || msg->msgid == MSGID_SBIE_INI_INS_SETTING || msg->msgid == MSGID_SBIE_INI_DEL_SETTING)
	{
...
MSG_HEADER* SbieIniServer::GetVersion(MSG_HEADER* msg)
{
	...
	ULONG rpl_len                 = sizeof(SBIE_INI_GET_USER_RPL) + (ver_len + 1) * sizeof(WCHAR);
	SBIE_INI_GET_VERSION_RPL* rpl = (SBIE_INI_GET_VERSION_RPL*)LONG_REPLY(rpl_len);
	if (!rpl)
	{
...
MSG_HEADER* SbieIniServer::GetWaitHandle(HANDLE idProcess)
{
	...
	{
		HANDLE pHandle;
		BOOL ok = DuplicateHandle(GetCurrentProcess(), GetCurrentProcess(), hCallerProcess, &pHandle, SYNCHRONIZE, FALSE, 0);
		if (!ok)
		{
			LastError = GetLastError();
		}
		...
		if (ok)
		{
			...
			SBIE_INI_GET_WAIT_HANDLE_RPL* rpl = (SBIE_INI_GET_WAIT_HANDLE_RPL*)LONG_REPLY(rpl_len);
			if (!rpl)
			{
				LastError = GetLastError();
			}
...
MSG_HEADER* SbieIniServer::GetUser(MSG_HEADER* msg)
{
	...
	ULONG rpl_len              = sizeof(SBIE_INI_GET_USER_RPL) + (name_len + 1) * sizeof(WCHAR);
	SBIE_INI_GET_USER_RPL* rpl = (SBIE_INI_GET_USER_RPL*)LONG_REPLY(rpl_len);
	if (!rpl)
	{
		return SHORT_REPLY(STATUS_INSUFFICIENT_RESOURCES);
	}
...
MSG_HEADER* SbieIniServer::GetPath(MSG_HEADER* msg)
{
	...
	{
		ULONG rpl_len              = sizeof(SBIE_INI_GET_PATH_RPL) + (wcslen(IniPath) + 1) * sizeof(WCHAR);
		SBIE_INI_GET_PATH_RPL* rpl = (SBIE_INI_GET_PATH_RPL*)LONG_REPLY(rpl_len);
...
bool SbieIniServer::SetUserSettingsSectionName(HANDLE hToken)
{
	...
	NTSTATUS status = SbieApi_QueryConfAsIs(_portable, NULL, 0, info.value, sizeof(info.value));
	...
	BOOL ok        = GetTokenInformation(hToken, TokenUser, &info, info_len, &info_len);
	if (!ok)
	{
		return false;
	}
	...
	ok = LookupAccountSid(NULL, info.user.User.Sid, m_username, &username_len, domain, &domain_len, &use);
	if (!ok || !m_username[0])
	{
		return false;
	}
	...
	ULONG crc = CRC_Adler32((UCHAR*)m_username, wcslen(m_username) * sizeof(WCHAR));
	...
	if (b)
	{
		...
		if (!b)
		{
			...
			if (GetVersionEx(&osvi) && osvi.dwMajorVersion >= 6)
			{
				...
				b = GetTokenInformation(hToken, (TOKEN_INFORMATION_CLASS)TokenElevationType, &elevationType, sizeof(elevationType), &len);
				if (b && (elevationType != TokenElevationTypeFull && elevationType != TokenElevationTypeLimited))
				{
					b = FALSE;
				}
			}
...
bool SbieIniServer::HashPassword(const WCHAR* Password, WCHAR* Hash41)
{
	...
	ok = CryptAcquireContext(&hCryptProv, NULL, MS_DEF_PROV, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT | CRYPT_SILENT);
	if (!ok)
	{
		ErrorLevel = 0x11;
		goto finish;
...
finish:
	...
	if (!ok)
	{
		SbieApi_LogEx(m_session_id, 2323, L"[%d / %08X]", ErrorLevel, LastError);
	}
...
ULONG SbieIniServer::IsCallerAuthorized(HANDLE hToken, const WCHAR* Password)
{
	...
	buf[41] = L'\0';
	SbieApi_QueryConfAsIs(NULL, L"EditPassword", 0, buf, 41 * sizeof(WCHAR));
	if (!buf[0])
	{
		return STATUS_SUCCESS;
	}
...
ULONG SbieIniServer::SetSetting(MSG_HEADER* msg)
{
	...
	bool delete_section_request = (*req->setting == L'*' && (!have_value)) ? true : false;
	...
	while (1)
	{
		...
		{
			status = SbieApi_QueryConfAsIs(NULL, NULL, section_idx | CONF_GET_NO_TEMPLS, section_name, sizeof(WCHAR) * 66);
		...
		bool section_match = (_wcsicmp(section_name, req->section) == 0);
		...
		while (1)
		{
			status = SbieApi_QueryConfAsIs(section_name, NULL, setting_idx | CONF_GET_NO_TEMPLS, setting, sizeof(WCHAR) * 66);
			...
			while (1)
			{
				ULONG remaining_buf_len = sizeof(m_line) - sizeof(WCHAR) * 4 - (ptr + 1 - setting) * sizeof(WCHAR);

				status = SbieApi_QueryConfAsIs(section_name, setting, value_idx | CONF_GET_NO_GLOBAL | CONF_GET_NO_TEMPLS, ptr + 1, remaining_buf_len);
...
ULONG SbieIniServer::AddSetting(MSG_HEADER* msg, bool insert)
{
	...
	while (1)
	{
		...
		{
			status = SbieApi_QueryConfAsIs(req->section, req->setting, index | CONF_GET_NO_GLOBAL | CONF_GET_NO_TEMPLS, setting, sizeof(m_line) - sizeof(WCHAR) * 4);
		...
		if (text)
		{
			WCHAR* text2 = (WCHAR*)HeapReAlloc(GetProcessHeap(), 0, text, text_len);
			if (!text2)
			{
...
ULONG SbieIniServer::SetTemplate(MSG_HEADER* msg)
{
	...
	NTSTATUS status;
	ULONG req2_len             = sizeof(SBIE_INI_SETTING_REQ) + value_len * sizeof(WCHAR);
	SBIE_INI_SETTING_REQ* req2 = (SBIE_INI_SETTING_REQ*)HeapAlloc(GetProcessHeap(), 0, req2_len);
	if (!req2)
	{
...
ULONG SbieIniServer::SetOrTestPassword(MSG_HEADER* msg)
{
	...
	{
		ULONG req2_len             = sizeof(SBIE_INI_SETTING_REQ) + 128 * sizeof(WCHAR);
		SBIE_INI_SETTING_REQ* req2 = (SBIE_INI_SETTING_REQ*)HeapAlloc(GetProcessHeap(), 0, req2_len);
		if (!req2)
		{
			status = STATUS_INSUFFICIENT_RESOURCES;
...
bool SbieIniServer::AddText(const WCHAR* line)
{
	...
	if ((!m_text_base) || (m_text - m_text_base + line_len + 32 > m_text_max_len))
	{
		ULONG text_max_len = m_text_max_len + line_len + 1024;
		WCHAR* text_base   = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, text_max_len * sizeof(WCHAR));
		if (!text_base)
		{
			return false;
		}
...
ULONG SbieIniServer::RefreshConf()
{
	...
	const ULONG CONFLICTING_ATTRS = (FILE_ATTRIBUTE_DEVICE | FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_REPARSE_POINT | FILE_ATTRIBUTE_ENCRYPTED | FILE_ATTRIBUTE_OFFLINE);
	...
	const ULONG REMOVABLE_ATTRS = (FILE_ATTRIBUTE_READONLY | FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM);
	...
	hFile = CreateFile(IniPath, FILE_GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
bool SbieIniServer::GetIniPath(WCHAR** IniPath, WCHAR** TmpPath, BOOLEAN* IsHomePath)
{
...
void SbieIniServer::LockConf(WCHAR* IniPath)
{
	...
	m_hLockFile = CreateFile(IniPath, FILE_GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
...
MSG_HEADER* SbieIniServer::RunSbieCtrl(HANDLE idProcess, bool isSandboxed)
{
	...
	if (ok && isSandboxed)
	{
		...
		if (ok2)
		{
			SbieApi_QueryConfAsIs(m_sectionname, _Setting, 0, buf, 8 * sizeof(WCHAR));
			ch = towlower(buf[0]);
		}
		if (!ch)
		{
	...
	if (ok)
	{
		...
		if (SbieDll_RunFromHome(SBIECTRL_EXE, args, &si, NULL))
		{
			...
			ok = CreateProcessAsUser(hToken, NULL, CmdLine, NULL, NULL, FALSE, (Environ ? CREATE_UNICODE_ENVIRONMENT : 0), Environ, NULL, &si, &pi);
