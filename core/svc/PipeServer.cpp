...
bool PipeServer::Start()
{
	...
	InitializeObjectAttributes(&objattrs, &PortName, OBJ_CASE_INSENSITIVE, NULL, sd);

	status = NtCreatePort((HANDLE*)&m_hServerPort, &objattrs, 0, MAX_PORTMSG_LENGTH, NULL);
	...
	for (i = 0; i < NUMBER_OF_THREADS; ++i)
	{
		m_Threads[i] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ThreadStub, this, 0, &idThread);
		if (!m_Threads[i])
		{
...
void PipeServer::Thread()
{
	...
	while (1)
	{
		...
		else if (msg->u2.s2.Type == LPC_PORT_CLOSED || msg->u2.s2.Type == LPC_CLIENT_DIED)
		{
			PortDisconnect(msg);
		}
	}
...
void PipeServer::PortConnect(PORT_MESSAGE* msg)
{
	...
	if (!clientProcess)
	{
		clientProcess = (CLIENT_PROCESS*)Pool_Alloc(m_pool, sizeof(CLIENT_PROCESS));
		if (clientProcess)
		{
			...
			HANDLE hProcess = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, (ULONG)(ULONG_PTR)msg->ClientId.UniqueProcess);
			if (hProcess)
			{
	...
	{
		clientThread = (CLIENT_THREAD*)Pool_Alloc(m_pool, sizeof(CLIENT_THREAD));
		if (clientThread)
		{
...
void PipeServer::PortDisconnectByCreateTime(LARGE_INTEGER* CreateTime)
{
	...
	if (!init_done)
	{
		pGetProcessIdOfThread = (P_GetProcessIdOfThread)GetProcAddress(_Kernel32, "GetProcessIdOfThread");
	...
	while (clientProcess)
	{
		...
		{
			CLIENT_THREAD* clientThread = (CLIENT_THREAD*)List_Head(&clientProcess->threads);
			while (clientThread)
			{
				//
				...
				HANDLE hThread = OpenThread(THREAD_QUERY_INFORMATION, FALSE, (ULONG)(ULONG_PTR)clientThread->idThread);
				if (hThread)
				{
...
void PipeServer::PortRequest(HANDLE PortHandle, PORT_MESSAGE* msg, void* voidClient)
{
	...
	if (!client->buf_hdr)
	{
		...
		if (msgid && buf_len && buf_len < MAX_REQUEST_LENGTH && buf_len >= sizeof(MSG_HEADER) && buf_len >= msg->u1.s1.DataLength)
		{
...
void PipeServer::PortReply(PORT_MESSAGE* msg, void* voidClient)
{
	...
	buf_len = client->buf_hdr->length - (ULONG)(client->buf_ptr - (UCHAR*)client->buf_hdr);
...
{
	UCHAR* buf = (UCHAR*)msg;
	if (*(ULONG*)(buf + msg->length) != 0 || *(ULONG*)(buf + msg->length + sizeof(ULONG)) != tzuk)
	{
...
{
	CLIENT_TLS_DATA* TlsData = (CLIENT_TLS_DATA*)TlsGetValue(m_instance->m_TlsIndex);
...
ULONG PipeServer::ImpersonateCaller(MSG_HEADER** pmsg)
{
	...
	{
		status = NtImpersonateClientOfPort(TlsData->PortHandle, TlsData->PortMessage);
...
{
	PORT_MESSAGE PortMsg;
	PortMsg.ClientId.UniqueProcess = (HANDLE)(ULONG_PTR)GetCurrentProcessId();
	PortMsg.ClientId.UniqueThread  = (HANDLE)(ULONG_PTR)GetCurrentThreadId();
