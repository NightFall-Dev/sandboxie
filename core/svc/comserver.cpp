...
#define MAX_MAP_BUFFER_LENGTH (COM_SLAVE_MAP_SIZE - FIELD_OFFSET(COM_SLAVE_MAP, Buffer))
...
static const GUID IID_IWbemServices    = {0x9556DC99, 0x828C, 0x11CF, {0xA3, 0x7E, 0x00, 0xAA, 0x00, 0x32, 0x40, 0xC7}};
static const GUID IID_IWbemClassObject = {0xDC12A681, 0x737F, 0x11CF, {0x88, 0x4D, 0x00, 0xAA, 0x00, 0x4B, 0x2E, 0x24}};
...
MSG_HEADER* ComServer::GetClassObjectHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	ULONG exc;
	if (!SbieDll_IsOpenClsid(guids[0], CLSCTX_LOCAL_SERVER, slave->BoxName))
	{
	...
	COM_GET_CLASS_OBJECT_RPL* rpl = (COM_GET_CLASS_OBJECT_RPL*)LONG_REPLY(sizeof(COM_GET_CLASS_OBJECT_RPL));
	if (rpl)
	{
		rpl->hr     = pMap->hr;
...
MSG_HEADER* ComServer::CreateInstanceHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	COM_CREATE_INSTANCE_RPL* rpl = (COM_CREATE_INSTANCE_RPL*)LONG_REPLY(sizeof(COM_CREATE_INSTANCE_RPL));
	if (rpl)
	{
		rpl->hr     = pMap->hr;
		rpl->objidx = pMap->objidx;
...
MSG_HEADER* ComServer::QueryInterfaceHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	COM_QUERY_INTERFACE_RPL* rpl = (COM_QUERY_INTERFACE_RPL*)LONG_REPLY(sizeof(COM_QUERY_INTERFACE_RPL));
	if (rpl)
	{
		rpl->hr     = pMap->hr;
		rpl->objidx = pMap->objidx;
...
MSG_HEADER* ComServer::AddRefReleaseHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	COM_ADD_REF_RELEASE_RPL* rpl = (COM_ADD_REF_RELEASE_RPL*)LONG_REPLY(sizeof(COM_ADD_REF_RELEASE_RPL));
	if (rpl)
	{
		rpl->hr       = pMap->hr;
...
MSG_HEADER* ComServer::InvokeMethodHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
...
MSG_HEADER* ComServer::UnmarshalInterfaceHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	COM_UNMARSHAL_INTERFACE_RPL* rpl = (COM_UNMARSHAL_INTERFACE_RPL*)LONG_REPLY(sizeof(COM_UNMARSHAL_INTERFACE_RPL));
	if (rpl)
	{
		rpl->hr     = pMap->hr;
		rpl->objidx = pMap->objidx;
...
MSG_HEADER* ComServer::MarshalInterfaceHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	ULONG rpl_len                  = sizeof(COM_MARSHAL_INTERFACE_RPL) + slave->pMap->BufferLength;
	COM_MARSHAL_INTERFACE_RPL* rpl = (COM_MARSHAL_INTERFACE_RPL*)LONG_REPLY(rpl_len);
	if (rpl)
	{
		rpl->hr           = pMap->hr;
		rpl->BufferLength = pMap->BufferLength;
...
MSG_HEADER* ComServer::QueryBlanketHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	COM_QUERY_BLANKET_RPL* rpl = (COM_QUERY_BLANKET_RPL*)LONG_REPLY(sizeof(COM_QUERY_BLANKET_RPL));
	if (rpl)
	{
		rpl->hr = pMap->hr;
		...
		memcpy(rpl->ServerPrincName, buf->ServerPrincName, sizeof(rpl->ServerPrincName));
	}
...
MSG_HEADER* ComServer::SetBlanketHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	COM_SET_BLANKET_RPL* rpl = (COM_SET_BLANKET_RPL*)LONG_REPLY(sizeof(COM_SET_BLANKET_RPL));
	if (rpl)
	{
		rpl->hr = pMap->hr;
	}
...
MSG_HEADER* ComServer::CopyProxyHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	...
	COM_COPY_PROXY_RPL* rpl = (COM_COPY_PROXY_RPL*)LONG_REPLY(sizeof(COM_COPY_PROXY_RPL));
	if (rpl)
	{
		rpl->hr     = pMap->hr;
		rpl->objidx = pMap->objidx;
...
MSG_HEADER* ComServer::NonComHandler(MSG_HEADER* msg, void* _slave, BOOLEAN* deleted)
{
	if (msg->length < sizeof(MSG_HEADER) || msg->length >= MAX_MAP_BUFFER_LENGTH)
	{
...
void* ComServer::LockSlave(HANDLE idProcess, ULONG msgid)
{
	...
	LONG rc = SbieApi_QueryProcess(idProcess, u.s.boxname, NULL, u.s.sid, &session_id);
	if (rc != 0)
	{
		return NULL;
	}
	...
	if (pIsWow64Process)
	{
		HANDLE hProcess = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, (ULONG)(ULONG_PTR)idProcess);
		if (hProcess)
		{
			BOOL xwow64 = FALSE;
	...
	#ifdef DEBUG_COMSERVER
			WCHAR txt[256];
			wsprintf(txt, L"LockSlave     idProcess=%d Wow64=%d msgid=%X\n", idProcess, IsWow64, msgid);
		...
		{
			WCHAR txt[256];
			wsprintf(txt, L"LockSlave     Cannot determine wow64ness for idProcess=%d\n", idProcess);
			OutputDebugString(txt);
		}
	#endif
	}
...
RetryLockSlave:
	...
	while (slave)
	{
		...
		else if (_wcsicmp(slave->SidString, u.s.sid) == 0 && _wcsicmp(slave->BoxName, u.s.boxname) == 0 && slave->SessionId == session_id
		    && slave->IsWow64 == IsWow64)
		{
#ifdef DEBUG_COMSERVER
			WCHAR txt[256];
			wsprintf(txt, L"LockSlave     idProcess=%d SlaveProcess=%d\n", idProcess, slave->idProcess);
			OutputDebugString(txt);
#endif
	...
	if (slave)
	{
		...
		{
			if (GetTickCount() - slave->lock_ticks >= SLAVE_TIMEOUT_SECONDS * 2 * 1000)
			{
				//
			...
			if (slave)
			{
				...
				WaitForSingleObject(m_SlaveReleasedEvent, SLAVE_TIMEOUT_SECONDS * 1000);
	...
	if ((!slave) && (msgid == MSGID_COM_GET_CLASS_OBJECT || msgid == MSGID_COM_UNMARSHAL_INTERFACE || ((msgid & 0xFF) > 0x80)))
	{
		...
		slave = (COM_SLAVE*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(COM_SLAVE));
		if (!slave)
		{
			error = 0x11;
		...
		wsprintf(u.path, _tmpl, slave->SidString, slave->BoxName, slave->SessionId, slave->IsWow64, L"Mutex");
		...
		wsprintf(u.path, _tmpl, slave->SidString, slave->BoxName, slave->SessionId, slave->IsWow64, L"Event1");
		...
		wsprintf(u.path, _tmpl, slave->SidString, slave->BoxName, slave->SessionId, slave->IsWow64, L"Event2");
		...
		wsprintf(u.path, _tmpl, slave->SidString, slave->BoxName, slave->SessionId, slave->IsWow64, L"Map");
		slave->hMap = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, COM_SLAVE_MAP_SIZE, u.path);
		if (!slave->hMap)
		{
			error = 0x15;
			goto slave_create_done;
		...
		slave->pMap = (COM_SLAVE_MAP*)MapViewOfFile(slave->hMap, FILE_MAP_ALL_ACCESS, 0, 0, COM_SLAVE_MAP_SIZE);
		if (!slave->pMap)
		{
			error = 0x16;
			goto slave_create_done;
		...
		ok = OpenThreadToken(GetCurrentThread(), TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY, FALSE, &hImpToken);
		if (!ok)
		{
			error = 0x18;
			goto slave_create_done;
		...
		ok = DuplicateTokenEx(hImpToken, 0, NULL, SecurityImpersonation, TokenPrimary, &hPriToken);
		if (!ok)
		{
			error = 0x19;
			goto slave_create_done;
		...
		wsprintf(u.path, _tmpl, slave->SidString, slave->BoxName, slave->SessionId, slave->IsWow64, L":");
		...
		ok = CreateProcessAsUser(hPriToken, NULL, cmdline, NULL, NULL, FALSE, CREATE_UNICODE_ENVIRONMENT, env, NULL, &si, &pi);
		if (!ok)
		{
			error = 0x22;
			goto slave_create_done;
...
#ifdef DEBUG_COMSERVER
		WCHAR txt[256];
		wsprintf(txt, L"LockSlave     SlaveProcess=%d created for idProcess=%d\n", slave->idProcess, idProcess);
		OutputDebugString(txt);
#endif
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"CallSlave error, Pid = %d, status = %d\n", GetCurrentProcessId(), status);
	OutputDebugString(txt);
#endif
...
void ComServer::NotifyAllSlaves(HANDLE idProcess)
{
	...
	ULONG64* ThreadData = (ULONG64*)HeapAlloc(GetProcessHeap(), 0, sizeof(ULONG64) * 2);
	if (!ThreadData)
	{
		return;
	}
	...
	hThread       = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)NotifyAllSlaves2, ThreadData, 0, &idThread);
	if (hThread)
	{
		CloseHandle(hThread);
	}
...
void ComServer::RunSlave(const WCHAR* cmdline)
{
	...
	HANDLE hParentProcessMutex = OpenMutex(MUTEX_MODIFY_STATE | SYNCHRONIZE, FALSE, objname);
	...
	HANDLE hEvent1 = OpenEvent(EVENT_MODIFY_STATE | SYNCHRONIZE, FALSE, objname);
	if (!hEvent1)
	{
		return;
	}
	...
	HANDLE hEvent2 = OpenEvent(EVENT_MODIFY_STATE | SYNCHRONIZE, FALSE, objname);
	if (!hEvent1)
	{
		return;
	}
	...
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)MapViewOfFile(hMap, FILE_MAP_ALL_ACCESS, 0, 0, COM_SLAVE_MAP_SIZE);
	if (!pMap)
	{
		return;
	}
...
void* ComServer::FindSlaveObject(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	...
	while (obj)
	{
		if (pMap->idProcess == obj->idProcess && pMap->objidx == obj->objidx)
		{
			break;
		}
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"FindSlaveObject         objidx=%08X idProcess=%06d obj=%08X\n", pMap->objidx, pMap->idProcess, obj);
	OutputDebugString(txt);
#endif
...
void* ComServer::RefOrAllocSlaveObject(ULONG idProcess, void* pUnknown, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_OBJECT* obj = (COM_OBJECT*)List_Head(ObjectsList);
	while (obj)
	{
		if (idProcess == obj->idProcess && pUnknown == obj->pUnknown)
		{
			obj->pUnknown->AddRef();
			break;
		}
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"RefSlaveObject          pUnknown=%08X idProcess=%06d obj=%08X\n", pUnknown, idProcess, obj);
	OutputDebugString(txt);
#endif
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"DeleteSlaveObject       objidx=%08X idProcess=%06d obj=%08X\n", obj->objidx, obj->idProcess, obj);
	OutputDebugString(txt);
#endif
...
void ComServer::GetClassObjectSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
	...
	if (pMap->ProcNum)
	{
		...
		*hr = CoGetObject(moniker, (BIND_OPTS*)&bindopts, (REFIID)guids[1], (void**)&pUnknown);
	}
	else
	{
		//
		...
		*hr = CoGetClassObject((REFCLSID)guids[0], CLSCTX_LOCAL_SERVER, NULL, (REFIID)guids[1], (void**)&pUnknown);
		if (FAILED(*hr))
		{
			*hr = CoGetClassObject((REFCLSID)guids[0], CLSCTX_INPROC_SERVER, NULL, (REFIID)guids[1], (void**)&pUnknown);
			if (FAILED(*hr))
			{
				return;
			}
	...
	COM_OBJECT* obj = (COM_OBJECT*)RefOrAllocSlaveObject(pMap->idProcess, pUnknown, ObjectsList, exc, hr);
	if (!obj)
	{
		return;
	}
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) GetClassObjectSlave     objidx=%08X pUnknown=%08X pChannel=%08X pStub=%08X\n", List_Count(ObjectsList), obj->objidx, obj->pUnknown, obj->pChannel, obj->pStub);
	OutputDebugString(txt);
#endif
}
...
void ComServer::CreateInstanceSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
	...
	COM_OBJECT* obj = (COM_OBJECT*)FindSlaveObject(pMap, ObjectsList, exc, hr);
	if (!obj)
	{
		return;
	}
	...
	*hr                          = pClassFactory->CreateInstance(NULL, *guid, (void**)&pUnknown);
	...
	obj = (COM_OBJECT*)RefOrAllocSlaveObject(pMap->idProcess, pUnknown, ObjectsList, exc, hr);
	if (!obj)
	{
		return;
	}
	...
	if (memcmp(&obj->iid, &GUID_NULL, sizeof(GUID)) == 0)
	{
		...
		*hr = SbieDll_ComCreateStub(*guid, pUnknown, (void**)&obj->pStub, (void**)&obj->pChannel);
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) CreateInstanceSlave     objidx=%08X pUnknown=%08X pChannel=%08X pStub=%08X\n", List_Count(ObjectsList), obj->objidx, obj->pUnknown, obj->pChannel, obj->pStub);
	OutputDebugString(txt);
#endif
}
...
void ComServer::QueryInterfaceSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) QueryInterfaceSlave     objidx=%08X pUnknown=%08X pChannel=%08X pStub=%08X\n", List_Count(ObjectsList), obj->objidx, obj->pUnknown, obj->pChannel, obj->pStub);
	OutputDebugString(txt);
#endif
}
...
void ComServer::AddRefReleaseSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) AddRefReleaseSlave      objidx=%08X pUnknown=%08X RefCount=%d\n", List_Count(ObjectsList), obj->objidx, obj->pUnknown, pMap->ProcNum);
	OutputDebugString(txt);
#endif
}
...
void ComServer::InvokeMethodSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
	...
	if (obj->Flags & FLAG_WMI)
	{
		...
		if (pMap->ProcNum == 3 || pMap->ProcNum == 6 || pMap->ProcNum == 18 || pMap->ProcNum == 20 || pMap->ProcNum == 23)
		{
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) InvokeMethodSlave       objidx=%08X pUnknown=%08X pChannel=%08X pStub=%08X HR=%08X\n", List_Count(ObjectsList), obj->objidx, obj->pUnknown, obj->pChannel, obj->pStub, *hr);
	OutputDebugString(txt);
#endif
}
...
void ComServer::UnmarshalInterfaceSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
	...
	*hr = CoUnmarshalInterface(pStream, buf->riid, (void**)&pUnknown);
	...
	if (memcmp(&obj->iid, &GUID_NULL, sizeof(GUID)) == 0)
	{
		...
		*hr = SbieDll_ComCreateStub(obj->iid, pUnknown, (void**)&obj->pStub, (void**)&obj->pChannel);
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) UnmarshalInterfaceSlave objidx=%08X pUnknown=%08X pChannel=%08X pStub=%08X\n", List_Count(ObjectsList), obj->objidx, obj->pUnknown, obj->pChannel, obj->pStub);
	OutputDebugString(txt);
#endif
}
...
void ComServer::MarshalInterfaceSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
	...
	if (pMap->objidx == -1)
	{
		obj = (COM_OBJECT*)FindOrCreateDummySlaveObject(pMap, ObjectsList, exc, hr);
	}
	else
	{
	...
	*hr                 = CoMarshalInterface(pStream, *guid, obj->pUnknown, dwDestContext, NULL, mshlflags);
	if (SUCCEEDED(*hr))
	{
		LARGE_INTEGER zero;
		ULARGE_INTEGER size;
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) MarshalInterfaceSlave objidx=%08X pUnknown=%08X pChannel=%08X pStub=%08X\n", List_Count(ObjectsList), obj->objidx, obj->pUnknown, obj->pChannel, obj->pStub);
	OutputDebugString(txt);
#endif
}
...
void ComServer::QueryBlanketSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
	...
	*hr = CoQueryProxyBlanket(obj->pUnknown, &buf->AuthnSvc, &buf->AuthzSvc, &ServerPrincName, &buf->AuthnLevel, &buf->ImpLevel, NULL, &buf->Capabilities);
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) QueryBlanketSlave objidx=%08X\n", List_Count(ObjectsList), obj->objidx);
	OutputDebugString(txt);
#endif
}
...
void ComServer::SetBlanketSlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
	...
	*hr = CoSetProxyBlanket(obj->pUnknown, buf->AuthnSvc, buf->AuthzSvc, pServerPrincName, buf->AuthnLevel, buf->ImpLevel, NULL, buf->Capabilities);

#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) SetBlanketSlave objidx=%08X\n", List_Count(ObjectsList), obj->objidx);
	OutputDebugString(txt);
#endif
}
...
void ComServer::CopyProxySlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;
...
#ifdef DEBUG_COMSERVER
	WCHAR txt[256];
	wsprintf(txt, L"(%04d) CopyProxy objidx=%08X on objidx=%08X\n", List_Count(ObjectsList), obj->objidx, pMap->objidx);
	OutputDebugString(txt);
#endif
...
void ComServer::ProcessNotifySlave(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	COM_SLAVE_MAP* pMap = (COM_SLAVE_MAP*)_map;

#ifdef DEBUG_COMSERVER
	WCHAR txt[128];
	wsprintf(txt, L"(%04d) ProcessNotifySlave      Before idProcess=%06d\n", List_Count(ObjectsList), pMap->idProcess);
	OutputDebugString(txt);
#endif
