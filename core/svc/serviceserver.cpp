...
MSG_HEADER* ServiceServer::QueryHandler(MSG_HEADER* msg)
{
	...
	SC_HANDLE handle2 = OpenService(handle1, req->name, SERVICE_QUERY_STATUS | SERVICE_QUERY_CONFIG);
	if (!handle2)
	{
		error = GetLastError();
		CloseServiceHandle(handle1);
	...
	if (req->with_service_config)
	{
		...
		{
			QueryServiceConfig2(handle2, req->with_service_config, NULL, 0, &cfg_len);
		}
	...
	if (rpl)
	{
		if (req->with_service_status)
		{
			...
			if (!QueryServiceStatusEx(handle2, SC_STATUS_PROCESS_INFO, (BYTE*)&rpl->service_status, len, &len))
			{
				error = GetLastError();
			}
		}
		...
		if ((!error) && req->with_service_config)
		{
			...
			if (req->with_service_config == (USHORT)-1)
			{
				b = QueryServiceConfig(handle2, cfg, cfg_len, &rpl->service_config_len);
			}
			else
			{
				b = QueryServiceConfig2(handle2, req->with_service_config, (BYTE*)cfg, cfg_len, &rpl->service_config_len);
			}
...
#define ADJUST_MEMBER(x) \
	if (cfga->x)         \
	cfga->x -= (ULONG_PTR)cfga
			...
			else if (req->with_service_config == SERVICE_CONFIG_DESCRIPTION)
			{
				SERVICE_DESCRIPTIONA* cfga = (SERVICE_DESCRIPTIONA*)cfg;
				ADJUST_MEMBER(lpDescription);
			}
			else if (req->with_service_config == SERVICE_CONFIG_FAILURE_ACTIONS)
			{
				SERVICE_FAILURE_ACTIONSA* cfga = (SERVICE_FAILURE_ACTIONSA*)cfg;
...
MSG_HEADER* ServiceServer::ListHandler(MSG_HEADER* msg)
{
	...
	while (1)
	{
		...
		i = j  = 0;
		BOOL b = EnumServicesStatus(handle1, req->type_filter, req->state_filter, buf, buf_len, &i, &num_services, &j);
		if (b)
		{
			break;
		}
