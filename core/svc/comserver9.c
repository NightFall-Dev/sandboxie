...
static IUnknown* ComServer_MyClassFactory_New(P_MyCreateInstance pMyCreateInstance);


static HRESULT ComServer_IClassFactory_QueryInterface(IClassFactory* This, REFIID riid, void** ppvObject);
...
static HRESULT ComServer_IClassFactory_CreateInstance(IClassFactory* This, IUnknown* pUnkOuter, REFIID riid, void** ppvObject);
static HRESULT ComServer_IClassFactory_LockServer(IClassFactory* This, BOOL fLock);


static IMyUnknown* ComServer_MyUnknown_New(P_MyQueryInterface pMyQueryInterface, ULONG SizeofVtbls);


static HRESULT ComServer_IUnknown_QueryInterface(IUnknown* This, REFIID riid, void** ppvObject);
...
_FX void ComServer_Main9(const WCHAR* OriginalCmdLine)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		hr_lvl = 2;
		hr = CoRegisterClassObject(pClsid, ComServer_MyClassFactory_New(pMyCreateInstance), CLSCTX_LOCAL_SERVER, REGCLS_MULTIPLEUSE, &cookie);
	}
...
abort:
	...
	while (1)
	{
		...
		if (b != -1)
		{
			...
			{
				if (ComServer_Object_RefCount == 0 && ComServer_Factory_RefCount <= 1 && ComServer_Factory_Locked == FALSE)
				{
...
_FX void ComServer_RestartProgram(const WCHAR* arg)
{
	...
	ULONG len  = (wcslen(ComServer_ExeName) + wcslen(arg) + 32) * sizeof(WCHAR);
	...
	SbieDll_RunSandboxed(ComServer_BoxName, cmd, dir, 0, &StartupInfo, &ProcessInformation);
}
...
_FX IUnknown* ComServer_MyClassFactory_New(P_MyCreateInstance pMyCreateInstance)
{
...
_FX HRESULT ComServer_IClassFactory_QueryInterface(IClassFactory* This, REFIID riid, void** ppvObject)
{
#ifdef COMSERVER_DEBUG
	WCHAR txt[128];
...
_FX HRESULT ComServer_IClassFactory_CreateInstance(IClassFactory* This, IUnknown* pUnkOuter, REFIID riid, void** ppvObject)
{
	HRESULT hr;
	IUnknown* obj;
...
_FX HRESULT ComServer_IClassFactory_LockServer(IClassFactory* This, BOOL fLock)
{
...
_FX IMyUnknown* ComServer_MyUnknown_New(P_MyQueryInterface pMyQueryInterface, ULONG SizeofVtbls)
{
	ULONG len;
	IMyUnknown* This;
	ULONG_PTR* ptr;
...
_FX HRESULT ComServer_IUnknown_QueryInterface(IUnknown* This, REFIID riid, void** ppvObject)
{
