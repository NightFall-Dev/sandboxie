...
#define NAMED_PIPE_DESIRED_ACCESS (GENERIC_READ | GENERIC_WRITE | SYNCHRONIZE | FILE_READ_ATTRIBUTES)

#define NAMED_PIPE_SHARE_ACCESS (FILE_SHARE_READ | FILE_SHARE_WRITE)

#define NAMED_PIPE_CREATE_DISPOSITION (FILE_OPEN)

#define NAMED_PIPE_CREATE_OPTIONS (FILE_NON_DIRECTORY_FILE | FILE_OPEN_NO_RECALL)
...
{
	m_pNtAlpcConnectPort = GetProcAddress(_Ntdll, "NtAlpcConnectPort");

	m_pNtAlpcSendWaitReceivePort = GetProcAddress(_Ntdll, "NtAlpcSendWaitReceivePort");

	m_ProxyHandle = new ProxyHandle(NULL, sizeof(PROXY_PIPE), CloseCallback, NULL);
...
MSG_HEADER* NamedPipeServer::OpenHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	if (_wcsicmp(req->name, L"lsarpc") == 0 || _wcsicmp(req->name, L"srvsvc") == 0 || _wcsicmp(req->name, L"wkssvc") == 0 || _wcsicmp(req->name, L"samr") == 0 || _wcsicmp(req->name, L"netlogon") == 0)
	{
		permitted = true;
	}
	if (!permitted)
	...
	if (rpl)
	{
		...
		if (RestrictToken())
		{
			rpl->h.status = NtCreateFile(&ProxyPipe.hPipe, NAMED_PIPE_DESIRED_ACCESS, &objattrs, &IoStatusBlock, NULL, 0, NAMED_PIPE_SHARE_ACCESS, NAMED_PIPE_CREATE_DISPOSITION, req->create_options & NAMED_PIPE_CREATE_OPTIONS, NULL, 0);
		}
		else
		{
...
MSG_HEADER* NamedPipeServer::CloseHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	PROXY_PIPE* ProxyPipe = (PROXY_PIPE*)m_ProxyHandle->Find(idProcess, req->handle);
...
MSG_HEADER* NamedPipeServer::SetHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	if (rpl)
	{
		...
		rpl->h.status = NtSetInformationFile(ProxyPipe->hPipe, &IoStatusBlock, req->data, req->data_len, FilePipeInformation);
...
MSG_HEADER* NamedPipeServer::ReadHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	if (rpl)
	{
		...
		rpl->h.status = NtReadFile(ProxyPipe->hPipe, ProxyPipe->hEvent, NULL, NULL, &IoStatusBlock, rpl->data, rpl->data_len, &li, NULL);
...
MSG_HEADER* NamedPipeServer::WriteHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	if (rpl)
	{
		...
		if (RestrictToken())
		{
			rpl->h.status = NtWriteFile(ProxyPipe->hPipe, ProxyPipe->hEvent, NULL, NULL, &IoStatusBlock, req->data, req->data_len, &li, NULL);
		}
		else
		{
			rpl->h.status = STATUS_ACCESS_DENIED;
...
MSG_HEADER* NamedPipeServer::LpcConnectHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	WCHAR port_name[96];
	port_name[0] = L'\0';
	if (_wcsicmp(req->name, L"ntsvcs") == 0 || _wcsicmp(req->name, L"plugplay") == 0)
	{
	...
	if (RestrictToken())
	{
		...
		if (req->max_msg_len == -1)
		{ // old lpc

			status = NtConnectPort(&hPort, &objname, &qos, NULL, NULL, &max_msg_len, req->info_data, &req->info_len);
		}
		else
		{ // new alpc
			...
			{
				status = ((P_NtAlpcConnectPort)m_pNtAlpcConnectPort)(&hPort,
				    &objname,
				    &objattrs,
				    &alpc,
	...
	ULONG rpl_len = sizeof(NAMED_PIPE_LPC_CONNECT_RPL) + req->info_len;
...
MSG_HEADER* NamedPipeServer::LpcRequestHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	ULONG status                    = STATUS_INVALID_PARAMETER;
	...
	else
	{
		...
		status = NtRequestWaitReplyPort(ProxyPipe->hPipe, port_msg, (PORT_MESSAGE*)rpl->data);
	}
...
MSG_HEADER* NamedPipeServer::AlpcRequestHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	ULONG status                     = STATUS_INVALID_PARAMETER;
	...
	const P_NtAlpcSendWaitReceivePort pNtAlpcSendWaitReceivePort = (P_NtAlpcSendWaitReceivePort)m_pNtAlpcSendWaitReceivePort;
	...
	rcv_msg = (PORT_MESSAGE*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, msg_len);
	if (!rcv_msg)
	{
		status = STATUS_INSUFFICIENT_RESOURCES;
	}
	...
	else
	{
		...
		if (NT_SUCCESS(status) && ((msg_len > PIPE_MAX_DATA_LEN / 2) || (rcv_msg->u1.s1.TotalLength > PIPE_MAX_DATA_LEN / 2)))
		{
	...
	if (NT_SUCCESS(status))
	{
		...
		if ((view.ReceiveFlags & 0x40000000) && view.u.s2.ViewBase && view.u.s2.ViewSize)
		{
			//
		...
		ULONG rpl_len = sizeof(NAMED_PIPE_ALPC_REQUEST_REQ) + msg_len + ViewSize + 8;
