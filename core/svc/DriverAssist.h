...
private:
	...
	void LookupSid2(const WCHAR* SidString, WCHAR* UserName, ULONG UserNameLen);
	...
	void* InjectLow_CopyCode(HANDLE hProcess, BOOLEAN iswow64, UCHAR* code, ULONG code_len);
	bool InjectLow_BuildTramp(void* _msg, BOOLEAN long_diff, UCHAR* code, ULONG_PTR addr);
	void* InjectLow_CopySyscalls(HANDLE hProcess);
	bool InjectLow_CopyData(HANDLE hProcess, void* remote_addr, void* local_data);
#ifdef _WIN64
	void* InjectLow_getPage(HANDLE hProcess, void* remote_addr);
	bool DriverAssist::Has32BitJumpHorizon(void* target, void* detour);
#endif
	bool InjectLow_WriteJump(HANDLE hProcess, void* remote_addr, BOOLEAN long_diff, void* local_data);
