...
void DriverAssist::LogMessage_Single(void* data)
{
	...
	if (LogVer == L'2')
	{
		WCHAR* text2 = (WCHAR*)LocalAlloc(LMEM_FIXED, (wcslen(text) + 64) * sizeof(WCHAR));
		if (!text2)
		{
			LocalFree(text);
			return;
		...
		wsprintf(text2, L"%04d-%02d-%02d %02d:%02d:%02d %s", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, text);
...
void DriverAssist::LogMessage_Multi(ULONG msgid, const WCHAR* path, const WCHAR* text)
{
	union {
		KEY_VALUE_PARTIAL_INFORMATION info;
		WCHAR space[256];
	...
	WCHAR* path2 = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, len * sizeof(WCHAR));
	if (!path2)
	{
		return;
	}
...
{
	HANDLE hFile = CreateFile(path, FILE_GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
