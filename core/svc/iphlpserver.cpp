...
typedef ULONG (*P_IcmpSendEcho2)(HANDLE IcmpHandle, HANDLE Event, void* ApcRoutine, void* ApcContext, IPAddr DstAddr, void* RequestData, WORD RequestSize, PIP_OPTION_INFORMATION RequestOptions, void* ReplyBuffer, ULONG ReplySize, ULONG Timeout);

typedef ULONG (*P_IcmpSendEcho2Ex)(HANDLE IcmpHandle, HANDLE Event, void* ApcRoutine, void* ApcContext, IPAddr SrcAddr, IPAddr DstAddr, void* RequestData, WORD RequestSize, PIP_OPTION_INFORMATION RequestOptions, void* ReplyBuffer, ULONG ReplySize, ULONG Timeout);

typedef ULONG (*P_Icmp6SendEcho2)(HANDLE IcmpHandle, HANDLE Event, void* ApcRoutine, void* ApcContext, void* SrcAdddr, void* DstAddr, void* RequestData, WORD RequestSize, PIP_OPTION_INFORMATION RequestOptions, void* ReplyBuffer, ULONG ReplySize, ULONG Timeout);
...
IpHlpServer::IpHlpServer(PipeServer* pipeServer)
{
	...
	m_ProxyHandle = new ProxyHandle(NULL, sizeof(PROXY_ICMP_HANDLE), CloseCallback, this);
...
MSG_HEADER* IpHlpServer::CreateHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	if (0 != SbieApi_CheckInternetAccess(idProcess, req->ip6 ? L"Ip6" : L"Ip", TRUE))
	{
		return SHORT_REPLY(ERROR_ACCESS_DENIED);
	}

	const ULONG rpl_len        = sizeof(IPHLP_CREATE_FILE_RPL);
	IPHLP_CREATE_FILE_RPL* rpl = (IPHLP_CREATE_FILE_RPL*)LONG_REPLY(rpl_len);
...
MSG_HEADER* IpHlpServer::CloseHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	PROXY_ICMP_HANDLE* ProxyIcmp = (PROXY_ICMP_HANDLE*)m_ProxyHandle->Find(idProcess, req->handle);
...
MSG_HEADER* IpHlpServer::SendEchoHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	IPHLP_SEND_ECHO_RPL* rpl = (IPHLP_SEND_ECHO_RPL*)LONG_REPLY(rpl_len);
	if (rpl)
	{
		...
		if (req->ip6)
		{
			num_replies = ((P_Icmp6SendEcho2)p_IcmpSendEcho)(ProxyIcmp->handle, NULL, NULL, NULL, req->src_addr, req->dst_addr, req->request_data, RequestSize, pRequestOptions, rpl->reply_data, reply_size, req->timeout);
		}
		else if (req->ex2)
		{
			num_replies = ((P_IcmpSendEcho2Ex)
			        p_IcmpSendEcho)(ProxyIcmp->handle, NULL, NULL, NULL, *(ULONG*)&req->src_addr, *(ULONG*)&req->dst_addr, req->request_data, RequestSize, pRequestOptions, rpl->reply_data, reply_size, req->timeout);
		}
		else
		{
			num_replies = ((P_IcmpSendEcho2)p_IcmpSendEcho)(ProxyIcmp->handle, NULL, NULL, NULL, *(ULONG*)&req->dst_addr, req->request_data, RequestSize, pRequestOptions, rpl->reply_data, reply_size, req->timeout);
		}
		...
		if ((!req->ip6) && num_replies)
		{
			...
			if (req->iswow64)
			{
				ICMP_ECHO_REPLY32* rpl32 = (ICMP_ECHO_REPLY32*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, reply_size);
				if (!rpl32)
				{
					rpl->h.status = ERROR_NOT_ENOUGH_MEMORY;
				}
				else
				{
					...
					ICMP_ECHO_REPLY* replies = (ICMP_ECHO_REPLY*)rpl->reply_data;
			...
			else
			{
				...
				for (i = 0; i < num_replies; ++i)
				{
					...
					reply->Data                = (void*)((ULONG_PTR)reply->Data - (ULONG_PTR)replies);
				}
