...
bool GuiServer::InitProcess(HANDLE hProcess, ULONG process_id, ULONG session_id, BOOLEAN add_to_job)
{
	ULONG status;
	ULONG errlvl;
	BOOL is_in_job;
...
ULONG GuiServer::SendMessageToSlave(ULONG session_id, ULONG process_id, BOOLEAN add_to_job)
{
	//
	// prepare a QUEUE_PUTREQ_REQ message to send to the slave process
	//

	const ULONG req_len = sizeof(QUEUE_PUTREQ_REQ) + sizeof(GUI_INIT_PROCESS_REQ);
...
ULONG GuiServer::StartSlave(ULONG session_id)
{
	...
	WCHAR* cmdline = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, 768 * sizeof(WCHAR));
	if (!cmdline)
	{
	...
	else
	{
		...
		wsprintf(cmdptr, L"%s_GuiProxy_%08X,%d", SANDBOXIE, session_id, GetCurrentProcessId());
	...
	if (ok)
	{
		ok = DuplicateTokenEx(hOldToken, TOKEN_RIGHTS, NULL, SecurityAnonymous, TokenPrimary, &hNewToken);
		if (!ok)
		{
	...
	if (ok)
	{
		ok = SetTokenInformation(hNewToken, TokenSessionId, &session_id, sizeof(ULONG));
		if (!ok)
		{
	...
	if (ok)
	{
		...
		ok = CreateProcessAsUser(hNewToken, NULL, cmdline, NULL, NULL, FALSE, ABOVE_NORMAL_PRIORITY_CLASS, NULL, NULL, &si, &pi);
		if (!ok)
		{
		...
		else
		{
			...
			else
			{
				...
				slave = (GUI_SLAVE*)HeapAlloc(GetProcessHeap(), 0, sizeof(GUI_SLAVE));
				if (!slave)
				{
					status = STATUS_INSUFFICIENT_RESOURCES;
					ok     = FALSE;
...
void GuiServer::RunSlave(const WCHAR* cmdline)
{
	...
	HANDLE hParentProcess = OpenProcess(SYNCHRONIZE, FALSE, pThis->m_ParentPid);
	if (!hParentProcess)
	{
...
void* GuiServer::GetNtUserQueryWindow(void)
{
	...
	UCHAR* code = (UCHAR*)GetProcAddress(GetModuleHandle(L"user32.dll"), "IsHungAppWindow");
	if (code)
	{
		//
		...
		{
			code = code + _E8_Offset + 5 + (LONG_PTR) * (LONG*)(code + _E8_Offset + 1);
			...
			if (*(ULONG*)code == 0xB8D18B4C && (*(USHORT*)(code + 8) == 0x050F) || *(USHORT*)(code + 8) == 0x04F6)
			{
				ok = TRUE;
			}
			...
			if (code[0] == 0xB8 && code[5] == 0xBA && *(USHORT*)(code + 10) == 0x12FF)
			{
				ok = TRUE;
			}

			// 32-bit Windows 8
			if (code[0] == 0xB8 && code[5] == 0xE8 && *(USHORT*)(code + 10) == 0x08C2)
			{
				ok = TRUE;
			}
...
bool GuiServer::CreateQueueSlave(const WCHAR* cmdline)
{
	...
	*ptr = L'\0'; // terminate queue name
	...
	const ULONG m_SlaveFuncs_len = sizeof(SlaveFunc) * (GUI_MAX_REQUEST_CODE + 4);
	m_SlaveFuncs                 = (SlaveFunc*)HeapAlloc(GetProcessHeap(), 0, m_SlaveFuncs_len);
	...
	HANDLE WaitHandle;
	if (!RegisterWaitForSingleObject(&WaitHandle, m_QueueEvent, QueueCallbackSlave, (void*)this, INFINITE, WT_EXECUTEDEFAULT))
	{
		return false;
	}
...
bool GuiServer::QueueCallbackSlave2(void)
{
	...
	ULONG status = SbieDll_QueueGetReq(m_QueueName, &args.pid, NULL, &request_id, &data_ptr, &data_len);
	if (status != 0)
	{
	...
	if (msgid < GUI_MAX_REQUEST_CODE)
	{
		...
		if (SlaveFuncPtr)
		{
			...
			{
				ULONG session_id;
				status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)args.pid, NULL, NULL, NULL, &session_id);
	...
	*rpl_buf = status;
	status   = SbieDll_QueuePutRpl(m_QueueName, request_id, rpl_buf, rpl_len);
...
HANDLE GuiServer::GetJobObjectForAssign(const WCHAR* boxname)
{
	...
	while (job)
	{
		if (_wcsicmp(job->boxname, boxname) == 0)
		{
			...
			BOOL ok = QueryInformationJobObject(job->handle, JobObjectBasicAccountingInformation, &info, sizeof(info), NULL);
			if (!ok)
			{
	...
	if (!hJobObject)
	{
		...
		if (!_jobsd)
		{
			...
			if (!ConvertStringSecurityDescriptorToSecurityDescriptor(L"O:SYG:SYD:(A;;GA;;;SY)", SDDL_REVISION_1, &_jobsd, NULL))
			{
				// this should not happen
		...
		{
			WCHAR jobname[128];
			wsprintf(jobname, L"%s_%s_Session_%d_Job_%08X", SANDBOXIE, boxname, m_SessionId, GetTickCount());
			...
			if (hJobObject)
			{
				...
				info.UIRestrictionsClass = JOB_OBJECT_UILIMIT_EXITWINDOWS | JOB_OBJECT_UILIMIT_HANDLES | JOB_OBJECT_UILIMIT_SYSTEMPARAMETERS | JOB_OBJECT_UILIMIT_READCLIPBOARD;

				BOOL ok = FALSE; // set TRUE to skip UIRestrictions
				if (!ok)
				{
					ok = SetInformationJobObject(hJobObject, JobObjectBasicUIRestrictions, &info, sizeof(info));
					...
					if (ok)
					{
						...
						ok = UserHandleGrantAccess(GetDesktopWindow(), hJobObject, TRUE);
					}
				}
				if (!ok)
				{
...
HANDLE GuiServer::GetJobObjectForGrant(ULONG pid)
{
	...
	WCHAR BoxName[48];
	ULONG SessionId;
	ULONG status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)pid, BoxName, NULL, NULL, &SessionId);
...
bool GuiServer::GetWindowStationAndDesktopName(WCHAR* out_name)
{
	...
	if (ConvertStringSecurityDescriptorToSecurityDescriptor(L"S:(ML;;NW;;;LW)", SDDL_REVISION_1, &label_sd, NULL))
	{
		ULONG_PTR sacl_offset = (ULONG_PTR)((SECURITY_DESCRIPTOR_RELATIVE*)label_sd)->Sacl;
		...
		if (!GetSecurityDescriptorSacl(label_sd, &sacl_present, &sacl, &sacl_defaulted))
		{
			sacl = NULL;
		}
	}
	...
	_hWinSta = CreateWindowStation(_CombinedName, CWF_CREATE_ONLY, WINSTA_ALL_ACCESS | WRITE_OWNER, &sa);
	if (!_hWinSta)
	{
		errlvl = 0x61;
	}
	...
	else
	{
		...
		else
		{
			...
			wsprintf(desktop_name, L"\\%s_Desktop_%d", SANDBOXIE, GetTickCount());
			++desktop_name;

			const ACCESS_MASK DESKTOP_ALL_ACCESS = 0x1FF; // see WinUser.h
			_hDesk = CreateDesktop(desktop_name, NULL, NULL, 0, DESKTOP_ALL_ACCESS | WRITE_OWNER, &sa);
			if (!_hDesk)
			{
				errlvl = 0x63;
			}
			...
			else
			{
				...
				if (sacl)
				{
					...
					errlvl = SetSecurityInfo(_hWinSta, SE_WINDOW_OBJECT, LABEL_SECURITY_INFORMATION, NULL, NULL, NULL, sacl);
					if (errlvl)
					{
						SetLastError(errlvl);
						errlvl = 0x64;
					...
					{
						errlvl = SetSecurityInfo(_hDesk, SE_WINDOW_OBJECT, LABEL_SECURITY_INFORMATION, NULL, NULL, NULL, sacl);
...
ULONG GuiServer::InitProcessSlave(SlaveArgs* args)
{
	...
	HANDLE hProcess = OpenProcess(PROCESS_SET_QUOTA | PROCESS_TERMINATE, FALSE, req->process_id);
	if (!hProcess)
	{
		status = GetLastError();
		errlvl = 0x91;
	...
	status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)req->process_id, boxname, NULL, NULL, &session_id);
	if (status != 0)
	{
		errlvl = 0x92;
		goto finish;
...
ULONG GuiServer::GetWindowStationSlave(SlaveArgs* args)
{
	...
	HANDLE hProcess = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_DUP_HANDLE, FALSE, args->pid);
	if (!hProcess)
	{
		status = GetLastError();
		errlvl = 0x71;
	...
	{
		GUI_GET_WINDOW_STATION_RPL* rpl = (GUI_GET_WINDOW_STATION_RPL*)args->rpl_buf;
		...
		ULONG _WinStaAccess = WINSTA_ENUMDESKTOPS | WINSTA_READATTRIBUTES
		...
		ULONG64 ProcessFlags = SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)args->pid, 0);
		if (ProcessFlags & SBIE_FLAG_OPEN_ALL_WIN_CLASS)
		{
			_WinStaAccess |= WINSTA_WRITEATTRIBUTES;
		}

		if (!DuplicateHandle(NtCurrentProcess(), local_winsta, hProcess, (HANDLE*)&rpl->hwinsta, _WinStaAccess, FALSE, 0))
		{
			status = GetLastError();
			errlvl = 0x75;
			goto finish;
		...
		if (!DuplicateHandle(NtCurrentProcess(), local_desktop, hProcess, (HANDLE*)&rpl->hdesk, 0, FALSE, DUPLICATE_SAME_ACCESS))
		{
			status = GetLastError();
			errlvl = 0x76;
			goto finish;
		...
		PROCESS_BASIC_INFORMATION info;
		status = NtQueryInformationProcess(hProcess, ProcessBasicInformation, &info, sizeof(PROCESS_BASIC_INFORMATION), &errlvl);
		if (status != 0)
		{
			errlvl = 0x77;
			goto finish;
...
ULONG GuiServer::CreateConsoleSlave(SlaveArgs* args)
{
	...
	HANDLE hProcess = OpenProcess(PROCESS_DUP_HANDLE | SYNCHRONIZE, FALSE, args->pid);
	if (!hProcess)
	{
	...
	ULONG session_id;
	ULONG status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)args->pid, NULL, image_name, NULL, &session_id);
	if (status != 0 || session_id != m_SessionId)
	{
		if (status == 0)
		{
	...
	ok = DuplicateHandle(hProcess, (HANDLE)(ULONG_PTR)req->token, NtCurrentProcess(), &hToken1, TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY, FALSE, 0);
	if (!ok)
	{
		hToken1 = NULL;
	...
	ok = DuplicateTokenEx(hToken1, MAXIMUM_ALLOWED, NULL, SecurityAnonymous, TokenPrimary, &hToken2);
	if (!ok)
	{
		hToken2 = NULL;
		status  = STATUS_ACCESS_DENIED;
	...
	cmdline = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, 768 * sizeof(WCHAR));
	if (!cmdline)
	{
		status = STATUS_INSUFFICIENT_RESOURCES;
		goto finish;
	...
	ok = CreateProcessAsUser(hToken2, NULL, cmdline, NULL, NULL, FALSE, ABOVE_NORMAL_PRIORITY_CLASS, NULL, NULL, &si, &pi);
	...
	if (RegisterWaitForSingleObject(&WaitHandles[0], hProcess, ConsoleCallbackSlave, (void*)WaitHandles, INFINITE, WT_EXECUTEONLYONCE))
	{
		//
...
ULONG GuiServer::QueryWindowSlave(SlaveArgs* args)
{
	...
	P_NtUserQueryWindow NtUserQueryWindow = (P_NtUserQueryWindow)GetNtUserQueryWindow();
...
{
	GUI_GET_WINDOW_HANDLE_REQ* req = (GUI_GET_WINDOW_HANDLE_REQ*)args->req_buf;
	GUI_GET_WINDOW_HANDLE_RPL* rpl = (GUI_GET_WINDOW_HANDLE_RPL*)args->rpl_buf;
	HWND hwnd                      = (HWND)(LONG_PTR)req->hwnd;
...
ULONG GuiServer::EnumWindowsSlave(SlaveArgs* args)
{
	...
	if (req->which == 'W')
	{
		rpl->ok = EnumWindows(EnumWindowsSlaveEnumProc, (LPARAM)&rpl->num_hwnds);
	}
	else if (req->which == 'C')
	{
		HWND hwnd = (HWND)(LONG_PTR)req->arg;
		rpl->ok   = EnumChildWindows(hwnd, EnumWindowsSlaveEnumProc, (LPARAM)&rpl->num_hwnds);
	}
	else if (req->which == 'T')
	{
		ULONG thread_id = req->arg;
		rpl->ok         = EnumThreadWindows(thread_id, EnumWindowsSlaveEnumProc, (LPARAM)&rpl->num_hwnds);
	}
	else
	{
		return STATUS_INVALID_PARAMETER;
	...
	args->rpl_len = sizeof(GUI_ENUM_WINDOWS_RPL) + (ULONG)rpl->num_hwnds * sizeof(ULONG);
...
{
	ULONG max_hwnds = (MAX_RPL_BUF_SIZE - sizeof(GUI_ENUM_WINDOWS_RPL)) / sizeof(ULONG);
...
ULONG GuiServer::EnumWindowsFilterSlave(ULONG pid, void* rpl_buf)
{
	...
	WCHAR boxname[48];
	ULONG status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)pid, boxname, NULL, NULL, NULL);
	if (status != 0)
	{
		return status;
	}
...
{
	GUI_MAP_WINDOW_POINTS_REQ* req = (GUI_MAP_WINDOW_POINTS_REQ*)args->req_buf;
	GUI_MAP_WINDOW_POINTS_RPL* rpl = (GUI_MAP_WINDOW_POINTS_RPL*)args->rpl_buf;
	...
	else
	{
		...
		ULONG max_points = (MAX_RPL_BUF_SIZE - sizeof(GUI_MAP_WINDOW_POINTS_RPL)) / sizeof(POINT);
		...
		rpl->retval = MapWindowPoints(hwnd_from, hwnd_to, rpl->pts, rpl->num_pts);
	}
...
ULONG GuiServer::SetWindowPosSlave(SlaveArgs* args)
{
	...
	{
		SetLastError(req->error);
		rpl->retval = SetWindowPos(hwnd, hwnd_insert_after, req->x, req->y, req->w, req->h, req->flags);
		rpl->error  = GetLastError();
	}
...
ULONG GuiServer::CloseClipboardSlave(SlaveArgs* args)
{
	...
	{
		ULONG caller_il = (ULONG)(ULONG_PTR)SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)args->pid, 'pril');
...
{
	GUI_GET_CLIPBOARD_DATA_REQ* req = (GUI_GET_CLIPBOARD_DATA_REQ*)args->req_buf;
	GUI_GET_CLIPBOARD_DATA_RPL* rpl = (GUI_GET_CLIPBOARD_DATA_RPL*)args->rpl_buf;
	...
	else
	{
		...
		if (mem_handle)
		{
			...
			else
			{
				...
				rpl->section_handle = (ULONG64)(ULONG_PTR)GetClipboardDataSlave2(args->pid, mem_ptr, mem_len);
				...
				if (req->format == CF_BITMAP || req->format == CF_ENHMETAFILE)
				{
...
HANDLE GuiServer::GetClipboardDataSlave2(ULONG pid, void* mem_ptr, SIZE_T mem_len)
{
	...
	HANDLE hSectionLocal = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE | SEC_COMMIT, size_high, size_low, NULL);
	if (hSectionLocal)
	{
		void* data = MapViewOfFile(hSectionLocal, FILE_MAP_WRITE, 0, 0, mem_len);
		...
		if (data)
		{
			...
			if (!DuplicateHandle(NtCurrentProcess(), hSectionLocal, hProcess, &hSectionRemote, SECTION_QUERY | SECTION_MAP_READ, FALSE, 0))
			{
				hSectionRemote = NULL;
			}
		}
		...
		if (mem_handle)
		{
			if (GlobalSize(mem_handle) >= sizeof(METAFILEPICT))
			{
				...
				if (mf)
				{
					...
					if (mem_len)
					{
						void* mem_ptr = HeapAlloc(GetProcessHeap(), 0, mem_len);
						if (mem_ptr)
						{
							if (GetMetaFileBitsEx(mf->hMF, mem_len, mem_ptr) == mem_len)
							{
								rpl->section_length = mem_len;
								rpl->section_handle = (ULONG64)(ULONG_PTR)GetClipboardDataSlave2(args->pid, mem_ptr, mem_len);
...
{
	GUI_SEND_POST_MESSAGE_REQ* req = (GUI_SEND_POST_MESSAGE_REQ*)args->req_buf;
	GUI_SEND_POST_MESSAGE_RPL* rpl = (GUI_SEND_POST_MESSAGE_RPL*)args->rpl_buf;
	...
	if (msg == WM_DDE_ACK && (req->which == 'sm w' || req->which == 'sm a' || req->which == 'smtw' || req->which == 'smta'))
	{
		//
		...
		if ((req->which == 'sm w') || (req->which == 'sm a') && IsWindow(hwnd))
		{
			rpl->lresult1 = 0;
			rpl->lresult2 = 0;

			ULONG_PTR* DdeArgs = (ULONG_PTR*)HeapAlloc(GetProcessHeap(), 0, (sizeof(ULONG_PTR) * 4));
			if (DdeArgs)
			{
				...
				ULONG tid;
				HANDLE hThread = CreateThread(NULL, 0, DdeProxyThreadSlave, (void*)DdeArgs, 0, &tid);
				if (hThread)
				{
					CloseHandle(hThread);
					rpl->lresult1 = 1;
	...
	if (access)
	{
		...
		if ((!CompareIntegrityLevels(args->pid, hwnd)) && (!ShouldIgnoreIntegrityLevels(args->pid, hwnd)))
		{
			access = false;
		}
	...
	{
		DWORD_PTR x;
		rpl->lresult1 = SendMessageTimeoutW(hwnd, msg, wparam, lparam, req->flags, req->timeout, &x);
		rpl->lresult2 = x;
	}
...
ULONG GuiServer::SendCopyDataSlave(SlaveArgs* args)
{
	...
	ULONG max_offset = FIELD_OFFSET(GUI_SEND_COPYDATA_REQ, cds_buf) + req->cds_len;
	...
	else if (req->which == 'dde ')
	{
		...
		HGLOBAL hGlobal = GlobalAlloc(GMEM_DDESHARE | GMEM_MOVEABLE, req->cds_len);
		if (hGlobal)
		{
			...
			if (pGlobal)
			{
				...
				lparam = PackDDElParam(WM_DDE_DATA, (UINT_PTR)hGlobal, (UINT_PTR)req->cds_key);

				// we can't post WM_DDE_DATA from this thread because the
				// DdeProxyThreadSlave proxy window is in another thread
				rpl->lresult1 = PostMessage(DDE_Request_ProxyWnd, (WM_USER + 0x123), tzuk, lparam);
...
ULONG GuiServer::SetForegroundWindowSlave(SlaveArgs* args)
{
	...
	GUI_SET_FOREGROUND_WINDOW_REQ* req = (GUI_SET_FOREGROUND_WINDOW_REQ*)args->req_buf;
...
{
	GUI_MONITOR_FROM_WINDOW_REQ* req = (GUI_MONITOR_FROM_WINDOW_REQ*)args->req_buf;
	GUI_MONITOR_FROM_WINDOW_RPL* rpl = (GUI_MONITOR_FROM_WINDOW_RPL*)args->rpl_buf;
...
ULONG GuiServer::SplWow64Slave(SlaveArgs* args)
{
	...
	if (req->set)
	{
		status = SbieApi_QueryProcessEx2((HANDLE)(ULONG_PTR)args->pid, 0, boxname, NULL, NULL, NULL, &create_time);
		if (status != 0)
		{
	...
	status = SbieApi_QueryProcessEx2((HANDLE)(ULONG_PTR)_SplWow64Pid, 0, boxname, NULL, NULL, NULL, &create_time);

	if (status != 0 || create_time != _SplWow64CreateTime || _wcsicmp(boxname, _SplWow64BoxName) != 0)
	{
		//
	...
	status = SbieApi_QueryProcessEx2((HANDLE)(ULONG_PTR)args->pid, 0, boxname, NULL, NULL, NULL, NULL);
...
void GuiServer::SplWow64SlaveWin8()
{
	...
	typedef BOOL (*P_QueryFullProcessImageNameW)(HANDLE hProcess, ULONG dwFlags, WCHAR* lpExeName, ULONG* lpdwSize);

	HMODULE KernelBase             = GetModuleHandle(L"KernelBase.dll");
	P_EnumProcesses pEnumProcesses = (P_EnumProcesses)GetProcAddress(KernelBase, "EnumProcesses");
	P_QueryFullProcessImageNameW pQueryFullProcessImageNameW = (P_QueryFullProcessImageNameW)GetProcAddress(KernelBase, "QueryFullProcessImageNameW");
	...
	if (pEnumProcesses(pids, pids_len, &pids_len))
	{
		...
		{
			if (ProcessIdToSessionId(pids[i], &SessionId) && (SessionId == m_SessionId))
			{
				HANDLE hProcess = OpenProcess(PROCESS_TERMINATE | PROCESS_QUERY_INFORMATION | SYNCHRONIZE, FALSE, pids[i]);

				if (hProcess)
				{
					ULONG path_len = 128;
					if (pQueryFullProcessImageNameW(hProcess, 1, path, &path_len))
					{
						WCHAR* ptr = wcsrchr(path, L'\\');
						if (ptr && _wcsicmp(ptr + 1, L"splwow64.exe") == 0)
						{
							if (SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)pids[i], 0) == 0)
							{
...
ULONG GuiServer::ChangeDisplaySettingsSlave(SlaveArgs* args)
{
	...
	GUI_CHANGE_DISPLAY_SETTINGS_REQ* req = (GUI_CHANGE_DISPLAY_SETTINGS_REQ*)args->req_buf;
	GUI_CHANGE_DISPLAY_SETTINGS_RPL* rpl = (GUI_CHANGE_DISPLAY_SETTINGS_RPL*)args->rpl_buf;
	...
	if (req->unicode)
	{
		rpl->retval = ChangeDisplaySettingsExW((LPCWSTR)devname, (DEVMODEW*)devmode, NULL, req->flags, NULL);
	}
	else
	{
		rpl->retval = ChangeDisplaySettingsExA((LPCSTR)devname, (DEVMODEA*)devmode, NULL, req->flags, NULL);
	}
	rpl->error = GetLastError();
...
ULONG GuiServer::GetProcessPathList(ULONG pid, void** out_pool, LIST** out_list)
{
...
bool GuiServer::CheckSameProcessBoxes(ULONG in_pid, WCHAR* boxname, HWND hwnd, ULONG* out_pid)
{
	*out_pid       = 0;
	...
	WCHAR boxname2[48];
	ULONG status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)*out_pid, boxname2, NULL, NULL, NULL);
	if (!NT_SUCCESS(status))
	{
		return false;
	}
	...
	{
		WCHAR boxname1[48];
		status = SbieApi_QueryProcess((HANDLE)(ULONG_PTR)in_pid, boxname1, NULL, NULL, NULL);
		if (!NT_SUCCESS(status))
		{
			return false;
		}
...
bool GuiServer::CheckWindowAccessible(ULONG pid, WCHAR* boxname, LIST* list, HWND hwnd)
{
	//
	...
	{
		ULONG x;
		NTSTATUS status = NtQueryInformationProcess(hProcess, ProcessImageFileName, clsnm, sizeof(clsnm) - 8, &x);
		x               = 0;
...
{
	ULONG src_il = (ULONG)(ULONG_PTR)SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)src_pid, 'pril');
	...
	ULONG len;
	ok = GetTokenInformation(hToken, (TOKEN_INFORMATION_CLASS)TokenIntegrityLevel, &info, sizeof(info), &len);
	CloseHandle(hToken);
	if (!ok)
	{
		return false;
...
bool GuiServer::AllowSendPostMessage(ULONG pid, ULONG msg, bool IsSendMsg, HWND hwnd)
{
...
#define WM_MOUSEHOVERLAST 0x02A3
	...
	ULONG64 ProcessFlags = SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)pid, 0);
...
#define IS_INPUT_MESSAGE(msg) \
	(msg == WM_INPUT || (msg >= WM_KEYFIRST && msg <= WM_KEYLAST) || (msg >= WM_MOUSEFIRST && msg <= WM_MOUSELAST) || (msg >= WM_NCMOUSEFIRST && msg <= WM_NCMOUSELAST) || (msg >= WM_MOUSEHOVERFIRST && msg <= WM_MOUSEHOVERLAST) || (msg >= WM_TABLET_FIRST && msg <= WM_TABLET_LAST))
	...
	if (msg < WM_USER)
	{
		static const ULONG sysmsgs[] = {0x0002, // WM_DESTROY
		    0x000B,                             // WM_SETREDRAW
		    0x0010,                             // WM_CLOSE
...
#define ISWNDCLASS(n, s) (len == n && _wcsicmp(clsnm, s) == 0)
			...
			if (blocked && (!IsSendMsg) && (msg == 0x0412) && ISWNDCLASS(7, L"Progman"))
			{
				blocked = false;
			}
			...
			if (blocked &&
			    ...
			    //                0x448 0x44B 0x457
			    ((IsSendMsg && (msg == 0x044C || msg == 0x044D || msg == 0x044E || msg == 0x044F || msg == 0x0450 || msg == 0x0451 || msg == 0x0455)) || ((!IsSendMsg) && (msg == 0x0440 || msg == 0x0441 || msg == 0x0443 || msg == 0x0444 || msg == 0x0447 || msg == 0x0448 || msg == 0x044B || msg == 0x0457)))
			    /* then if wndclass ... */
			...
			if (blocked && (!(ProcessFlags & SBIE_FLAG_IMAGE_FROM_SANDBOX)) && ISWNDCLASS(14, L"MSTaskSwWClass"))
			{
				WCHAR sender_exename[128];
				SbieApi_QueryProcess((HANDLE)(ULONG_PTR)pid, NULL, sender_exename, NULL, NULL);

				if (_wcsicmp(sender_exename, L"excel.exe") == 0 || _wcsicmp(sender_exename, L"powerpnt.exe") == 0)
				{
					blocked = false;
				}
			}
...
void GuiServer::AdjustConsoleTaskbarButton()
{
	...
	static const GUID IID_IPropertyStore = {0x886D8EEB, 0x8CF2, 0x4446, {0x8D, 0x02, 0xCD, 0xBA, 0x1D, 0xBD, 0xCF, 0x99}};

	typedef HRESULT (*P_SHGetPropertyStoreForWindow)(HWND hwnd, REFIID riid, IPropertyStore** ppv);

	IPropertyStore* pPropertyStore;
	PROPVARIANT v;
	HRESULT hr;

	P_SHGetPropertyStoreForWindow SHGetPropertyStoreForWindow = (P_SHGetPropertyStoreForWindow)GetProcAddress(LoadLibrary(L"shell32.dll"), "SHGetPropertyStoreForWindow");
	if (!SHGetPropertyStoreForWindow)
	{
		return;
	}

	hr = SHGetPropertyStoreForWindow(GetConsoleWindow(), IID_IPropertyStore, &pPropertyStore);
	if (FAILED(hr))
	{
		return;
	}
	...
	pPropertyStore->lpVtbl->SetValue(pPropertyStore, &PKEY_AppUserModel_PreventPinning, v);

	v.vt      = VT_BSTR;
	v.bstrVal = SANDBOXIE L"_v4_Proxy_Console_Window";
	pPropertyStore->lpVtbl->SetValue(pPropertyStore, &PKEY_AppUserModel_ID, v);
...
ULONG GuiServer::DdeProxyThreadSlave(void* xDdeArgs)
{
	...
	HWND hProxyWnd = CreateWindowEx(0, (LPCWSTR)_atom, L"", 0, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, GetModuleHandle(NULL), NULL);
	if (!hProxyWnd)
	{
		return 0;
	}
	...
	while (1)
	{
		...
		if ((msg.message == WM_DDE_EXECUTE || msg.message == WM_DDE_REQUEST) && (HWND)msg.wParam == hClientWnd)
		{
			//
			...
			if (msg.message == WM_DDE_EXECUTE)
			{
				...
				if (cds.lpData)
				{
					SendMessage(hServerWnd, WM_COPYDATA, (WPARAM)hProxyWnd, (LPARAM)&cds);
			...
			else if (msg.message == WM_DDE_REQUEST)
			{
				...
				wsprintf(prop_name, L"//" SANDBOXIE L"//DDE//REQ//%08X", lParam);
		...
		if (msg.message == (WM_USER + 0x123) && msg.wParam == tzuk)
		{
			...
			PostMessage(hClientWnd, WM_DDE_DATA, (WPARAM)hProxyWnd, msg.lParam);
		}
		...
		if (msg.message == WM_DDE_TERMINATE && (HWND)msg.wParam == hClientWnd)
		{
