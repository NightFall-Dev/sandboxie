...
void* ComServer::FindOrCreateDummySlaveObject(void* _map, LIST* ObjectsList, ULONG* exc, HRESULT* hr)
{
	...
	while (obj)
	{
		if (pMap->idProcess == obj->idProcess && pMap->objidx == -1 && memcmp(pMap->Buffer, &obj->iid, sizeof(GUID)) == 0)
		{
			break;
		}
...
class MySearchNotifyInlineSite : public IUnknown
{
	...
	{
		if (memcmp(&riid, &IID_ISearchNotifyInlineSite, sizeof(GUID)) == 0 || memcmp(&riid, &IID_IUnknown, sizeof(GUID)) == 0)
		{
			*ppvObject = this;
			return S_OK;
		}
	...
	virtual HRESULT STDMETHODCALLTYPE OnItemIndexedStatusChange(ULONG sipStatus, ULONG dwNumEntries, void* rgItemStatusEntries)
	{
		return S_OK;
	}

	virtual HRESULT STDMETHODCALLTYPE OnCatalogStatusChange(REFGUID guidCatalogResetSignature, REFGUID guidCheckPointSignature, ULONG dwLastCheckPointNumber)
	{
		return S_OK;
	}
};
...
ULONG ComServer::CryptProtectDataSlave(void* Buffer)
{
	...
	if (req->mode == L'P')
	{
		...
		ok = CryptProtectData(&DataIn, DataDescr, pEntropy, NULL, &PromptStruct, req->flags, &DataOut);

		DataDescr = NULL;
	}
	else if (req->mode == L'U')
	{
		ok = CryptUnprotectData(&DataIn, &DataDescr, pEntropy, NULL, &PromptStruct, req->flags, &DataOut);
	}
	else
	{
		return ERROR_INVALID_PARAMETER;
	...
	rpl_len = sizeof(COM_CRYPT_PROTECT_DATA_RPL) + DataOut.cbData;
	...
	{
		COM_CRYPT_PROTECT_DATA_RPL* rpl = (COM_CRYPT_PROTECT_DATA_RPL*)Buffer;
