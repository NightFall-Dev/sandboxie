...
bool ServiceServer::CanCallerDoElevation(HANDLE idProcess, const WCHAR* ServiceName, ULONG* pSessionId)
{
	WCHAR boxname[48];
	WCHAR exename[99];
	...
	if (DropRights && ServiceName)
	{
		...
		while (1)
		{
			NTSTATUS status = SbieApi_QueryConfAsIs(boxname, L"StartService", index, buf, 64 * sizeof(WCHAR));
			++index;
...
void ServiceServer::ReportError2218(HANDLE idProcess, ULONG errlvl)
{
	...
	if (0 == SbieApi_QueryProcess(idProcess, boxname, imagename, NULL, &session_id))
	{
...
WCHAR* ServiceServer::BuildPathForStartExe(HANDLE idProcess, const WCHAR* devmap, const WCHAR* svcname, const WCHAR* InArgs, WCHAR** OutArgs)
{
...
MSG_HEADER* ServiceServer::RunHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	else
	{
		...
		error = RunHandler2(idProcess, idSession, req->devmap, svcname, req->path);
	}
...
ULONG ServiceServer::RunHandler2(HANDLE idProcess, ULONG idSession, const WCHAR* devmap, const WCHAR* svcname, const WCHAR* path)
{
	...
	{
		errlvl  = 0x21;
		ExePath = BuildPathForStartExe(idProcess, devmap, svcname, path, NULL);
		if (!ExePath)
		{
			ok = FALSE;
	...
	{
		errlvl = 0x24;
		ok     = SetTokenInformation(hNewToken, TokenSessionId, &idSession, sizeof(ULONG));
	}
	...
	if (ok)
	{
		...
		errlvl = 0x25;
		ok     = CreateProcessAsUser(hNewToken, NULL, ExePath, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
...
void ServiceServer::SetTokenDefaultDacl(HANDLE hNewToken, HANDLE idProcess)
{
	...
	hProcess = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, (ULONG)(ULONG_PTR)idProcess);
	if (!hProcess)
	{
		goto finish;
	}
	...
	ok = GetTokenInformation(hNewToken, TokenDefaultDacl, pDacl, (1024 - 128), &len);
	if (!ok)
	{
		goto finish;
	}
	...
	ok = SetTokenInformation(hNewToken, TokenDefaultDacl, pDacl, (1024 - 128));
...
MSG_HEADER* ServiceServer::UacHandler(MSG_HEADER* msg, HANDLE idProcess, HANDLE idThread)
{
	SetThreadToken(NULL, NULL);
	...
	{
		error = UacHandler2(idProcess, idThread, req->devmap, (ULONG_PTR)req->uac_pkt_addr, req->uac_pkt_len);
	}
...
ULONG ServiceServer::UacHandler2(HANDLE idProcess, HANDLE idThread, const WCHAR* devmap, ULONG_PTR pkt_addr, ULONG pkt_len)
{
	...
	if (ok)
	{
		...
		wsprintf(cmdline,
		    L"%s_UacProxy:%08X_%08X_%08X_%08X_@%s",
		    SANDBOXIE,
		    (ULONG)(ULONG_PTR)idProcess,
		    pkt_addr_64.HighPart,
		    pkt_addr_64.LowPart,
		    pkt_len,
		    devmap);
	...
	{
		errlvl  = 0x42;
		hThread = OpenThread(THREAD_QUERY_INFORMATION, FALSE, (ULONG)(ULONG_PTR)idThread);
		if (!hThread)
		{
			ok = FALSE;
		}
	...
	if (ok)
	{
		...
		{
			ULONG LastError = GetLastError();
			if (LastError == ERROR_NO_TOKEN || LastError == ERROR_BAD_IMPERSONATION_LEVEL || LastError == ERROR_CANT_OPEN_ANONYMOUS)
			{
				errlvl   = 0x45;
				hProcess = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, (ULONG)(ULONG_PTR)idProcess);
				if (!hProcess)
				...
				{
					errlvl = 0x46;
					ok     = OpenProcessToken(hProcess, TOKEN_RIGHTS, &hNewToken);
				}
			}
		}
	}
...
void ServiceServer::RunUacSlave(const WCHAR* cmdline)
{
	...
	else
	{
		...
		RunUacSlave3(idProcess, (ULONG_PTR)pkt_addr.QuadPart, pkt_len, false, NULL);
	}
}
...
void ServiceServer::RunUacSlave2(ULONG_PTR* ThreadArgs)
{
	...
	WCHAR* AppName = NULL;
	RunUacSlave3(idProcess, (ULONG_PTR)ThreadArgs[3], (ULONG)ThreadArgs[4], false, &AppName);
	...
	if (isAdmin)
	{
		CreateThread(NULL, 0, RunUacSlave2Thread1, (void*)ThreadArgs, 0, NULL);
		CreateThread(NULL, 0, RunUacSlave2Thread2, (void*)ThreadArgs, 0, NULL);
	...
	if (1)
	{
		...
		typedef HRESULT (*P_SHGetStockIconInfo)(ULONG_PTR siid, UINT uFlags, SHSTOCKICONINFO* psii);
		...
		if (hShell32)
		{
			P_SHGetStockIconInfo pSHGetStockIconInfo = (P_SHGetStockIconInfo)GetProcAddress(hShell32, "SHGetStockIconInfo");
			if (pSHGetStockIconInfo)
			{
				SHSTOCKICONINFO sii;
				memzero(&sii, sizeof(SHSTOCKICONINFO));
				sii.cbSize = sizeof(SHSTOCKICONINFO);
				HRESULT hr = pSHGetStockIconInfo(SIID_SHIELD, SHGSI_ICON, &sii);
				if (SUCCEEDED(hr))
				{
	...
	HWND hWnd = CreateWindowEx(WS_EX_TOPMOST | (rtl ? WS_EX_LAYOUTRTL : 0), (LPCWSTR)atom, SANDBOXIE, WS_SYSMENU | WS_MINIMIZEBOX, 10, 10, 600, 400, NULL, NULL, NULL, strings);
	...
	if (hmonitor)
	{
		...
		if (GetMonitorInfo(hmonitor, &monitor))
		{
			...
			if (x || y)
			{
				SetWindowPos(hWnd, NULL, x, y, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOSENDCHANGING | SWP_NOREDRAW | SWP_NOACTIVATE | SWP_DEFERERASE);
			}
		}
	}
...
LRESULT ServiceServer::RunUacSlave2WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	HFONT hfont = CreateFont(-height, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, L"Arial");
	...
	hfont = CreateFont(-height, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, L"Arial");
	SelectObject(hdc, hfont);
...
int ServiceServer::RunUacSlave2WndProcTextOut(HDC hdc, int y, int height, int msgid)
{
	WCHAR txt[512];
...
ULONG ServiceServer::RunUacSlave2Thread2(void* lpParameters)
{
	...
	HANDLE hProcess = OpenProcess(SYNCHRONIZE, FALSE, (ULONG)(ULONG_PTR)idProcess);
	if (hProcess)
	{
...
void ServiceServer::RunUacSlave3(HANDLE idProcess, ULONG_PTR pkt_addr, ULONG pkt_len, bool JustFail, WCHAR** OutAppName)
{
	HANDLE hProcess        = NULL;
	...
	{
		WCHAR BoxName[48];
		SbieApi_QueryProcess((HANDLE)(ULONG_PTR)GetCurrentProcessId(), BoxName, NULL, NULL, NULL);
		if (BoxName[0])
		{
			errlvl   = 0x89;
			hProcess = NULL;
		...
		{
			errlvl = 0x82;
			hProcess = OpenProcess(PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_DUP_HANDLE, FALSE, (ULONG)(ULONG_PTR)idProcess);
			...
			if ((!hProcess) && OutAppName)
			{
				...
				WCHAR* AppName = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, 99 * sizeof(WCHAR));
				if (AppName)
				{
					SbieApi_QueryProcess(idProcess, NULL, AppName, NULL, NULL);
					if (*AppName)
					{
						*OutAppName = AppName;
						return;
	...
	{
		errlvl = 0x84;
		ok     = ReadProcessMemory(hProcess, (void*)(ULONG_PTR)pkt_addr, pkt, pkt_len, &copy_len);
		if (ok && (copy_len != pkt_len || pkt->tzuk != tzuk || pkt->len != pkt_len || pkt_len != ~pkt->inv_len))
		{
			ok = FALSE;
	...
	if (ok)
	{
		...
		errlvl = 0x86;
		ok = WriteProcessMemory(hProcess, (void*)(ULONG_PTR)copy_addr, &pkt->hResult, sizeof(ULONG64) * 2, &copy_len);

		if (ok && pkt->hEvent)
		{
			errlvl = 0x87;
			ok = DuplicateHandle(hProcess, (HANDLE)(ULONG_PTR)pkt->hEvent, GetCurrentProcess(), &hEvent, EVENT_MODIFY_STATE, FALSE, 0);
			if (ok)
			{
				errlvl = 0x88;
				ok     = SetEvent(hEvent);
...
bool ServiceServer::RunUacSlave4(HANDLE hClientProcess, void* xpkt, WCHAR** OutAppName)
{
	...
	app = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, (app_len + 1) * sizeof(WCHAR));
	if (!app)
	{
		return false;
	}
	...
	cmd = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, (cmd_len + 1) * sizeof(WCHAR));
	if (!cmd)
	{
		return false;
	}
	...
	dir = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, (dir_len + 1) * sizeof(WCHAR));
	if (!dir)
	{
		return false;
	}
	...
	if (memcmp(app, L"*MSI*", 5 * sizeof(WCHAR)) == 0 && memcmp(app, cmd, 5 * sizeof(WCHAR)) == 0 && memcmp(app, dir, 5 * sizeof(WCHAR)) == 0)
	{
		HANDLE hOldToken, hNewToken;

		ok = OpenProcessToken(GetCurrentProcess(), TOKEN_ALL_ACCESS, &hOldToken);
		if (ok)
		{
			ok = DuplicateToken(hOldToken, SecurityImpersonation, &hNewToken);

			if (ok)
			{
				ok = DuplicateHandle(GetCurrentProcess(), hNewToken, hClientProcess, (HANDLE*)&pkt->hResult, 0, FALSE, DUPLICATE_SAME_ACCESS);
	...
	{
		ok = DuplicateHandle(GetCurrentProcess(), pi.hProcess, hClientProcess, (HANDLE*)&pkt->hResult, 0, FALSE, DUPLICATE_SAME_ACCESS);
...
bool ServiceServer::CallUacDialogHook(ULONG_PTR* ThreadArgs, HANDLE idProcess, const WCHAR* cmdline, bool isAdmin)
{
	...
	P_UacDialog pUacDialog = (P_UacDialog)GetProcAddress(extdll, "UacDialog");
	#ifndef _WIN64
	if (!pUacDialog)
	{
	...
	LONG_PTR ReturnCode = pUacDialog(GetModuleHandle(SBIEDLL L".dll"), (ULONG)(ULONG_PTR)idProcess, cmdline, isAdmin ? 1 : 0);
