...
FileServer::FileServer(PipeServer* pipeServer)
{
	...
	BOOL b = LookupPrivilegeValue(L"", SE_RESTORE_NAME, &privs->Privileges[0].Luid);
	if (b)
	{
	...
	wsprintf(m_winsxs, L"\\drive\\%c%s\\winsxs\\", m_windows[0], &m_windows[2]);
...
MSG_HEADER* FileServer::SetAttributes(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	HANDLE handle;
	NTSTATUS status = OpenBoxFile(idProcess, req->path, FILE_WRITE_ATTRIBUTES | SYNCHRONIZE, FILE_SYNCHRONOUS_IO_NONALERT, &handle);
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtSetInformationFile(handle, &IoStatusBlock, &req->info, sizeof(req->info), FileBasicInformation);
...
MSG_HEADER* FileServer::SetShortName(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	HANDLE handle;
	NTSTATUS status = OpenBoxFile(idProcess, req->path, FILE_GENERIC_WRITE | DELETE, FILE_SYNCHRONOUS_IO_NONALERT | FILE_OPEN_FOR_BACKUP_INTENT, &handle);
	...
	if (NT_SUCCESS(status))
	{
		...
		IO_STATUS_BLOCK IoStatusBlock;
		status = NtSetInformationFile(handle, &IoStatusBlock, &req->info, sizeof(req->info), FileShortNameInformation);
...
NTSTATUS FileServer::OpenBoxFile(HANDLE idProcess, WCHAR* request_path, ACCESS_MASK desired_access, ULONG create_options, HANDLE* out_handle)
{
	...
	IO_STATUS_BLOCK IoStatusBlock;
	status = NtCreateFile(out_handle, desired_access, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, create_options, NULL, 0);
...
MSG_HEADER* FileServer::LoadKey(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	NTSTATUS status = SbieApi_QueryProcess(idProcess, NULL, NULL, NULL, NULL);
	if (!NT_SUCCESS(status))
	{
		return SHORT_REPLY(STATUS_ACCESS_DENIED);
	};
	...
	InitializeObjectAttributes(&source, &KeyPath_uni, OBJ_CASE_INSENSITIVE, NULL, NULL);

	RtlInitUnicodeString(&FilePath_uni, req->KeyPath);
	InitializeObjectAttributes(&target, &FilePath_uni, OBJ_CASE_INSENSITIVE, NULL, NULL);
...
MSG_HEADER* FileServer::GetAllHandles(HANDLE idProcess)
{
	...
	for (i = 0; i < 5; ++i)
	{
		info = (struct MY_SYSTEM_HANDLE_INFORMATION*)HeapAlloc(m_heap, 0, info_len);
		if (!info)
		{
	...
	if (NT_SUCCESS(status))
	{
		ULONG rpl_len = sizeof(FILE_GET_ALL_HANDLES_RPL) + info->Count * sizeof(ULONG64);

		FILE_GET_ALL_HANDLES_RPL* rpl = (FILE_GET_ALL_HANDLES_RPL*)LONG_REPLY(rpl_len);
		...
		else
		{
			for (i = 0; i < info->Count; ++i)
			{
				...
				{
					ULONG handle = (ULONG)(ULONG_PTR)info->HandleInfo[i].Handle;

					ULONG objtype = (ULONG)info->HandleInfo[i].ObjectTypeNumber;

					rpl->handles[rpl->num_handles] = (handle & 0x00FFFFFFU) | (objtype << 24);
...
bool FileServer::CopySingleFile(const WCHAR* src_path, const WCHAR* dst_path, HANDLE idProcess)
{
	NTSTATUS status;
	UNICODE_STRING objname;
	OBJECT_ATTRIBUTES objattrs;
	...
	InitializeObjectAttributes(&objattrs, &objname, OBJ_CASE_INSENSITIVE, NULL, m_PublicSd);
	...
	do
	{
		...
		status = NtCreateFile(&src_handle, FILE_GENERIC_READ, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE, NULL, 0);
		...
		*LastBackslashPtr       = L'\0';
		...
		status = NtCreateFile(&dst_handle, FILE_GENERIC_WRITE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_DIRECTORY_FILE, NULL, 0);
		...
		if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_OBJECT_PATH_NOT_FOUND)
		{
			...
			status = NtCreateFile(&dst_handle, FILE_GENERIC_WRITE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN_IF, FILE_SYNCHRONOUS_IO_NONALERT | FILE_DIRECTORY_FILE, NULL, 0);
		}

		if (!NT_SUCCESS(status))
		{
			SbieApi_LogEx(session_id, 2216, L"[%08X] %S", status, AssemblyNamePtr);
		}
		...
		*LastBackslashPtr = L'\\';
		...
		status = NtCreateFile(&dst_handle, FILE_GENERIC_WRITE, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OVERWRITE_IF, FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE, NULL, 0);
		...
		if (NT_SUCCESS(status))
		{
			status = NtQueryInformationFile(src_handle, &IoStatusBlock, &open_info, sizeof(FILE_NETWORK_OPEN_INFORMATION), FileNetworkOpenInformation);
		}
		...
		while (NT_SUCCESS(status) && open_info.EndOfFile.QuadPart)
		{
			...
			status = NtReadFile(src_handle, NULL, NULL, NULL, &IoStatusBlock, buffer, buffer_size, NULL, NULL);
			...
			if (NT_SUCCESS(status))
			{
				...
				status = NtWriteFile(dst_handle, NULL, NULL, NULL, &IoStatusBlock, buffer, buffer_size, NULL, NULL);
			}
		}
...
NTSTATUS FileServer::MyFindFirstFile(const WCHAR* path, HANDLE* handle, FILE_DIRECTORY_INFORMATION** info, FILE_DIRECTORY_INFORMATION** info_ptr)
{
	NTSTATUS status;
	OBJECT_ATTRIBUTES objattrs;
	UNICODE_STRING objname;
	...
	*info_ptr = NULL;
	...
	status = NtCreateFile(handle, FILE_GENERIC_READ, &objattrs, &IoStatusBlock, NULL, 0, FILE_SHARE_VALID_FLAGS, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_DIRECTORY_FILE, NULL, 0);
	...
	FILE_DIRECTORY_INFORMATION* entry = (FILE_DIRECTORY_INFORMATION*)MyAlloc(DIR_INFO_LENGTH);
	if (!entry)
	{
...
NTSTATUS FileServer::MyFindNextFile(HANDLE handle, FILE_DIRECTORY_INFORMATION* info, FILE_DIRECTORY_INFORMATION** info_ptr)
{
	//
	...
	FILE_DIRECTORY_INFORMATION* entry = (FILE_DIRECTORY_INFORMATION*)(*info_ptr);

	if (entry->NextEntryOffset)
	{
		ULONG SavedNextEntryOffset = entry->FileIndex;

		entry = (FILE_DIRECTORY_INFORMATION*)((UCHAR*)entry + entry->NextEntryOffset);
	...
	else
	{
		...
		status = NtQueryDirectoryFile(handle, NULL, NULL, NULL, &IoStatusBlock, info, DIR_INFO_LENGTH, FileDirectoryInformation, FALSE, NULL, FALSE);
	...
	if (entry->NextEntryOffset)
	{
		FILE_DIRECTORY_INFORMATION* next_entry = (FILE_DIRECTORY_INFORMATION*)((UCHAR*)entry + entry->NextEntryOffset);
...
MSG_HEADER* FileServer::OpenWow64Key(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	while (1)
	{
		rc = RegOpenKeyEx(hRootKey, lpSubKey, 0, KEY_READ | KEY_WOW64_32KEY, &hKey);
		if (rc == 0)
		{
			break;
		}
	...
	ULONG rpl_len = sizeof(FILE_OPEN_WOW64_KEY_RPL) + (len + 2) * sizeof(WCHAR);
	if (BackslashPtr)
	{
		*BackslashPtr = L'\\';
	...
	FILE_OPEN_WOW64_KEY_RPL* rpl = (FILE_OPEN_WOW64_KEY_RPL*)LONG_REPLY(rpl_len);
	if (rpl)
	{
...
NTSTATUS FileServer::CheckBoxFilePath(HANDLE idProcess, WCHAR* request_path, const WCHAR* extra_path)
{
	//
	...
	ULONG len       = 0;
	NTSTATUS status = SbieApi_QueryProcessPath(idProcess, NULL, NULL, NULL, &len, NULL, NULL);
	if (!NT_SUCCESS(status))
	{
		return status;
	}

	WCHAR* sandbox_path = (WCHAR*)MyAlloc(len + 8 + wcslen(extra_path) * sizeof(WCHAR));
	if (!sandbox_path)
	{
		return STATUS_INSUFFICIENT_RESOURCES;
	}

	status = SbieApi_QueryProcessPath(idProcess, sandbox_path, NULL, NULL, &len, NULL, NULL);
	if (!NT_SUCCESS(status))
	{
		MyFree(sandbox_path);
		return status;
...
NTSTATUS FileServer::CheckBoxKeyPath(HANDLE idProcess, WCHAR* request_path, const WCHAR* extra_path)
{
	//
	...
	ULONG len       = 0;
	NTSTATUS status = SbieApi_QueryProcessPath(idProcess, NULL, NULL, NULL, NULL, &len, NULL);
	if (!NT_SUCCESS(status))
	{
		return status;
	}
	...
	status = SbieApi_QueryProcessPath(idProcess, NULL, sandbox_path, NULL, NULL, &len, NULL);
	if (!NT_SUCCESS(status))
	{
		MyFree(sandbox_path);
		return status;
