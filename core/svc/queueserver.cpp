...
MSG_HEADER* QueueServer::CreateHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	status = OpenProcess(idProcess, &hProcess, PROCESS_DUP_HANDLE | PROCESS_QUERY_INFORMATION);
	...
	ULONG queue_obj_len = sizeof(QUEUE_OBJ) + (wcslen(QueueName) + 1) * sizeof(WCHAR);
...
MSG_HEADER* QueueServer::GetReqHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	ULONG rpl_len = sizeof(QUEUE_GETREQ_RPL) + RequestObj->req_data_len + 8;
...
MSG_HEADER* QueueServer::PutReqHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	REQUEST_OBJ* RequestObj = (REQUEST_OBJ*)HeapAlloc(m_heap, 0, sizeof(REQUEST_OBJ));
	if (!RequestObj)
	{
		status = STATUS_INSUFFICIENT_RESOURCES;
		goto finish;
...
MSG_HEADER* QueueServer::GetRplHandler(MSG_HEADER* msg, HANDLE idProcess)
{
	...
	ULONG rpl_len = sizeof(QUEUE_GETRPL_RPL) + RequestObj->rpl_data_len + 8;
...
LONG QueueServer::OpenProcess(HANDLE idProcess, HANDLE* out_hProcess, ULONG DesiredAccess)
{
	LONG status;
	...
	status                 = NtOpenProcess(out_hProcess, DesiredAccess, &ObjectAttributes, &ClientId);
	if (!NT_SUCCESS(status))
	{
		*out_hProcess = NULL;
		if (status == STATUS_ACCESS_DENIED)
...
LONG QueueServer::DuplicateEvent(HANDLE hProcess, ULONG64 hEvent, HANDLE* out_hEvent)
{
	LONG status;
	if (!hEvent)
	{
	...
	{
		status = NtDuplicateObject(hProcess, (HANDLE)(ULONG_PTR)hEvent, GetCurrentProcess(), out_hEvent, EVENT_MODIFY_STATE, 0, 0);
		if (!NT_SUCCESS(status))
		{
			*out_hEvent = NULL;
			if (status == STATUS_ACCESS_DENIED)
...
WCHAR* QueueServer::MakeQueueName(HANDLE idProcess, WCHAR* req_name, ULONG* out_status)
{
	LONG status;
	...
	else
	{
		...
		status = SbieApi_QueryProcessPath(idProcess, NULL, NULL, NULL, NULL, NULL, &ipc_len);
		if (status != 0)
		{
		...
		status = SbieApi_QueryProcessPath(idProcess, NULL, NULL, name, NULL, NULL, &ipc_len);
		if (status != 0)
		{
...
void* QueueServer::FindQueueObj(const WCHAR* QueueName)
{
	...
	while (QueueObj)
	{
		if (QueueObj->queue_name_len == QueueNameLen && _wcsicmp(QueueObj->queue_name, QueueName) == 0)
		{
			break;
		}
	...
	if (QueueObj)
	{
		...
		HANDLE hProcess = NULL;
		ULONG status    = OpenProcess(QueueObj->server_pid, &hProcess, PROCESS_QUERY_INFORMATION);
		if (status == 0)
		{
			DWORD nExitCode = 0;
			...
			if (GetExitCodeProcess(hProcess, &nExitCode) && nExitCode == STILL_ACTIVE)
			{
				...
				if (GetProcessTimes(hProcess, &time1, &time2, &time3, &time4))
				{
					...
					if (time1.dwHighDateTime == time2.dwHighDateTime && time1.dwLowDateTime == time2.dwLowDateTime)
					{
...
void QueueServer::NotifyHandler(HANDLE idProcess)
{
	...
	while (QueueObj)
	{
		...
		else
		{
			...
			while (RequestObj)
			{
				REQUEST_OBJ* RequestObjNext = (REQUEST_OBJ*)List_Next(RequestObj);
