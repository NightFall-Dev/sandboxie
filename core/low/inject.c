...
static UCHAR* FindDllExport2(void* DllBase, IMAGE_DATA_DIRECTORY* dir0, const UCHAR* ProcName);
...
_FX UCHAR* FindDllExport2(void* DllBase, IMAGE_DATA_DIRECTORY* dir0, const UCHAR* ProcName)
{
	void* proc = NULL;
	ULONG i, j, n;
...
#endif
{
	...
	if (data->is_wow64)
	{
		extern ULONG64 SbieApi_QueryProcessInfo(SBIELOW_DATA * data, ULONG info_type);
	...
	//
	LdrCode = FindDllExport(ntdll_base, (UCHAR*)extra + extra->LdrLoadDll_offset);
	if (!LdrCode)
	{
		return;
	}
	...
	LdrCode = FindDllExport(ntdll_base, (UCHAR*)extra + extra->LdrGetProcAddr_offset);
	if (!LdrCode)
	{
		return;
	}
	...
	LdrCode = FindDllExport(ntdll_base, (UCHAR*)extra + extra->NtRaiseHardError_offset);
	if (!LdrCode)
	{
		return;
	}
	...
	LdrCode = FindDllExport(ntdll_base, (UCHAR*)extra + extra->RtlFindActCtx_offset);
	if (!LdrCode)
	{
		return;
	}
	...
	inject->KerneDll_Buf32   = (ULONG)((ULONG_PTR)extra + extra->KernelDll_offset);
	inject->KerneDll_Buf64   = (ULONG64)((ULONG_PTR)extra + extra->KernelDll_offset);
	...
	inject->SbieDll_Buf32  = (ULONG)((ULONG_PTR)extra + extra->NativeSbieDll_offset);
	inject->SbieDll_Buf64  = (ULONG)((ULONG_PTR)extra + extra->NativeSbieDll_offset);
	...
	SBIELOW_CALL(NtProtectVirtualMemory)(NtCurrentProcess(), &RegionBase, &RegionSize, PAGE_EXECUTE_READWRITE, &OldProtect);

	*(ULONG_PTR*)(MyHookCode - 8) = (ULONG_PTR)inject;

	SBIELOW_CALL(NtProtectVirtualMemory)(NtCurrentProcess(), &RegionBase, &RegionSize, OldProtect, &OldProtect);

	RegionBase = (void*)&LdrCode[0];
	RegionSize = 12;
	SBIELOW_CALL(NtProtectVirtualMemory)(NtCurrentProcess(), &RegionBase, &RegionSize, PAGE_EXECUTE_READWRITE, &inject->RtlFindActCtx_Protect);
...
_FX void InitInjectWow64(SBIELOW_DATA* data, void* RtlFindActivationContextSectionString)
{
	...
	inject->SbieDll_MaxLen = inject->SbieDll_Length + sizeof(WCHAR);
	inject->SbieDll_Buf32  = (ULONG)((ULONG_PTR)extra + extra->Wow64SbieDll_offset);
