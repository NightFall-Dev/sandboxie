...
NTSTATUS SystemServiceC(SBIELOW_DATA* data, ULONG syscall_index, ULONG_PTR* syscall_args);
...
_FX NTSTATUS SbieApi_Ioctl(SBIELOW_DATA* data, void* parms)
{
	...
	*(ULONG_PTR*)&MyIoStatusBlock = (ULONG_PTR)MyIoStatusBlock32;
	...
	return SBIELOW_CALL(NtDeviceIoControlFile)((HANDLE)data->api_device_handle, NULL, NULL, NULL, &MyIoStatusBlock, API_SBIEDRV_CTLCODE, parms, sizeof(ULONG64) * 8, NULL, 0);
}
...
_FX void InitSyscalls(SBIELOW_DATA* data, void* SystemService)
{
	...
	*(ULONG_PTR*)(SystemServiceAsm + OFFSET_ULONG_PTR) = (ULONG_PTR)data;
	...
	SyscallPtr = (ULONG*)(data->syscall_data + sizeof(ULONG) // size of buffer
	...
	while (SyscallPtr[0] || SyscallPtr[1])
	{
		...
		// jmp rax
		    ...
		    ZwXxxPtr[1] == 0xb8 && *(USHORT*)&ZwXxxPtr[10] == 0xe0ff) /* jmp rax */
		{
		...
		*(ULONG*)&ZwXxxPtr[6] = (ULONG)(ULONG_PTR)(SystemServiceAsm - (ZwXxxPtr + 10));
...
#endif
{
	...
	else if (OldProcessHeap == -1)
	{
		...
		while (*ProcessHeap == -1)
		{
			const P_NtDelayExecution NtDelayExecution = (P_NtDelayExecution)&data->NtDelayExecution_code;
