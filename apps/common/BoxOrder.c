...
_FX WCHAR* BoxOrder_ReadSetting(const WCHAR* section_name)
{
	...
	if (section_name)
	{
		...
		if (result)
		{
			...
			while (1)
			{
				status = SbieApi_QueryConfAsIs(section_name, BoxOrder_SettingName, setting_idx | CONF_GET_NO_GLOBAL | CONF_GET_NO_TEMPLS, result_ptr, (result_len - 8) * sizeof(WCHAR));
...
_FX void BoxOrder_ReadMissing(BOX_ORDER_ENTRY* parent)
{
	...
	while (1)
	{
		box_index = SbieApi_EnumBoxesEx(box_index | CONF_GET_NO_TEMPLS, work_name, TRUE);
		if (box_index == -1)
		{
			break;
		}
	...
	while (name_index < box_count)
	{
		...
		box_index = SbieApi_EnumBoxesEx(box_index | CONF_GET_NO_TEMPLS, name, TRUE);
		if (box_index == -1)
		{
			break;
		}
...
_FX int BoxOrder_WriteSetting(WCHAR* text)
{
	...
	for (ptr = text; ptr; text = ptr)
	{
		...
		status = SbieDll_UpdateConf(opcode, NULL, L"UserSettings_?", BoxOrder_SettingName, text);
		if (status != 0)
		{
			return 0;
		}
