...
static LRESULT CALLBACK MyMsgBox_Hook(int nCode, WPARAM wParam, LPARAM lParam);

static void MyMsgBox_InsertCheckBox(HWND hwnd);

static LRESULT CALLBACK MyInputBox_Hook(int nCode, WPARAM wParam, LPARAM lParam);
...
_FX LRESULT CALLBACK MyMsgBox_Hook(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (MyMsgBox_hwnd != (HWND)-1)
	{
		...
		if (nCode == HCBT_DESTROYWND && hwnd == MyMsgBox_hwnd)
		{
			...
			if (BST_CHECKED == SendDlgItemMessage(hwnd, 'tz', BM_GETCHECK, 0, 0))
			{
...
_FX void MyMsgBox_InsertCheckBox(HWND hwnd)
{
	...
	button = CreateWindow(L"BUTTON", MyMsgBox_CheckText, BS_AUTOCHECKBOX | WS_CHILD | WS_VISIBLE, 0, 0, -1, -1, hwnd, (HMENU)'tz', NULL, NULL);
	...
	GetTextExtentPoint32(dc, MyMsgBox_CheckText, wcslen(MyMsgBox_CheckText), &sz);
	...
	stat = CreateWindow(L"STATIC", L"", WS_CHILD | WS_VISIBLE, 0, rc.bottom - sz.cy * 2, rc.right, rc.bottom, hwnd, (HMENU)-1, NULL, NULL);
...
_FX int MessageCheckBox(HWND hwnd, const WCHAR* text, const WCHAR* caption, UINT flags, const WCHAR* check)
{
	HHOOK hhk;
	int rv;
	...
	hhk = SetWindowsHookEx(WH_CBT, MyMsgBox_Hook, NULL, GetCurrentThreadId());
...
_FX LRESULT CALLBACK MyInputBox_Hook(int nCode, WPARAM wParam, LPARAM lParam)
{
	HWND hwnd = (HWND)wParam;
	...
	if (MyMsgBox_hwnd != (HWND)-1)
	{
		...
		if (nCode == HCBT_DESTROYWND && hwnd == MyMsgBox_hwnd)
		{
			int len = (int)SendDlgItemMessage(hwnd, 'tz', WM_GETTEXTLENGTH, 0, 0);
			if (len)
			{
				MyInputBox_EditText = LocalAlloc(LMEM_FIXED, (len + 4) * sizeof(WCHAR));
				SendDlgItemMessage(hwnd, 'tz', WM_GETTEXT, (WPARAM)(len + 2), (LPARAM)MyInputBox_EditText);
			}
...
_FX void MyInputBox_InsertEditBox(HWND hwnd)
{
	...
	edit = CreateWindowEx(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | MyInputBox_EditStyle, 0, 0, -1, -1, hwnd, (HMENU)'tz', NULL, NULL);
	...
	MoveWindow(edit, 20, (p.y - sz.cy) / 2, rc.right - 20 * 2, sz.cy, TRUE);
...
_FX WCHAR* InputBox(HWND hwnd, const WCHAR* caption, UINT flags, const WCHAR* def)
{
	HHOOK hhk;
	int rv;
	WCHAR* input;
	...
	hhk = SetWindowsHookEx(WH_CBT, MyInputBox_Hook, NULL, GetCurrentThreadId());
...
_FX void MyMsgBox_PushPop(BOOLEAN Push)
{
	if (Push)
	{
		...
		{
			MyMsgBox_Data[MyMsgBox_Level] = LocalAlloc(LMEM_FIXED, sizeof(MY_MSG_BOX_DATA));
		}
