...
#define MyHeapAlloc(len) (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, (len))
...
void Show_Error(WCHAR* Descr)
{
	...
	if (ErrorCode)
	{
		...
		wsprintf(msg, L"%s\n\n%s\n\n%s (%d)", Descr, SysErrText, ErrorText, ErrorCode);
	...
	{
		MessageBox(NULL, msg, Sandboxie_Start_Title, MB_ICONEXCLAMATION | MB_OK | MB_SETFOREGROUND | MB_TOPMOST | (layout_rtl ? MB_RTLREADING | MB_RIGHT : 0));
	}
}
...
BOOL Validate_Box_Name(void)
{
	...
	{
		WCHAR* errmsg = SbieDll_FormatMessage1(MSG_2331, SbieDll_GetStartError());
		SetLastError(0);
		Show_Error(errmsg);
	...
	{
		if (_wcsicmp(BoxName, L"__ask__") == 0 || _wcsicmp(BoxName, L"current") == 0)
		{
...
_FX WCHAR* Get_Default_Browser(void)
{
	...
	cmdlen = _cmdline;
	rc     = AssocQueryString(0, ASSOCSTR_EXECUTABLE, assoc_name, NULL, pgmname, &cmdlen);
	...
	{
		cmdlen = _cmdline;
		rc     = AssocQueryString(ASSOCF_INIT_BYEXENAME, ASSOCSTR_EXECUTABLE, L"iexplore", NULL, pgmname, &cmdlen);

		if (rc != S_OK)
		{
			rc = SHGetFolderPath(NULL, CSIDL_PROGRAM_FILES, NULL, SHGFP_TYPE_CURRENT, pgmname);
			if (rc != S_OK)
			{
				memzero(pgmname, 16);
...
BOOL Parse_Command_Line(void)
{
	...
	while (*cmd == L'/')
	{
		...
		if (_wcsnicmp(cmd, L"box:", 4) == 0)
		{
			...
			if (*tmp == L'-' && (!SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)GetCurrentProcessId(), 0)))
			{
				//
				...
				if (*cmd != L' ' || model_pid >= 0 || (!SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)-model_pid, 0)))
				{
					cmd = tmp;
				}
			}
		...
		{
			if (SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)GetCurrentProcessId(), 0))
			{
		...
		else if (_wcsnicmp(cmd, L"terminate:*", 11) == 0 || _wcsnicmp(cmd, L"terminate_all", 13) == 0)
		{
		...
		else if (_wcsnicmp(cmd, L"disable_force", 13) == 0 || _wcsnicmp(cmd, L"dfp", 3) == 0)
		{
			cmd = Eat_String(cmd);
	...
	if (wcsncmp(cmd, default_browser, wcslen(default_browser)) == 0 || _wcsnicmp(cmd, L"http://", 7) == 0 || _wcsnicmp(cmd, L"https://", 8) == 0)
	{
		ULONG cmdlen;
	...
	else if (wcsncmp(cmd, mail_agent, wcslen(mail_agent)) == 0)
	{
		...
		HRESULT rc = AssocQueryString(0, ASSOCSTR_EXECUTABLE, L"mailto", NULL, pgmname, &cmdlen);
	...
	else if (wcsncmp(cmd, start_menu, wcslen(start_menu)) == 0)
	{
		...
		else
		{
			...
			if (*colon == L':')
			{
				...
				StartMenuSectionName = MyHeapAlloc((name_len + 4) * sizeof(WCHAR));
...
int Program_Start(void)
{
	...
	if (GetEnvironmentVariable(L"00000000_" SBIE L"_CURRENT_DIRECTORY", curdir, 1020))
	{
		SetCurrentDirectory(curdir);
		SetEnvironmentVariable(L"00000000_" SBIE L"_CURRENT_DIRECTORY", NULL);
	}
	...
	if (GetEnvironmentVariable(L"00000000_" SBIE L"_SERVICE_NAME", NULL, 0))
	{
		expanded = MyHeapAlloc(8192 * sizeof(WCHAR));
		ExpandEnvironmentStrings(cmdline, expanded, 8192);

		ok = CreateProcess(NULL, expanded, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
	...
	do
	{
		...
		shExecInfo.fMask  = SEE_MASK_FLAG_NO_UI | SEE_MASK_DOENVSUBST | SEE_MASK_FLAG_DDEWAIT | SEE_MASK_NOZONECHECKS;
		if (wait_for_process)
		{
		...
		if ((!ok) && (err == ERROR_NO_ASSOCIATION) && process_was_registered && run_elevated_2)
		{
...
int __stdcall WinMainCRTStartup(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	int rc;
	USHORT KeyState;
	STARTUPINFO si;
	...
	if (1)
	{
		...
		if (shell32)
		{
			...
			P_Set pSet = (P_Set)GetProcAddress(shell32, "SetCurrentProcessExplicitAppUserModelID");
			if (pSet)
			{
