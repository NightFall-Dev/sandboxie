...
extern void DeleteSandbox(const WCHAR* BoxName, BOOL bLogoff, BOOL bSilent, int phase);
...
static void TranslateCommand(WCHAR* cmd1, WCHAR* cmd2, const WCHAR* BoxFolder);
...
void Error(const WCHAR* Descr, NTSTATUS Status)
{
	...
	if (!g_Silent)
	{
		...
		if (Status)
		{
			...
			{
				wcscat(text, L"\n\n");
				wcscat(text, SbieDll_FormatMessage0(MSG_3215));
			}
...
void DeleteSandbox(const WCHAR* BoxName, BOOL bLogoff, BOOL bSilent, int phase)
{
	g_BoxName = BoxName;
	...
	if ((phase <= 1) && SbieApi_QueryConfBool(g_BoxName, L"NeverDelete", FALSE))
	{
		SetLastError(0);
	...
	else
	{
		...
		wsprintf(evname, SANDBOXIE L"_Delete_Sandbox_Session_%d", session_id);
...
void RenameSandbox(void)
{
	...
	RtlMoveMemory(boxpath + 4, boxpath, (wcslen(boxpath) + 1) * sizeof(WCHAR));
	...
	status = NtCreateFile(&hSandbox, DELETE | SYNCHRONIZE, &objattrs, &MyIoStatusBlock, NULL, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN, FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, NULL);
	...
	*backslash = L'\0';
	...
	status = NtCreateFile(&hTopLevel, FILE_GENERIC_READ, &objattrs, &MyIoStatusBlock, NULL, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN, FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, NULL);
	...
	wsprintf(u.info.FileName, L"__Delete_%s_%08X%08X", g_BoxName, ft.dwHighDateTime, ft.dwLowDateTime);
	u.info.FileNameLength = wcslen(u.info.FileName) * sizeof(WCHAR);

	ULONG retries = 0;
	while (1)
	{
		status = NtSetInformationFile(hSandbox, &MyIoStatusBlock, &u.info, sizeof(u), FileRenameInformation);
...
void DeleteFilesInBox(const WCHAR* boxname)
{
	...
	static const WCHAR* _DefaultDeleteCommand = L"%SystemRoot%\\System32\\cmd.exe /c rmdir /s /q \"%SANDBOX%\"";
	...
	ULONG len  = 2048;
	WCHAR* cmd = (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, len);
	memzero(cmd, len);
	len -= 8;
	...
	WCHAR* tmpl = (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, 256);
	wsprintf(tmpl, L"__Delete_%s_", boxname);

	WCHAR* BoxFolder = (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, 2048);
	...
	while (1)
	{
		if ((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && wcslen(data.cFileName) == wcslen(tmpl) + 16 && _wcsnicmp(data.cFileName, tmpl, wcslen(tmpl)) == 0)
		{
...
NOINLINE void WaitForFolder(const WCHAR* folder, ULONG seconds)
{
	...
	{
		status = NtCreateFile(&handle, DELETE | SYNCHRONIZE, &objattrs, &MyIoStatusBlock, NULL, FILE_ATTRIBUTE_NORMAL, 0, FILE_OPEN, FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
...
bool RemoveFileAttributes(HANDLE heap, const WCHAR* parent, const WCHAR* child)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtCreateFile(&handle, FILE_READ_ATTRIBUTES | FILE_WRITE_ATTRIBUTES | SYNCHRONIZE, &objattrs, &MyIoStatusBlock, NULL, 0, 0, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	}
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtQueryInformationFile(handle, &MyIoStatusBlock, &u.info, sizeof(u), FileBasicInformation);
		...
		if (NT_SUCCESS(status))
		{
			...
			status = NtSetInformationFile(handle, &MyIoStatusBlock, &u.info, sizeof(u), FileBasicInformation);
		}
	}
...
bool RenameSingleFile(HANDLE heap, const WCHAR* parent, const WCHAR* child, const WCHAR* BoxPath)
{
	static ULONG counter = 0;
	UNICODE_STRING uni;
	OBJECT_ATTRIBUTES objattrs;
	...
	wsprintf(dst_path, L"\\??\\%s\\%08X-%08X-%08X", BoxPath, now.dwHighDateTime, now.dwLowDateTime, counter);
	...
	*dst_name       = L'\0';
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtCreateFile(&src_handle, DELETE | SYNCHRONIZE, &objattrs, &MyIoStatusBlock, NULL, 0, 0, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	}
	...
	if (NT_SUCCESS(status))
	{
		...
		status = NtCreateFile(&dst_handle, FILE_GENERIC_READ, &objattrs, &MyIoStatusBlock, NULL, FILE_ATTRIBUTE_NORMAL, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN, FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	}
	...
	if (NT_SUCCESS(status))
	{
		...
		{
			status = NtSetInformationFile(src_handle, &MyIoStatusBlock, &u.info, sizeof(u), FileRenameInformation);
...
{
	ULONG len      = sizeof(PATHELEM) + (wcslen(parent) + wcslen(child) + 4) * sizeof(WCHAR);
...
{
	static const WCHAR* deviceNames[] = {L"aux", L"clock$", L"con", L"nul", L"prn", L"com1", L"com2", L"com3", L"com4", L"com5", L"com6", L"com7", L"com8", L"com9", L"lpt1", L"lpt2", L"lpt3", L"lpt4", L"lpt5", L"lpt6", L"lpt7", L"lpt8", L"lpt9", NULL};
	static const UCHAR valid_chars[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
	                                   "0123456789 ^&@{}[],$=!-#()%.+~_";
	bool anyRenames;
		...
		while (1)
		{
			...
			else if (data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				...
				if (data.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT)
				{
					...
					WCHAR* fullpath = (WCHAR*)HeapAlloc(heap, 0, len * sizeof(WCHAR));
					if (fullpath)
					{
				...
				if (!removed)
				{
					PATHELEM* elem_new = AllocPathElem(heap, elem->path, name);
...
ALIGNED WCHAR* GetBoxFilePath(const WCHAR* boxname, ULONG extra)
{
	...
	status = SbieApi_QueryBoxPath(boxname, NULL, NULL, NULL, &len, NULL, NULL);
	if (status != 0)
	{
		return NULL;
	}
	...
	if (path)
	{
		status = SbieApi_QueryBoxPath(boxname, path, NULL, NULL, &len, NULL, NULL);
