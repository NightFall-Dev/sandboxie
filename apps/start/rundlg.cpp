...
void LoadHistory(void)
{
	...
	hr = pst->ReadItem(PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &GUID_SANDBOXIE, RunHistory, &len, &data, NULL, 0);
	if (FAILED(hr) || len == 0)
	{
		return;
	}
	...
	while (*ptr)
	{
		...
		hist_item = (HIST_ITEM*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, len);
...
void SaveHistory(void)
{
	...
	data      = (BYTE*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY, len);
	ptr       = (WCHAR*)data;
	hist_item = (HIST_ITEM*)List_Head(&history);
	while (hist_item)
	{
	...
	*ptr = L'\0'; // terminator null
	...
	hr = pst->WriteItem(PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &GUID_SANDBOXIE, RunHistory, len, data, NULL, 0, 0);
	if (FAILED(hr))
	{
		typeinfo.cbSize        = sizeof(typeinfo);
		typeinfo.szDisplayName = (WCHAR*)Sandboxie;
		hr                     = pst->CreateType(PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &typeinfo, 0);
		hr = pst->CreateSubtype(PST_KEY_CURRENT_USER, &GUID_SANDBOXIE, &GUID_SANDBOXIE, &typeinfo, NULL, 0);
...
HWND CreateHistoryCombo(HWND hwnd)
{
	HWND ctrl;
	RECT rc;
	LRESULT lResult;
	...
	ctrl = CreateWindowEx(0,
	    WC_COMBOBOXEX,
	    L"",
	    CBS_DROPDOWN | CBS_AUTOHSCROLL | WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | WS_TABSTOP,
	    rc.left,
	    rc.top,
	    rc.right - rc.left,
	    300,
	...
	while (hist_item)
	{
		...
		hist_item->idx  = SendMessage(ctrl, CBEM_INSERTITEM, 0, (LPARAM)&cbei);
		hist_item       = (HIST_ITEM*)List_Next(hist_item);
	}
...
void DeleteHistoryItem(HWND hwndCombo)
{
	HIST_ITEM *hist_item, *next_item;
	COMBOBOXEXITEM cbei;
	WCHAR text[512];
...
void PrepareRunAsAdmin(HWND hwnd, const WCHAR* BoxName, BOOLEAN JustAdmin)
{
	...
	if (BoxName)
	{
		if ((BoxName == (WCHAR*)-1) || SbieApi_QueryConfBool(BoxName, L"DropAdminRights", FALSE))
		{
	...
	if (JustAdmin)
	{
		...
		if (ctrl)
		{
			SendMessage(ctrl, BM_SETCHECK, run_elevated_2 ? BST_CHECKED : BST_UNCHECKED, 0);
...
INT_PTR RunDialogProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SetDlgItemText(hwnd, IDRUNDLGTEXT, SbieDll_FormatMessage0(MSG_3103));

		SetDlgItemText(hwnd, IDRUNDLGTEXT2, SbieDll_FormatMessage0(MSG_3104));
		...
		{
			SetFocus(hwndCombo);
			hwndEdit = (HWND)SendMessage(hwndCombo, CBEM_GETEDITCONTROL, 0, 0);
			SHAutoComplete(hwndEdit, SHACF_FILESYSTEM | SHACF_AUTOSUGGEST_FORCE_ON | SHACF_AUTOAPPEND_FORCE_ON);
		}
		else
		{
			EndDialog(hwnd, 0);
		...
		hwndToolTip = CreateWindowEx(WS_EX_TOPMOST, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_NOANIMATE | TTS_NOFADE, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, hwnd, NULL, (HINSTANCE)lParam, NULL);
	...
	case WM_COMMAND:
		...
		else if (LOWORD(wParam) == IDBROWSE)
		{
			...
			ofn.Flags        = OFN_DONTADDTORECENT | OFN_NODEREFERENCELINKS | OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
...
WCHAR* DoRunDialog(HINSTANCE hInstance)
{
	...
	if (!initialized)
	{
		...
		editbox_len   = 10240;
		editbox_space = (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, editbox_len * sizeof(WCHAR));

		hDeleteIcon = (HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDDELETE), IMAGE_ICON, 16, 16, 0);

		LoadHistory();

		msg_3308
			= SbieDll_FormatMessage0(MSG_3308);
			while (1)
			{
	...
	if (layout_rtl)
	{
		LPCDLGTEMPLATE tmpl = (LPCDLGTEMPLATE)Common_DlgTmplRtl(hInstance, MAKEINTRESOURCE(RUN_DIALOG));
		r                   = DialogBoxIndirectParam(hInstance, tmpl, NULL, RunDialogProc, (LPARAM)hInstance);
	}
	else
	{
		r = DialogBoxParam(hInstance, MAKEINTRESOURCE(RUN_DIALOG), NULL, RunDialogProc, (LPARAM)hInstance);
	}
