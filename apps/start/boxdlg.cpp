...
BOOLEAN CanDisableForce(void)
{
	...
	if (!init)
	{
		if (SbieApi_QueryConfBool(NULL, L"ForceDisableAdminOnly", FALSE))
		{
			...
			SBIE_INI_GET_USER_RPL* rpl = (SBIE_INI_GET_USER_RPL*)CallSbieSvcGetUser();
			if (rpl)
			{
				if (rpl->admin)
				{
...
BOOL GetBoxName(HWND listbox, int index, WCHAR* boxname)
{
	...
	if (text)
	{
		LRESULT lResult = SendMessage(listbox, LB_GETTEXT, index, (LPARAM)text);
		if (lResult != LB_ERR)
		{
			WCHAR* ptr = text;
			while (*ptr == L' ')
...
void TrackToolTip(HWND hwnd, int first_time)
{
	...
	if (!boxtext)
	{
		boxtext = (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, (2048 + 8) * sizeof(WCHAR));
	}
	...
	else if (0 != SbieApi_QueryConfAsIs(boxname, L"Description", 0, boxtext, 2048 * sizeof(WCHAR)))
	{
		boxtext[0] = L'\0';
...
dotooltip:
	...
	if (!htip)
	{
		...
		htip = CreateWindowEx(exStyle, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_BALLOON | TTS_NOANIMATE | TTS_NOFADE, 0, 0, 0, 0, NULL, NULL, GetModuleHandle(NULL), NULL);
...
int AddBoxOrder(BOX_ORDER_ENTRY* order_entry, HWND hwnd, int indent, const WCHAR* compare_text)
{
	WCHAR* text;
	int i, rv = -1;
	...
	while (order_entry)
	{
		if (order_entry->children)
		{
			...
			SendDlgItemMessage(hwnd, IDBOXLIST, LB_ADDSTRING, 0, (LPARAM)text);

			i = AddBoxOrder(order_entry->children, hwnd, indent + 12, compare_text);
			if (rv == -1)
			{
				rv = i;
			}
		...
		else
		{
			...
			i = (int)SendDlgItemMessage(hwnd, IDBOXLIST, LB_ADDSTRING, 0, (LPARAM)text);

			if (rv == -1 && i != LB_ERR && _wcsicmp(order_entry->name, compare_text) == 0)
			{
				rv = i;
			}
		}
...
INT_PTR ListBoxProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == WM_CHAR)
	{
...
INT_PTR BoxDialogProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SetDlgItemText(hwnd, IDRUNDLGTEXT, SbieDll_FormatMessage0(MSG_3106));
		...
		if (0 == SbieApi_QueryProcess((HANDLE)(ULONG_PTR)GetCurrentProcessId(), boxname, NULL, NULL, NULL))
		{
			SendDlgItemMessage(hwnd, IDBOXLIST, LB_ADDSTRING, 0, (LPARAM)boxname);
		}
		else
		{
			//
			...
			if (box_order)
			{
				...
				{
					index = AddBoxOrder(box_order->children, hwnd, 0, L"DefaultBox");
				}
			...
			if (index != -1)
			{
				SendDlgItemMessage(hwnd, IDBOXLIST, LB_SETCURSEL, index, 0);
				set_sel = FALSE;
			}

			index = (int)SendDlgItemMessage(hwnd, IDBOXLIST, LB_GETCOUNT, 0, 0);
			if (index <= 0)
			{
				EndDialog(hwnd, 0);
			}
			...
			if (boxdlg_run_outside_sandbox && CanDisableForce())
			{
				...
				boxname[40] = 0;
				SendDlgItemMessage(hwnd, IDBOXLIST, LB_INSERTSTRING, -1, (LPARAM)boxname);
				dfp_index = (int)SendDlgItemMessage(hwnd, IDBOXLIST, LB_INSERTSTRING, -1, (LPARAM)text);
			}
		}
		...
		if (1)
		{
			...
			WNDPROC oldproc = (WNDPROC)GetWindowLongPtr(hListBox, GWLP_WNDPROC);
			SetProp(hListBox, L"Ptr6392", oldproc);
			SetWindowLongPtr(hListBox, GWLP_WNDPROC, (ULONG_PTR)ListBoxProc);
		}
	...
	case WM_WINDOWPOSCHANGED:
		...
		if (hide_tool_tip || uMsg == WM_DESTROY || uMsg == WM_KILLFOCUS || (uMsg == WM_ACTIVATE && LOWORD(wParam) == WA_INACTIVE))
		{
			x = 9;
		}
	...
	case WM_COMMAND:
		...
		index = (int)SendDlgItemMessage(hwnd, IDBOXLIST, LB_GETCURSEL, 0, 0);
		if (index == LB_ERR)
		{
			is_box = FALSE;
		}
		...
		{
			is_box = GetBoxName(GetDlgItem(hwnd, IDBOXLIST), index, boxname);
		}

		if (LOWORD(wParam) == IDBOXLIST && HIWORD(wParam) == LBN_SELCHANGE)
		{
			TrackToolTip(hwnd, 0);
		...
		else if (LOWORD(wParam) == IDOK || (LOWORD(wParam) == IDBOXLIST && HIWORD(wParam) == LBN_DBLCLK))
		{
			if (dfp_index != LB_ERR && index == dfp_index)
			{
				...
				rc = MessageBox(hwnd, SbieDll_FormatMessage0(MSG_3253), Sandboxie_Start_Title, MB_ICONQUESTION | MB_YESNO | (layout_rtl ? MB_RTLREADING | MB_RIGHT : 0));
...
void Adjust_Window_Height(HWND hwnd)
{
	...
	SetWindowPos(hwnd, HWND_TOPMOST, rc.left, rc.top, rc.right - rc.left + 1, pt.y, 0);
	...
	SetWindowPos(hlist, 0, 0, 0, rc.right - rc.left + 1, rc.bottom - rc.top + 1, SWP_NOMOVE);
	...
	for (i = 0; ControlIDs[i]; ++i)
	{
		...
		SetWindowPos(hctrl, 0, pt.x, pt.y, rc.right - rc.left + 1, rc.bottom - rc.top + 1, 0);
	}
}
...
WCHAR* DoBoxDialog(void)
{
	...
	if (!initialized)
	{
		...
		hProgramIcon = (HICON)LoadImage(hInstance, MAKEINTRESOURCE(IDICON), IMAGE_ICON, 0, 0, LR_DEFAULTSIZE);
	...
	if (layout_rtl)
	{
		LPCDLGTEMPLATE tmpl = (LPCDLGTEMPLATE)Common_DlgTmplRtl(hInstance, MAKEINTRESOURCE(BOX_DIALOG));
		r                   = DialogBoxIndirectParam(hInstance, tmpl, NULL, BoxDialogProc, (LPARAM)hInstance);
	}
	else
	{
		r = DialogBoxParam(hInstance, MAKEINTRESOURCE(BOX_DIALOG), NULL, BoxDialogProc, (LPARAM)hInstance);
	}
