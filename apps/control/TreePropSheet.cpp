...
namespace TreePropSheet
{
	...
	BOOL CTreePropSheet::OnInitDialog()
	{
		...
		DWORD dwTreeStyle = WS_TABSTOP | WS_CHILD | WS_VISIBLE | TVS_SHOWSELALWAYS | TVS_TRACKSELECT | TVS_HASLINES | TVS_LINESATROOT | TVS_HASBUTTONS | TVS_SINGLEEXPAND;
...
// MFC7-support here (Thanks to Rainer Wollgarten)
#if _MFC_VER >= 0x0700
		{
			m_pwndPageTree->CreateEx(dwTreeExStyle, dwTreeStyle, rectTree, this, s_unPageTreeId);
		}
#else
		{
			m_pwndPageTree->CreateEx(dwTreeExStyle, _T("SysTreeView32"), _T("PageTree"), dwTreeStyle, rectTree, this, s_unPageTreeId);
		}
#endif
		...
		if (!m_checkbox_text.IsEmpty())
		{
			...
			m_checkbox.Create(m_checkbox_text, BS_AUTOCHECKBOX | WS_CHILD | WS_VISIBLE, rect, this, 'tz');
			if (0 && m_bLayoutRTL)
			{
