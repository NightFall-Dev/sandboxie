...
{
	m_combo.Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP, CRect(), this, ID_GROUP_COMBO);
...
void CMyListCtrl::InitImageList(const CString& prefix)
{
	...
	HICON iconEmpty = (HICON)LoadImage(AfxGetInstanceHandle(), prefix + L"EMPTY", IMAGE_ICON, 0, 0, 0);

	HICON iconMinus = (HICON)LoadImage(AfxGetInstanceHandle(), prefix + L"MINUS", IMAGE_ICON, 0, 0, 0);

	HICON iconPlus = (HICON)LoadImage(AfxGetInstanceHandle(), prefix + L"PLUS", IMAGE_ICON, 0, 0, 0);
...
int CMyListCtrl::AddToImageList(HICON icon)
{
	...
	if (ImageList_GetIconSize(imglist->m_hImageList, &imglist_width, &imglist_height))
	{
		ICONINFO icon_info;
		if (GetIconInfo(icon, &icon_info))
		{
			...
			bool ok_clr = AddToImageList2(&bmp_clr, imglist_width, imglist_height, false);
			bool ok_msk = AddToImageList2(&bmp_msk, imglist_width, imglist_height, true);
...
bool CMyListCtrl::AddToImageList2(CBitmap* bmp, int w, int h, bool mask)
{
	...
	BITMAP info;
	if (bmp->GetBitmap(&info) && (info.bmWidth < w || info.bmHeight < h))
	{
		CDC* dc = GetDC();
...
void CMyListCtrl::RebuildImageList()
{
	...
	{
		lvi.iItem = ListIndex;
		if (CListCtrl::GetItem(&lvi) && lvi.iImage != m_imgEmpty && lvi.iImage != m_imgMinus && lvi.iImage != m_imgPlus)
		{
	...
	{
		if (ImageIndex != m_imgEmpty && ImageIndex != m_imgMinus && ImageIndex != m_imgPlus)
		{
...
void CMyListCtrl::SelectIndex(int ListIndex)
{
	...
	CListCtrl::SetItemState(ListIndex, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
}
