...
class CBoxPage : public CPropertyPage
{
	...
	int AddStringAndSetWidth(CListBox* pListBox, const CString& str, int index = -1);
	...
	BOOL CheckIfBrackets(CListBox* pListBox, int index, const CString& SettingName, const CString& Prefix = CString());
	...
	void DeleteCommand_SetDeleteProgram(const WCHAR* PgmName, const WCHAR* PgmArgs);
	...
	static int CALLBACK ForceLinger_OnCmdAddFolder_Callback(HWND hwnd, UINT uMsg, LPARAM lParam, LPARAM lpData);
...
public:
	CBoxPage(ULONG type, const CString& BoxName, ULONG GroupMsgId, ULONG TitleMsgId);

	~CBoxPage();

	static void DoPropertySheet(CWnd* pParentWnd, const CString& BoxName, bool* ReloadBoxes, const CString& ProgramName);
};
