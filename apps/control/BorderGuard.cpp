...
CBorderGuard::CBorderGuard()
{
	...
	if (dwmapi)
	{
		m_DwmIsCompositionEnabled = GetProcAddress(dwmapi, "DwmIsCompositionEnabled");
		if (m_DwmIsCompositionEnabled)
		{
			m_DwmGetWindowAttribute = GetProcAddress(dwmapi, "DwmGetWindowAttribute");
	...
	m_border_hwnd = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW | WS_EX_TOPMOST, (LPCWSTR)atom, WindowClassName, WS_POPUP | WS_CLIPSIBLINGS, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, NULL, NULL);
	if (!m_border_hwnd)
	{
		return;
	}
...
void CBorderGuard::MyTimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	m_instance->Refresh();
}
...
void CBorderGuard::Refresh()
{
	...
	if (boxname[0])
	{
		...
		if (m_DwmIsCompositionEnabled)
		{
			...
			P_DwmIsCompositionEnabled pDwmIsCompositionEnabled = (P_DwmIsCompositionEnabled)m_DwmIsCompositionEnabled;
			...
			if (SUCCEEDED(hr) && dwmEnabled)
			{
				...
				P_DwmGetWindowAttribute pDwmGetWindowAttribute = (P_DwmGetWindowAttribute)m_DwmGetWindowAttribute;
		...
		if (pid == m_active_pid && hwnd == m_active_hwnd)
		{
			if (memcmp(&rect, &m_active_rect, sizeof(RECT)) == 0)
			{
				...
				if (m_title_mode)
				{
					...
					if (have_cursor && cursor.x >= m_title_rect.left && cursor.x <= m_title_rect.right
					    && cursor.y >= m_title_rect.top && cursor.y <= m_title_rect.bottom)
					{
						title_mode = +1;
					}
...
void CBorderGuard::RefreshBorder(HWND hwnd, ULONG style, RECT* rect, const WCHAR* boxname)
{
	//
	...
	if (rect->left <= desktop->left && rect->top <= desktop->top && rect->right >= desktop->right
	    && rect->bottom >= desktop->bottom && (style & WS_CAPTION) != WS_CAPTION)
	{
		return;
	}
...
#define w (*rect)
		...
		if (t.left < w.left || t.left > w.right || t.top < w.top || t.top > w.bottom || t.right < w.left || t.right > w.right
		    || t.bottom < w.top || t.bottom > w.bottom || t.right - t.left <= m_thumb_width || t.bottom - t.top <= m_thumb_height)
		{
...
#undef t
		...
		m_title_mode = -1;
		POINT cursor;
		if (GetCursorPos(&cursor) && cursor.x >= m_title_rect.left && cursor.x <= m_title_rect.right
		    && cursor.y >= m_title_rect.top && cursor.y <= m_title_rect.bottom)
		{
			m_title_mode = +1;
		}
		...
		while (testwnd)
		{
			...
			if (exStyle & WS_EX_TOPMOST)
			{
				...
				if (m_windows_8)
				{
					...
					if (n == 19 && _wcsicmp(clsnm, L"EdgeUiInputWndClass") == 0)
					{
...
#undef ADD_POINT
	...
	SetWindowPos(m_border_hwnd, NULL, ax, ay, aw, ah, SWP_SHOWWINDOW | SWP_NOACTIVATE);
