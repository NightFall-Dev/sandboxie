...
CBoxPage::CBoxPage(ULONG type, const CString& BoxName, ULONG GroupMsgId, ULONG TitleMsgId) :
    m_BoxName(BoxName)
{
	...
	if (m_type == BoxPageOpenFile || m_type == BoxPageOpenPipe || m_type == BoxPageReadFile || m_type == BoxPageWriteFile || m_type == BoxPageClosedFile || m_type == BoxPageOpenKey || m_type == BoxPageReadKey || m_type == BoxPageWriteKey || m_type == BoxPageClosedKey || m_type == BoxPageOpenIpc || m_type == BoxPageClosedIpc || m_type == BoxPageWinClass || m_type == BoxPageComClass)
	{
...
void CBoxPage::SetPageTemplate(const WCHAR* TemplateName)
{
	...
	{
		m_template_copy = Common_DlgTmplRtl(AfxGetInstanceHandle(), m_psp.pszTemplate);
...
BOOL CBoxPage::CheckIfBrackets(CListBox* pListBox, int index, const CString& SettingName, const CString& Prefix)
{
		...
		while (pos)
		{
			...
			if (local || global)
			{
				...
				if (found)
				{
					...
					CString ClassName = CTemplateListBox::GetTemplateClass(nameTemplate);
...
void CBoxPage::AdjustFont()
{
	...
	if (!m_font)
	{
		...
		{
			CDC* pCdc      = GetDC();
			LONG NewHeight = -MulDiv(10, GetDeviceCaps(pCdc->m_hDC, LOGPIXELSY), 72);
			ReleaseDC(pCdc);
...
int CBoxPage::AddStringAndSetWidth(CListBox* pListBox, const CString& str, int index)
{
	if (!str.IsEmpty())
	{
...
void CBoxPage::MakeLTR(UINT id)
{
	if (CMyApp::m_LayoutRTL)
	{
		...
		wnd->ModifyStyleEx(WS_EX_RIGHT | WS_EX_RTLREADING | WS_EX_LAYOUTRTL, 0, _flag);
...
bool CBoxPage::SelectUsers(CStringList& users)
{
	...
	hr = CoCreateInstance(CLSID_DsObjectPicker, NULL, CLSCTX_INPROC_SERVER, IID_IDsObjectPicker, (void**)&pObjectPicker);
	if (FAILED(hr))
	{
		return false;
	}
	...
	ScopeInit.FilterFlags.Uplevel.flBothModes = DSOP_FILTER_USERS | DSOP_FILTER_WELL_KNOWN_PRINCIPALS | DSOP_FILTER_BUILTIN_GROUPS | DSOP_FILTER_UNIVERSAL_GROUPS_SE | DSOP_FILTER_GLOBAL_GROUPS_SE | DSOP_FILTER_DOMAIN_LOCAL_GROUPS_SE;
...
BOOL CBoxPage::OnInitDialog()
{
	...
	if (m_type == BoxPageForceFol || m_type == BoxPageForcePgm || m_type == BoxPageLingerPgm || m_type == BoxPageLeaderPgm)
	{
	...
	if (!m_titleForPage.IsEmpty())
	{
		if (!m_groupbox.m_hWnd)
		{
			...
			m_groupbox.Create(L"", WS_CHILD | WS_VISIBLE | BS_GROUPBOX, rc, this, ID_PAGE_TITLE);
...
BOOL CBoxPage::OnKillActive()
{
	if (m_modified)
	{
		TreePropSheet::CTreePropSheet* parent = (TreePropSheet::CTreePropSheet*)GetParent();
...
{
	const WCHAR* cmd = L"%SystemRoot%\\System32\\cmd.exe /c RMDIR /s /q \"%SANDBOX%\"";
...
void CBoxPage::DeleteCommand_SDelete()
{
	...
	const WCHAR* PgmArgs = L"-p 3 -s -q \"%SANDBOX%\"";
...
void CBoxPage::DeleteCommand_EraserL()
{
	...
	const WCHAR* PgmArgs = L"-folder \"%SANDBOX%\" -subfolders "
...
void CBoxPage::DeleteCommand_Eraser6()
{
	...
	const WCHAR* PgmArgs = L"addtask --quiet --method=ecbf4998-0b4f-445c-9a06-23627659e419 "
...
void CBoxPage::DeleteCommand_SetDeleteProgram(const WCHAR* PgmName, const WCHAR* PgmArgs)
{
	CString pgm1;
...
{
	if (m_type == BoxPageForcePgm && ForceLinger_MatchPattern(pgm, _ExplorerExe))
	{
...
void CBoxPage::ForceLinger_OnCmdAddFolder()
{
	...
	if (ok && m_type == BoxPageForceFol)
	{
		...
		if (len_path >= len_windir && (path[len_windir] == L'\\' || path[len_windir] == L'\0') && _wcsnicmp(path, windir, len_windir) == 0)
		{
		...
		if (len_path == 3 && len_windir >= 3 && _wcsnicmp(path, windir, len_path) == 0)
		{
...
int CALLBACK CBoxPage::ForceLinger_OnCmdAddFolder_Callback(HWND hwnd, UINT uMsg, LPARAM lParam, LPARAM lpData)
{
...
void CBoxPage::ForceLinger_OnCmdRemove()
{
	...
	if (index != LB_ERR)
	{
		...
		if ((m_type == BoxPageInternetPgm || m_type == BoxPageStartRunPgm) && (pListBox->GetCount() == 0))
		{
...
bool CBoxPage::ForceLinger_MatchPattern(const CString& pat, const CString& test)
{
...
void CBoxPage::FileAccess_OnInitDialog(CBox& box)
{
	...
	if (m_type == BoxPageWinClass || m_type == BoxPageComClass)
	{
		...
		GetDlgItem(ID_PAGE_LABEL_1)->SetWindowPos(NULL, 0, 0, rc1.right - rc1.left, rc2.bottom - rc1.top, SWP_NOMOVE | SWP_NOREPOSITION);
	...
	if (m_type != BoxPageComClass)
	{
		...
		{
			exclamation_bitmap = MyGdi_CreateFromResource(L"BLUE_EXCLAMATION");
		}
...
void CBoxPage::FileAccess_SwitchPgm(const CString& pgm)
{
	...
	GetDlgItem(ID_FILE_NEGATE)->ShowWindow((FileAccess_index == 0) ? SW_HIDE : SW_SHOW);
...
void CBoxPage::FileAccess_OnAddEntry()
{
	...
	if (m_type == BoxPageOpenKey || m_type == BoxPageClosedKey || m_type == BoxPageReadKey || m_type == BoxPageWriteKey || m_type == BoxPageOpenIpc || m_type == BoxPageClosedIpc || m_type == BoxPageWinClass || m_type == BoxPageComClass)
	{
...
void CBoxPage::FileAccess_OnEditEntry()
{
	...
	else
	{
		...
		if (m_type == BoxPageOpenFile || m_type == BoxPageOpenPipe || m_type == BoxPageClosedFile || m_type == BoxPageReadFile || m_type == BoxPageWriteFile)
		{
...
void CBoxPage::UserAccounts_OnOK(CBox& box)
{
	...
	if ((!UserAccounts_restricted) && (STATUS_ACCOUNT_RESTRICTION == SbieApi_IsBoxEnabled(box.GetName())))
	{
...
void CBoxPage::DoPropertySheet(CWnd* pParentWnd, const CString& BoxName, bool* ReloadBoxes, const CString& ProgramName)
{
	...
	CBoxPage pageAppearance(BoxPageAppearance, BoxName, 0, MSG_3811);

	CBoxPage pageQuickRecover(BoxPageQuickRecover, BoxName, MSG_3821, MSG_3822);
	CBoxPage pageAutoRecover(BoxPageAutoRecover, BoxName, MSG_3821, MSG_3715);

	CBoxPage pageAutoDelete(BoxPageAutoDelete, BoxName, MSG_3841, MSG_3842);
	CBoxPage pageDeleteCommand(BoxPageDeleteCommand, BoxName, MSG_3841, MSG_3852);

	CBoxPage pageProgramGroups(BoxPageProgramGroups, BoxName, 0, MSG_3971);

	CBoxPage pageForceFol(BoxPageForceFol, BoxName, MSG_3861, MSG_3862);
	CBoxPage pageForcePgm(BoxPageForcePgm, BoxName, MSG_3861, MSG_3872);

	CBoxPage pageLingerPgm(BoxPageLingerPgm, BoxName, MSG_3881, MSG_3882);
	CBoxPage pageLeaderPgm(BoxPageLeaderPgm, BoxName, MSG_3881, MSG_3892);

	CBoxPage pageFileMigrate(BoxPageFileMigrate, BoxName, 0, MSG_3901);

	CBoxPage pageInternetPgm(BoxPageInternetPgm, BoxName, MSG_3911, MSG_3912);
	CBoxPage pageStartRunPgm(BoxPageStartRunPgm, BoxName, MSG_3911, MSG_3922);
	CBoxPage pageDropRights(BoxPageDropRights, BoxName, MSG_3911, MSG_3942);
	CBoxPage pageBlockNetworkFiles(BoxPageNetworkFiles, BoxName, MSG_3911, MSG_3955);

	CBoxPage pageOpenFile(BoxPageOpenFile, BoxName, 0, MSG_4001);
	CBoxPage pageOpenPipe(BoxPageOpenPipe, BoxName, 0, MSG_4011);
	CBoxPage pageClosedFile(BoxPageClosedFile, BoxName, 0, MSG_4021);
	CBoxPage pageReadFile(BoxPageReadFile, BoxName, 0, MSG_4031);
	CBoxPage pageWriteFile(BoxPageWriteFile, BoxName, 0, MSG_4111);
	CBoxPage pageOpenKey(BoxPageOpenKey, BoxName, 0, MSG_4041);
	CBoxPage pageClosedKey(BoxPageClosedKey, BoxName, 0, MSG_4051);
	CBoxPage pageReadKey(BoxPageReadKey, BoxName, 0, MSG_4061);
	CBoxPage pageWriteKey(BoxPageWriteKey, BoxName, 0, MSG_4121);
	CBoxPage pageOpenIpc(BoxPageOpenIpc, BoxName, 0, MSG_4071);
	CBoxPage pageClosedIpc(BoxPageClosedIpc, BoxName, 0, MSG_4081);
	CBoxPage pageWinClass(BoxPageWinClass, BoxName, 0, MSG_4091);
	CBoxPage pageComClass(BoxPageComClass, BoxName, 0, MSG_4101);

	CBoxPage pageUserAccounts(BoxPageUserAccounts, BoxName, 0, MSG_5101);
