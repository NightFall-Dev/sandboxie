...
void CBoxes::ReloadBoxes()
{
	...
	while (1)
	{
		...
		{
			index = SbieApi_EnumBoxesEx(index | CONF_GET_NO_TEMPLS, name, TRUE);
			if (index == -1)
			{
				break;
			}
	...
	if (GetSize() == 1 && SbieApi_IsBoxEnabled(m_DefaultBox) == STATUS_OBJECT_NAME_NOT_FOUND)
	{
...
BOOL CBoxes::WriteExpandedView()
{
	...
	if (ok)
	{
		...
		while (!names.IsEmpty())
		{
			...
			if (name.GetLength() >= len && name.Left(len).CompareNoCase(setting) == 0)
			{
