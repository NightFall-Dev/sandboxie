...
CProgramSelector::CProgramSelector(CWnd* pParentWnd, CBox* box, bool withProgramGroups, const CString& groupName) :
...
void CProgramSelector::GetProgramNamesFromGroups(CBox& box, const CStringList& groups, CStringList& out_pgms)
{
...
void CProgramSelector::GetProgramNamesFromPathLists(CBox& box, CStringList& pgms)
{
	CStringList list;
...
void CProgramSelector::OnOK()
{
	...
	if (!m_pgm1.IsEmpty())
	{
		...
		{
			bool isProgramGroup =
			    ((m_pgm1.GetLength() > 1 && m_pgm1.GetAt(0) == L'<')) || ((m_pgm1.GetLength() > 2 && m_pgm1.Left(2) == L"!<"));
