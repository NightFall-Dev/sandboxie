	...
	if (bMenuBar && bSelected)
	{
		...
		for (int x = 0; x < SM_CXSHADOW; x++)
		{
			for (int y = (x < 2) ? 2 - x : 0; y < rc.Height() - x - ((x > 0) ? 1 : 2); y++)
			{
				...
				COLORREF cr2 = RGB(((nMakeSpec * int(GetRValue(cr))) / 100), ((nMakeSpec * int(GetGValue(cr))) / 100), ((nMakeSpec * int(GetBValue(cr))) / 100));
		...
		if (GetCaption(sCaption) > 0)
		{
			...
			else
			{
				...
				if (GetChecked())
				{
					COLORREF crHighLight = ::GetSysColor(COLOR_HIGHLIGHT);
					CPenDC pen(*pDC, crHighLight);
					CBrushDC brush(*pDC, crBackImg = GetDisabled() ? HLS_TRANSFORM(::GetSysColor(COLOR_MENU), +73, 0) : (bSelected ? HLS_TRANSFORM(crHighLight, +50, -50) : HLS_TRANSFORM(crHighLight, +70, -57)));
				...
				if (m_ImgDesc.m_hImgList != NULL && m_ImgDesc.m_nIndex != -1)
				{
					...
					if (GetDisabled() || (bSelected && !GetChecked()))
					{
						...
						pDC->DrawState(CPoint(pRect->left + (bOver ? 4 : 3), rc.top + (bOver ? 4 : 3)), CSize(IMGWIDTH, IMGHEIGHT), hIcon, DSS_MONO, CBrush(bOver ? HLS_TRANSFORM(::GetSysColor(COLOR_HIGHLIGHT), +50, -66) : HLS_TRANSFORM(::GetSysColor(COLOR_MENU), -27, 0)));
						DestroyIcon(hIcon);
					}
					if (!GetDisabled())
					{
						::ImageList_Draw(m_ImgDesc.m_hImgList, m_ImgDesc.m_nIndex, pDC->m_hDC, pRect->left + ((bSelected && !GetChecked()) ? 2 : 3), rc.top + ((bSelected && !GetChecked()) ? 2 : 3), ILD_TRANSPARENT);
					}
				}
				else if (GetChecked())
				{
...
void CMenuXP::OnMeasureItem(MEASUREITEMSTRUCT* pMeasureItemStruct)
{
	if (pMeasureItemStruct->CtlType == ODT_MENU)
	{
		...
		{
			pMeasureItemStruct->itemWidth = IMGWIDTH + IMGPADDING + HIWORD(pMeasureItemStruct->itemData) + TEXTPADDING
			    + HIBYTE(LOWORD(pMeasureItemStruct->itemData)) + TEXTPADDING + 4;
		}
	}
}
...
LRESULT CMenuXP::OnMenuChar(HMENU hMenu, UINT nChar, UINT nFlags)
{
	if ((nFlags & (MF_POPUP | MF_SYSMENU)) == MF_POPUP || nFlags == 0)
	{
		...
		for (int i = 0; i < nItemCount; i++)
		{
			...
			{
				if (sCaption[nPos] == '&' && (UINT)toupper(sCaption[nPos + 1]) == nChar && (nPos == 0 || sCaption[nPos - 1] != '&'))
				{
