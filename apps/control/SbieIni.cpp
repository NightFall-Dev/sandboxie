...
CString CSbieIni::GetPath(BOOL* IsHomePath)
{
	...
	SBIE_INI_GET_PATH_REQ* req = (SBIE_INI_GET_PATH_REQ*)malloc(REQUEST_LEN);
	if (req)
	{
		req->h.msgid  = MSGID_SBIE_INI_GET_PATH;
		req->h.length = sizeof(SBIE_INI_GET_PATH_REQ);

		SBIE_INI_GET_PATH_RPL* rpl = (SBIE_INI_GET_PATH_RPL*)SbieDll_CallServer(&req->h);
		if (rpl)
		{
			if (rpl->h.status == 0)
			{
...
BOOL CSbieIni::GetUser(CString& Section, CString& Name, BOOL& IsAdmin)
{
	...
	SBIE_INI_GET_USER_REQ* req = (SBIE_INI_GET_USER_REQ*)malloc(REQUEST_LEN);
	if (req)
	{
		req->h.msgid  = MSGID_SBIE_INI_GET_USER;
		req->h.length = sizeof(SBIE_INI_GET_USER_REQ);

		SBIE_INI_GET_USER_RPL* rpl = (SBIE_INI_GET_USER_RPL*)SbieDll_CallServer(&req->h);
		if (rpl)
		{
			if (rpl->h.status == 0)
			{
...
void CSbieIni::GetSettingsNames(const CString& Section, CStringList& Names)
{
	...
	while (1)
	{
		LONG rc = SbieApi_QueryConfAsIs(Section, NULL, index, buf, buf_len - 4);
		if (rc != 0)
		{
			break;
		}
...
BOOL CSbieIni::SetText(const CString& Section, const CString& Setting, const CString& Value)
{
	CString oldValue;
...
BOOL CSbieIni::SetNum(const CString& Section, const CString& Setting, int Value)
{
	CString ValueStr;
...
BOOL CSbieIni::SetNum64(const CString& Section, const CString& Setting, __int64 Value)
{
	CString ValueStr;
...
BOOL CSbieIni::SetBool(const CString& Section, const CString& Setting, BOOL Value)
{
...
void CSbieIni::GetText(const CString& Section, const CString& Setting, CString& Value, const CString& Default) const
{
	if (Section.IsEmpty())
	{
		Value = Default;
	...
	SbieApi_QueryConfAsIs(Section, Setting, flags, buf, buf_len - 4);
	if (buf[0])
	{
		Value = buf;
	}
...
void CSbieIni::GetNum(const CString& Section, const CString& Setting, int& Value, int Default) const
{
	CString ValueStr;
...
void CSbieIni::GetNum64(const CString& Section, const CString& Setting, __int64& Value, __int64 Default) const
{
	CString ValueStr;
	GetText(Section, Setting, ValueStr);
...
void CSbieIni::GetBool(const CString& Section, const CString& Setting, BOOL& Value, BOOL Default) const
{
	CString ValueStr;
	GetText(Section, Setting, ValueStr);
	WCHAR ch = 0;
...
void CSbieIni::GetTextList(const CString& Section, const CString& Setting, CStringList& Value, BOOL withBrackets) const
{
	if (Section.IsEmpty())
	{
		return;
	...
	while (1)
	{
		SbieApi_QueryConfAsIs(Section, Setting, index | flags, buf, buf_len - 4);
		if (!buf[0])
		{
			break;
		}
...
BOOL CSbieIni::InsertText(const CString& Section, const CString& Setting, const CString& Value)
{
...
BOOL CSbieIni::AppendText(const CString& Section, const CString& Setting, const CString& Value)
{
...
BOOL CSbieIni::DelText(const CString& Section, const CString& Setting, const CString& Value)
{
...
BOOL CSbieIni::DelText2(const CString& Section, const CString& Setting, const CString& Value, const CString& ProcessGroup)
{
	BOOL ok = CallSbieSvc(MSGID_SBIE_INI_DEL_SETTING, Section, Setting, Value);

	if (ok)
	{
		CString value1 = L"!" + ProcessGroup + L"," + Value;
		ok             = CallSbieSvc(MSGID_SBIE_INI_DEL_SETTING, Section, Setting, value1);
	}
...
BOOL CSbieIni::CallSbieSvc(ULONG msgid, const CString& Section, const CString& Setting, const CString& Value)
{
	if (Section.IsEmpty())
	{
		return TRUE;
	...
	if (req)
	{
		...
		ok = CallServerWithPassword(req, req->password, Section, Setting);
...
{
	CString keyPath(L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\");
	if (WithUser)
	{
		keyPath += L"User ";
	}
...
CString CSbieIni::MakeGenericPath(const CString& path)
{
	...
	while (pos)
	{
		...
		if (path.GetLength() >= val.GetLength() && path.Mid(0, val.GetLength()).CompareNoCase(val) == 0)
		{
...
CString CSbieIni::MakeSpecificPath(const CString& path)
{
	...
	while (pos)
	{
		...
		if (path.GetLength() >= key.GetLength() && path.Mid(0, key.GetLength()).CompareNoCase(key) == 0)
		{
			CString retval = val;
...
BOOL CSbieIni::RenameOrCopySection(const CString& OldName, const CString& NewName, bool deleteOldName)
{
	...
	while (1)
	{
		status = SbieApi_QueryConfAsIs(OldName, NULL, setting_index | CONF_GET_NO_TEMPLS, buf, buf_len - 4);
		...
		while (1)
		{
			status = SbieApi_QueryConfAsIs(OldName, setting_name, value_index | CONF_GET_NO_GLOBAL | CONF_GET_NO_TEMPLS, buf, buf_len - 4);
			if (status == STATUS_RESOURCE_NAME_NOT_FOUND)
			{
				status = STATUS_SUCCESS;
				break;
...
BOOL CSbieIni::SetTemplateVariable(const CString& Name, const CString& Value, BOOL UserSpecific)
{
	BOOL ok = FALSE;

	SBIE_INI_TEMPLATE_REQ* req = (SBIE_INI_TEMPLATE_REQ*)malloc(REQUEST_LEN);
	if (req)
	{
		...
		req->h.length = sizeof(SBIE_INI_TEMPLATE_REQ) + req->value_len * sizeof(WCHAR);

		ok = CallServerWithPassword(req, req->password, m_TemplateSettings, Name);
...
void CSbieIni::GetTemplateClasses(CStringList& list)
{
	...
	while (1)
	{
		...
		CString section(buf);
		SbieApi_QueryConfAsIs(section, L"Tmpl.Class", CONF_GET_NO_GLOBAL, buf, buf_len - 4);
		if (!buf[0])
		{
			break;
		}
...
CString CSbieIni::GetTemplateMetaData(const CString& Template, const CString& Setting)
{
	CString TemplateSectionName = m_Template_ + Template;
...
NOINLINE void CSbieIni::GetTextListWithBrackets(const CString& Section, const CString& Setting, CStringList& Value, WCHAR* buf, ULONG buf_len) const
{
	int index = 0;
	while (1)
	{
		SbieApi_QueryConfAsIs(Section, m_Template, index | CONF_GET_NO_TEMPLS, buf, buf_len - 4);
		if (!buf[0])
		{
			break;
		}
		...
		while (1)
		{
			SbieApi_QueryConfAsIs(tmpl_name, Setting, tmpl_index | CONF_GET_NO_GLOBAL, buf, buf_len - 4);
			if (!buf[0])
			{
				break;
			}
...
BOOL CSbieIni::CallServerWithPassword(void* RequestBuf, WCHAR* pPasswordWithinRequestBuf, const CString& SectionName, const CString& SettingName)
{
	if (m_ServiceStopped)
	{
		return FALSE;
...
BOOL CSbieIni::SetOrTestPassword(WCHAR* NewPassword66)
{
	...
	if (req)
	{
		...
		ok = CallServerWithPassword(req, req->old_password, _GlobalSettings, L"*");
	}
...
void CSbieIni::GetRestrictions(BOOL& EditAdminOnly, BOOL& DisableForceAdminOnly, BOOL& NeedPassword, BOOL& HavePassword, BOOL& ForgetPassword)
{
	GetBool(_GlobalSettings, L"EditAdminOnly", EditAdminOnly, FALSE);

	GetBool(_GlobalSettings, L"ForceDisableAdminOnly", DisableForceAdminOnly, FALSE);
...
void CSbieIni::SetRestrictions(BOOL EditAdminOnly, BOOL DisableForceAdminOnly, BOOL ForgetPassword)
{
	SetBool(_GlobalSettings, L"EditAdminOnly", EditAdminOnly);

	SetBool(_GlobalSettings, L"ForceDisableAdminOnly", DisableForceAdminOnly);

	SetBool(_GlobalSettings, L"ForgetPassword", ForgetPassword);
}
...
{
	typedef ULONG (*P_CredUIPromptForCredentials)(void* pUiInfo, const WCHAR* pszTargetName, void* pContext, ULONG dwAuthError, WCHAR* pszUserName, ULONG ulUserNameBufferSize, WCHAR* pszPassword, ULONG ulPasswordBufferSize, BOOL* save, ULONG dwFlags);
	...
	if (!pCredUIPromptForCredentials)
	{
		...
		if (credui)
		{
			pCredUIPromptForCredentials = (P_CredUIPromptForCredentials)GetProcAddress(credui, "CredUIPromptForCredentialsW");
		}
	}
	if (!pCredUIPromptForCredentials)
	{
	...
	ULONG rc = pCredUIPromptForCredentials(&info, SANDBOXIE, NULL, 0, UserName, 64 + 1, Password66, 64 + 1, &save, CREDUI_FLAGS_DO_NOT_PERSIST | CREDUI_FLAGS_GENERIC_CREDENTIALS);
...
ULONG CSbieIni::MyPromptForCredentials(void* pUiInfo, const WCHAR* pszTargetName, void* pContext, ULONG dwAuthError, WCHAR* pszUserName, ULONG ulUserNameBufferSize, WCHAR* pszPassword, ULONG ulPasswordBufferSize, BOOL* save, ULONG dwFlags)
{
	...
	WCHAR* edit = ::InputBox(info->hwndParent, info->pszMessageText, flags, NULL);
	if (!edit)
	{
...
void CSbieIni::MonitorService(CWnd* pWnd, ULONG idExitCmd)
{
	...
	SBIE_INI_GET_WAIT_HANDLE_RPL* rpl = (SBIE_INI_GET_WAIT_HANDLE_RPL*)SbieDll_CallServer(&req.h);
	if (rpl)
	{
		if (rpl->h.status == 0)
		{
			...
			CreateThread(NULL, 0, MonitorServiceThread, ThreadArgs, 0, &ThreadId);
		}
		SbieDll_FreeMem(rpl);
	}
}
