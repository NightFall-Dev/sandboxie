    ...
    CBaseDialog(pParentWnd, L"PROC_SETTINGS_DIALOG")
{
	...
	m_process         = boxproc.GetProcessImageName(boxproc.GetIndexForProcessId(pid));
...
void CProcSettingsDialog::UpdateAccess(const CString& list, ULONG idList, ULONG idButton, ULONG msgidAll, ULONG msgidSome)
{
...
{
	UpdateAccess(m_internet_list, ID_SETTINGS_INTERNET_LIST, ID_SETTINGS_INTERNET, MSG_3762, MSG_3763);
}
...
{
	UpdateAccess(m_startrun_list, ID_SETTINGS_STARTRUN_LIST, ID_SETTINGS_STARTRUN, MSG_3766, MSG_3767);
}
...
void CProcSettingsDialog::OnCmdForce()
{
	...
	pButton = (CButton*)GetDlgItem(ID_SETTINGS_FORCE);
	if (pButton->GetCheck() == BST_CHECKED && m_process.CompareNoCase(_ExplorerExe) == 0)
	{
