...
static const CString _NotifyInternetAccessDenied(L"NotifyInternetAccessDenied");
static const CString _NotifyStartRunAccessDenied(L"NotifyStartRunAccessDenied");
...
void CBox::SetDefaultSettings()
{
	...
	{
		ok = AddOrRemoveQuickRecoveryFolder(L"%{374DE290-123F-4565-9164-39C4925E467B}%", TRUE);
	}
...
void CBox::GetProcessGroupName(const CString& GroupName, CString& LocalGroupName, CString& GlobalGroupName)
{
...
CString CBox::GetProcessGroup(const CString& GroupName)
{
	...
	while (pos)
	{
		...
		if (comma != -1 && (text.Mid(0, comma) == LocalGroupName || text.Mid(0, comma) == GlobalGroupName))
		{
			//
...
CString CBox::GetInternetProcess(BOOL* pNotify)
{
	...
	while (pos)
	{
		CString& text = list.GetNext(pos);
		int comma     = text.Find(L',');
		if (comma == -1 && text == _AfdX || text == _InternetAccessDevices)
		{
			return _Star;
		}
	}
	...
	CString LocalGroupName, GlobalGroupName;
	GetProcessGroupName(_InternetAccessGroup, LocalGroupName, GlobalGroupName);
	...
	while (pos)
	{
		CString& text = list.GetNext(pos);
		int comma     = text.Find(L',');
		if (comma != -1 && (text.Mid(comma + 1) == _AfdX || text.Mid(comma + 1) == _InternetAccessDevices))
		{
...
BOOL CBox::SetInternetProcess(const CString& image, BOOL* pNotify)
{
	...
	BOOL ok = ini.DelText2(m_name, _ClosedFilePath, _InternetAccessDevices, LocalGroupName);
	...
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _AfdX, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _TcpX, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _IpX, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _Ip, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _Tcp, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _Udp, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _RawIp, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _Ip6, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _Tcp6, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _Udp6, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _RawIp6, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _Nsi, GlobalGroupName);
	}
	if (ok)
	{
		ok = ini.DelText2(m_name, _ClosedFilePath, _Http, GlobalGroupName);
	}
	...
	if (!image.IsEmpty())
	{
		...
		if (ok)
		{
			ok = ini.InsertText(m_name, _ClosedFilePath, pfx + _InternetAccessDevices);
		}
	}
...
CString CBox::GetStartRunProcess(BOOL* pNotify)
{
	...
	CString LocalGroupName, GlobalGroupName;
	GetProcessGroupName(_StartRunAccessGroup, LocalGroupName, GlobalGroupName);
...
void CBox::GetXxxPathListForPgm(ULONG type, CStringList& list, const CString& pgm, BOOL withBrackets)
{
	CStringList list1;
...
void CBox::SetXxxPathListForPgm(ULONG type, const CStringList& list, const CString& pgm)
{
	CSbieIni& ini = CSbieIni::GetInstance();
...
void CBox::GetSetUserAccounts(const CString& BoxName, BOOL set, CStringList& list)
{
	CSbieIni& ini = CSbieIni::GetInstance();
	if (set)
	{
	...
	else
	{
		...
		if (text.GetLength() && (text.GetAt(0) == L'y' || text.GetAt(0) == L'Y'))
		{
