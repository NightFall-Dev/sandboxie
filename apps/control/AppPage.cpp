...
#define MODIF_BROWSER (MODIF_IEXPLORE | MODIF_FIREFOX | MODIF_CHROME | MODIF_OTHERWB)
...
void CAppPage::Access_OnInitDialog(CBox& box)
{
	...
	GetDlgItem(ID_APP_SCREEN_READER)->SetWindowText(CTemplateListBox::GetTemplateTitle(L"ScreenReader"));
...
void CAppPage::Template_OnLink()
{
	...
	if (index != LB_ERR)
	{
		...
		{
			const CString& section = m_tmpl_sections.GetAt(pos);
			CString url            = CSbieIni::GetInstance().GetTemplateMetaData(section, L"Url");
			if (!url.IsEmpty())
			{
...
void CAppPage::Template_Filter()
{
	...
	while (pos)
	{
		...
		BOOL ff = (name.Find(L"Firefox_") != -1) || (name.Find(L"Waterfox_") != -1) || (name.Find(L"PaleMoon_") != -1);
		BOOL ch = (name.Find(L"Chrome_") != -1) || (name.Find(L"Iron_") != -1) || (name.Find(L"Dragon_") != -1) || (name.Find(L"Vivaldi_") != -1) || (name.Find(L"Neon_") != -1);
...
void CAppPage::AddPages(CPropertySheet& sheet, const CString& BoxName)
{
	...
	m_app_pages.AddTail(new CAppPage(AppPageFolders, BoxName, MSG_4209));

	m_app_pages.AddTail(new CAppPage(AppPageAccess, BoxName, MSG_4302));
...
void CAppPage::UpdateWebTemplates(CBox& box)
{
	...
	UpdateTemplates2(box, L"firefox.exe", L"Firefox_Bookmarks_DirectAccess", L"%AppData%\\Mozilla\\Firefox\\Profiles\\*\\bookmark*", L"%AppData%\\Mozilla\\Firefox\\Profiles\\*\\places*");
	...
	const CString& SeaMonkeyPath1(L"%AppData%\\Mozilla\\Profiles\\*\\bookmark*");
	const CString& SeaMonkeyPath2(L"%AppData%\\Mozilla\\Profiles\\*\\places*");

	UpdateTemplates2(box, L"seamonkey.exe", SeaMonkeyTemplate, SeaMonkeyPath1, SeaMonkeyPath2);

	UpdateTemplates2(box, L"seamon~1.exe", SeaMonkeyTemplate, SeaMonkeyPath1, SeaMonkeyPath2);
	...
	UpdateTemplates2(box, L"opera.exe", L"Opera_Bookmarks_DirectAccess", L"%AppData%\\Opera\\Opera\\Profile\\opera6.adr", L"%AppData%\\Opera\\Opera\\Profile\\notes.adr");
}
...
void CAppPage::UpdateEmailTemplates(CBox& box)
{
	...
	_OutlookExpressKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Outlook Express");
	_OutlookExpressKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Internet Account Manager");
	_OutlookExpressKeys.AddTail(L"HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Outlook Express");
	_OutlookExpressKeys.AddTail(L"HKEY_LOCAL_MACHINE\\Software\\Wow6432Node\\"
	                            L"Microsoft\\Outlook Express");

	UpdateTemplates3(box, L"msimn.exe", L"Outlook_Express", _OutlookExpressFiles, _OutlookExpressKeys);
	...
	_OfficeOutlookKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Office");
	_OfficeOutlookKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows NT\\"
	                           L"CurrentVersion\\Windows Messaging Subsystem\\Profiles\\Outlook");

	UpdateTemplates3(box, L"outlook.exe", L"Office_Outlook", _OfficeOutlookFiles, _OfficeOutlookKeys);
	...
	_WindowsMailKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows Mail");

	UpdateTemplates3(box, L"winmail.exe", L"Windows_Vista_Mail", _WindowsMailFiles, _WindowsMailKeys);
	...
	_LiveMailKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows Live");
	_LiveMailKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows Live Mail");

	UpdateTemplates3(box, L"wlmail.exe", L"Windows_Live_Mail", _LiveMailFiles, _LiveMailKeys);
	...
	_ThunderbirdKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Mozilla Thunderbird");
	_ThunderbirdKeys.AddTail(L"HKEY_LOCAL_MACHINE\\Software\\Mozilla Thunderbird");
	_ThunderbirdKeys.AddTail(L"HKEY_LOCAL_MACHINE\\Software\\Wow6432Node\\Mozilla Thunderbird");

	CString _Thunderbird(L"Thunderbird");

	UpdateTemplates3(box, L"thunderbird.exe", _Thunderbird, _ThunderbirdFiles, _ThunderbirdKeys);

	UpdateTemplates3(box, L"thunde~1.exe", _Thunderbird, _ThunderbirdFiles, _ThunderbirdKeys);
	...
	_SeaMonkeyKeys.AddTail(L"HKEY_CURRENT_USER\\Software\\Mozilla*\\SeaMonkey*");
	_SeaMonkeyKeys.AddTail(L"HKEY_LOCAL_MACHINE\\Software\\Mozilla\\SeaMonkey*");
	_SeaMonkeyKeys.AddTail(L"HKEY_LOCAL_MACHINE\\Software\\Wow6432Node\\Mozilla\\SeaMonkey*");

	CString _SeaMonkey(L"SeaMonkey");

	UpdateTemplates3(box, L"seamonkey.exe", _SeaMonkey, _SeaMonkeyFiles, _SeaMonkeyKeys);
	...
	UpdateTemplates2(box, L"opera.exe", L"Opera_Mail", _OperaPath1, _OperaPath1);
}
...
void CAppPage::UpdateTemplates2(CBox& box, const CString& image_name, const CString& tmpl_name, const CString& path1, const CString& path2)
{
	BOOL found = FALSE;
...
void CAppPage::UpdateTemplates3(CBox& box, const CString& image_name, const CString& tmpl_name, CStringList& files, CStringList& keys)
{
	CStringList list;
	box.GetXxxPathListForPgm('fo', list, image_name, TRUE);
	...
	while (!keys.IsEmpty())
	{
		...
		while (pos)
		{
			POSITION pos0 = pos;
			CString& item = list.GetNext(pos);
			if (item.IsEmpty() || item.GetAt(0) == L'[' || item.CompareNoCase(curItem) == 0)
			{
				list.RemoveAt(pos0);
				pos = list.GetHeadPosition();
				continue;
