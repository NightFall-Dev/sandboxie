...
CInitWait::CInitWait(CWinApp* myApp)
{
	...
	CreateEx(0, (LPCTSTR)CMyApp::m_atom, CMyApp::m_appTitle, WS_OVERLAPPEDWINDOW | WS_CAPTION | WS_SYSMENU, 0, 0, 0, 0, NULL, NULL, NULL);
...
void CInitWait::GetVersions()
{
	...
	if (m_svc_ver.GetAt(0) == L'?')
	{
		...
		SBIE_INI_GET_VERSION_RPL* rpl = (SBIE_INI_GET_VERSION_RPL*)SbieDll_CallServer(&req.h);
		if (rpl)
		{
