...
void CMessageDialog::ReloadConf()
{
	...
	while (!list.IsEmpty())
	{
		...
		else if (head.Mid(0, 4).CompareNoCase(SBIE_IN_MSGS) == 0 || head.Mid(0, 4).CompareNoCase(L"SBIE") == 0
...
BOOL CMessageDialog::IsHiddenMessage(ULONG code, const WCHAR* detail_1, const WCHAR* detail_2)
{
	code = code & 0xFFFF;
	...
	if (code == 1406 && 0 == wcscmp(detail_2, L"{374DE290-123F-4565-9164-39C4925E467B}"))
	{
		return TRUE;
	}

	if (code == 1412 && wcsstr(detail_1, L"RecoverFolder=%{374DE290-123F-4565-9164-39C4925E467B}%"))
	{
		return TRUE;
	}
	...
	while (i > 0)
	{
		...
		if (entry->code == code)
		{
			...
			BOOL match = (!entry->detail.GetLength()) || (entry->detail.CompareNoCase(detail_1) == 0);
			if (match)
			{
				return TRUE;
			}
...
void CMessageDialog::OnTimer()
{
	...
	while (1)
	{
		...
		len           = sizeof(MsgEntry) + (str1_len + 4 + str2_len + 4) * sizeof(WCHAR);
		...
		if (msg)
		{
			...
			if (msg->code == MSG_1307 || msg->code == MSG_1308)
			{
				...
				if (!image_copy)
				{
					ULONG new_code = (msg->code == MSG_1307) ? MSG_2221 : MSG_2222;
					MsgEntry* msg  = (MsgEntry*)new UCHAR[len];
					msg->code      = new_code;
					wmemcpy(msg->str1, str1, str1_len + 1);
					msg->str2 = msg->str1 + str1_len + 1;
...
void CMessageDialog::MyDoModal(CDialog* pDialog)
{
	...
	{
		pMainWnd->SetWindowPos(&wndBottom, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
	}
}
...
CString CMessageDialog::GetSBIExxxx(ULONG* pCode, CString* pDetail)
{
	...
	if (index != LB_ERR)
	{
		...
		if (pos != -1)
		{
			...
			if (pDetail)
			{
				CString* p_detail = (CString*)m_listbox->GetItemDataPtr(index);
				*pDetail          = *p_detail;
			}
		}
	}
...
void CMessageDialog::DiscardMessages(const CString& sbie, const CString& detail)
{
	while (1)
	{
		int index;
		...
		for (index = 0; index < size; ++index)
		{
			...
			if (text.Find(sbie) != -1)
			{
				...
				BOOL match = (detail.IsEmpty() || p_detail->IsEmpty()) || (p_detail->CompareNoCase(detail) == 0);
				if (match)
				{
...
void CMessageDialog::OnHide()
{
	...
	CString text;
	text.Format(L"%04d%c%s", code, (detail.IsEmpty() ? L'\0' : L','), detail);
	...
	if (code == 2221 || code == 2222)
	{
		...
		if (pos1 != -1)
		{
			...
			if (pos2 != -1)
			{
				...
				CMyMsg msg((code == 2221 ? MSG_3920 : MSG_3930), boxname, pgmname);
				int rc = CMyApp::MsgBox(this, msg, MB_OKCANCEL);
				if (rc != IDOK)
				{
					return;
			...
			if (pos2 != -1)
			{
				...
				if (pos3 != -1)
				{
					...
					if (!box.GetName().IsEmpty())
					{
						...
						CMyMsg msg(MSG_3909, boxname, limitOldTxt, limitNewTxt);
						int rc = CMyApp::MsgBox(this, msg, MB_OKCANCEL);
						if (rc != IDOK)
						{
							return;
...
void CMessageDialog::DoRecovery()
{
	...
	if (!m_qr)
	{
		...
		*space = L'\0';
		...
		{
			m_GettingStartedWindow->PostMessage(WM_COMMAND, ID_SANDBOX_MENU | ID_SANDBOX_RECOVER);
		}
...
void CMessageDialog::ReadEventLog()
{
	...
	while (hEventLog)
	{
		...
		BOOL ok = ::ReadEventLog(hEventLog, EVENTLOG_SEQUENTIAL_READ | EVENTLOG_FORWARDS_READ, 0, m_buf, m_buf_len, &bytesRead, &bytesNeeded);
		...
		while (bytesRead > 0)
		{
			...
			{
				ULONG len     = sizeof(MsgEntry) + rec->Length;
				MsgEntry* msg = (MsgEntry*)new UCHAR[len];
				if (msg)
				{
