...
public:
	...
	static void RunStartExe(const CString& cmd, const CString& box, BOOL wait = FALSE, BOOL inherit = FALSE);

	static int MsgBox(CWnd* pWnd, const WCHAR* text, UINT flags);

	static int MsgBox(CWnd* pWnd, ULONG textid, UINT flags);

	static int MsgCheckBox(CWnd* pWnd, const CString& text, ULONG checkid, UINT flags);

	static int MsgCheckBox(CWnd* pWnd, ULONG textid, ULONG checkid, UINT flags);

	static CString InputBox(CWnd* pWnd, ULONG textid);
	static void InputBox(CWnd* pWnd, ULONG textid, CString& str);

	static void GetProgramFileName(CWnd* pParentWnd, CString& pgm1, CString& pgm2);
