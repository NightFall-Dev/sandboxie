...
ON_COMMAND_RANGE(ID_SANDBOX_MENU, ID_SANDBOX_MENU_LAST_ID, OnCmdSandbox)
...
CMyFrame::CMyFrame(BOOL ForceVisible, BOOL ForceSync)
{
	...
	CreateEx(exStyle, (LPCTSTR)CMyApp::m_atom, CMyApp::m_appTitle, WS_OVERLAPPEDWINDOW | WS_CAPTION | WS_SYSMENU, left, top, width, height, NULL, NULL, NULL);
...
bool CMyFrame::AdjustSizePosition(int& left, int& top, int& width, int& height)
{
...
void CMyFrame::OnCmdThirdParty()
{
	...
	if (GetCurrentMessage()->message == WM_COMMAND && GetCurrentMessage()->wParam == ID_CONF_THIRD_PARTY_ALERT)
	{
...
void CMyFrame::OnCmdSandbox(UINT nID)
{
	...
	switch (command)
	{
	case ID_SANDBOX_RUN_BROWSER: CMyApp::RunStartExe(L"default_browser", box.GetName()); break;

	case ID_SANDBOX_RUN_MAILER: CMyApp::RunStartExe(L"mail_agent", box.GetName()); break;

	case ID_SANDBOX_RUN_ANY: CMyApp::RunStartExe(L"run_dialog", box.GetName()); break;

	case ID_SANDBOX_RUN_MENU: CMyApp::RunStartExe(L"start_menu", box.GetName()); break;

	case ID_SANDBOX_RUN_EXPLORER: RunExplorer(box, TRUE); break;

	case ID_SANDBOX_TERMINATE: TerminateProcesses(box, TRUE); break;
	...
	case ID_SANDBOX_DELETE:
		...
		{
			BOOL spawned;
			BOOL WithDelete = (command == ID_SANDBOX_DELETE) ? TRUE : FALSE;
			CDeleteDialog dlg(this, box.GetName(), WithDelete, FALSE, &spawned);
			if (spawned)
			{
	...
	case ID_SANDBOX_EXPLORE: RunExplorer(box, FALSE); break;

	case ID_SANDBOX_SETTINGS:
		if (!CBaseDialog::InModalState())
		{
			CBoxPage::DoPropertySheet(this, box.GetName(), &ReloadBoxes, CString());
		}
		break;

	case ID_SANDBOX_RENAME: RemoveOrRename(box, FALSE, TRUE, &ReloadBoxes); break;

	case ID_SANDBOX_REMOVE: RemoveOrRename(box, TRUE, FALSE, &ReloadBoxes); break;
	}
...
void CMyFrame::RunExplorer(CBox& box, BOOL sandboxed)
{
	...
	if (tip)
	{
		int rv = CMyApp::MsgCheckBox(this, (sandboxed ? MSG_3533 : MSG_3534), 0, MB_OK);
		if (rv < 0)
		{
			rv = -rv;
	...
	if (sandboxed)
	{
		...
		CMyApp::RunStartExe(L"explorer.exe /e,::{20D04FE0-3AEA-1069-A2D8-08002B30309D}", box.GetName(), FALSE);
	}
	else
	{
		//
		...
		if (!BoxPath.IsEmpty())
		{
			const ULONG ShareFlags = (FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE);

			HANDLE hFile = INVALID_HANDLE_VALUE;

			while (1)
			{
				hFile = CreateFile(BoxPath, FILE_GENERIC_READ, ShareFlags, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
		...
		if (!BoxPath.IsEmpty())
		{
			int x = (UINT)(ULONG_PTR)ShellExecute(m_hWnd, NULL, BoxPath, NULL, NULL, SW_SHOWNORMAL);
			if (x <= 32)
			{
				BoxPath = CString();
			}
...
void CMyFrame::RemoveOrRename(CBox& box, BOOL remove, BOOL rename, bool* ReloadBoxes)
{
	...
	else
	{
		...
		if (rename)
		{
			...
			CSbieIni::GetInstance().RenameOrCopySection(box.GetName(), newName, true);
		}
		else if (remove)
		{
...
void CMyFrame::InitSandboxMenu(CMenu* pMenu, int posChild, int posInsert)
{
	...
	if (box_order)
	{
		...
		{
			CMenu* model = (posChild == 0) ? m_pTraySandboxMenu : m_SandboxMenu;
			InitSandboxMenu1(box_order->children, model, sandbox, posInsert);
		}
...
void CMyFrame::InitSandboxMenu1(void* _box_order, CMenu* ModelMenu, CMenu* SandboxMenu, int posInsert)
{
	...
	while (order_entry)
	{
		...
		if (order_entry->children)
		{
			...
			InitSandboxMenu1(order_entry->children, ModelMenu, &ChildMenu, 0);
		...
		else
		{
			...
			if (i)
			{
				...
				InitSandboxMenu2(ModelMenu, &ChildMenu, i * ID_SANDBOX_MENU_SIZE + ID_SANDBOX_MENU);
			}
		}
		...
		SandboxMenu->InsertMenu(posInsert, MF_BYPOSITION | MF_STRING | MF_POPUP, (INT_PTR)ChildMenu.GetSafeHmenu(), title);
		++posInsert;
...
void CMyFrame::InitSandboxMenu2(CMenu* model, CMenu* child, UINT BaseId)
{
	...
	for (int j = 0; j < model->GetMenuItemCount(); ++j)
	{
		...
		if (id == -1)
		{
			...
			child->AppendMenu(MF_STRING | MF_POPUP, (INT_PTR)child2.GetSafeHmenu(), title);
...
void CMyFrame::OnContextMenu(CWnd* pWnd, CPoint pt)
{
	...
	if (pBox)
	{
		...
		CMenu child;
		InitSandboxMenu2(m_SandboxMenu, &child, index * ID_SANDBOX_MENU_SIZE + ID_SANDBOX_MENU);
...
LRESULT CMyFrame::OnTrayIcon(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_GettingStartedWindow && (msg == WM_LBUTTONDOWN || msg == WM_LBUTTONUP || msg == WM_LBUTTONDBLCLK))
	{
		m_GettingStartedWindow->PostMessage(WM_COMMAND, ID_SANDBOX_MENU | ID_SANDBOX_EXPLORE);
	...
	if (msg == WM_RBUTTONUP)
	{
		...
		HMONITOR hMonitor = MonitorFromPoint(m_TrayPoint, MONITOR_DEFAULTTONULL);
		if (hMonitor)
		{
			MONITORINFO mi;
		...
		{
			m_GettingStartedWindow->PostMessage(WM_COMMAND, ID_SANDBOX_MENU | ID_SANDBOX_TERMINATE);
		}

		SetForegroundWindow();

		UINT nID =
		    (UINT)pMenu->TrackPopupMenu(TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, m_TrayPoint.x, m_TrayPoint.y, this, NULL);
		...
		if (m_GettingStartedWindow && nID != ID_EXIT)
		{
			...
			m_GettingStartedWindow->PostMessage(WM_COMMAND, ID_SANDBOX_MENU | msg);
		}
		else if (nID)
		{
...
void CMyFrame::RefreshTrayIcon(BOOL ForceDeleteIcon)
{
	...
	CString EventName;
	EventName.Format(SANDBOXIE L"_Delete_Sandbox_Session_%d", CMyApp::m_session_id);
...
{
	if ((nType == SIZE_RESTORED && IsWindowVisible()) || nType == SIZE_MAXIMIZED || nType == SIZE_MINIMIZED)
	{
		m_hidden = FALSE;
	}
	...
	if (pMyListCtrl)
	{
		...
		CRect rc;
		GetClientRect(&rc);
		pMyListCtrl->SetWindowPos(NULL, rc.left, rc.top, rc.right, rc.bottom, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER);
