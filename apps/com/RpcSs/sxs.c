...
static void Sxs_Request(UCHAR* data, ULONG data_len, UCHAR** rsp_data, ULONG* rsp_len);

static void Sxs_Generate(UCHAR* buf, HANDLE* hSection);

static IUnknown* Sxs_CreateStreamFromBuffer(UCHAR* Text);

static void Sxs_GenerateHelper(SXS_GENERATE_ACTIVATION_CONTEXT_ARGS* args, WCHAR* LangNames);


//---------------------------------------------------------------------------


typedef HRESULT (*P_CreateStreamOnHGlobal)(HGLOBAL hGlobal, BOOL fDeleteOnRelease, void** ppStream);

typedef ULONG (*P_SxsGenerateActivationContext)(SXS_GENERATE_ACTIVATION_CONTEXT_ARGS* args);
...
_FX void Sxs_Error(ULONG level, ULONG status)
{
	...
	SbieApi_Log(2203, L"%S - (%S) [%02X/%08X]", Sxs_QueueName, ServiceTitle, level, status);
}
...
_FX BOOLEAN Sxs_Init(void)
{
	...
	if (mod)
	{
		pSxsGenerateActivationContext = (P_SxsGenerateActivationContext)GetProcAddress(mod, "SxsGenerateActivationContext");
	}
	else
	{
	...
	if (mod)
	{
		pCreateStreamOnHGlobal = (P_CreateStreamOnHGlobal)GetProcAddress(mod, "CreateStreamOnHGlobal");
	}
	else
	{
...
_FX ULONG Sxs_Thread(void* arg)
{
	...
	while (1)
	{
		...
		while (1)
		{
			status = SbieDll_QueueGetReq(Sxs_QueueName, NULL, NULL, &req_id, &data_ptr, &data_len);
			...
			status = SbieDll_QueuePutRpl(Sxs_QueueName, req_id, rsp_data, rsp_len);
...
_FX void Sxs_Request(UCHAR* data, ULONG data_len, UCHAR** rsp_data, ULONG* rsp_len)
{
	...
	else if (hSection)
	{
		...
		status = NtMapViewOfSection(hSection, NtCurrentProcess(), &MappedBase, 0, 0, &ZeroOffset, &ViewSize, xViewShare, MEM_PHYSICAL, PAGE_READONLY);
...
_FX void Sxs_GenerateHelper(SXS_GENERATE_ACTIVATION_CONTEXT_ARGS* args, WCHAR* LangNames)
{
