...
_FX SOCKET my_WSASocketW(int af, int type, int protocol, LPWSAPROTOCOL_INFO lpProtocolInfo, GROUP g, DWORD dwFlags)
{
...
_FX BOOL my_OpenThreadToken(HANDLE ThreadHandle, DWORD DesiredAccess, BOOL OpenAsSelf, PHANDLE TokenHandle)
{
	typedef BOOL (*P_OpenThreadToken)(HANDLE ThreadHandle, DWORD DesiredAccess, BOOL OpenAsSelf, PHANDLE TokenHandle);

	BOOL ok = ((P_OpenThreadToken)__sys_OpenThreadToken)(ThreadHandle, DesiredAccess, OpenAsSelf, TokenHandle);

	if ((!ok) && GetLastError() == ERROR_BAD_IMPERSONATION_LEVEL && (!OpenAsSelf))
	{
		OpenAsSelf = TRUE;

		ok = ((P_OpenThreadToken)__sys_OpenThreadToken)(ThreadHandle, DesiredAccess, OpenAsSelf, TokenHandle);
	}
...
_FX LONG my_RegOpenKeyExW(HKEY hKey, LPCWSTR lpSubKey, DWORD ulOptions, REGSAM samDesired, PHKEY phkResult)
{
	typedef BOOL (*P_RegOpenKeyExW)(HKEY hKey, LPCWSTR lpSubKey, DWORD ulOptions, REGSAM samDesired, PHKEY phkResult);
	typedef BOOL (*P_SetThreadToken)(PHANDLE Thread, HANDLE Token);

	LONG rc = ((P_RegOpenKeyExW)__sys_RegOpenKeyExW)(hKey, lpSubKey, ulOptions, samDesired, phkResult);
	...
	if (rc == ERROR_BAD_IMPERSONATION_LEVEL)
	{
		...
		HANDLE Token;
		BOOL ok = OpenThreadToken(NtCurrentThread(), TOKEN_IMPERSONATE, TRUE, &Token);
		if (ok && Token)
		{
			((P_SetThreadToken)__sys_SetThreadToken)(NULL, NULL);
			rc = ((P_RegOpenKeyExW)__sys_RegOpenKeyExW)(hKey, lpSubKey, ulOptions, samDesired, phkResult);
...
_FX LONG my_RegQueryValueExW(HKEY hKey, LPCWSTR lpValueName, LPDWORD lpReserved, LPDWORD lpType, LPBYTE lpData, LPDWORD lpcbData)
{
	typedef BOOL (*P_RegQueryValueExW)(HKEY hKey, LPCWSTR lpValueName, LPDWORD lpReserved, LPDWORD lpType, LPBYTE lpData, LPDWORD lpcbData);
	typedef BOOL (*P_SetThreadToken)(PHANDLE Thread, HANDLE Token);

	LONG rc = ((P_RegQueryValueExW)__sys_RegQueryValueExW)(hKey, lpValueName, lpReserved, lpType, lpData, lpcbData);
	...
	if (rc == ERROR_BAD_IMPERSONATION_LEVEL)
	{
		...
		{
			((P_SetThreadToken)__sys_SetThreadToken)(NULL, NULL);
			rc = ((P_RegQueryValueExW)__sys_RegQueryValueExW)(hKey, lpValueName, lpReserved, lpType, lpData, lpcbData);
			((P_SetThreadToken)__sys_SetThreadToken)(NULL, Token);
		}
...
BOOL StartRpcEptMapper(void)
{
	...
	ok = Service_Start_ServiceMain(ServiceName2, L"rpcepmap.dll", "ServiceMain", TRUE);
	if (!ok)
	{
		return FALSE;
	}
...
_FX int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	{
		ULONG idThread;
		HANDLE hThread = CreateThread(NULL, 0, Sxs_Thread, 0, CREATE_SUSPENDED, &idThread);
		if (hThread)
		{
	...
	if (!SbieDll_IsOpenCOM())
	{
		...
		ComPlusCOMRegTable = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE | SEC_COMMIT, 0, (PAGE_SIZE * 2), ComPlusCOMRegTable_Name);
	...
	{
		ULONG idThread;
		HANDLE hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Dde_Thread, 0, CREATE_SUSPENDED, &idThread);
		if (hThread)
		{
			SetThreadPriority(hThread, THREAD_PRIORITY_ABOVE_NORMAL);
			ResumeThread(hThread);
