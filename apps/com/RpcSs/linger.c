...
_FX void Add_LL_Entry(LINGER_LEADER** lingers, ULONG* inout_count, const WCHAR* image)
{
	...
	lingers[linger_count] = HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, sizeof(LINGER_LEADER) + (wcslen(image) + 4) * sizeof(WCHAR));
...
int DoLingerLeader(void)
{
	...
	wnd_pids = HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, sizeof(ULONG) * 512);
	...
	if (1)
	{
		...
		lingers = (LINGER_LEADER**)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, sizeof(LINGER_LEADER*) * MAX_LINGER_LEADER_COUNT);

		while (1)
		{
			rc = SbieApi_QueryConfAsIs(NULL, L"LingerProcess", linger_count, image, sizeof(WCHAR) * 120);
			if (rc != 0)
			{
				break;
			}
		...
		leaders = HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, sizeof(LINGER_LEADER*) * MAX_LINGER_LEADER_COUNT);

		while (1)
		{
			rc = SbieApi_QueryConfAsIs(NULL, L"LeaderProcess", leader_count, image, sizeof(WCHAR) * 120);
			if (rc != 0)
			{
				break;
			}
	...
	if (1)
	{
		...
		for (i = 0;; ++i)
		{
			rc = SbieApi_QueryConfAsIs(NULL, L"StartService", i, image, sizeof(WCHAR) * 120);
			if (rc != 0)
			{
				break;
			}
		...
		for (i = 0;; ++i)
		{
			rc = SbieApi_QueryConfAsIs(NULL, L"StartProgram", i, image, sizeof(WCHAR) * 120);
			if (rc != 0)
			{
				break;
			}
	...
	while (1)
	{
		...
		for (i = 1; i <= pids[0]; ++i)
		{
			...
			if (0 == SbieApi_QueryProcess(pids_i, NULL, image, NULL, NULL) && _wcsicmp(image, _SandboxieCrypto) != 0)
			{
				//
				...
				if (!is_local_system_sid)
				{
					...
					ULONG64 ProcessFlags = SbieApi_QueryProcessInfo(pids_i, 0);

					if (ProcessFlags & (SBIE_FLAG_FORCED_PROCESS | SBIE_FLAG_PARENT_WAS_START_EXE | SBIE_FLAG_PROCESS_IS_START_EXE))
					{
		...
		if (terminate_and_stop)
		{
			for (i = 1; i <= pids[0]; ++i)
			{
				...
				ULONG64 ProcessFlags = SbieApi_QueryProcessInfo((HANDLE)(ULONG_PTR)pids[i], 0);
