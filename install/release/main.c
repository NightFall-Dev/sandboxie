...
int GetResourceId(void)
{
	...
	P_IsWow64Process __sys_IsWow64Process = (P_IsWow64Process)GetProcAddress(kernel32, "IsWow64Process");
...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	wsprintf(name, L"SandboxieInstall-%d-bit-%d.exe", (rsid / 100), GetTickCount());

	hfile = CreateFile(path, FILE_GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if (byteswritten == exesize)
	{
		...
		if (icmd)
		{
			ocmd = HeapAlloc(GetProcessHeap(), 0, (wcslen(icmd) + 8) * sizeof(WCHAR));
			if (!ocmd)
			{
				goto finish;
			}
		...
		if (CreateProcess(path, ocmd, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
		{
