...
static LRESULT Kmd_WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
_FX void Kmd_ScanDll_2(ULONG* pids, PGM* pgms)
{
	...
	for (i = 0; pids[i] != -1; ++i)
	{
		...
		if (!j)
		{
			typedef BOOL (*P_EnumProcessModulesEx)(HANDLE hProcess, HMODULE* lphModule, DWORD cb, LPDWORD lpcbNeeded, DWORD dwFilterFlag);
			static P_EnumProcessModulesEx pEnumProcessModulesEx = NULL;
			if (!pEnumProcessModulesEx)
			{
				pEnumProcessModulesEx = (P_EnumProcessModulesEx)GetProcAddress(GetModuleHandle(L"psapi.dll"), "EnumProcessModulesEx");
			}
			if (pEnumProcessModulesEx)
			{
				j = pEnumProcessModulesEx(hProcess, mods, 262144, &len, LIST_MODULES_ALL);
			}
		}
#endif _WIN64
		if (!j)
			...
			for (j = 0; j < len; ++j)
			{
				if (GetModuleBaseName(hProcess, mods[j], name, 60))
				{
					if (_wcsicmp(name, SBIEDLL L".dll") == 0)
					{
						...
						if (!ProcessIdToSessionId(pgms[z].process_id, &pgms[z].session_id))
						{
							pgms[z].session_id = 0;
						}

						if (!GetModuleBaseName(hProcess, mods[0], pgms[z].image, 124))
						{
							pgms[z].image[0] = L'?';
							pgms[z].image[1] = L'\0';
						}
...
_FX void EnableDebugPriv(void)
{
	...
	BOOL b = LookupPrivilegeValue(L"", SE_DEBUG_NAME, &privs->Privileges[0].Luid);
	if (b)
	{
...
_FX ULONG Kmd_DoWindow(WCHAR* text, PGM* pgms, BOOLEAN silent)
{
	...
	hWnd = CreateWindowEx(WS_EX_APPWINDOW, (LPCWSTR)atom, SANDBOXIE, WS_POPUPWINDOW | WS_CAPTION, x, y, w, h, NULL, NULL, NULL, &OkClicked);
	...
	hStatic = CreateWindowEx(i, L"STATIC", SbieDll_FormatMessage0(8106), SS_LEFT | WS_CHILD | WS_VISIBLE, 5, 5, w - 15, 40, hWnd, NULL, NULL, NULL);
	...
	hBtnOk = CreateWindowEx(0, L"BUTTON", text, BS_PUSHBUTTON | WS_CHILD | WS_TABSTOP | WS_VISIBLE, xOk, y, 100, 25, hWnd, (HMENU)IDOK, NULL, NULL);
	...
	hBtnCancel = CreateWindowEx(0, L"BUTTON", text, BS_PUSHBUTTON | WS_CHILD | WS_TABSTOP | WS_VISIBLE, xCancel, y, 100, 25, hWnd, (HMENU)IDCANCEL, NULL, NULL);
	...
	hList = CreateWindowEx(0, L"LISTBOX", NULL, LBS_NOINTEGRALHEIGHT | LBS_NOSEL | WS_HSCROLL | WS_VSCROLL | WS_BORDER | WS_CHILD | WS_TABSTOP | WS_VISIBLE, 5, 45, w - 15, h - 115, hWnd, NULL, NULL, NULL);
	...
	for (i = 0; pgms[i].process_id; ++i)
	{
		...
		swprintf(text, L"%-48.48s  PID %5d  Session %d", pgms[i].image, pgms[i].process_id, pgms[i].session_id);
...
terminate_and_return:
	...
	for (i = 0; pgms[i].process_id; ++i)
	{
		if (!pgms[i].skip)
		{
			HANDLE hProcess = OpenProcess(PROCESS_TERMINATE, FALSE, pgms[i].process_id);
			if (hProcess)
			{
...
_FX LRESULT Kmd_WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
