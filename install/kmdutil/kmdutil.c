...
void Display_Error(PWSTR SubFuncName, DWORD LastError)
{
	...
	{
		FormatMessage(FormatFlags, NULL, LastError, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&ErrorText, 0, NULL);
...
BOOL Parse_Command_Line(COMMAND* Command, PWSTR* Driver_Name, PWSTR* Driver_Path, PWSTR* Driver_Display, PWSTR* Driver_MsgFile, PWSTR* Driver_Altitude, PWSTR* Driver_Group, OPTIONS* Options)
{
	PWSTR* xargs;
	WCHAR* args[128];
	int num_args_given;
	...
	if (num_args_given > 1 && _wcsnicmp(args[1], L"/lang=", 6) == 0)
	{
		...
		--num_args_given;
		memcpy(&args[1], &xargs[2], sizeof(ULONG_PTR) * (num_args_given - 1));
	}

	if (num_args_given == 1)
	{
		*Command = CMD_ERROR;
		MessageBox(NULL, L"Missing command", L"KmdUtil", MB_ICONEXCLAMATION | MB_OK);
		return FALSE;
	}
	...
	{
		*Command = CMD_ERROR;
		MessageBox(NULL, L"Invalid command", L"KmdUtil", MB_ICONEXCLAMATION | MB_OK);
		return FALSE;
	}
	...
	{
		*Command = CMD_ERROR;
		MessageBox(NULL, L"Not enough arguments", L"KmdUtil", MB_ICONEXCLAMATION | MB_OK);
		return FALSE;
	}
	...
	*Options = OPT_NONE;
	...
	{
		*Command = CMD_ERROR;
		MessageBox(NULL, L"Invalid option", L"KmdUtil", MB_ICONEXCLAMATION | MB_OK);
		return FALSE;
	}
...
HKEY Kmd_Open_Services_Key(const WCHAR* subkey)
{
	...
	retcode = RegOpenKeyEx(HKEY_LOCAL_MACHINE, str, 0, KEY_READ | KEY_WRITE, &hkey);
...
BOOL Kmd_Delete_Service(const wchar_t* Driver_Name)
{
	SC_HANDLE service;

	// try to open the service for the driver

	service = OpenService(ScMgr, Driver_Name, SERVICE_INTERROGATE | DELETE);
...
BOOL Kmd_Unregister_Event_Source(const wchar_t* Driver_Name)
{
	...
	retcode = RegOpenKeyEx(system_key, Driver_Name, 0, KEY_READ | KEY_WRITE, &driver_key);

	if (retcode != 0)
	{
		if (retcode == ERROR_FILE_NOT_FOUND || retcode == ERROR_SERVICE_DOES_NOT_EXIST || retcode == ERROR_RESOURCE_TYPE_NOT_FOUND)
		{
			return TRUE;
		}
...
BOOL Kmd_Install_Service(const wchar_t* Service_Name, const wchar_t* Service_Path, const wchar_t* Service_Display, const wchar_t* Service_Group, OPTIONS Options)
{
	SC_HANDLE service;
...
BOOL Kmd_Register_Event_Source(const wchar_t* Driver_Name, const wchar_t* Driver_Path)
{
	...
	do
	{
		retcode = RegCreateKeyEx(system_key, Driver_Name, 0, NULL, 0, KEY_READ | KEY_WRITE, NULL, &driver_key, NULL);
	...
	retcode = RegSetValueEx(driver_key, L"EventMessageFile", 0, REG_EXPAND_SZ, (BYTE*)Driver_Path, (wcslen(Driver_Path) + 1) * sizeof(WCHAR));
	if (retcode != 0)
	{
	...
	retcode = RegSetValueEx(driver_key, L"TypesSupported", 0, REG_DWORD, (BYTE*)&types_supported, sizeof(types_supported));
	if (retcode != 0)
	{
...
BOOL Kmd_Register_MiniFilter(const wchar_t* Driver_Name, const wchar_t* Driver_Altitude)
{
	...
	retcode = RegSetValueEx(driver_key, L"DependsOnService", 0, REG_MULTI_SZ, (BYTE*)L"FltMgr\0\0", 8 * sizeof(WCHAR));
	if (retcode != 0)
	{
	...
	retcode = RegCreateKeyEx(driver_key, L"Instances", 0, NULL, 0, KEY_READ | KEY_WRITE, NULL, &instances_key, NULL);
	if (retcode != 0)
	{
	...
	retcode = RegSetValueEx(instances_key, L"DefaultInstance", 0, REG_SZ, (BYTE*)InstanceName, (wcslen(InstanceName) + 1) * sizeof(WCHAR));
	if (retcode != 0)
	{
	...
	retcode = RegCreateKeyEx(instances_key, InstanceName, 0, NULL, 0, KEY_READ | KEY_WRITE, NULL, &an_instance_key, NULL);
	if (retcode != 0)
	{
	...
	retcode = RegSetValueEx(an_instance_key, L"Altitude", 0, REG_SZ, (BYTE*)Driver_Altitude, (wcslen(Driver_Altitude) + 1) * sizeof(WCHAR));
	if (retcode != 0)
	{
	...
	zero    = 0;
	retcode = RegSetValueEx(an_instance_key, L"Flags", 0, REG_DWORD, (BYTE*)&zero, sizeof(DWORD));
	if (retcode != 0)
	{
		Display_Error(L"RegSetValueEx (Altitude)", retcode);
		return FALSE;
...
BOOL Kmd_Start_Service(const wchar_t* Driver_Name)
{
	SC_HANDLE service;
	...
	service = OpenService(ScMgr, Driver_Name, SERVICE_INTERROGATE | SERVICE_START);
	if (!service)
	{
		Display_Error(L"OpenService", 0);
		return FALSE;
	...
	if (!ControlService(service, SERVICE_CONTROL_INTERROGATE, &service_status))
	{
...
BOOL Kmd_Stop_Service(const wchar_t* Driver_Name)
{
	...
	service = OpenService(ScMgr, Driver_Name, SERVICE_INTERROGATE | SERVICE_STOP);
	...
	for (retries = 0;; ++retries)
	{
		...
		if (!ControlService(service, SERVICE_CONTROL_INTERROGATE, &service_status))
		{
			...
			if ((GetLastError() == ERROR_SERVICE_REQUEST_TIMEOUT || GetLastError() == ERROR_PIPE_BUSY) && retries < 3)
			{
				continue;
			}
...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!Parse_Command_Line(&Command, &Driver_Name, &Driver_Path, &Driver_Display, &Driver_MsgFile, &Driver_Altitude, &Driver_Group, &Options))
	{
		return EXIT_FAILURE;
	}

	ScMgr = OpenSCManager(NULL, SERVICES_ACTIVE_DATABASE, SC_MANAGER_CREATE_SERVICE);
	...
	{
		ok = Kmd_Install_Service(Driver_Name, Driver_Path, Driver_Display, Driver_Group, Options);
		if (ok)
		{
			if (!Driver_MsgFile)
			{
