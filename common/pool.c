...
#define PAGE_BITMAP_SIZE PAD_CELL(((POOL_PAGE_SIZE - PAGE_HEADER_SIZE) / POOL_CELL_SIZE + 7) / 8)

// number of cells in a single pool page
#define NUM_PAGE_CELLS ((POOL_PAGE_SIZE - PAGE_HEADER_SIZE - PAGE_BITMAP_SIZE) / POOL_CELL_SIZE)
...
//---------------------------------------------------------------------------
	...
	#define ABEND(why) KeBugCheckEx(DRIVER_CORRUPTED_MMPOOL, (ULONG_PTR)POOL_TAG, why, 0, 0)
#else /* ! KERNEL_MODE || POOL_DEBUG */
	#define ABEND(why)                                     \
		{                                                  \
			OutputDebugString(L"pool abend " L#why L"\n"); \
			__debugbreak();                                \
		}
#endif
...
#else
	ptr = VirtualAlloc(0, size, MEM_RESERVE | MEM_COMMIT | MEM_TOP_DOWN, ((UCHAR)tag == 0xFF ? PAGE_EXECUTE_READWRITE : PAGE_READWRITE));
#endif
...
ALIGNED POOL* Pool_CreateTagged(ULONG tag)
{
	...
	pool->lock = ExAllocatePoolWithTag(NonPagedPool, sizeof(ERESOURCE), tag);
	if (!pool->lock)
	{
...
ALIGNED void* Pool_Get_Cells(POOL* pool, ULONG size)
{
	...
	ptr = (UCHAR*)page + PAGE_HEADER_SIZE + PAGE_BITMAP_SIZE + index * POOL_CELL_SIZE;
...
ALIGNED void Pool_Free_Cells(void* ptr, ULONG size)
{
	...
	ULONG index   = (((ULONG)(ULONG_PTR)ptr & POOL_MASK_RIGHT) - PAGE_HEADER_SIZE - PAGE_BITMAP_SIZE) / POOL_CELL_SIZE;
	...
	if (page->num_free < FULL_PAGE_THRESHOLD && page->num_free + size >= FULL_PAGE_THRESHOLD)
	{
...
ALIGNED void* Pool_Get_Large_Chunk(POOL* pool, ULONG size)
{
	...
	large_chunk_size = (size + LARGE_CHUNK_SIZE + POOL_PAGE_SIZE - 1) & ~(POOL_PAGE_SIZE - 1);
	...
	large_chunk = (LARGE_CHUNK*)((UCHAR*)ptr + large_chunk_size - LARGE_CHUNK_SIZE);
