...
NTSTATUS Stream_Read_More(IN STREAM* stream);
...
__declspec(align(16)) NTSTATUS Stream_Open(OUT STREAM** out_stream, IN HANDLE Handle)
{
	STREAM* stream;
...
__declspec(align(16)) NTSTATUS Stream_Open(OUT STREAM** out_stream, IN const WCHAR* FullPath, IN ACCESS_MASK DesiredAccess, IN ULONG FileAttributes, IN ULONG ShareAccess, IN ULONG CreateDisposition, IN ULONG CreateOptions)
{
	NTSTATUS status;
	UNICODE_STRING uni;
	...
	*out_stream = NULL;
	...
	InitializeObjectAttributes(&objattrs, &uni, OBJ_KERNEL_HANDLE | OBJ_CASE_INSENSITIVE, NULL, NULL);

	for (retries = 0; retries < 10; ++retries)
	{
		status = IoCreateFile(&stream->handle,
...
__declspec(align(16)) void Stream_Close(IN STREAM* stream)
{
#ifndef KERNEL_MODE
...
__declspec(align(16)) NTSTATUS Stream_Read_More(IN STREAM* stream)
{
	NTSTATUS status;
	IO_STATUS_BLOCK MyIoStatusBlock;
...
__declspec(align(16)) NTSTATUS Stream_Flush(IN STREAM* stream)
{
	NTSTATUS status;
	IO_STATUS_BLOCK MyIoStatusBlock;
	...
	if (stream->data_len != 0)
	{
		...
		if (NT_SUCCESS(status) && MyIoStatusBlock.Information != stream->data_len)
		{
...
NTSTATUS Stream_Write_Bytes(IN STREAM* stream, IN ULONG len, OUT UCHAR* v)
{
	while (len)
	{
		STREAM_PUT_BYTE(*v);
